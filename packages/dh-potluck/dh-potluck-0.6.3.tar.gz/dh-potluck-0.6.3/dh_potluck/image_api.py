...
class ImageApi:
    ...
    def _get_image_api_host_for_url(cls, url_orig):
        prod_cdn_urls = current_app.config.get('DH_POTLUCK_PROD_IMAGE_CDN_URLS',
                                               DEFAULT_PROD_CDN_URLS)
        for cdn_url in prod_cdn_urls:
            if url_orig.startswith(cdn_url):
                host = current_app.config.get('DH_POTLUCK_PROD_IMAGE_API_URL',
                                              DEFAULT_PROD_IMAGE_API_URL)
                break
        else:
