import json
from archive.CCA_methods import *
from archive.CCA_methods import load_hcp_data, score_feat_corr, reduce_dimensions
from archive.CCA_methods.plot_utils import plot_connectome_correlations, plot_weights, plot_results
...
def main():
    ...
    for run in range(runs):
        ...
        ## Scikit-learn
        scikit = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='scikit', max_iter=max_als_iter).fit(
            train_set_1,
            train_set_2)
        ...
        params = {'c_1': 0, 'c_2': 0}
        als = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='l2', params=params,
        ...
        # kernel cca
        params = {'reg': 1e+4}
        kernel_linear = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='kernel',
        ...
        kernel_reg = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='kernel',
        ...
        kernel_gaussian = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='kernel', max_iter=max_als_iter).cv_fit(train_set_1, train_set_2, folds=cv_folds,
                                                                                                                                         param_candidates=param_candidates)
        ...
        kernel_poly = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='kernel', max_iter=max_als_iter).cv_fit(train_set_1, train_set_2, folds=cv_folds,
                                                                                                                                     param_candidates=param_candidates)
        ...
        ridge_als = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='l2', max_iter=max_als_iter).cv_fit(train_set_1, train_set_2,
                                                                                                                               param_candidates=param_candidates,
                                                                                                                               folds=cv_folds)
        ...
        sparse_als = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='scca',
        ...
        sparse_generalized_als = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='sgcca',
                                                                        max_iter=max_als_iter).cv_fit(train_set_1,
                                                                              train_set_2,
                                                                              param_candidates=param_candidates,
                                                                              folds=cv_folds, verbose=True)
        ...
        pmd = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='pmd',
        ...
        parkohomenko = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='parkhomenko',
                                                              max_iter=max_als_iter).cv_fit(train_set_1,
                                                                                          train_set_2,
                                                                                          param_candidates=param_candidates,
                                                                                          folds=cv_folds)
        ...
        elastic_als = archive.CCA_methods.linear.DeepWrapper(outdim_size=outdim_size, method='elastic',
                                                             max_iter=max_als_iter).cv_fit(train_set_1, train_set_2,
                                                                  param_candidates=param_candidates,
                                                                  folds=cv_folds)
        ...
        # Deep FCN Torch
        deep = archive.CCA_methods.deep.DeepWrapper(outdim_size=outdim_size, epoch_num=epoch_num)
