...
class Dselect:
	...
	def set(self, filename: str, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST>]:ENCC:PDCCh:DSELect \n
		Snippet: driver.source.bb.eutra.dl.subf.encc.pdcch.dselect.set(filename = '1', stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST>]:ENCC:PDCCh:DSELect \n
		Snippet: value: str = driver.source.bb.eutra.dl.subf.encc.pdcch.dselect.get(stream = repcap.Stream.Default) \n
		Selects an existing data list file from the default directory or from the specific directory. Refer to 'Accessing Files
		in the Default or Specified Directory' for general information on file handling in the default and in a specific
		directory. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Subf')
