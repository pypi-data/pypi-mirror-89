...
class TtInterval:
	...
	def get(self, stream=repcap.Stream.Default) -> enums.TdscdmaEnhTchTti:
		...
		Snippet: value: enums.TdscdmaEnhTchTti = driver.source.bb.tdscdma.up.cell.enh.dch.hsdpa.ttInterval.get(stream = repcap.Stream.Default) \n
		Queries the transmission time interval (TTI) . \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: tt_interval: 5MS"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:ENH:DCH:HSDPA:TTINterval?')
