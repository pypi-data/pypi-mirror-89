...
def _valid_plot_kwargs():
    ...
    vkwargs = {
        ...
        'volume_exponent'           : { 'Default'     : 'legacy',
...
def plot( data, **kwargs ):
    ...
    if config['volume']:
        ...
        if config['volume_exponent'] == 'legacy':
            volumeAxes.figure.canvas.draw()  # This is needed to calculate offset
            offset = volumeAxes.yaxis.get_major_formatter().get_offset()
            if len(offset) > 0:
                offset = (' x '+offset)
        elif isinstance(config['volume_exponent'],int) and config['volume_exponent'] >= 0:
            xp = config['volume_exponent']
