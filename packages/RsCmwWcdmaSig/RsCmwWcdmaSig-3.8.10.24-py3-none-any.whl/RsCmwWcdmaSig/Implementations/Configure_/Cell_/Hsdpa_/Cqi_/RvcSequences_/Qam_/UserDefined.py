...
class UserDefined:
	...
	def set(self, structure: UserDefinedStruct, quadratureAM=repcap.QuadratureAM.Default) -> None:
		"""SCPI: CONFigure:WCDMa:SIGNaling<instance>:CELL:HSDPa:CQI:RVCSequences:QAM<nr>:UDEFined \n
		Snippet: driver.configure.cell.hsdpa.cqi.rvcSequences.qam.userDefined.set(value = [PROPERTY_STRUCT_NAME](), quadratureAM = repcap.QuadratureAM.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HSDPa:CQI:RVCSequences:QAM{quadratureAM_cmd_val}:UDEFined', structure)

	def get(self, quadratureAM=repcap.QuadratureAM.Default) -> UserDefinedStruct:
		"""SCPI: CONFigure:WCDMa:SIGNaling<instance>:CELL:HSDPa:CQI:RVCSequences:QAM<nr>:UDEFined \n
		Snippet: value: UserDefinedStruct = driver.configure.cell.hsdpa.cqi.rvcSequences.qam.userDefined.get(quadratureAM = repcap.QuadratureAM.Default) \n
		Specifies an RV coding sequence to be used for signals with 16-QAM or 64-QAM modulation if UDEFined is set via
		CONFigure:WCDMa:SIGN<i>:CELL:HSDPa:CQI:RVCSequences:QAM<no>. \n
			:param quadratureAM: optional repeated capability selector. Default value: QAM16 (settable in the interface 'Qam')
			:return: structure: for return value, see the help for UserDefinedStruct structure arguments."""
		quadratureAM_cmd_val = self._base.get_repcap_cmd_value(quadratureAM, repcap.QuadratureAM)
		return self._core.io.query_struct(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HSDPa:CQI:RVCSequences:QAM{quadratureAM_cmd_val}:UDEFined?', self.__class__.UserDefinedStruct())
