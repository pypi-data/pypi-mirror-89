...
class Gain:
	...
	def set(self, gain: float, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.evdo.terminal.rriChannel.gain.set(gain = 1.0, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:EVDO:TERMinal<ST>:RRIChannel:GAIN \n
		Snippet: value: float = driver.source.bb.evdo.terminal.rriChannel.gain.get(stream = repcap.Stream.Default) \n
		(enabled for an access terminal working in traffic mode) Sets the gain of the Reverse Rate Indicator (RRI) channel
		relative to the pilot channel power. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Terminal')
