...
class UeCategory:
	...
	def get_manual(self) -> int:
		...
		Snippet: value: int = driver.configure.cell.hsupa.ueCategory.get_manual() \n
		Configures the UE category to be used by the R&S CMW if no reported value is available or usage of the reported value is
		disabled, see method RsCmwWcdmaSig.Configure.Cell.Hsupa.UeCategory.Reported.set. \n
			:return: ue_cat_manual: Range: 1 to 9
		"""
		response = self._core.io.query_str('CONFigure:WCDMa:SIGNaling<Instance>:CELL:HSUPa:UECategory:MANual?')
		return Conversions.str_to_int(response)

	def set_manual(self, ue_cat_manual: int) -> None:
		"""SCPI: CONFigure:WCDMa:SIGNaling<instance>:CELL:HSUPa:UECategory:MANual \n
		Snippet: driver.configure.cell.hsupa.ueCategory.set_manual(ue_cat_manual = 1) \n
		Configures the UE category to be used by the R&S CMW if no reported value is available or usage of the reported value is
		disabled, see method RsCmwWcdmaSig.Configure.Cell.Hsupa.UeCategory.Reported.set. \n
			:param ue_cat_manual: Range: 1 to 9
		"""
		param = Conversions.decimal_value_to_str(ue_cat_manual)
		self._core.io.write(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HSUPa:UECategory:MANual {param}')
