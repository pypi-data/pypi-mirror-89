...
def run_image(input_path:str, output_path:str, weights_path=None):
    ...
    if weights_path is None:
        weights_path = tf.keras.utils.get_file('cdcl.h5',
                                               'https://github.com/fahima10/high-vision/releases/download/v0.0/cdcl_model.h5',
...
def run_video(input_path:str, output_path:str, fps:int = 25, weights_path=None):
...
def run_camera(cam, check_fps = True, frames_per_second = 25, output_video_name = None, 
                          show_frames = True, frame_name = "Output", verbose = 0, weights_path= None):
    ...
    while True:
        ...
        if ret:
          ...
          flipImg = cv2.flip(frame_copy, 1)
          oriImg = (frame_copy / 256.0) - 0.5
          flipImg = (flipImg / 256.0) - 0.5
          
          output, heatmap, paf, people, seg = process(oriImg, flipImg, params, model_params, model, frame)
