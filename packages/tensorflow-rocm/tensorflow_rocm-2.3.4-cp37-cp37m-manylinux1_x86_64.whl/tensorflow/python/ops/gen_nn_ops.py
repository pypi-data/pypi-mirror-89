...
def dropout(input, rate, noise_shape, seed, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "Dropout", name,
        tld.op_callbacks, input, rate, noise_shape, seed)
...
def dropout_eager_fallback(input, rate, noise_shape, seed, name, ctx):
  _attr_T, _inputs_T = _execute.args_to_matching_eager([input, rate], ctx)
...
def dropout_grad(gradients, rate, mask, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "DropoutGrad", name,
        tld.op_callbacks, gradients, rate, mask)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
...
def dropout_grad_eager_fallback(gradients, rate, mask, name, ctx):
  _attr_T, _inputs_T = _execute.args_to_matching_eager([gradients, rate], ctx)
...
def gelu(features, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "Gelu", name, tld.op_callbacks,
        features)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
...
def gelu_grad(gradients, outputs, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "GeluGrad", name,
        tld.op_callbacks, gradients, outputs)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
