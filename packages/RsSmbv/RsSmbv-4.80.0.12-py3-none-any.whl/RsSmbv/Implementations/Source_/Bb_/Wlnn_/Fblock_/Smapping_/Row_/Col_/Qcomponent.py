...
class Qcomponent:
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default, column=repcap.Column.Default) -> float:
		...
		Snippet: value: float = driver.source.bb.wlnn.fblock.smapping.row.col.qcomponent.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default, column = repcap.Column.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:WLNN:FBLock{channel_cmd_val}:SMAPping:ROW{stream_cmd_val}:COL{column_cmd_val}:Q?')
