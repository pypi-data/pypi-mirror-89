...
def spec_validator():
    ...
    test_schema = [
        {
            "SIMPLE": {"expand": False, "required": True, "type": "bool", "default_value": "T"},
            "BITPIX": {
                "expand": False,
                "required": True,
                "type": "int",
...
@pytest.fixture(
    scope="function",
    params=["valid_hdu_list", "valid_fits_header", "valid_fits_file", "valid_compressed.fits.fz"],
)
...
def invalid_test_header_params(tmpdir_factory):
    ...
    yield {
        ...
        "invalid_compressed.fits.fz": Path(file_name),
    }
