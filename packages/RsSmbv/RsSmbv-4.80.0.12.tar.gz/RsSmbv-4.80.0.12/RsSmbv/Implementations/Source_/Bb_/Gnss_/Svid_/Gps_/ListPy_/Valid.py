...
class Valid:
	...
	def get(self, stream=repcap.Stream.Default) -> List[int]:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID:GPS<ST>:LIST:[VALid] \n
		Snippet: value: List[int] = driver.source.bb.gnss.svid.gps.listPy.valid.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_int_list(f'SOURce<HwInstance>:BB:GNSS:SVID:GPS{stream_cmd_val}:LIST:VALid?')
		return response
