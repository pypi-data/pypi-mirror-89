...
class Azero:
	...
	def set(self, azero: int, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:SBAS<ST>:MSAS:UTC:AZERo \n
		Snippet: driver.source.bb.gnss.time.conversion.sbas.msas.utc.azero.set(azero = 1, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:SBAS{stream_cmd_val}:MSAS:UTC:AZERo {param}')

	def get(self, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:SBAS<ST>:MSAS:UTC:AZERo \n
		Snippet: value: int = driver.source.bb.gnss.time.conversion.sbas.msas.utc.azero.get(stream = repcap.Stream.Default) \n
		Sets the constant term of polynomial, A0. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sbas')
			:return: azero: integer Range: -2147483648 to 2147483647"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:SBAS{stream_cmd_val}:MSAS:UTC:AZERo?')
