...
class RsCmwBluetoothMeas:
	"""730 total commands, 8 Sub-groups, 0 group commands"""
	driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.90.0022'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwBluetoothMeas.driver_options, options, direct_session)
		self._core.driver_version = '3.7.90.0022'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.90.0022'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwBluetoothMeas version failed. Current version: '3.7.90.0022', minimum required version: '{min_version}'")
	...
	@property
	def configure(self):
		"""configure commands group. 5 Sub-classes, 4 commands."""
