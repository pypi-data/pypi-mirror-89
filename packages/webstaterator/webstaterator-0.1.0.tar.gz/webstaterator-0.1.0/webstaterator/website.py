...
#--- links ---------------------------------------------------------------------
    ...
    def generate_links(self):
        ...
        for page in self._pages:
            ...
            obj = self._model
            if '.' in model_obj:
                nestedmodel_obj = model_obj.split('.')
                breadcrumb = ""
                ...
                if model_obj not in obj:
                    err_str = model_err_fmt.format(model_obj,'model')
                    raise InvalidModelException(err_str)
            ...
            if isinstance(obj, Mapping):
                ...
                if PageAttributes.ID not in page:
                    err_str = "Pages using the list attribute must include an id attribute or target a dictionary"
                    raise InvalidPagesException(err_str)
                model_id  = page[PageAttributes.ID]

                index = 0
                for item_obj in obj:
                    if model_id in item_obj:
