...
class Carrier:
	...
	def set(self, hsn: int, carrier=repcap.Carrier.Default) -> None:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:HSN:TCH[:CARRier<Carrier>] \n
		Snippet: driver.configure.rfSettings.hopping.hsn.tch.carrier.set(hsn = 1, carrier = repcap.Carrier.Default) \n
		Specifies the hopping sequence number (HSN) to be used. \n
			:param hsn: Range: 0 to 63
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')"""
		param = Conversions.decimal_value_to_str(hsn)
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		self._core.io.write(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:HSN:TCH:CARRier{carrier_cmd_val} {param}')

	def get(self, carrier=repcap.Carrier.Default) -> int:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:HSN:TCH[:CARRier<Carrier>] \n
		Snippet: value: int = driver.configure.rfSettings.hopping.hsn.tch.carrier.get(carrier = repcap.Carrier.Default) \n
		Specifies the hopping sequence number (HSN) to be used. \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
			:return: hsn: Range: 0 to 63"""
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		response = self._core.io.query_str(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:HSN:TCH:CARRier{carrier_cmd_val}?')
