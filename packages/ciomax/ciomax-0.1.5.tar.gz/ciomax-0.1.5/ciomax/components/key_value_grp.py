...
class KeyValueGrpList(QtWidgets.QWidget):
    ...
    def entries(self):
        return [e for e in self.entries_component.children() if e.metaObject().className() == "KeyValuePairGrp" and e.isVisible() ]

    def __iter__(self):
        for entry in self.entries():
            key = entry.key_field.text().strip()
            value = entry.value_field.text().strip()
            if key and value:
                if self.has_checkbox:
                    checked = entry.checkbox.isChecked()
                    yield [key, value, checked]
                else:
                    yield [key, value]
...
def remove_widget(widget):
    ...
    #  of the parent, it will include  this widget. By setting it invisible, we
    #  can check for visibility in KeyValueGrpList.entries() and only consider
    #  visible widgets when saving to the store.
