...
class Secondary:
	...
	def get_stype(self) -> enums.ServerType:
		...
		Snippet: value: enums.ServerType = driver.configure.data.control.dns.secondary.get_stype() \n
		...
		"""
		response = self._core.io.query_str_with_opc('CONFigure:DATA:CONTrol:DNS:SECondary:STYPe?')
		return Conversions.str_to_scalar_enum(response, enums.ServerType)

	def set_stype(self, stype: enums.ServerType) -> None:
		"""SCPI: CONFigure:DATA:CONTrol:DNS:SECondary:STYPe \n
		Snippet: driver.configure.data.control.dns.secondary.set_stype(stype = enums.ServerType.FOReign) \n
		Select the primary and secondary DNS server type. \n
		...
		self._core.io.write_with_opc(f'CONFigure:DATA:CONTrol:DNS:SECondary:STYPe {param}')
