...
class Poffset:
	...
	def fetch(self, freqOffset=repcap.FreqOffset.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.listPy.smodulation.poffset.fetch(freqOffset = repcap.FreqOffset.Default) \n
		Return the burst power at the carrier frequency minus/plus a selected frequency offset, for all measured list mode
		segments of the spectrum due to modulation measurement. The values described below are returned by FETCh commands.
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:GSM:MEASurement<Instance>:MEValuation:LIST:SMODulation:POFFset{freqOffset_cmd_val}?', suppressed)
		return response
	...
	def calculate(self, freqOffset=repcap.FreqOffset.Default) -> List[enums.ResultStatus2]:
		...
		Snippet: value: List[enums.ResultStatus2] = driver.multiEval.listPy.smodulation.poffset.calculate(freqOffset = repcap.FreqOffset.Default) \n
		Return the burst power at the carrier frequency minus/plus a selected frequency offset, for all measured list mode
		segments of the spectrum due to modulation measurement. The values described below are returned by FETCh commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
		Use RsCmwGsmMeas.reliability.last_value to read the updated reliability indicator. \n
		...
		freqOffset_cmd_val = self._base.get_repcap_cmd_value(freqOffset, repcap.FreqOffset)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:GSM:MEASurement<Instance>:MEValuation:LIST:SMODulation:POFFset{freqOffset_cmd_val}?', suppressed)
