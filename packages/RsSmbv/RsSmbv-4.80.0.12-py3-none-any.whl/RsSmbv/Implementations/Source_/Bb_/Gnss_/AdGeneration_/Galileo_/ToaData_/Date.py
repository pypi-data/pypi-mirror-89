...
class Date:
	...
	def set(self, structure: DateStruct, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:TOAData:DATE \n
		Snippet: driver.source.bb.gnss.adGeneration.galileo.toaData.date.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> DateStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:TOAData:DATE \n
		Snippet: value: DateStruct = driver.source.bb.gnss.adGeneration.galileo.toaData.date.get(stream = repcap.Stream.Default) \n
		Enabled for UTC or GLONASS timebase (method RsSmbv.Source.Bb.Gnss.AdGeneration.Gps.ToaData.Tbasis.set) . Enters the date
		for the assistance data in DMS format of the Gregorian calendar. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Galileo')
