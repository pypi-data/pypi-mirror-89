...
class StatusBar():
    def __init__(self):
        self.distance = 0
        self.hp = 2
        with open("score.json", "r") as self.f:
    ...
    def draw_top_score(self):
        ...
        draw_text(f"最高分：{self.top_score}", pos_x, pos_y, ac.color.BLUE, font_name=("simhei", "PingFang"))


    def draw_bar(self):
        draw_rectangle_filled(
            SCREEN_WIDTH // 2, SCREEN_HEIGHT - 15, SCREEN_WIDTH, 30, ac.color.WHITE
        )
    
    def draw_distance(self):
        pos_x = 10
        pos_y = SCREEN_HEIGHT - 20
        draw_text(f"路程：{self.distance}", pos_x, pos_y, ac.color.BLUE, font_name=("simhei", "PingFang"))
    
    def draw_hp(self):
        pos_x = SCREEN_WIDTH//2 -50
        pos_y = SCREEN_HEIGHT - 20
        draw_text(f"血量：", pos_x, pos_y, ac.color.BLUE, font_name=("simhei", "PingFang"))
        hearts = SpriteList()
        for i in range(self.hp):
            heart = Sprite("heal.png")
...
class MyCar(Window):
    ...
    def setup(self):
        self.road1 = Road("road.png")
        self.road2 = Road("road.png")
        self.road2.center_y = SCREEN_HEIGHT // 2 + SCREEN_HEIGHT
        self.small_car_list = [f"small_car{str(i)}" for i in range(1, 5)]
    ...
    def on_key_release(self, symbol: int, modifiers: int):
        if symbol == ac.key.LEFT and self.small_car.center_x >= SCREEN_WIDTH //2:
            self.small_car.center_x -= 200
        elif symbol == ac.key.RIGHT and self.small_car.center_x <= SCREEN_WIDTH //2:
            self.small_car.center_x += 200

    def create_carts(self):
        cart_list = ["big_car1.png", "big_car2.png", "big_car3.png"]
        num = 2
