...
class Bits:
	...
	def set(self, segment_bits: enums.SegmentBits, segment=repcap.Segment.Default) -> None:
		...
		self._core.io.write(f'CONFigure:EVDO:SIGNaling<Instance>:RPControl:SEGMent{segment_cmd_val}:BITS {param}')
	...
	def get(self, segment=repcap.Segment.Default) -> enums.SegmentBits:
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		response = self._core.io.query_str(f'CONFigure:EVDO:SIGNaling<Instance>:RPControl:SEGMent{segment_cmd_val}:BITS?')
