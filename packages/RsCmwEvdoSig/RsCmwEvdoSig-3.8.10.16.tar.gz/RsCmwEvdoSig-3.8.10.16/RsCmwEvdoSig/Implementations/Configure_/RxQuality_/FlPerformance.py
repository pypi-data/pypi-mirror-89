...
class FlPerformance:
	...
	def get_mframes(self) -> int:
		...
		Snippet: value: int = driver.configure.rxQuality.flPerformance.get_mframes() \n
		...
		"""
		response = self._core.io.query_str('CONFigure:EVDO:SIGNaling<Instance>:RXQuality:FLPFormance:MFRames?')
		return Conversions.str_to_int(response)

	def set_mframes(self, max_frames: int) -> None:
		"""SCPI: CONFigure:EVDO:SIGNaling<instance>:RXQuality:FLPFormance:MFRames \n
		Snippet: driver.configure.rxQuality.flPerformance.set_mframes(max_frames = 1) \n
		Defines the maximum duration of the 'Forward Link Throughput' / 'Reverse Link Throughput' measurement as a number of 26.
		66 ms CDMA2000 frames. \n
			:param max_frames: Range: 1 to 10000
		"""
		param = Conversions.decimal_value_to_str(max_frames)
		self._core.io.write(f'CONFigure:EVDO:SIGNaling<Instance>:RXQuality:FLPFormance:MFRames {param}')
