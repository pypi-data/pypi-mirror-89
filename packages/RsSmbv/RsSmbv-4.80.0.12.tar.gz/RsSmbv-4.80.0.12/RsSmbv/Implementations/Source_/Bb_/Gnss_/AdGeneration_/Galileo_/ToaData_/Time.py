...
class Time:
	...
	def set(self, structure: TimeStruct, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:TOAData:TIME \n
		Snippet: driver.source.bb.gnss.adGeneration.galileo.toaData.time.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> TimeStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:TOAData:TIME \n
		Snippet: value: TimeStruct = driver.source.bb.gnss.adGeneration.galileo.toaData.time.get(stream = repcap.Stream.Default) \n
		Enabled for UTC or GLONASS timebase (method RsSmbv.Source.Bb.Gnss.AdGeneration.Gps.ToaData.Tbasis.set) . Enters the exact
		start time for the assistance data in UTC time format. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Galileo')
