...
class Range:
	...
	def get(self, hsrQAM=repcap.HsrQAM.Default) -> GetStruct:
		...
		32-QAM modulated DL signal with higher symbol rate (HSR) . \n
		...
		return self._core.io.query_struct(f'SENSe:GSM:SIGNaling<Instance>:RREPort:HSRQam{hsrQAM_cmd_val}:MBEP:RANGe?', self.__class__.GetStruct())
