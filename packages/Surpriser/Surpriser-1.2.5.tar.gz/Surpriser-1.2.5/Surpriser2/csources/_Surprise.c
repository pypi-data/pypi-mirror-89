...
/*           Initialization                 */
// *******************************************
PyMODINIT_FUNC initsurprise(void) {
    PyObject* mod;
...
static int Surpriser_init(Surprise *self, PyObject *args, PyObject *kwds) {
    ...
    for (i=0; i<K; i++) {
        ...
        for (j=0; j<K; j++) {
            *(y + i*K + j) = (int) PyInt_AsSsize_t(PyList_GetItem(linha,j));
        }
    }
    ...
    if (Nc>0) {
        ...
        for (i=0;i<Nc;i++) {
            ...
            *(comsizes + i) = cs;
            for (j=0;j<cs;j++) {
                *(*(communities+i) + j) = (int) PyInt_AsSsize_t(PyList_GetItem(community, j));
            }
        }
    }
    //printf("Here 1!\n");
...
static PyObject *Surpriser_partition(Surprise *self) {
    ...
    for (j=0; j<K; j++) {
        PyObject *num = PyInt_FromSsize_t(*(part+j));
...
static PyObject *Surpriser_linksin(Surprise *self, PyObject *args) {
    ...
    for (i=0; i<N; i++) {
        *(ccom + i) = (int) PyInt_AsSsize_t(PyList_GetItem(com, i));
    }
...
static PyObject *Surpriser_community(Surprise *self, PyObject *args) {
    ...
    for (j=0; j<cs; j++) {
        ...
        PyObject *num = PyInt_FromSsize_t(node);
...
static PyObject *Function_compare(PyObject *self, PyObject *args) {   
    ...
    for (i=0; i<K; i++) {
        co1 = (int) PyInt_AsSsize_t(PyList_GetItem(Pyli1, i));
        co2 = (int) PyInt_AsSsize_t(PyList_GetItem(Pyli2, i));
        *(li1 + i) = co1;
        *(li2 + i) = co2;
        if (co1>n1) {n1 = co1;}
        if (co2>n2) {n2 = co2;}
