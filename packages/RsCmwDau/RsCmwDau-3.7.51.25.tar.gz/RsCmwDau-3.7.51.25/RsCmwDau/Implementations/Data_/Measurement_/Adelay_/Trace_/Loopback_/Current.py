...
class Current:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.data.measurement.adelay.trace.loopback.current.fetch() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:DATA:MEASurement<MeasInstance>:ADELay:TRACe:LOOPback:CURRent?', suppressed)
		return response

	def read(self) -> List[float]:
		"""SCPI: READ:DATA:MEASurement<Instance>:ADELay:TRACe:LOOPback[:CURRent] \n
		Snippet: value: List[float] = driver.data.measurement.adelay.trace.loopback.current.read() \n
		Query the values of the audio delay traces 'Uplink', 'Downlink' and 'Loopback'. The trace values are returned from right
		to left, from sample number 0 to sample number -N. N equals the configured maximum number of samples minus one, see
		method RsCmwDau.Configure.Data.Measurement.Adelay.msamples. \n
		Use RsCmwDau.reliability.last_value to read the updated reliability indicator. \n
			:return: results: Comma-separated list of delay values, one result per sample Unit: s"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:DATA:MEASurement<MeasInstance>:ADELay:TRACe:LOOPback:CURRent?', suppressed)
		return response
