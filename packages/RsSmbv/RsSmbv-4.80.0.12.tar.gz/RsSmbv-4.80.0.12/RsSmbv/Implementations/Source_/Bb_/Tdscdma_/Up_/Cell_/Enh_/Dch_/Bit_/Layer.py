...
class Layer:
	...
	def set(self, layer: enums.EnhBitErr, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.up.cell.enh.dch.bit.layer.set(layer = enums.EnhBitErr.PHYSical, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.EnhBitErr:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:ENH:DCH:BIT:LAYer \n
		Snippet: value: enums.EnhBitErr = driver.source.bb.tdscdma.up.cell.enh.dch.bit.layer.get(stream = repcap.Stream.Default) \n
		Sets the layer in the coding process at which bit errors are inserted. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
