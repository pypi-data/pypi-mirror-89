...
class Dpdch:
	...
	def set(self, structure: DpdchStruct, carrier=repcap.Carrier.Default) -> None:
		"""SCPI: CONFigure:WCDMa:MEASurement<instance>:UECHannels:CARRier<carrier>:DPDCh \n
		Snippet: driver.configure.ueChannels.carrier.dpdch.set(value = [PROPERTY_STRUCT_NAME](), carrier = repcap.Carrier.Default) \n
		...
		configuration \n
			:param structure: for set value, see the help for DpdchStruct structure arguments.
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')"""
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		self._core.io.write_struct(f'CONFigure:WCDMa:MEASurement<Instance>:UECHannels:CARRier{carrier_cmd_val}:DPDCh', structure)

	def get(self, carrier=repcap.Carrier.Default) -> DpdchStruct:
		"""SCPI: CONFigure:WCDMa:MEASurement<instance>:UECHannels:CARRier<carrier>:DPDCh \n
		Snippet: value: DpdchStruct = driver.configure.ueChannels.carrier.dpdch.get(carrier = repcap.Carrier.Default) \n
		Specifies the presence of a DPDCH in the uplink signal and the beta factor and spreading factor of the channel.
			INTRO_CMD_HELP: For the combined signal path scenario, use: \n
			- Beta factor setting:
			Table Header:  \n
