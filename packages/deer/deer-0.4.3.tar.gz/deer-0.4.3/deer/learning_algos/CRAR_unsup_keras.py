...
from .NN_CRAR_unsup_keras import NN # Default Neural network used
...
class CRAR(LearningAlgo):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_norm=0, freeze_interval=1000, batch_size=32, update_rule="rmsprop", random_state=np.random.RandomState(), double_Q=False, neural_network=NN, **kwargs):
        ...
        self.loss_productl=0
        self.loss_gamma=0
        self.loss_discrim_masked=0
        self.loss_discrim_masked2=0
        ...
        self.encoder_discrim = self.learn_and_plan.encoder_model()
        self.encoder_diff = self.learn_and_plan.encoder_diff_model(self.encoder)
        self.encoder_productl = self.learn_and_plan.encoder_productl_model(self.encoder)
        ...
        self.discrim_masked = self.learn_and_plan.discrim_masked_model(self.encoder_discrim)
        self.discrim_masked_full = self.learn_and_plan.discrim_masked_full_model(self.encoder, self.transition, self.R, self.gamma, self.discrim_masked)
        self.discrim_masked_full2 = self.learn_and_plan.discrim_masked_full_model(self.encoder, self.transition, self.R, self.gamma, self.discrim_masked)        
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        mask_ones=np.ones((self._batch_size,self.learn_and_plan.n_channels_internal_dim))
        states_val=list(states_val)
        next_states_val=list(next_states_val)
        
        # FIXME more general inputs
        #shift_s=np.random.randint(4)
        #states_val[0]=states_val[0][:,:,shift_s:-4+shift_s,shift_s:-4+shift_s]
        #next_states_val[0]=next_states_val[0][:,:,shift_s:-4+shift_s,shift_s:-4+shift_s]
            
        Es_=self.encoder.predict(next_states_val)
        Es=self.encoder.predict(states_val)
        ETs=self.transition.predict([Es,mask_ones,onehot_actions])
        R=self.R.predict([Es,mask_ones,onehot_actions])
        ...
        # Fit transition
        mask=np.random.randint(2, size=(self._batch_size,self.learn_and_plan.n_channels_internal_dim))
        mask=mask.astype(float)
        self.loss_T+=self.diff_Tx_x_.train_on_batch( states_val+next_states_val+[mask]+[onehot_actions]+[(1-terminals_val)] , np.zeros_like(Es) )
        
        # Fit rewards
        #mask=np.ones((self._batch_size,self.learn_and_plan.n_channels_internal_dim))
        #for layer in self.encoder.layers: print(layer.get_config(), layer.get_weights())
        self.lossR+=self.full_R.train_on_batch(states_val+[mask]+[onehot_actions], rewards_val)
        #for layer in self.encoder.layers: print(layer.get_config(), layer.get_weights())
        
        # Fit gammas
        self.loss_gamma+=self.full_gamma.train_on_batch(states_val+[mask]+[onehot_actions], (1-terminals_val[:])*self._df)
        ...
        x=np.random.randint(states_val[0].shape[-2]-16)
        y=np.random.randint(states_val[0].shape[-1]-16)
        sub_arrays_states=states_val[0][:,:,x:x+16,y:y+16]
        rolled=np.roll(sub_arrays_states,1,axis=0)
        self.loss_disambiguate2+=self.encoder_diff.train_on_batch([sub_arrays_states,rolled],np.reshape(np.zeros_like(Es),(self._batch_size,-1)))
        #self.loss_productl+=self.encoder_productl.train_on_batch( [states_val[0]] , np.zeros((self._batch_size,9,9)) ) #np.zeros((self._batch_size,dim[-2] // self.learn_and_plan._pooling_encoder,dim[-2] // self.learn_and_plan._pooling_encoder)) )

        sub_arrays_next_states=next_states_val[0][:,:,x:x+16,y:y+16]     
        self.loss_disentangle_t+=self.diff_s_s_.train_on_batch([sub_arrays_states,sub_arrays_next_states], np.reshape(np.zeros_like(Es),(self._batch_size,-1)))

#        # Increase the entropy in the abstract features of two states
#        # This works only when states_val is made up of only one observation --> FIXME
#        layer=random.randint(self.learn_and_plan.n_channels_internal_dim, size=(self._batch_size))
#        sub_arrays_states=states_val[0][:,layer,:,:]
#        rolled=np.roll(sub_arrays_states,1,axis=0)
#        self.loss_disambiguate2+=self.encoder_diff.train_on_batch([sub_arrays_states,rolled],np.reshape(np.zeros_like(Es),(self._batch_size,-1)))
#        #self.loss_productl+=self.encoder_productl.train_on_batch( [states_val[0]] , np.zeros((self._batch_size,9,9)) ) #np.zeros((self._batch_size,dim[-2] // self.learn_and_plan._pooling_encoder,dim[-2] // self.learn_and_plan._pooling_encoder)) )
#
#        sub_arrays_next_states=next_states_val[0][:,layer,:,:]     
#        self.loss_disentangle_t+=self.diff_s_s_.train_on_batch([sub_arrays_states,sub_arrays_next_states], np.reshape(np.zeros_like(Es),(self._batch_size,-1)))

        
        # Disantanglement loss
        masked_index=random.sample(list(range(self._batch_size)), self._batch_size//2)
        non_masked_index=list( set(np.arange(self._batch_size)).symmetric_difference(set(masked_index)) )
        masked=np.ones((self._batch_size)) #random.randint(2, size=(self._batch_size))
        masked[masked_index]=0
        
        masked_layer=np.ones((self._batch_size, self.learn_and_plan.n_channels_internal_dim))
        indices_masked= np.array(  ( [ [masked_index[i],np.random.randint(self.learn_and_plan.n_channels_internal_dim,size=self._batch_size//2)[i]] for i in range(self._batch_size//2) ] )  )
        masked_layer[ indices_masked[:,0], indices_masked[:,1] ] = 0

        masked_layer2=copy.deepcopy(masked_layer)
        #print ("masked_layer2")
        #print (masked_layer2)
        indices_masked2= np.array(  ( [ [non_masked_index[i],np.random.randint(self.learn_and_plan.n_channels_internal_dim,size=self._batch_size//2)[i]] for i in range(self._batch_size//2) ] )  )
        #print (indices_masked2)
        masked_layer2[indices_masked2[:,0], indices_masked2[:,1]]=0
        #print (masked_layer2)

        if(self.update_counter%500==0):
            print (states_val+[masked_layer]+[onehot_actions])
            print (masked)
            print ("predict")
            print (self.discrim_masked_full.predict(states_val+[masked_layer]+[onehot_actions]+[masked_layer2]+[(1-terminals_val)]))
        
        
        #print ("aa")
        #for layer in self.transition.layers: print(layer.get_config(), layer.get_weights())
        #for layer in self.gamma.layers: print(layer.get_config(), layer.get_weights())
        self.loss_discrim_masked+=self.discrim_masked_full.train_on_batch(states_val+[masked_layer]+[onehot_actions]+[masked_layer2]+[(1-terminals_val)], masked)
        #print ("aaa")
        #for layer in self.gamma.layers: print(layer.get_config(), layer.get_weights())
        #for layer in self.transition.layers: print(layer.get_config(), layer.get_weights())
        #self.loss_discrim_masked2+=self.discrim_masked_full2.train_on_batch(states_val+[masked_layer]+[onehot_actions]+[masked_layer2]+[(1-terminals_val)], 1-masked)
        ...
        if(self.update_counter%500==0):
            print ("self.loss_T/500., self.lossR/500., self.loss_gamma/500., self.loss_Q/500., self.loss_disentangle_t/500., self.loss_disambiguate1/500., self.loss_disambiguate2/500., self.loss_productl/500.")
            print (self.loss_T/500., self.lossR/500.,self.loss_gamma/500., self.loss_Q/500., self.loss_disentangle_t/500., self.loss_disambiguate1/500., self.loss_disambiguate2/500., self.loss_productl/500.)
            print ("self.loss_discrim_masked/500.,self.loss_discrim_masked2/500.")
            print (self.loss_discrim_masked/500.,self.loss_discrim_masked2/500.)
...
#
#        self.loss_Q+=self.full_Q.train_on_batch(states_val , q_vals ) 
        ...
        return 0,0#np.sqrt(loss),loss_ind
...
#        ## END DEBUG PURPOSES
    ...
    def qValues_planning_abstr(self, state_abstr_val, R, gamma, T, Q, d, branching_factor=None):
        ...
        if (d==0):
            ...
            next_x_predicted=T.predict([tile3_encoded_x,repeat_identity])#[-1] # [-1] means that it select the most complex transition model
    ...
    def _compile(self):
        ...
        if (self._update_rule=="sgd"):
            ...
            optimizer8=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipnorm=self._clip_norm)
            optimizer9=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipnorm=self._clip_norm)
            optimizer10=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipnorm=self._clip_norm)
        ...
        self.encoder_diff.compile(optimizer=optimizer5,
                  loss=exp_dec_error)
        self.encoder_productl.compile(optimizer=optimizer8,
        ...
        self.discrim_masked_full.compile(optimizer=optimizer9, loss='binary_crossentropy') #mse')#binary_crossentropy')        
        self.transition.trainable = True
        self.R.trainable = True
        self.gamma.trainable = True

        self.encoder.trainable = False
        self.discrim_masked.trainable = False
        self.discrim_masked_full2.compile(optimizer=optimizer10, loss='binary_crossentropy') #mse')#binary_crossentropy')        
        self.encoder.trainable = True
        self.discrim_masked.trainable = True
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.encoder_diff.optimizer.lr, self._lr/20.)
        K.set_value(self.encoder_productl.optimizer.lr, self._lr/5.)

        K.set_value(self.diff_s_s_.optimizer.lr, self._lr/100.) # /5. for simple laby or simple catcher; /1. for distrib of laby
        
        K.set_value(self.discrim_masked_full.optimizer.lr, self._lr/20.)
        K.set_value(self.discrim_masked_full2.optimizer.lr, self._lr/20.)
