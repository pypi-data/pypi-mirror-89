    ...
    def select_from_detail(self):
        ...
        for i in range(len(fetchers)):
            detailinfo = {"id": "", "date": "", "runtime": 0, "javdblink": "", "title": "", "director": "",
                          ...
                          "smallimage": "", "bigimage": "", "previewimages": "", "magnets": ""}
            for j in range(len(detailinfo.keys())):
                key=detailinfo.keys()[j]
                detailinfo[key]=fetchers[i][j]
            result.append(detailinfo)
        return result

    def toJvedio(self,targetpath):
        ...
        for detailinfo in detailinfos:
            sqltext = "INSERT INTO movie(id,title,releasedate,director,genre,tag,actor,actorid," \
                      "studio,rating,plot,outline,year,runtime,sourceurl,source," \
                      "actressimageurl,smallimageurl,bigimageurl,extraimageurl) " \
                      f"values('{detailinfo['id']}','{detailinfo['title']}','{detailinfo['date']}','{detailinfo['director']}','{detailinfo['genres']}','{detailinfo['tag']}'," \
                      ...
                      f"ON CONFLICT(id) DO UPDATE SET title='{detailinfo['title']}',releasedate='{detailinfo['date']}',director='{detailinfo['director']}',genre='{detailinfo['genres']}',tag='{detailinfo['tag']}'," \
        ...
        for detailinfo in detailinfos:
            ...
            sqltext = f"insert into  javdb(id,code) values('{detailinfo['id']}','{link}') ON CONFLICT(id) DO UPDATE SET code='{link}'";
            cursor.execute(sqltext)
            conn.commit()
        conn.close()
