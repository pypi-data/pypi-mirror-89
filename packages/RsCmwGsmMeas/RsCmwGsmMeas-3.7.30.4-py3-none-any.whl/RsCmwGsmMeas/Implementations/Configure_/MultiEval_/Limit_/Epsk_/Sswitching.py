...
class Sswitching:
	...
	def get_plevel(self) -> PlevelStruct:
		...
		Snippet: value: PlevelStruct = driver.configure.multiEval.limit.epsk.sswitching.get_plevel() \n
		...
		"""
		return self._core.io.query_struct('CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SSWitching:PLEVel?', self.__class__.PlevelStruct())

	def set_plevel(self, value: PlevelStruct) -> None:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SSWitching:PLEVel \n
		Snippet: driver.configure.multiEval.limit.epsk.sswitching.set_plevel(value = PlevelStruct()) \n
		Define and activate reference power values for the modulation schemes 8PSK and 16-QAM. These values are relevant in the
		context of CONFigure:GSM:MEAS<i>:MEValuation:LIMit:GMSK:SSWitching:MPOint<no> and
		CONFigure:GSM:MEAS<i>:MEValuation:LIMit:QAM<m>:SSWitching:MPOint<no>. \n
			:param value: see the help for PlevelStruct structure arguments.
		"""
		self._core.io.write_struct('CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SSWitching:PLEVel', value)
