...
class Cp:
	...
	@property
	def isignal(self):
		"""isignal commands group. 5 Sub-classes, 0 commands."""
		if not hasattr(self, '_isignal'):
			from .Cp_.Isignal import Isignal
			self._isignal = Isignal(self._core, self._base)
		return self._isignal

	@property
	def qsignal(self):
		"""qsignal commands group. 5 Sub-classes, 0 commands."""
		if not hasattr(self, '_qsignal'):
			from .Cp_.Qsignal import Qsignal
