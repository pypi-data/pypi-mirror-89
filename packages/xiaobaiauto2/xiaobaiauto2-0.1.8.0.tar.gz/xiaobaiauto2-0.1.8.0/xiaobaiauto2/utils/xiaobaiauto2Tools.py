...
def compare_str(s0: Optional[str], s1: Optional[str], t: Optional[str], is_xiaobaiauto2: Optional[int] = 0):
    '''
...
def compare_dict(d0: Optional[dict], d1: Optional[dict], is_xiaobaiauto2: Optional[int] = 0) -> dict:
    ...
    :return:
    '''
    if not isinstance(d0, dict) and not isinstance(d1, dict):
        return {}
    diff = d0.keys() & d1
    ...
    if diffr.__len__() > 0:
        for v in diffr:
            if v[0] == 'method' and d0['method'] != d1['method']:
                ...
                try:
                    d0[v[0]] = compare_str(v[1], v[2], t=d0['headers']['content-type'], is_xiaobaiauto2=is_xiaobaiauto2)
                except KeyError as e:
                    try:
                        d0[v[0]] = compare_str(v[1], v[2], t=d0['headers']['Content-Type'], is_xiaobaiauto2=is_xiaobaiauto2)
                    except KeyError as e:
                        d0[v[0]] = compare_str(v[1], v[2], t='text', is_xiaobaiauto2=is_xiaobaiauto2)
    if diff_api:
        return {}
    else:
        d1.update(d0)
