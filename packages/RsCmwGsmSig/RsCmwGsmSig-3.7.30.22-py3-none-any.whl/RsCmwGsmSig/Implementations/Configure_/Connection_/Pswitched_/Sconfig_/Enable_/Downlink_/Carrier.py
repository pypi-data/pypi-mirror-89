...
class Carrier:
	...
	def set(self, enable: List[bool], carrier=repcap.Carrier.Default) -> None:
		...
		Snippet: driver.configure.connection.pswitched.sconfig.enable.downlink.carrier.set(enable = [True, False, True], carrier = repcap.Carrier.Default) \n
		Specifies the downlink timeslots the mobile has to use in a packet switched connection. Timeslot 0 cannot be enabled
		(always OFF) . \n
		...
		self._core.io.write(f'CONFigure:GSM:SIGNaling<Instance>:CONNection:PSWitched:SCONfig:ENABle:DL:CARRier{carrier_cmd_val} {param}')

	def get(self, carrier=repcap.Carrier.Default) -> List[bool]:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:CONNection:PSWitched:SCONfig:ENABle:DL:CARRier<Carrier> \n
		Snippet: value: List[bool] = driver.configure.connection.pswitched.sconfig.enable.downlink.carrier.get(carrier = repcap.Carrier.Default) \n
		Specifies the downlink timeslots the mobile has to use in a packet switched connection. Timeslot 0 cannot be enabled
		(always OFF) . \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
			:return: enable: OFF | ON List of 8 values for timeslot 0 to 7"""
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		response = self._core.io.query_str(f'CONFigure:GSM:SIGNaling<Instance>:CONNection:PSWitched:SCONfig:ENABle:DL:CARRier{carrier_cmd_val}?')
