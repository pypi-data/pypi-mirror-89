...
class CacheFileRequest(object):
    ...
    def get_cache_file(*args, **kwargs):
        ...
        if kwargs.get("request"):
            ...
            body = kwargs.get("request").body.decode("utf-8")
    ...
    def process_request(self, request, spider) -> process_request_type:
        ...
        if should_read_cache(request, spider):
            try:
                logger.debug(f"===读缓存===  {cache_file_path.split(os.sep)[-1]}   {request.url}")
                return pickle.load(open(cache_file_path, "rb"))
            except Exception as e:
                logger.debug(f"==缓存读取失败===  Reason is {e}")
                Fh.remove_file(cache_file_path)
                traceback.print_exc()
        else:
            # 不读缓存
            # logger.info("===不应该读缓存===   " + request.url)
            pass
    ...
    def process_response(self, request, response, spider) -> process_response_type:
            ...
            if exists_file:
                if exclude:
                    if self.cache_file_exclude_ts:
                        if time.time() - Fh.get_update_ts(cache_file_path) > self.cache_file_exclude_ts:
                            _ = f"排除缓存文件过期。需要重写。{cache_file_path}{Fh.get_update_ts(cache_file_path)}  {request.url}"
                            logger.debug(_)
                            return True
                        else:
                            _ = f"排除的URL没过期，不重写。过了{int(time.time() - Fh.get_update_ts(cache_file_path))} 秒？ {cache_file_path}"
                            logger.debug(_)
                            return False
                    else:
        ...
        if response.status < 300:
            ...
            try:
                ...
                logger.info(f"== Can't cache == Reason:{e}")
                Fh.remove_file(cache_file_path)
        else:
            logger.debug(f"===Status Code ERROR === Code is {response.status} " + response.url)
        return response
...
class CacheFileByDayRequest(object):
    ...
    def get_cache_response(self, request, spider):
        ...
        if Fh.is_file_exit(cache_file_path):
            try:
                logger.debug("===Read cache===\t" + request.url + "\t" + cache_file_path)
                return pickle.load(open(cache_file_path, "rb"))
            except Exception as e:
    ...
    def process_response(self, request, response, spider) -> process_response_type:
        if response.status == 200:
            ...
            if not cache_response:
                # 不存在缓存文件就要写入
                cache_file_path = self.cache_file_path_by_day(
                    url=request.url,
                    spider=spider.name,
                    root_path_cache=self.settings.get("ROOT_PATH_CACHE"))
...
class CacheRedisHashRequest(object):
    ...
    def __init__(self):
        from XX.DB.RedisHelper import RedisHelper as RH
        self.conn_redis7 = RH.get_redis_connect(
