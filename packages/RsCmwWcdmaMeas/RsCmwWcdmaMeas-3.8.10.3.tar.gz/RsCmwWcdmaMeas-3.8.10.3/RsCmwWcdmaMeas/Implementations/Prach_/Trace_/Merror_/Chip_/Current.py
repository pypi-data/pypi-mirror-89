...
class Current:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.prach.trace.merror.chip.current.fetch() \n
		Return the values of the magnitude error vs. chip diagram. See also 'Detailed Views: Modulation' \n
		Use RsCmwWcdmaMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: mag_error_chip: float Comma-separated list of 4096 values, one per chip of the preselected preamble Range: -100 % to 100 %, Unit: %"""
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.prach.trace.merror.chip.current.read() \n
		Return the values of the magnitude error vs. chip diagram. See also 'Detailed Views: Modulation' \n
		Use RsCmwWcdmaMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: mag_error_chip: float Comma-separated list of 4096 values, one per chip of the preselected preamble Range: -100 % to 100 %, Unit: %"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
