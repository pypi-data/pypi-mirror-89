...
class Sformat:
	...
	def get(self, stream=repcap.Stream.Default) -> str:
		...
		Snippet: value: str = driver.source.bb.tdscdma.up.cell.enh.dch.hsupa.sformat.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:ENH:DCH:HSUPA:SFORmat?')
