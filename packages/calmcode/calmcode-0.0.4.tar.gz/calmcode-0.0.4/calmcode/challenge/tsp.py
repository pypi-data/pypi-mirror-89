...
class TravellingSalesmanShip:
    """
    This challenge represents a TSP problem with an extra penalty for making large turns.
    """
    ...
    @classmethod
    def from_predefined(cls, name):
        """
        Generates a predefined problem.
        """
        names = {
            'sail-base': TravellingSalesmanShip(n_ports=250, shape="uniform", angle_penalty=1.75, seed=42)
        }
        return names[name]
    ...
    def check_solution(self, solution):
        ...
        """
        if "tour" not in solution.keys():
            raise ValueError("Your solution needs to have a key `tour`. It is currently missing.")
        tour = solution["tour"]
        ...
        if not all([v == 2 for v in double.values()]):
            ...
            extra = [k for k, v in Counter(tour).items() if v > 1]
            on_top = [k for k in tour if k not in all_ports]
            report = f"Incorrect submission. The following ports were visited more than once: {extra}. The following ports were missing {missing}. The following ports should not appear {on_top}."
