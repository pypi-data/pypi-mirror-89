...
from keras.layers import Input, Layer, Dense, Flatten, Activation, Conv2D, DepthwiseConv2D, MaxPooling2D, UpSampling2D, Reshape, Permute, Add, Subtract, Dot, Multiply, Average, Lambda, Concatenate, BatchNormalization, merge, RepeatVector, AveragePooling2D
...
class NN():
    ...
    def transition_model(self):
        ...
        if(self._high_int_dim==True):
            ...
            n_rows = dim[-2] // self._pooling_encoder
            n_cols = dim[-1] // self._pooling_encoder
            inputs = [ Input(shape=(n_rows,n_cols,self.n_channels_internal_dim)), Input( shape=(self._n_actions,) ) ]     # data_format='channels_last'
            
            layers_action=inputs[1]
            layers_action=RepeatVector(n_rows*n_cols*self.n_channels_internal_dim)(layers_action)
            #layers_action=Permute((2,1), input_shape=((dim[-2] // self._pooling_encoder)*(dim[-1] // self._pooling_encoder),self._n_actions))(layers_action)
            layers_action=Reshape((n_rows,n_cols,self.n_channels_internal_dim,self._n_actions))(layers_action)
            #layers_action=Permute((2,3,1), input_shape=(self._n_actions,(dim[-2] // self._pooling_encoder),(dim[-1] // self._pooling_encoder)))(layers_action)    #data_format='channels_last'
            
            #x = Concatenate(axis=-1)([layers_action,inputs[0]])
            
            x=inputs[0]

            #x = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=self._n_actions, activation='tanh')(x)
            #x = Reshape((n_rows, n_cols, self.n_channels_internal_dim, self._n_actions))(x)
            #x = Multiply()([x,layers_action])
            #x = Lambda(lambda x: K.sum(x,axis=-1), output_shape=(n_rows, n_cols, self.n_channels_internal_dim))(x)

            x = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=self._n_actions, activation='tanh')(x)
            x = Reshape((n_rows, n_cols, self.n_channels_internal_dim, self._n_actions))(x)
            x = Multiply()([x,layers_action])
            x1 = Lambda(lambda x: K.sum(x,axis=-1), output_shape=(n_rows, n_cols, self.n_channels_internal_dim))(x)
            
            #x = Conv2D(self.n_channels_internal_dim, (1, 1), padding='same')(x)
            layers_action=inputs[1]
            layers_action=RepeatVector(n_rows*n_cols)(layers_action)
            layers_action=Reshape((n_rows,n_cols,self._n_actions))(layers_action)
    ...
    def transition_model2(self):
        ...
        if(self._high_int_dim==True):
            ...
            features_layer1 = Lambda(lambda x: x[:,:,:,0:1], output_shape=((dim[-2] // self._pooling_encoder),(dim[-1] // self._pooling_encoder),1))(inputs[0])
            x_a = Concatenate(axis=-1)([layers_action,features_layer1])
            
            #x = Conv2D(8, (1, 1), padding='same', activation='tanh')(inputs[0])
            x = Conv2D(1, (3, 3), padding='same', activation='tanh')(x_a)
            ...
            xf1_1 = Conv2D(1, (1, 1), padding='same')(x) # x1 is independant on the action
            #xf1_1 = Lambda(lambda x: x * 0)(xf1_1)
            
            x = Conv2D(16, (3, 3), padding='same', activation='tanh')(x_a)
            x = Conv2D(32, (3, 3), padding='same', activation='tanh')(x)
            x = Conv2D(16, (3, 3), padding='same', activation='tanh')(x)
            x = Conv2D(4, (3, 3), padding='same', activation='tanh')(x)
            #x = Conv2D(8, (2, 2), padding='same', activation='tanh')(x)
            #x = Conv2D(8, (1, 1), padding='same', activation='tanh')(x)
            xf1_2 = Conv2D(1, (1, 1), padding='same')(x) # x2 is dependant on the action
            #xf1_2 = Lambda(lambda x: x * 0)(xf1_2)
            
#            x = Conv2D(self.n_channels_internal_dim, (1, 1), padding='same')(x)



            features_layer2 = Lambda(lambda x: x[:,:,:,1:2], output_shape=((dim[-2] // self._pooling_encoder),(dim[-1] // self._pooling_encoder),1))(inputs[0])
            x_a = Concatenate(axis=-1)([layers_action,features_layer2])
            
            x = Conv2D(1, (3, 3), padding='same', activation='tanh')(x_a)
            #x = Conv2D(2, (3, 3), padding='same', activation='tanh')(x)
            x = Conv2D(1, (3, 3), padding='same', activation='tanh')(x)
            xf2_1 = Conv2D(1, (1, 1), padding='same')(x) # x1 is independant on the action
            #xf2_1 = Lambda(lambda x: x * 0)(xf2_1)

            x = Conv2D(16, (3, 3), padding='same', activation='tanh')(x_a)
            x = Conv2D(32, (3, 3), padding='same', activation='tanh')(x)
            x = Conv2D(16, (3, 3), padding='same', activation='tanh')(x)
            x = Conv2D(4, (3, 3), padding='same', activation='tanh')(x)
            xf2_2 = Conv2D(1, (1, 1), padding='same')(x) # x2 is dependant on the action


            transition_1 = Concatenate(axis=-1)([ xf1_1 , xf2_1 ])
            transition_2 = Concatenate(axis=-1)([ xf1_1 , xf2_1 ]) #Add()([xf1_1,xf1_2]) , Add()([xf2_1,xf2_2]) ])

            x_approx1 = Add()([inputs[0],transition_1]) # x' with the simplest transition
            x_approx2 = Add()([inputs[0],transition_2]) # x' with the more complex transition
            
            model = Model(inputs=inputs, outputs=[x_approx1,x_approx2])
...
### OLD NN_CRAR_modif_keras
        x1 = Subtract()([Tx[0],enc_x_])
        x2 = Subtract()([Tx[1],enc_x_])
        x = Concatenate(axis=-1)([x1,x2]) # Concat the diff due to the different levels of complexity
