...
class EvMagnitude:
	...
	def set(self, structure: EvMagnitudeStruct, qamOrder=repcap.QamOrder.Default) -> None:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM<ModOrder>:EVMagnitude \n
		Snippet: driver.configure.multiEval.limit.qam.evMagnitude.set(value = [PROPERTY_STRUCT_NAME](), qamOrder = repcap.QamOrder.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:EVMagnitude', structure)

	def get(self, qamOrder=repcap.QamOrder.Default) -> EvMagnitudeStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM<ModOrder>:EVMagnitude \n
		Snippet: value: EvMagnitudeStruct = driver.configure.multiEval.limit.qam.evMagnitude.get(qamOrder = repcap.QamOrder.Default) \n
		Defines and activates upper limits for the RMS, peak and 95th percentile values of the error vector magnitude (EVM) . \n
			:param qamOrder: optional repeated capability selector. Default value: Nr16 (settable in the interface 'Qam')
			:return: structure: for return value, see the help for EvMagnitudeStruct structure arguments."""
		qamOrder_cmd_val = self._base.get_repcap_cmd_value(qamOrder, repcap.QamOrder)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:EVMagnitude?', self.__class__.EvMagnitudeStruct())
