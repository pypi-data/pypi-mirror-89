...
class NeuralAgent(object):
    ...
    def train(self):
        ...
        try:
            states, actions, rewards, next_states, terminals, rndValidIndices = self._dataSet.randomBatch(self._batchSize, True)
            loss, diff = self._network.train(states, actions, rewards, next_states, terminals)
            self._trainingLossAverages.append(loss)
            self._dataSet.update_priority(self, np.pow(diff,2), rndValidIndices)
...
class DataSet(object):
    ...
    def __init__(self, env, randomState=None, maxSize=1000, priority=False):
        """Initializer.
        ...
        if (priority):
            self._priority = CircularBuffer(maxSize)
    ...
    def priority(self):
        ...
        return self._priority.getSlice(0)
    ...
    def update_priority(self, priorities, rndValidIndices):
        """
        """
        self._priority.setSliceBySeq(dat=priorities, seq=rndValidIndices)
    ...
    def addSample(self, obs, action, reward, isTerminal, priority=False):
