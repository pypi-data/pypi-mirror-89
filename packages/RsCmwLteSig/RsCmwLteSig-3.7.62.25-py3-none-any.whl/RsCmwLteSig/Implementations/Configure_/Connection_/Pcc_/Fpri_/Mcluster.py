...
class Mcluster:
	...
	def get_downlink(self) -> DownlinkStruct:
		...
		Snippet: value: DownlinkStruct = driver.configure.connection.pcc.fpri.mcluster.get_downlink() \n
		...
		"""
		return self._core.io.query_struct('CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FPRI:MCLuster:DL?', self.__class__.DownlinkStruct())

	def set_downlink(self, value: DownlinkStruct) -> None:
		"""SCPI: CONFigure:LTE:SIGNaling<instance>:CONNection[:PCC]:FPRI:MCLuster:DL \n
		Snippet: driver.configure.connection.pcc.fpri.mcluster.set_downlink(value = DownlinkStruct()) \n
		Configures the downlink for the scheduling type 'Follow WB PMI-RI', with multi-cluster allocation. The allowed input
		ranges have dependencies and are described in the background information, see 'CQI Channels' and especially Table 'RBG
		parameters'. \n
			:param value: see the help for DownlinkStruct structure arguments.
		"""
		self._core.io.write_struct('CONFigure:LTE:SIGNaling<Instance>:CONNection:PCC:FPRI:MCLuster:DL', value)
