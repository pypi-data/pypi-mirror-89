...
class State:
	...
	def set(self, state: bool, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.impairment.iqOutput.state.set(state = False, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce:BB:IMPairment:IQOutput{channel_cmd_val}:STATe {param}')

	def get(self, channel=repcap.Channel.Default) -> bool:
		"""SCPI: [SOURce]:BB:IMPairment:IQOutput<CH>:STATe \n
		Snippet: value: bool = driver.source.bb.impairment.iqOutput.state.get(channel = repcap.Channel.Default) \n
		Activates the impairment or correction values LEAKage, QUADrature and IQRatio for the corresponding stream.
			INTRO_CMD_HELP: The suffix <ch> has the following values: \n
			- <ch>= 0: I/Q Analog Outputs
			- <ch>= 1: I/Q Modulator Digital Impairments \n
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce:BB:IMPairment:IQOutput{channel_cmd_val}:STATe?')
