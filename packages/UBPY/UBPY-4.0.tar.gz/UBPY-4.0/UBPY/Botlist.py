...
class UBPYsearch:
    ...
    async def searchuser(self, ID,page=1):
        ...
        headers = {"Authorization": f"Bearer {token}", "content-type": "application/json"}
        data = {
            "query": "query{user(id: "f'"{ID}"'"){bots(page:" f'{page}'"){result{tag, heartCount, discordVerified,guilds,id,status,brief,avatar,prefix,invite,locked,library,description}}}"}
        async with aiohttp.ClientSession() as cs:
            async with cs.post(url, headers=headers, json=data) as r:
                response = await r.read()
                sid = response.decode('utf-8')
