...
from alnlp.modules.endpoint_span_extractor import EndpointSpanExtractor
from alnlp.modules.gated_sum import GatedSum
from alnlp.modules.self_attentive_span_extractor import SelfAttentiveSpanExtractor
...
class CoreferenceResolver(torch.nn.Module):
    """
    ...
    def __init__(
            self,
            text_field_embedder: torch.nn.Embedding,
            context_layer: Seq2SeqEncoder,
            mention_feedforward: FeedForward,
            antecedent_feedforward: FeedForward,
            feature_size: int,
    ...
    def forward(
        ...
        text_embeddings = self._lexical_dropout(self._text_field_embedder(batch))
        ...
        text_mask = util.get_text_field_mask(batch) if mask is None else mask
        ...
        if span_labels is not None:
            ...
            self._mention_recall(top_spans, batch)
            self._conll_coref_scores(
                top_spans, top_antecedent_indices, predicted_antecedents, batch
            )

            output_dict["loss"] = negative_marginal_log_likelihood

        # if batch is not None:
        #     output_dict["document"] = [x["original_text"] for x in batch]
        return output_dict
