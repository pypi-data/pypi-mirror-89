...
class Elevation:
	...
	def get(self, time_basis: enums.TimeBasis, year: int, month: int, day: int, hour: int, minutes: int, seconds: float, week_number: int, time_of_week: float, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RT:GALileo<ST>:SVID<CH>:ELEVation \n
		Snippet: value: float = driver.source.bb.gnss.rt.galileo.svid.elevation.get(time_basis = enums.TimeBasis.BDT, year = 1, month = 1, day = 1, hour = 1, minutes = 1, seconds = 1.0, week_number = 1, time_of_week = 1.0, stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
