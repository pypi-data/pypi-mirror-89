...
class Pipeline:
    def __init__(self, lang, cache_dir=None, gpu=True):
        ...
        for lang in self.added_langs:
            ...
            if tbname2training_id[treebank_name] % 2 == 1:
                self._mwt_model[lang] = MWTWrapper(self._config, treebank_name=treebank_name, gpu_run=self._gpu)
        self._lemma_model = {}
        for lang in self.added_langs:
            treebank_name = lang2treebank[lang]
            self._lemma_model[lang] = LemmaWrapper(self._config, treebank_name=treebank_name, gpu_run=self._gpu)
        # ner if possible
        self._ner_model = {}
        for lang in self.added_langs:
    ...
    def _setup_config(self, lang):
        ...
        master_config.device = torch.device('cuda' if self._gpu and torch.cuda.is_available() else 'cpu')
            ...
            for t, wp_p in zip(para_text, wp_pred):
                ...
                if wp_p >= 1:
                    ...
                    if (wp_p == 2 or wp_p == 4):
                        ...
                        sentences.append(
                            {ID: len(sentences) + 1, TEXT: in_doc[sent_span[0]: sent_span[1]], DSPAN: sent_span})
                        current_sent = []
            ...
            for t, wp_p in zip(para_text, wp_pred):
                ...
                if wp_p >= 1:
                    ...
                    if (wp_p == 2 or wp_p == 4):
                        ...
                        doc.append({
                            ID: len(doc) + 1,
                            TEXT: in_doc[processed_sent[0][DSPAN][0]: processed_sent[-1][DSPAN][1]],
    ...
    def posdep(self, in_sent):  # assuming input is a sentence
        ...
        for batch in DataLoader(test_set, batch_size=tbname2tagbatchsize.get(self._config.treebank_name, 10),
                                shuffle=False, collate_fn=test_set.collate_fn):
    ...
    def __call__(self, in_doc):
        tokenized_doc = self.dtokenize(in_doc)
        tagged_doc = self.dposdep(tokenized_doc)
        final_doc = self.dlemmatize(tagged_doc)
        if self._config.active_lang in lang2nercorpus:  # ner if possible
            final_doc = self.dner(final_doc)
