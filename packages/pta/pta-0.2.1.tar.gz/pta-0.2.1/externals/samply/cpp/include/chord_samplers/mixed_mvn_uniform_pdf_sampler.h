...
class MixedMvnUniformPdfSampler : 
	public MvnPdfSampler<ScalarType>, public UniformPdfSampler<ScalarType> {
public:
	/**
	...
	SamplingHelper& sampling_helper)
{
	...
	if (non_degenerate_ray_ids.size() > 0)
		...
		t(non_degenerate_ray_ids) = MvnPdfSampler<Scalar>::sample1d(
			in_rays(non_degenerate_ray_ids),
			in_intersections(non_degenerate_ray_ids),
			sampling_helper
		);
	if (degenerate_ray_ids.size() > 0)
		...
		t(degenerate_ray_ids) = UniformPdfSampler<Scalar>::sample1d(
			in_rays(degenerate_ray_ids),
			in_intersections(degenerate_ray_ids),
			sampling_helper
		);
	return t;
}
...
MixedMvnUniformPdfSampler<ScalarType>::
	get_optimally_reparametrized_sampler() const
{
	AffineTransform<double> from_optimal_parametrization =
		vars_to_mvn_vars_.inverse() * 
		MvnPdfSampler<ScalarType>::samples_transform_;
	AffineTransform<double> to_optimal_parametrization =
		MvnPdfSampler<ScalarType>::samples_transform_.inverse() * 
		vars_to_mvn_vars_;
