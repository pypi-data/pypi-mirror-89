...
class Internal:
	...
	def depth(self):
		...
		if not hasattr(self, '_depth'):
			from .Internal_.Depth import Depth
			self._depth = Depth(self._core, self._base)
		return self._depth

	# noinspection PyTypeChecker
	def get_source(self) -> enums.AmSourceInt:
		"""SCPI: [SOURce<HW>]:AM:INTernal:SOURce \n
		Snippet: value: enums.AmSourceInt = driver.source.am.internal.get_source() \n
		No command help available \n
			:return: source: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:AM:INTernal:SOURce?')
		return Conversions.str_to_scalar_enum(response, enums.AmSourceInt)

	def set_source(self, source: enums.AmSourceInt) -> None:
		"""SCPI: [SOURce<HW>]:AM:INTernal:SOURce \n
		Snippet: driver.source.am.internal.set_source(source = enums.AmSourceInt.LF1) \n
		No command help available \n
			:param source: No help available
		"""
		param = Conversions.enum_scalar_to_str(source, enums.AmSourceInt)
		self._core.io.write(f'SOURce<HwInstance>:AM:INTernal:SOURce {param}')
