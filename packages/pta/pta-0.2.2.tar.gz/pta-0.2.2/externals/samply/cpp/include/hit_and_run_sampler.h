...
template<
	typename Scalar,
	template<typename> typename SpaceDescriptor,
	template<typename> typename ChordSampler,
	template<typename, typename> typename Logger>
class HitAndRunSampler : public MarkovChain<
	HitAndRunSampler<Scalar, SpaceDescriptor, ChordSampler,	Logger>, 
	Matrix<Scalar>, Matrix<Scalar>, Logger> {
public:
	/**
	 ...
	 */
	typedef MarkovChain<
		HitAndRunSampler, Matrix<Scalar>, Matrix<Scalar>, Logger> Base;
	...
	HitAndRunSampler(
		...
		, chord_sampler_Fs(
			sampler.get_optimally_reparametrized_sampler())
		, transforms_(
			space_descriptor_Fd, space_descriptor_Fi, chord_sampler_Fs)
	{
	}
...
protected:
	...
	InternalState next_state(const InternalState& state) override
	{
		...
		auto directions_Fd = sampling_helper.get_random_directions<Scalar>(
		...
		RaysPacket<Scalar> rays_Fi = {
			state,
			transforms_.transform_directions_from_Fd_to_Fi(directions_Fd)
		};
		...
		RaysPacket<Scalar> rays_Fs = transforms_.transform_rays_from_Fi_to_Fs(
			rays_Fi, directions_Fd);
		Vector<Scalar> t = chord_sampler_Fs.sample1d(
			rays_Fs, intersections, sampling_helper);
		space_descriptor_Fi.update_position(t);

		return rays_Fi.at(t);
	}

	State convert_from_internal_state(
		const InternalState& internal_state) override
	{
