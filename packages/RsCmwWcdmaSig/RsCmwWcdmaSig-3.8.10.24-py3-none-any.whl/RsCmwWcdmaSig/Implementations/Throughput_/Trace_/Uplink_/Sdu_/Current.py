...
class Current:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.throughput.trace.uplink.sdu.current.fetch() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WCDMa:SIGNaling<Instance>:THRoughput:TRACe:UL:SDU:CURRent?', suppressed)
		return response

	def read(self) -> List[float]:
		"""SCPI: READ:WCDMa:SIGNaling<instance>:THRoughput:TRACe:UL:SDU:CURRent \n
		Snippet: value: List[float] = driver.throughput.trace.uplink.sdu.current.read() \n
		Return the values of the uplink PDU and SDU throughput traces. The results of the current and average traces can be
		retrieved. The number of trace values N depends on the configured <update interval> and <window size>: N = integer
		(<window size> / <update interval>) \n
		Use RsCmwWcdmaSig.reliability.last_value to read the updated reliability indicator. \n
			:return: uplink_sdu: Comma-separated list of N throughput trace values Range: 0 bit/s to 100E+6 bit/s, Unit: bit/s"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:WCDMa:SIGNaling<Instance>:THRoughput:TRACe:UL:SDU:CURRent?', suppressed)
		return response
