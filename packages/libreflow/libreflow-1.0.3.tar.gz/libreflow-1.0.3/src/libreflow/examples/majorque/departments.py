...
class PlayLastBlastAction(baseflow.file.OpenWithDefaultApp):
    ...
    def get_buttons(self):
        ...
        if not parent.files.has_mapped_name('preview'):
            self.message.set("<font color=#D5000D>No <i>preview</i> folder in %s files</font>" % parent.name())
            return ['Cancel']

        if not self._files:
            self.message.set("<font color=#D5000D>No playblast in <i>preview</i> folder.</font>")
            return ['Cancel']
    ...
    def needs_dialog(self):
        ...
        contextual_dict = get_contextual_dict(parent, 'settings', ['ROOT_DIR', 'path'])
        preview_folder_path = os.path.join(contextual_dict['ROOT_DIR'], contextual_dict['path'], 'preview')
