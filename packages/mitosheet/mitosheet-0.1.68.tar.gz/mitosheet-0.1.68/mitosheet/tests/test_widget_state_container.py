...
def test_column_exists_error():
    mito_widget = sheet(pd.DataFrame({'A': [123]}), tutorial_mode=True)
    assert mito_widget.tutorial_mode == True

    mito_widget = sheet(pd.DataFrame({'A': [123]}), tutorial_mode=False)
    assert mito_widget.tutorial_mode == False
