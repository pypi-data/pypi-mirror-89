...
class Traces:
	"""Traces commands group definition. 1 total commands, 1 Sub-groups, 0 group commands
	Repeated Capability: Trace, default value after init: Trace.Ix1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("traces", core, parent)
	...
	@property
	def fthroughput(self):
		"""fthroughput commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_fthroughput'):
			from .Traces_.Fthroughput import Fthroughput
			self._fthroughput = Fthroughput(self._core, self._base)
		return self._fthroughput

	def clone(self) -> 'Traces':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Traces(self._core, self._base.parent)
