define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" placeholder="{2}"/>', 'vp_pyPow', 'vp-input', 'base/expression');
        tblLayoutRequire.addRow("Input Base", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyExp', 'vp-input');
        tblLayoutRequire.addRow("Exponent", sbTagString.toString());
