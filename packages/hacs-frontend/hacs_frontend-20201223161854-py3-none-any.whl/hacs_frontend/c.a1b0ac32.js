            ...
            ${e.sort((t,e)=>"name"===this._sortBy?t.name.toLocaleLowerCase()<e.name.toLocaleLowerCase()?-1:1:t[this._sortBy]>e[this._sortBy]?-1:1).slice(0,this._load).map(t=>a`<paper-icon-item
                ...
                >
                  ${"integration"===t.category?a`
                        ...
                        />
                      `:a`<ha-svg-icon .path=${d} slot="item-icon"></ha-svg-icon>`}
