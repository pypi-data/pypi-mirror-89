...
def fetchCrypto_Exchange(fsym,tsym,top=5):
    ...
    print("\nToday's top 5 active exchanges for",fsym+"/"+tsym,"(in "+tsym+"1,000)")
    for e in vol:
...
def compCrypto_Price(product1,product2,days=30):
    ...
    #图示标题
    titletxt="Recent Prices of Cryptocurrency Prodcucts"
...
def compCrypto_Return(product1,product2,days=30):
    ...
    #图示标题
    titletxt="Recent Returns of Cryptocurrency Prodcucts"
    plt.title(titletxt,fontweight='bold')
...
def eval_Roi(fsym,tsym,market1,market2,roi):
    ...
    product="Trade("+fsym+"/"+tsym+") "
    mktpair="Market pair ("+market1+", "+market2+")"
    plt.figure(figsize=(8,5))
    plt.plot(np.array(roi)*100, 'r')
    plt.xlabel("Trade No. (Total="+str(ttltrdnum)+")")
