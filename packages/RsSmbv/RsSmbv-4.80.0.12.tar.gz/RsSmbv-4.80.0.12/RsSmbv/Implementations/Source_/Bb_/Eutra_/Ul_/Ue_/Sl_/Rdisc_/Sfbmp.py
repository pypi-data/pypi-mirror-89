...
class Sfbmp:
	...
	def set(self, structure: SfbmpStruct, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.sl.rdisc.sfbmp.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> SfbmpStruct:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:SL:RDISc:SFBMp \n
		Snippet: value: SfbmpStruct = driver.source.bb.eutra.ul.ue.sl.rdisc.sfbmp.get(stream = repcap.Stream.Default) \n
		Sets the subframe bitmap. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
