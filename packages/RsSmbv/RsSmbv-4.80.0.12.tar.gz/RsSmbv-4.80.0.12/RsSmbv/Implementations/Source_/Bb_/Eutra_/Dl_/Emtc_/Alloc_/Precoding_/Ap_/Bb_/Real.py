...
class Real:
	...
	def get(self, channel=repcap.Channel.Default, antennaPort=repcap.AntennaPort.Default, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:EMTC:ALLoc<CH>:PRECoding:AP<DIR>:BB<ST>:REAL \n
		Snippet: value: float = driver.source.bb.eutra.dl.emtc.alloc.precoding.ap.bb.real.get(channel = repcap.Channel.Default, antennaPort = repcap.AntennaPort.Default, stream = repcap.Stream.Default) \n
