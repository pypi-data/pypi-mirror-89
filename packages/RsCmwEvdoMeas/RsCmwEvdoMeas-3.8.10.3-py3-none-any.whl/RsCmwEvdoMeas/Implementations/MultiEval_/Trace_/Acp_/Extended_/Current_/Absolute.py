...
class Absolute:
	...
	def read(self) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:EVDO:MEASurement<Instance>:MEValuation:TRACe:ACP:EXTended:CURRent:ABSolute?', suppressed)
		return response
	...
	def fetch(self) -> List[float]:
		...
		Use RsCmwEvdoMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: curr_acp: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:TRACe:ACP:EXTended:CURRent:ABSolute?', suppressed)
		return response

	def calculate(self) -> List[float]:
		"""SCPI: CALCulate:EVDO:MEASurement<instance>:MEValuation:TRACe:ACP:EXTended:CURRent:ABSolute \n
		Snippet: value: List[float] = driver.multiEval.trace.acp.extended.current.absolute.calculate() \n
		Returns the adjacent channel absolute power measured in dBm at a series of frequencies. The current, average and maximum
		traces can be retrieved. The frequencies are determined by the offset values defined via the commands method
		RsCmwEvdoMeas.Configure.MultiEval.Acp.Foffsets.lower and method RsCmwEvdoMeas.Configure.MultiEval.Acp.Foffsets.upper. All
		defined offset values are considered (irrespective of their activation status) . The values described below are returned
		...
		Use RsCmwEvdoMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: curr_acp: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:TRACe:ACP:EXTended:CURRent:ABSolute?', suppressed)
		return response
