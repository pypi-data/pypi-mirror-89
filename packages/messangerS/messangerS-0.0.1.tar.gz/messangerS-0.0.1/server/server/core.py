...
from common.utils import send_mes, get_mes
from common.variables import *
from common.deskriptors import PortServer
...
class MesProcessor(threading.Thread):
    '''
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                log_server.info(f'Установлено соедение с ПК {client_address}')
                client.settimeout(5)
            ...
            try:
                ...
                log_server.error(f'Ошибка работы с сокетами: {err.errno}')
                ...
                for client_with_message in recv_data_lst:
                    try:
                        self.process_client_message(
                            get_mes(client_with_message),
                            client_with_message
                        )
                    except (OSError, json.JSONDecodeError, TypeError) as err:
                        log_server.debug(f'Getting data from client exception.', exc_info=err)
    ...
    def remove_client(self, client):
        ...
        '''
        log_server.info(f'Клиент {client.getpeername()} отключился от сервера.')
    ...
    def process_message(self, message):
        ...
        if message[DESTINATION] in self.names and self.names[message[DESTINATION]] in self.listen_sockets:
            try:
                send_mes(self.names[message[DESTINATION]], message)
                log_server.info(
    ...
    def process_client_message(self, message, client):
        ...
        # Если это запрос контакт-листа
            ...
            response[LIST_INFO] = self.database.get_users_contact(message[USER])
            try:
                send_mes(client, response)
            except OSError:
                self.remove_client(client)
        ...
        # Если это запрос известных пользователей
            ...
            response[LIST_INFO] = [user[0]
                                   for user in self.database.all_users_list()]
            try:
                send_mes(client, response)
            except OSError:
                self.remove_client(client)
    ...
    def autorize_user(self, message, sock):
        ...
        log_server.debug(f'Start auth process for {message[USER]}')
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            response = RESPONSE_400
            response[ERROR] = 'Имя пользователя уже занято.'
            try:
                log_server.debug(f'Username busy, sending {response}')
                send_mes(sock, response)
            except OSError:
                log_server.debug('OS Error')
                pass
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
            ...
            digest = hash.digest()
            log_server.debug(f'Auth message = {message_auth}')
            try:
                # Обмен с клиентом
                send_mes(sock, message_auth)
                ans = get_mes(sock)
            except OSError as err:
                log_server.debug('Error in auth, data:', exc_info=err)
                sock.close()
                return
    ...
    def service_update_lists(self):
        ...
        for client in self.names:
            try:
                send_mes(self.names[client], RESPONSE_205)
            except OSError:
