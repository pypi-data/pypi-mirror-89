...
class Score(Stage):
    ...
    def process(self, pipeline: "Pipeline", inputs: DataFrame) -> DataFrame:
        def is_goal(team_id):
            return (F.col("event_type") == "SHOT") & (F.col("result") == "GOAL") & (F.col("team_id") == team_id)

        windowSpec = Window.orderBy("period_id", "timestamp").rangeBetween(Window.unboundedPreceding, 0)
