...
public:
	 ...
	 */
	ReparametrizedObject<UniformPdfSampler>
...
template<typename ScalarType>
UniformPdfSampler<ScalarType>::UniformPdfSampler(
	const Eigen::Index num_dimensions)
	...
	SamplingHelper& sampling_helper)
{
	...
	if (intersections.one_ray_one_intersection) {
		...
		// directly.
		t = sampling_helper.get_random_uniform(
			intersections.segment_starts, intersections.segment_ends);
	}
	else {
				...
				const Eigen::Index ray_index) -> Scalar
		{
			...
			auto segment_lengths =
				segment_ends - segment_starts;
			Eigen::Index sampled_segment_idx =
				sampling_helper.sample_index_with_weights<Scalar>(
					segment_lengths);
