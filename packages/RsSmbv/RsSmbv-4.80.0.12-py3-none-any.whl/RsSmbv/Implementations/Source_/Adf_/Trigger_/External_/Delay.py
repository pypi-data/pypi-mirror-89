...
class Delay:
	...
	def set(self, delay: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.adf.trigger.external.delay.set(delay = 1.0, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:ADF:TRIGger:EXTernal{channel_cmd_val}:DELay {param}')

	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SOURce<HW>]:ADF:TRIGger:[EXTernal<CH>]:DELay \n
		Snippet: value: float = driver.source.adf.trigger.external.delay.get(channel = repcap.Channel.Default) \n
		No command help available \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
			:return: delay: No help available"""
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:ADF:TRIGger:EXTernal{channel_cmd_val}:DELay?')
