...
class Downlink:
	...
	def set(self, tti: float, number_rb: int, start_rb: int, cqi_idx: int, stream=repcap.Stream.Default) -> None:
		...
		indicating that the queried subframe is no DL subframe. \n
			...
			:param number_rb: Number of allocated resource blocks. The same value must be configured for all streams of the carrier.
			:param start_rb: Position of first resource block. The same value must be configured for all streams of the carrier.
			:param cqi_idx: CQI index Range: 1 to 15
			:param stream: optional repeated capability selector. Default value: S1 (settable in the interface 'Downlink')"""
		param = ArgSingleList().compose_cmd_string(ArgSingle('tti', tti, DataType.Float), ArgSingle('number_rb', number_rb, DataType.Integer), ArgSingle('start_rb', start_rb, DataType.Integer), ArgSingle('cqi_idx', cqi_idx, DataType.Integer))
