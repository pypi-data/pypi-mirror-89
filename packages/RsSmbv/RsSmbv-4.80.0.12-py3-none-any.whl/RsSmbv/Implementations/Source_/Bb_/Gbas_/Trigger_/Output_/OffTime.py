...
class OffTime:
	...
	def set(self, off_time: int, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.gbas.trigger.output.offTime.set(off_time = 1, channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GBAS:TRIGger:OUTPut<CH>:OFFTime \n
		Snippet: value: int = driver.source.bb.gbas.trigger.output.offTime.get(channel = repcap.Channel.Default) \n
		Sets the number of symbols in a period (On time + Off time) for marker RATio \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
