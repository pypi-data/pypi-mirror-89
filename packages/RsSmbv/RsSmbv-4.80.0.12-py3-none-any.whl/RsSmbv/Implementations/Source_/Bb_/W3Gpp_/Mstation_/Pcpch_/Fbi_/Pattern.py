...
class Pattern:
	...
	def set(self, structure: PatternStruct, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.pcpch.fbi.pattern.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> PatternStruct:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:PCPCh:FBI:PATTern \n
		Snippet: value: PatternStruct = driver.source.bb.w3Gpp.mstation.pcpch.fbi.pattern.get(stream = repcap.Stream.Default) \n
		The command determines the bit pattern for the FBI field when the PATTern data source is selected. The maximum length of
		the pattern is 32 bits. The first parameter determines the bit pattern (choice of hexadecimal, octal or binary notation) ,
		the second specifies the number of bits to use. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
