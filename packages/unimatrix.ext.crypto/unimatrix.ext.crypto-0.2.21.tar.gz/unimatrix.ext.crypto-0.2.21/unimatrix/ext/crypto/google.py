...
class GoogleManagedKey(PrivateKey):
    ...
    async def decrypt(self, ct, padding):
        response = await self._kms.asymmetric_decrypt({
    ...
    async def encrypt(self, pt, padding):
        if not self._public:
            pub = await self._kms.get_public_key({'name': self.resource_id})
    ...
    async def sign(self, blob: bytes, algorithm, **kwargs) -> bytes:
        ...
        response = await self._kms.asymmetric_sign({
            'name': self.resource_id,
    ...
    async def _get_crypto_key(self):
        f = self._kms.get_crypto_key\
            if not self.opts.get('version')\
            else self._kms.get_crypto_key_version
