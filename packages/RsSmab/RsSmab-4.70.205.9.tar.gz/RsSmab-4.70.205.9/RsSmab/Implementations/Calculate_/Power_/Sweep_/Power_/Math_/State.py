...
class State:
	...
	def set(self, state: bool, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.calculate.power.sweep.power.math.state.set(state = False, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'CALCulate:POWer:SWEep:POWer:MATH{channel_cmd_val}:STATe {param}')

	def get(self, channel=repcap.Channel.Default) -> bool:
		"""SCPI: CALCulate:[POWer]:SWEep:POWer:MATH<CH>:STATe \n
		Snippet: value: bool = driver.calculate.power.sweep.power.math.state.get(channel = repcap.Channel.Default) \n
		Activates the trace mathematics mode for 'Power' measurement. This feature enables you to calculate the difference
		between the measurement values of two traces. For further calculation, a math result can also be assigned to a trace. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Math')
			:return: state: 0| 1| OFF| ON"""
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'CALCulate:POWer:SWEep:POWer:MATH{channel_cmd_val}:STATe?')
