...
class Mode:
	...
	def set(self, mode: enums.SystConfOutpMapMatMode, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.sconfiguration.output.mapping.rf.mode.set(mode = enums.SystConfOutpMapMatMode.ADD, channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> enums.SystConfOutpMapMatMode:
		"""SCPI: SCONfiguration:OUTPut:MAPPing:RF<CH>:MODE \n
		Snippet: value: enums.SystConfOutpMapMatMode = driver.sconfiguration.output.mapping.rf.mode.get(channel = repcap.Channel.Default) \n
		No command help available \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Rf')
