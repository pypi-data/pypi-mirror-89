...
def JMETER_SCRIPT_HEADER(key, value):
    return f'''
              <elementProp name="" elementType="Header">
                <stringProp name="Header.name">{escape(key)}</stringProp>
                <stringProp name="Header.value">{escape(value)}</stringProp>
              </elementProp>'''
...
def JMETER_EQUEST(file_path: Optional[str] = '',
    ...
    if HTTPer.query != '':
        if HTTPer.fragment != '':
            path = HTTPer.path + '?' + HTTPer.query + '#' + HTTPer.fragment
        else:
            path = HTTPer.path + '?' + HTTPer.query
    else:
        path = HTTPer.path
...
def create_jmeter(
    ...
    latest = getWebList(url='https://mirror.bit.edu.cn/apache/jmeter/binaries/HEADER.html',
                        match='Apache JMeter (.+?) requires')
    version = latest[0] if len(latest) > 0 else jmeter_version
    if request is None:
        ...
        for i, v in enumerate(request):
            if isinstance(v, dict):
                code += JMETER_EQUEST(post_value=v.get('data'), url=v.get('url'), method=v.get('method'),
