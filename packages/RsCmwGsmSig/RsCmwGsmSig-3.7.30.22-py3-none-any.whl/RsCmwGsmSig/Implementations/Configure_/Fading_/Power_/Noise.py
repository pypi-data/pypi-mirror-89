...
class Noise:
	...
	def get_total(self) -> float:
		...
		"""
		response = self._core.io.query_str('CONFigure:GSM:SIGNaling<Instance>:FADing:POWer:NOISe:TOTal?')
	...
	def get_value(self) -> float:
		...
		Queries the calculated noise power on the downlink channel. \n
			:return: noise_power: Unit: dBm
		"""
		response = self._core.io.query_str('CONFigure:GSM:SIGNaling<Instance>:FADing:POWer:NOISe?')
		return Conversions.str_to_float(response)
