...
class Dynamic:
	...
	def set(self, structure: DynamicStruct, qamOrder=repcap.QamOrder.Default, usefulPart=repcap.UsefulPart.Default, rangePcl=repcap.RangePcl.Default) -> None:
		...
		Snippet: driver.configure.multiEval.limit.qam.powerVsTime.lower.upart.dynamic.set(value = [PROPERTY_STRUCT_NAME](), qamOrder = repcap.QamOrder.Default, usefulPart = repcap.UsefulPart.Default, rangePcl = repcap.RangePcl.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:PVTime:LOWer:UPARt{usefulPart_cmd_val}:DYNamic{rangePcl_cmd_val}', structure)
	...
	def get(self, qamOrder=repcap.QamOrder.Default, usefulPart=repcap.UsefulPart.Default, rangePcl=repcap.RangePcl.Default) -> DynamicStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM<ModOrder>:PVTime:LOWer:UPARt<nr>:DYNamic<Range> \n
		Snippet: value: DynamicStruct = driver.configure.multiEval.limit.qam.powerVsTime.lower.upart.dynamic.get(qamOrder = repcap.QamOrder.Default, usefulPart = repcap.UsefulPart.Default, rangePcl = repcap.RangePcl.Default) \n
		These commands define and activate dynamic (PCL-dependent) corrections to the lower limit lines for the measured power vs.
		time. The corrections apply to the modulation schemes GMSK, 8PSK (EPSK) or 16-QAM (QAM16) . Each limit line section can
		consist of different areas (<no>) . Each dynamic correction is defined for up to five different PCL ranges (<Range>) ).
		In the default configuration, the dynamic corrections for all lower limit lines are set to zero and disabled. \n
		...
		usefulPart_cmd_val = self._base.get_repcap_cmd_value(usefulPart, repcap.UsefulPart)
		rangePcl_cmd_val = self._base.get_repcap_cmd_value(rangePcl, repcap.RangePcl)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:PVTime:LOWer:UPARt{usefulPart_cmd_val}:DYNamic{rangePcl_cmd_val}?', self.__class__.DynamicStruct())
