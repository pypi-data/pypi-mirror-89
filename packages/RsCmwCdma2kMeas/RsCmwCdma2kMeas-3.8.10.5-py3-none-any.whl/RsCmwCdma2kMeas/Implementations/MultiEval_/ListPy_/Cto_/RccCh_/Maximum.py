...
class Maximum:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.listPy.cto.rccCh.maximum.fetch() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:CTO:RCCCh:MAXimum?', suppressed)
		return response

	def calculate(self) -> List[float]:
		"""SCPI: CALCulate:CDMA:MEASurement<Instance>:MEValuation:LIST:CTO:RCCCh:MAXimum \n
		Snippet: value: List[float] = driver.multiEval.listPy.cto.rccCh.maximum.calculate() \n
		Returns the time offset (statistical values) of the reverse common control channel for all active list mode segments. The
		values described below are returned by FETCh commands. CALCulate commands return limit check results instead, one value
		for each result listed below. \n
		Use RsCmwCdma2kMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: rcc_ch: float Comma-separated list of values, one per active segment. Range: -50 ns to 50 ns, Unit: s"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:LIST:CTO:RCCCh:MAXimum?', suppressed)
		return response
