...
class Device(object):
    ...
    def get_status(self):
        ...
        #Or, check if the device is offline
        status = resp_json["status"] #Get the status
        if status == "offline": #If the device is offline
            raise DeviceError("The requested device is offline.")
        ...
        try:
            ...
            self.status = Status(
                status=status, #The device status
        ...
        except Exception as e: #If something went wrong while initializing the status object
            raise ResponseError(f"Response \"{resp_json}\" could not be converted into a status object.")
