...
class Ploss:
	...
	def set(self, path_loss_state: bool, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:NAVic<ST>:POWer:PLOSs \n
		Snippet: driver.source.bb.gnss.svid.navic.power.ploss.set(path_loss_state = False, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:NAVic<ST>:POWer:PLOSs \n
		Snippet: value: bool = driver.source.bb.gnss.svid.navic.power.ploss.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		If enabled, the power of the SV ID signals is reduced to account for the free space attenuation. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Navic')
