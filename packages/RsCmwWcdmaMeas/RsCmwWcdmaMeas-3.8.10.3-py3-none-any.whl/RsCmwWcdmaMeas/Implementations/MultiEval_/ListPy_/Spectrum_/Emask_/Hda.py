...
class Hda:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("hda", core, parent)

	@property
	def current(self):
		"""current commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_current'):
			from .Hda_.Current import Current
	...
	def average(self):
		...
		if not hasattr(self, '_average'):
			from .Hda_.Average import Average
	...
	def maximum(self):
		...
		if not hasattr(self, '_maximum'):
			from .Hda_.Maximum import Maximum
	...
	def clone(self) -> 'Hda':
		...
		new_group = Hda(self._core, self._base.parent)
