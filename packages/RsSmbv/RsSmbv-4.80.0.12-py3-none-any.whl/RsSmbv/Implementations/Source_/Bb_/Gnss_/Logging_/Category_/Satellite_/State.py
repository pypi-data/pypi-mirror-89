...
class State:
	...
	def set(self, state: bool, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:LOGGing:CATegory:SATellite:STATe<ST> \n
		Snippet: driver.source.bb.gnss.logging.category.satellite.state.set(state = False, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:GNSS:LOGGing:CATegory:SATellite:STATe<ST> \n
		Snippet: value: bool = driver.source.bb.gnss.logging.category.satellite.state.get(stream = repcap.Stream.Default) \n
		Enables the logging of the selected category. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'State')
