...
class FormatPy:
	...
	def set(self, position_format: enums.PositionFormat, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GPS<ST>:COORdinates:FORMat \n
		Snippet: driver.source.bb.gnss.adGeneration.gps.coordinates.formatPy.set(position_format = enums.PositionFormat.DECimal, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS{stream_cmd_val}:COORdinates:FORMat {param}')

	# noinspection PyTypeChecker
	def get(self, stream=repcap.Stream.Default) -> enums.PositionFormat:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GPS<ST>:COORdinates:FORMat \n
		Snippet: value: enums.PositionFormat = driver.source.bb.gnss.adGeneration.gps.coordinates.formatPy.get(stream = repcap.Stream.Default) \n
		Sets the format in which the coordinates of the reference location are set. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Gps')
			:return: position_format: DMS| DECimal"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS{stream_cmd_val}:COORdinates:FORMat?')
