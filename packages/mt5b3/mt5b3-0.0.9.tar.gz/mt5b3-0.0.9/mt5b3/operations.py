...
def runMultiTraders(traders,ops):
    if traders==None: # or type(trader)!=b3.Trader:
        print("Error! Traders should be a list of object of class mt5b3.Trader or its subclass")
        return False
    dbars=startOps(ops)
    ...
    while not endedOps(ops):
        for trader in traders:
            ...
            executeOrders(orders,ops,dbars) # problema registro de ordem para multitraders
        dbars=getCurrBars(ops,dbars)
        time.sleep(delay)
    print('End of operation saving equity file in ',ops['file'])
    for trader in traders:
        trader.ending(dbars)
    df=saveEquityFile(ops) # problema para multitraders
    return df
