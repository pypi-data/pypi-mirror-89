...
class State:
	...
	def read(self, sequence=repcap.Sequence.Default) -> List[enums.StatePower]:
		...
		Snippet: value: List[enums.StatePower] = driver.oltr.sequence.trace.up.state.read(sequence = repcap.Sequence.Default) \n
		...
		Use RsCmwCdma2kMeas.reliability.last_value to read the updated reliability indicator. \n
			...
			:return: state_up_power: No help available"""
		sequence_cmd_val = self._base.get_repcap_cmd_value(sequence, repcap.Sequence)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:CDMA:MEASurement<Instance>:OLTR:SEQuence{sequence_cmd_val}:TRACe:UP:STATe?', suppressed)
	...
	def fetch(self, sequence=repcap.Sequence.Default) -> List[enums.StatePower]:
		...
		Snippet: value: List[enums.StatePower] = driver.oltr.sequence.trace.up.state.fetch(sequence = repcap.Sequence.Default) \n
		For each sequence, state commands return limit violation results 'State Down/Up Power' in 20 equidistant parts of the 100
		ms interval following the power down/up step. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
		Use RsCmwCdma2kMeas.reliability.last_value to read the updated reliability indicator. \n
		...
		sequence_cmd_val = self._base.get_repcap_cmd_value(sequence, repcap.Sequence)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'FETCh:CDMA:MEASurement<Instance>:OLTR:SEQuence{sequence_cmd_val}:TRACe:UP:STATe?', suppressed)
		return Conversions.str_to_list_enum(response, enums.StatePower)
	...
	def calculate(self, sequence=repcap.Sequence.Default) -> List[enums.ResultStatus2]:
		...
		Snippet: value: List[enums.ResultStatus2] = driver.oltr.sequence.trace.up.state.calculate(sequence = repcap.Sequence.Default) \n
		For each sequence, state commands return limit violation results 'State Down/Up Power' in 20 equidistant parts of the 100
		ms interval following the power down/up step. The values described below are returned by FETCh and READ commands.
		CALCulate commands return limit check results instead, one value for each result listed below. \n
		Use RsCmwCdma2kMeas.reliability.last_value to read the updated reliability indicator. \n
		...
		sequence_cmd_val = self._base.get_repcap_cmd_value(sequence, repcap.Sequence)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:CDMA:MEASurement<Instance>:OLTR:SEQuence{sequence_cmd_val}:TRACe:UP:STATe?', suppressed)
