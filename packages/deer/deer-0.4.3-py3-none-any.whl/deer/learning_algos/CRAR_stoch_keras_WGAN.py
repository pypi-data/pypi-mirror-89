...
def exp_dec_error(y_true, y_pred):
    return K.exp( - K.sqrt( K.clip(K.sum(K.square(y_pred), axis=-1, keepdims=True),0.000001,10) )  ) # tend to increase y_pred
...
class CRAR(LearningAlgo):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_norm=0, freeze_interval=1000, batch_size=32, update_rule="rmsprop", random_state=np.random.RandomState(), double_Q=False, neural_network=NN, **kwargs):
        ...
        self.encoder_g_full2 = self.learn_and_plan.encoder_g_full_model(self.encoder, self.transition_d)
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        terminals_val = terminals_val.astype('float') #FIXME
        noise = np.random.normal(0, 1, (self._batch_size,8,8,3)) #FIXME
        ...
        if(self.update_counter%500==0):
            ...
            #fig = plt.figure()
            plt.subplot(2,3,1)
            plt.imshow(states_val[0][0,0])
            plt.subplot(2,3,4)
            plt.imshow(Es[0])
            plt.subplot(2,3,5)
            plt.imshow(ETs[0])
            plt.subplot(2,3,3)
            plt.imshow(next_states_val[0][0,0])
            plt.subplot(2,3,6)
            plt.imshow(Es_[0])
        ...
        true_transitions=np.concatenate(  ( np.ones((self._batch_size//2,1)) , -np.ones((self._batch_size//2,1)) )  ,  axis=0  )
        ...
        rewards_val_noise=rewards_val+np.random.normal(0,0.4,rewards_val.shape)
        terminals_val_noise=terminals_val+np.random.normal(0,0.4,terminals_val.shape)
        ...
        if (self.wait_D==0):
            ...
            if(self.loss_D<0.6 and self.loss_G>0.8):
                self.wait_D=4
            elif(self.loss_D<0.5 and self.loss_G>0.9):
                self.wait_D=16
            elif(self.loss_D<0.3 and self.loss_G>1.5):
                self.wait_D=64
        else:
            self.wait_D-=1
        ...
        if (self.wait_G==0):
            ...
            self.loss_G2=0.95*self.loss_G+0.05*self.encoder_g_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[terminals_val,rewards_val], -np.ones((self._batch_size,1)) )
            self.transition_g_full2.train_on_batch(states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) )
            #self.encoder_g_full2.train_on_batch(states_val+[onehot_actions]+next_states_val+[terminals_val,rewards_val], np.ones((self._batch_size,1))*0.0001 )
            if(self.loss_G<0.6 and self.loss_D>0.8):
                self.wait_G=4
            elif(self.loss_G<0.5 and self.loss_D>0.9):
                self.wait_G=16
            elif(self.loss_G<0.3 and self.loss_D>1.5):
                self.wait_G=64
        else:
            self.wait_G-=1
            #print ("self.loss_G")
...
#        ## END DEBUG PURPOSES
        ...
        QD_plan=0
        for i in range(d+1):
            Qd=self.qValues_planning_abstr(encoded_x, transition, Q, d=i, branching_factor=[self._n_actions,2,2,2,2,2,2,2], repeat_factor=[3,3,3,3,3,3,3,3]).reshape(len(encoded_x),-1)
            print ("Qd,i")
            print (Qd,i)
            QD_plan+=Qd
        QD_plan=QD_plan/(d+1)
    ...
    def qValues_planning_abstr(self, state_abstr_val, transition, Q, d, branching_factor=None, repeat_factor=None):
        ...
        q_values=Q.predict(state_abstr_val)
        #print ("q_values[:5]")
        #print (q_values[:5])
    ...
    def _compile(self):
        ...
        if (self._update_rule=="sgd"):
            ...
            optimizer2=SGD(lr=self._lr, momentum=self._momentum, nesterov=False, clipnorm=self._clip_norm) # to possibly modify them separately
            ...
            optimizer2=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipnorm=self._clip_norm) # to possibly modify them separately
            optimizer2b=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipnorm=self._clip_norm) # to possibly modify them separately
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.transition_g_full.optimizer.lr, self._lr/100.)
        K.set_value(self.encoder_g_full.optimizer.lr, self._lr/100.)
        K.set_value(self.transition_g_full2.optimizer.lr, self._lr)
        K.set_value(self.encoder_g_full2.optimizer.lr, self._lr)
        ...
        K.set_value(self.diff_s_s_.optimizer.lr, self._lr*2) # /5. for simple laby or simple catcher; /1. for distrib of laby
