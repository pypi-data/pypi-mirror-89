...
class RvSequence:
	...
	def set(self, rv_sequence: str, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.up.cell.enh.dch.hsupa.rvSequence.set(rv_sequence = '1', stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:ENH:DCH:HSUPA:RVSequence \n
		Snippet: value: str = driver.source.bb.tdscdma.up.cell.enh.dch.hsupa.rvSequence.get(stream = repcap.Stream.Default) \n
		For HARQ mode set to constant NACK, sets the retransmission sequence. For HSUPA, the command is a query only. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
