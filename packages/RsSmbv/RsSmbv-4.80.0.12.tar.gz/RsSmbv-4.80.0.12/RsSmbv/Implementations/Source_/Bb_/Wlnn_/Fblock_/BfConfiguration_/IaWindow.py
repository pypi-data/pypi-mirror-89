...
class IaWindow:
	...
	def set(self, iawindow: List[str], channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.wlnn.fblock.bfConfiguration.iaWindow.set(iawindow = ['raw1', 'raw2', 'raw3'], channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> List[str]:
		"""SCPI: [SOURce<HW>]:BB:WLNN:FBLock<CH>:BFConfiguration:IAWindow \n
		Snippet: value: List[str] = driver.source.bb.wlnn.fblock.bfConfiguration.iaWindow.get(channel = repcap.Channel.Default) \n
		Sets the parameters necessary to support an IBSS (2 bytes) . The Information field contains the ATIM Window parameter. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fblock')
