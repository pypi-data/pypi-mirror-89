...
class Minimum:
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.dwcp.wtfi.minimum.fetch() \n
		...
		return self._core.io.query_struct(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:DWCP:WTFI:MINimum?', self.__class__.FetchStruct())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.dwcp.wtfi.minimum.calculate() \n
		Returns the scalar channel power for the data channel results for all active list mode segments. WOT represents W12
		(Walsh code one-two) and WTF represents W24 (Walsh code two-four) . The values described below are returned by FETCh
		commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:DWCP:WTFI:MINimum?', self.__class__.CalculateStruct())
