...
class AffineTransform : public AffineTransformBase<
	AffineTransform<Scalar, Storage>, Storage> {
public:
	using AffineTransformBase<AffineTransform<Scalar, Storage>, Storage>::AffineTransformBase;
	 ...
	 */
	AffineTransform(
		const Matrix<Scalar>& T, const Vector<Scalar>& shift);
	 ...
	 */
	template<typename OtherTransform, typename OtherStorage>
	auto operator*(const AffineTransformBase<
		OtherTransform, OtherStorage>& transform) const;
...
inline AffineTransform<Scalar, Storage>::AffineTransform(
	const Matrix<Scalar>& T, const Vector<Scalar>& shift)
	: AffineTransformBase<
		AffineTransform<Scalar>, AffineTransformStorage<Scalar>>(T, shift)
{
...
inline AffineTransform<Scalar, Storage>::AffineTransform(
	AffineTransformBase<OtherTransform, OtherStorage>&& other)
	: AffineTransformBase<AffineTransform<Scalar>, AffineTransformStorage<Scalar>>(std::move(other))
{
	reset_transform_type_();
}
	...
	const RaysPacket<Scalar>& rays) const
{
	...
	case TransformType_::AffineDiagonal:
		return RaysPacket<Scalar>{
			asAffineDiagonal_() * rays.origins,
	...
	case TransformType_::Dense:
		return RaysPacket<Scalar>{
			asDense_() * rays.origins,
			asLinear_() * rays.directions
		};
		break;
	case TransformType_::Identity:
		return RaysPacket<Scalar>{
			asIdentity_() * rays.origins,
			asIdentity_() * rays.directions
		};
		break;
	case TransformType_::Linear:
		return RaysPacket<Scalar>{
			asLinear_() * rays.origins,
			asLinear_() * rays.directions
		};
		break;
	case TransformType_::LinearDiagonal:
		return RaysPacket<Scalar>{
			asLinearDiagonal_() * rays.origins,
			asLinearDiagonal_() * rays.directions
		};
		break;
	case TransformType_::Translation:
		return RaysPacket<Scalar>{
			asTranslation_() * rays.origins,
			rays.directions
		};
		break;
	case TransformType_::Reset:
		return RaysPacket<Scalar>{
			asReset_() * rays.origins,
			asZero_() * rays.directions
		};
		break;
	case TransformType_::Zero:
		return RaysPacket<Scalar>{
			asZero_() * rays.origins,
			asZero_() * rays.directions
		};
		break;
	}
...
{
	return AffineTransform<Scalar>(this->storage_.get_linear(),
...
template<typename Scalar, typename Storage>
template<typename CastScalar>
inline AffineTransform<CastScalar>
AffineTransform<Scalar, Storage>::cast() const
{
...
{
	return AffineTransform<Scalar>(
		Matrix<Scalar>::Identity(n_dimensions, n_dimensions),
