...
#filepath = os.path.join(basepath, 'test.txt')`);
  ...
  var sync_bby_lesson = function () {
    const pathRegRes = /userdata\/lessons-(\d+)-(\w+)\.ipynb/.exec(
    ...
    const domain = isDev ? 'dev.boyuai.com' : 'boyuai.com';
    ...
    $.ajax({
      ...
      data: JSON.stringify(body), // Some data e.g. Valid JSON as a string
  ...
  var addButtons = function () {
    ...
    if (
      /userdata\/lessons-(\d+)-(\w+)\.ipynb/.test(
        Jupyter.notebook.notebook_path
      )
    ) {
