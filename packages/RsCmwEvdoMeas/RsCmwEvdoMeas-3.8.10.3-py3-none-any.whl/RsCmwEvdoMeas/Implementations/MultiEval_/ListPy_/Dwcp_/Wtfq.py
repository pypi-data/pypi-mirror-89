...
class Wtfq:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("wtfq", core, parent)

	@property
	def current(self):
		"""current commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_current'):
			from .Wtfq_.Current import Current
	...
	def average(self):
		...
		if not hasattr(self, '_average'):
			from .Wtfq_.Average import Average
	...
	def maximum(self):
		...
		if not hasattr(self, '_maximum'):
			from .Wtfq_.Maximum import Maximum
	...
	def minimum(self):
		...
		if not hasattr(self, '_minimum'):
			from .Wtfq_.Minimum import Minimum
	...
	def clone(self) -> 'Wtfq':
		...
		new_group = Wtfq(self._core, self._base.parent)
