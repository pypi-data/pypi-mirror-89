...
class IpAddress:
	...
	def set(self, index: enums.IpAddrIndex) -> None:
		...
		Selects the IP address to which signaling messages have to be sent for message monitoring. The address pool is configured
		...
		self._core.io.write(f'CONFigure:GSM:SIGNaling<Instance>:MMONitor:IPADdress {param}')
	...
	def get(self) -> GetStruct:
		...
		return self._core.io.query_struct(f'CONFigure:GSM:SIGNaling<Instance>:MMONitor:IPADdress?', self.__class__.GetStruct())
