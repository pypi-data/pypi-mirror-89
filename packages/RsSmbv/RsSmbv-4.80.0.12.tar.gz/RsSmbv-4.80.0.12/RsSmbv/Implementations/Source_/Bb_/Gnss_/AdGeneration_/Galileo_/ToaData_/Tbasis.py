...
class Tbasis:
	...
	def set(self, time_basis: enums.TimeBasis, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:TOAData:TBASis \n
		Snippet: driver.source.bb.gnss.adGeneration.galileo.toaData.tbasis.set(time_basis = enums.TimeBasis.BDT, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.TimeBasis:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GALileo<ST>:TOAData:TBASis \n
		Snippet: value: enums.TimeBasis = driver.source.bb.gnss.adGeneration.galileo.toaData.tbasis.get(stream = repcap.Stream.Default) \n
		Determines the timebase used to enter the time of assistance data parameters. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Galileo')
