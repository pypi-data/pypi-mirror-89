...
class Awin:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("awin", core, parent)

	def set(self, window_size: int) -> None:
		"""SCPI: CONFigure:CDMA:SIGNaling<Instance>:NETWork:SYSTem:AWIN \n
		Snippet: driver.configure.network.system.awin.set(window_size = 1) \n
		...
		self._core.io.write(f'CONFigure:CDMA:SIGNaling<Instance>:NETWork:SYSTem:AWIN {param}')
	...
	def get(self) -> GetStruct:
		"""SCPI: CONFigure:CDMA:SIGNaling<Instance>:NETWork:SYSTem:AWIN \n
		Snippet: value: GetStruct = driver.configure.network.system.awin.get() \n
			INTRO_CMD_HELP: Search window size (index) for \n
			- The active set and candidate set (SRCH_WIN_Asystem parameter → AWIN suffix)
			- The neighbor set (SRCH_WIN_N system parameter → NWIN suffix)
			- The remaining set (SRCH_WIN_R system parameter → RWIN suffix)
		...
		return self._core.io.query_struct(f'CONFigure:CDMA:SIGNaling<Instance>:NETWork:SYSTem:AWIN?', self.__class__.GetStruct())
