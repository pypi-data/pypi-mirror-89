...
class S(Server):
    ...
    async def init_gui(self):
        ...
        if (await self.mud.getMapUserData("room.ui_showName"))[0] is None:
            await self.mud.setMapUserData("room.ui_showName","1")
    ...
    async def set_room_label(self, r):
        await self.mud.setRoomName(r.id_mudlet, r.name)
        await self.mud.setRoomUserData(r.id_mudlet, "room.ui_showName","1")
        await self.mud.setRoomUserData(r.id_mudlet, "room.ui_nameOffset","%f %f" % (r.label_x,r.label_y))
    ...
    async def called_label_shift(self, dx, dy):
        ...
        for r in rooms:
            ...
            try:
                ...
                x += dx; y += dy
                await self.mud.setRoomUserData(r.id_mudlet, "room.ui_nameOffset","%f %f" % x, y)
