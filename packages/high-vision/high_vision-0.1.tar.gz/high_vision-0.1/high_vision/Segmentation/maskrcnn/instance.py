...
class instance_segmentation():
    ...
    def process_camera(self, cam, show_bboxes = True,  output_video_name = None, frames_per_second = 25, 
                       show_frames = True, frame_name = "Output", verbose = 0, check_fps = True, custom=None):
        capture = cam
        width = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
        codec = cv2.VideoWriter_fourcc(*'DIVX')
