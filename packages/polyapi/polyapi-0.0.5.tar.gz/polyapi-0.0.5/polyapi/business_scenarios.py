...
class BusinessLogic:
    ...
    def clone_current_olap_module(self) -> Dict:
        ...
        :return: (str) self.multisphere_module_id
        """
    ...
    def measure_rename_group(self, curr_name: str, new_name: str) -> [Dict, str]:
        """
        Переименовать группу фактов. [ID-2992]
        ...
        # список всех фактов мультисферы
        res = self.execute_olap_command(
            command_name="fact",
            state="list_rq"
        )

        measures_list = self.h.parse_result(res, "facts")

        # если в мультисфере есть такая такая группа фактов - переименовать группу
        ...
        if not query:
            logging.error("ERROR!!! Group <%s> not exist", curr_name)
            logging.error("APPLICATION STOPPED!!!")
            self.current_exception = "Group <%s> not exist" % curr_name
            if self.jupiter:
                return self.current_exception
            raise ValueError("Group <%s> not exist" % curr_name)
    ...
    def measure_remove_group(self, group_name: str) -> [Dict, str]:
        """
        Удаление группы фактов [ID-2992]
        ...
        # если такой группы не существует - выбросить исключение
        for item in measures_list:
        ...
        if not query:
            logging.error("ERROR!!! Group <%s> not exist", group_name)
            logging.error("APPLICATION STOPPED!!!")
            self.current_exception = "Group <%s> not exist" % group_name
            if self.jupiter:
                return self.current_exception
            raise ValueError("Group <%s> not exist" % group_name)
