...
class Constellation:
	...
	def set(self, filename: str, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:IMPort:QZSS<ST>:FILE:CONStellation \n
		Snippet: driver.source.bb.gnss.sv.importPy.qzss.file.constellation.set(filename = '1', stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:IMPort:QZSS<ST>:FILE:CONStellation \n
		Snippet: value: str = driver.source.bb.gnss.sv.importPy.qzss.file.constellation.get(stream = repcap.Stream.Default) \n
		Selects the file from that the satellites constellation and navigation data are extracted. Supported file types per GNSS
		system
			Table Header: GNSS system / *.txt / *.alm / *.al3 / *.xml / *.alg / *.rnx / *.<xx>n \n
			- GPS / x / x / x / x / x
