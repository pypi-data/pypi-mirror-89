...
class Woti:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("woti", core, parent)

	@property
	def current(self):
		"""current commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_current'):
			from .Woti_.Current import Current
	...
	def average(self):
		...
		if not hasattr(self, '_average'):
			from .Woti_.Average import Average
	...
	def maximum(self):
		...
		if not hasattr(self, '_maximum'):
			from .Woti_.Maximum import Maximum
	...
	def minimum(self):
		...
		if not hasattr(self, '_minimum'):
			from .Woti_.Minimum import Minimum
	...
	def clone(self) -> 'Woti':
		...
		new_group = Woti(self._core, self._base.parent)
