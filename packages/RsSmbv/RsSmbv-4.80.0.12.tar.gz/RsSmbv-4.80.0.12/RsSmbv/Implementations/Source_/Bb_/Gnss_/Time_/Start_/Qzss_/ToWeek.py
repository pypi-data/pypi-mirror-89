...
class ToWeek:
	...
	def get(self, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:STARt:QZSS<ST>:TOWeek \n
		Snippet: value: float = driver.source.bb.gnss.time.start.qzss.toWeek.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TIME:STARt:QZSS{stream_cmd_val}:TOWeek?')
