...
from CCA_methods_2.KCCA import *
from CCA_methods_2.plot_utils import cv_plot


class Wrapper:
    # The idea is that this can take some data and run one of my many CCA_archive methods
    def __init__(self, outdim_size=2, method='l2', params=None, max_iter=50, tol=1e-4):
    ...
    def predict_corr(self, X_test, Y_test):
        ...
        U_test, V_test = self.transform_view(X_new=X_test, Y_new=Y_test)
    ...
    def predict_view(self, X_new=None, Y_new=None):
        # Takes one dataset and predicts the other
    ...
    def transform_view(self, X_new=None, Y_new=None):
        ...
        if self.method == 'kernel':
            U_new, V_new = self.KCCA.transform(X_new, Y_new)
        elif self.method == 'pls':
            U_new, V_new = self.PLS.transform(X_new, Y_new)
        else:
            if X_new is not None:
                U_new = X_new @ self.X_rotation
            if Y_new is not None:
                V_new = Y_new @ self.Y_rotation
    ...
    def outer_loop(self, X_train, Y_train):
        ...
        for k in range(self.outdim_size):
            self.inner_loop = ALS_inner_loop(X_train_res, Y_train_res, C=C_train_res, params=self.params,
...
class ALS_inner_loop:
    ...
    def __init__(self, X, Y, C=None, max_iter=100, tol=1e-4, initialization='random', params=None,
                 method='l2'):
    ...
    def iterate(self):
        ...
        if any(self.method in _ for _ in ['scca', 'pmd', 'constrained_scca', 'sgcca', 'constrained_sgcca', 'elastic']):
            if self.X.shape[1] > self.X.shape[0] or self.Y.shape[1] > self.Y.shape[0]:
                warm_start = PLSCanonical(n_components=1, scale=False).fit(self.X, self.Y)
                target = self.X @ warm_start.x_weights_ + self.Y @ warm_start.y_weights_
                target /= np.linalg.norm(target)
                c = np.squeeze(warm_start.y_weights_)
            else:
                if self.initialization == 'random':
                    target = np.random.rand(self.X.shape[0])
                    target /= np.linalg.norm(target)
                    c = np.random.rand(self.Y.shape[1])
                    c /= np.linalg.norm(self.Y@c)
                else:
        ...
        for _ in range(self.max_iter):
            ...
            if self.method == 'l2':
                ...
                v = self.Y @ c
                w, self.lambda_1 = self.constrained_regression(self.X, v, self.params['c_1'], self.lambda_1)
                ...
                w, self.lambda_1 = self.constrained_regression(self.X, target, self.params['c_1'], self.lambda_1)
            ...
            if self.method == 'l2':
                ...
                u = self.X @ w
                c, self.lambda_2 = self.constrained_regression(self.Y, u, self.params['c_2'], self.lambda_2)
            elif self.method == 'scca':
                target = self.X @ w
                ...
                c, self.lambda_2 = self.constrained_regression(self.Y, target, self.params['c_2'], self.lambda_2)
                target = (self.Y @ c + self.X @ w) / 2
    ...
    @ignore_warnings(category=ConvergenceWarning)
    def constrained_regression(self, X, y, p, init_lambda=1, search='bin'):
        if p == 0:
...
def bin_search(current, previous, current_val, previous_val, min_, max_):
    ...
    if current_val < 0:
        if previous_val < 0:
            new = min(current * 1.618, (current + max_) / 2)
        if previous_val > 0:
    ...
    if current_val > 0:
        if previous_val > 0:
            new = max(current / 1.618, (current + min_) / 2)
        if previous_val < 0:
            new = (current + previous) / 2
        if current < max_:
            max_ = current
