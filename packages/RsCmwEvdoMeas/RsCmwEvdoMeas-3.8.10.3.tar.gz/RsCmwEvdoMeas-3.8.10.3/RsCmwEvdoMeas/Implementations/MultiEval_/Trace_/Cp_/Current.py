...
class Current:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.multiEval.trace.cp.current.read() \n
		...
		return self._core.io.query_struct(f'READ:EVDO:MEASurement<Instance>:MEValuation:TRACe:CP:CURRent?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:EVDO:MEASurement<instance>:MEValuation:TRACe:CP:CURRent \n
		Snippet: value: ResultData = driver.multiEval.trace.cp.current.fetch() \n
		Returns the channel power of the reverse link physical channels of both the I and Q signal. The slots for the pilot and
		the RRI channel are evaluated within the same measurement slot. The values described below are returned by FETCh and READ
		commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:TRACe:CP:CURRent?', self.__class__.ResultData())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.trace.cp.current.calculate() \n
		Returns the channel power of the reverse link physical channels of both the I and Q signal. The slots for the pilot and
		the RRI channel are evaluated within the same measurement slot. The values described below are returned by FETCh and READ
		commands. CALCulate commands return limit check results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:TRACe:CP:CURRent?', self.__class__.CalculateStruct())
