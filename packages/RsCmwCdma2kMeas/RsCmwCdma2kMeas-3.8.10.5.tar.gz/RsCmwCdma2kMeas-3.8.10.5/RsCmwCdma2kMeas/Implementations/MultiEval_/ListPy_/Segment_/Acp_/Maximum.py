...
class Maximum:
	...
	class FetchStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct('Maximum_Acp', DataType.FloatList, None, False, False, 21),
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:ACP:MAXimum?', self.__class__.FetchStruct())
	...
	class CalculateStruct(StructBase):
		...
		__meta_args_list = [
			ArgStruct.scalar_int('Reliability', 'Reliability'),
			ArgStruct.scalar_int('Seg_Reliability'),
			ArgStruct('Maximum_Acp', DataType.EnumList, enums.ResultStatus2, False, False, 21),
			ArgStruct.scalar_float('Ms_Power_Wide'),
			ArgStruct.scalar_float('Ms_Power_Narrow'),
			ArgStruct.scalar_float('Out_Of_Tol_Count'),
	...
	def calculate(self, segment=repcap.Segment.Default) -> CalculateStruct:
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:ACP:MAXimum?', self.__class__.CalculateStruct())
