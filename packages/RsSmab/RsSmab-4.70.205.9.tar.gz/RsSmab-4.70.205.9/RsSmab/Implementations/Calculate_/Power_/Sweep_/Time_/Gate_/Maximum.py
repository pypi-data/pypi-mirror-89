...
class Maximum:
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		...
		Snippet: value: float = driver.calculate.power.sweep.time.gate.maximum.get(channel = repcap.Channel.Default) \n
		...
		response = self._core.io.query_str(f'CALCulate:POWer:SWEep:TIME:GATE{channel_cmd_val}:MAXimum?')
