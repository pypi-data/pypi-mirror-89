...
class DcFading:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dcFading", core, parent)

	@property
	def flexible(self):
		"""flexible commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_flexible'):
			from .DcFading_.Flexible import Flexible
	...
	def get_external(self) -> ExternalStruct:
		...
		Snippet: value: ExternalStruct = driver.route.scenario.dcFading.get_external() \n
		...
		"""
		return self._core.io.query_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DCFading:EXTernal?', self.__class__.ExternalStruct())

	def set_external(self, value: ExternalStruct) -> None:
		"""SCPI: ROUTe:WCDMa:SIGNaling<instance>:SCENario:DCFading[:EXTernal] \n
		Snippet: driver.route.scenario.dcFading.set_external(value = ExternalStruct()) \n
		Activates the 'Dual Carrier Fading: External' scenario and selects the signal paths. For possible connector and converter
		values, see 'Values for Signal Path Selection'. \n
			:param value: see the help for ExternalStruct structure arguments.
		"""
		self._core.io.write_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DCFading:EXTernal', value)
	...
	def get_internal(self) -> InternalStruct:
		...
		Snippet: value: InternalStruct = driver.route.scenario.dcFading.get_internal() \n
		...
		"""
		return self._core.io.query_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DCFading:INTernal?', self.__class__.InternalStruct())

	def set_internal(self, value: InternalStruct) -> None:
		"""SCPI: ROUTe:WCDMa:SIGNaling<instance>:SCENario:DCFading:INTernal \n
		Snippet: driver.route.scenario.dcFading.set_internal(value = InternalStruct()) \n
		Activates the 'Dual Carrier Fading: Internal' scenario and selects the signal paths. For possible parameter values, see
		'Values for Signal Path Selection'. \n
			:param value: see the help for InternalStruct structure arguments.
		"""
		self._core.io.write_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DCFading:INTernal', value)

	def clone(self) -> 'DcFading':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = DcFading(self._core, self._base.parent)
