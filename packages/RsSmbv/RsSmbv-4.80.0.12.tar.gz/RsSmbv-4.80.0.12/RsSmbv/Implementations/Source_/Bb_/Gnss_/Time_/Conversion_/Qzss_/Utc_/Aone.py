...
class Aone:
	...
	def set(self, aone: int, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:QZSS<ST>:UTC:AONE \n
		Snippet: driver.source.bb.gnss.time.conversion.qzss.utc.aone.set(aone = 1, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:QZSS{stream_cmd_val}:UTC:AONE {param}')

	def get(self, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:QZSS<ST>:UTC:AONE \n
		Snippet: value: int = driver.source.bb.gnss.time.conversion.qzss.utc.aone.get(stream = repcap.Stream.Default) \n
		Sets the first order term of polynomial, A1. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Qzss')
			:return: aone: integer Range: -8388608 to 8388607"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:QZSS{stream_cmd_val}:UTC:AONE?')
