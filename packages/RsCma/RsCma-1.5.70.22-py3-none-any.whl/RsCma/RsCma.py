...
class RsCma:
	"""1840 total commands, 19 Sub-groups, 0 group commands"""
	driver_options = "SupportedInstrModels = CMA180, SupportedIdnPatterns = CMA, SimulationIdnString = 'Rohde&Schwarz,CMA180,100001,1.5.70.0022'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCma.driver_options, options, direct_session)
		self._core.driver_version = '1.5.70.0022'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '1.5.70.0022'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCma version failed. Current version: '1.5.70.0022', minimum required version: '{min_version}'")
