...
class CvPeriod:
	...
	def set(self, const_vel_period: float, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS<ST>:SDYNamics:CVPeriod \n
		Snippet: driver.source.bb.gnss.svid.qzss.sdynamics.cvPeriod.set(const_vel_period = 1.0, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS<ST>:SDYNamics:CVPeriod \n
		Snippet: value: float = driver.source.bb.gnss.svid.qzss.sdynamics.cvPeriod.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Sets the time period during that the velocity is kept constant. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Qzss')
