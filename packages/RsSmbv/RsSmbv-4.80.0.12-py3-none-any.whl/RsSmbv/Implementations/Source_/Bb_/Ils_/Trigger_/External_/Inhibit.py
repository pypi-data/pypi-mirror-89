...
class Inhibit:
	...
	def set(self, inhibit: int, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.ils.trigger.external.inhibit.set(inhibit = 1, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:ILS:TRIGger:EXTernal{channel_cmd_val}:INHibit {param}')

	def get(self, channel=repcap.Channel.Default) -> int:
		"""SCPI: [SOURce<HW>]:[BB]:ILS:TRIGger:[EXTernal<CH>]:INHibit \n
		Snippet: value: int = driver.source.bb.ils.trigger.external.inhibit.get(channel = repcap.Channel.Default) \n
		Specifies the duration by which a restart is inhibited. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'External')
			:return: inhibit: integer Range: 0 to 67108863"""
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ILS:TRIGger:EXTernal{channel_cmd_val}:INHibit?')
