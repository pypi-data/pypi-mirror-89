...
class WeChatQrcodeLoginView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        queryset = WeChatQrcodeLogin.objects.filter(
            event_id=event_id, code=code).first()
        token = ''
        if queryset:
            ...
            if user_queryset:
                token = jwt_create_token_wechat(
                    openid=user_queryset.openid, unionid=user_queryset.unionid, userid=str(user_queryset.pk), wechat_id=queryset.wechat_id)
