...
class Otasp:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("otasp", core, parent)

	@property
	def send(self):
		"""send commands group. 0 Sub-classes, 3 commands."""
		if not hasattr(self, '_send'):
			from .Otasp_.Send import Send
	...
	def receive(self):
		...
		if not hasattr(self, '_receive'):
			from .Otasp_.Receive import Receive
			self._receive = Receive(self._core, self._base)
		return self._receive

	def clone(self) -> 'Otasp':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Otasp(self._core, self._base.parent)
