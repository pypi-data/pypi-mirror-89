...
class Data:
	...
	def set(self, data: enums.DataSour, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.down.cell.enh.dch.hsdpa.data.set(data = enums.DataSour.DLISt, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.DataSour:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:DOWN:CELL<ST>:ENH:DCH:HSDPA:DATA \n
		Snippet: value: enums.DataSour = driver.source.bb.tdscdma.down.cell.enh.dch.hsdpa.data.get(stream = repcap.Stream.Default) \n
		The command determines the data source for the HSDPA/HSUPA channels. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
