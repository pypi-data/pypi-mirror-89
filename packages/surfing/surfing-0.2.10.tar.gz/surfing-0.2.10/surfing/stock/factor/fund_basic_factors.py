...
from .utils import _START_DATE, _END_DATE
...
class TradingDay(Factor):
    ...
    def calc(self):
        ...
        self._factor = self._factor[(self._factor.index >= pd.to_datetime(_START_DATE).date())
                                 & (self._factor.index <= pd.to_datetime(_END_DATE).date())]
...
class FundPersonHold(Factor):
    ...
    def calc(self):
        self._factor = DataManager.basic_data(func_name='get_fund_persional_hold', start_date=_START_DATE).drop(columns=['_update_time'], errors='ignore')
