...
class Ulink:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.data.measurement.adelay.ulink.fetch() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:DATA:MEASurement<MeasInstance>:ADELay:ULINk?', suppressed)
		return response

	def read(self) -> List[float]:
		"""SCPI: READ:DATA:MEASurement<Instance>:ADELay:ULINk \n
		Snippet: value: List[float] = driver.data.measurement.adelay.ulink.read() \n
		Query the statistical audio delay results for 'Uplink', 'Downlink' and 'Loopback'. \n
		Use RsCmwDau.reliability.last_value to read the updated reliability indicator. \n
			:return: results: Comma-separated list of four results: Current, Average, Minimum, Maximum value Unit: s"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:DATA:MEASurement<MeasInstance>:ADELay:ULINk?', suppressed)
		return response
