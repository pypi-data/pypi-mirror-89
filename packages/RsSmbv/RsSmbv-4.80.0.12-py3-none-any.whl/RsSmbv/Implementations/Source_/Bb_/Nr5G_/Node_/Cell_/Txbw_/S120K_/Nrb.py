...
class Nrb:
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CH>:TXBW:S120K<ST>:NRB \n
		Snippet: value: int = driver.source.bb.nr5G.node.cell.txbw.s120K.nrb.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:NR5G:NODE:CELL{channel_cmd_val}:TXBW:S120K{stream_cmd_val}:NRB?')
