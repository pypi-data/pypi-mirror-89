...
class Range:
	...
	def set(self, structure: RangeStruct, ims=repcap.Ims.Default) -> None:
		...
		Snippet: driver.configure.data.control.ims.update.evs.receive.bitrate.range.set(value = [PROPERTY_STRUCT_NAME](), ims = repcap.Ims.Default) \n
		Selects the bit-rate range supported in the EVS primary mode in the uplink (receive) direction, for a call update and
		separate configuration. \n
			:param structure: for set value, see the help for RangeStruct structure arguments.
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')"""
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		self._core.io.write_struct(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:RECeive:BITRate:RANGe', structure)

	def get(self, ims=repcap.Ims.Default) -> RangeStruct:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:EVS:RECeive:BITRate:RANGe \n
		Snippet: value: RangeStruct = driver.configure.data.control.ims.update.evs.receive.bitrate.range.get(ims = repcap.Ims.Default) \n
		Selects the bit-rate range supported in the EVS primary mode in the uplink (receive) direction, for a call update and
		separate configuration. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')
			:return: structure: for return value, see the help for RangeStruct structure arguments."""
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		return self._core.io.query_struct(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:RECeive:BITRate:RANGe?', self.__class__.RangeStruct())
