...
class Minimum:
	...
	def fetch(self, subVector=repcap.SubVector.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.listPy.powerVsTime.svector.subvector.minimum.fetch(subVector = repcap.SubVector.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:GSM:MEASurement<Instance>:MEValuation:LIST:PVTime:SVECtor:SUBVector{subVector_cmd_val}:MINimum?', suppressed)
		return response
