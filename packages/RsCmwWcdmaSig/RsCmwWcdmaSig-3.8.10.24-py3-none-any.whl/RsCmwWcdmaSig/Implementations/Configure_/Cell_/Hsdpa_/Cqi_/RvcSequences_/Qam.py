...
class Qam:
	...
	def set(self, sequence: enums.RvcSequence, quadratureAM=repcap.QuadratureAM.Default) -> None:
		...
		Snippet: driver.configure.cell.hsdpa.cqi.rvcSequences.qam.set(sequence = enums.RvcSequence.S1, quadratureAM = repcap.QuadratureAM.Default) \n
		Specifies an RV coding sequence to be used for signals with 16-QAM or 64-QAM modulation. If UDEFined is selected, the
		sequence is defined via method RsCmwWcdmaSig.Configure.Cell.Hsdpa.Cqi.RvcSequences.Qam.UserDefined.set. \n
		...
		self._core.io.write(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HSDPa:CQI:RVCSequences:QAM{quadratureAM_cmd_val} {param}')

	# noinspection PyTypeChecker
	def get(self, quadratureAM=repcap.QuadratureAM.Default) -> enums.RvcSequence:
		"""SCPI: CONFigure:WCDMa:SIGNaling<instance>:CELL:HSDPa:CQI:RVCSequences:QAM<nr> \n
		Snippet: value: enums.RvcSequence = driver.configure.cell.hsdpa.cqi.rvcSequences.qam.get(quadratureAM = repcap.QuadratureAM.Default) \n
		Specifies an RV coding sequence to be used for signals with 16-QAM or 64-QAM modulation. If UDEFined is selected, the
		sequence is defined via method RsCmwWcdmaSig.Configure.Cell.Hsdpa.Cqi.RvcSequences.Qam.UserDefined.set. \n
			:param quadratureAM: optional repeated capability selector. Default value: QAM16 (settable in the interface 'Qam')
		...
		quadratureAM_cmd_val = self._base.get_repcap_cmd_value(quadratureAM, repcap.QuadratureAM)
		response = self._core.io.query_str(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HSDPa:CQI:RVCSequences:QAM{quadratureAM_cmd_val}?')
