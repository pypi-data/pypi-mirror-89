	...
	if(post_info.fits)
	{
		...
		if (do_ground_shot || do_jump_shot || do_double_jump_shot)
		{
            ...
            if (in_2d_field(&ball_offset, &me->hitbox))
            {
                ...
                if (do_double_jump_shot && !r.found && slope > double_jump_shot_min_slope)
                {
		...
		if (do_aerial_shot && !r.found)
		{
            ...
            if (in_field(&ball_offset, &me->hitbox))
            {
                double slope = find_slope(shot_vector, car_to_ball);
                if (slope > aerial_shot_min_slope)
                {
