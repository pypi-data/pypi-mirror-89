...
class Smodulation:
	...
	def get_rpower(self) -> RpowerStruct:
		...
		Snippet: value: RpowerStruct = driver.configure.multiEval.limit.epsk.smodulation.get_rpower() \n
		...
		"""
		return self._core.io.query_struct('CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SMODulation:RPOWer?', self.__class__.RpowerStruct())

	def set_rpower(self, value: RpowerStruct) -> None:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SMODulation:RPOWer \n
		Snippet: driver.configure.multiEval.limit.epsk.smodulation.set_rpower(value = RpowerStruct()) \n
		Define two reference power values for the modulation schemes 8PSK and 16-QAM. These values are relevant in the context of
		CONFigure:GSM:MEAS<i>:MEValuation:LIMit:EPSK:SMODulation:MPOint<no> and
		CONFigure:GSM:MEAS<i>:MEValuation:LIMit:QAM<m>:SMODulation:MPOint<no>. \n
			:param value: see the help for RpowerStruct structure arguments.
		"""
		self._core.io.write_struct('CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SMODulation:RPOWer', value)
