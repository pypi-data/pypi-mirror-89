...
class Control:
	...
	def set(self, nav_msg_control: enums.NavMsgCtrl, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:SBAS<ST>:SIGNal:L1Band:CA:DATA:NMESsage:CONTrol \n
		Snippet: driver.source.bb.gnss.svid.sbas.signal.l1Band.ca.data.nmessage.control.set(nav_msg_control = enums.NavMsgCtrl.AUTO, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> enums.NavMsgCtrl:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:SBAS<ST>:SIGNal:L1Band:CA:DATA:NMESsage:CONTrol \n
		Snippet: value: enums.NavMsgCtrl = driver.source.bb.gnss.svid.sbas.signal.l1Band.ca.data.nmessage.control.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Defines whether the navigation message parameters can be changed or not. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sbas')
