...
class RsCmwGsmMeas:
	...
	driver_options = "SupportedInstrModels = CMW500/CMW270/CMW100/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.30.0004'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwGsmMeas.driver_options, options, direct_session)
		self._core.driver_version = '3.7.30.0004'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.30.0004'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwGsmMeas version failed. Current version: '3.7.30.0004', minimum required version: '{min_version}'")
