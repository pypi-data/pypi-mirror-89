...
class Mpoint:
	...
	def set(self, structure: MpointStruct, measPoint=repcap.MeasPoint.Default) -> None:
		...
		Snippet: driver.configure.multiEval.limit.epsk.smodulation.mpoint.set(value = [PROPERTY_STRUCT_NAME](), measPoint = repcap.MeasPoint.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SMODulation:MPOint{measPoint_cmd_val}', structure)

	def get(self, measPoint=repcap.MeasPoint.Default) -> MpointStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SMODulation:MPOint<nr> \n
		Snippet: value: MpointStruct = driver.configure.multiEval.limit.epsk.smodulation.mpoint.get(measPoint = repcap.MeasPoint.Default) \n
		Defines and activates a limit line for the modulation schemes 8PSK and 16-QAM and for a certain frequency offset.
		The specified limits apply above the high power reference value and below the low power reference value defined by method
		RsCmwGsmMeas.Configure.MultiEval.Limit.Epsk.Smodulation.rpower and method RsCmwGsmMeas.Configure.MultiEval.Limit.Qam.
		Smodulation.Rpower.set. Between the two reference power values, the limits are determined by linear interpolation. \n
		...
		measPoint_cmd_val = self._base.get_repcap_cmd_value(measPoint, repcap.MeasPoint)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:SMODulation:MPOint{measPoint_cmd_val}?', self.__class__.MpointStruct())
