...
class XanityClass(object):
    ...
    def _parse_args(self, args=None):
        ...
        if self.args.action is None and ('--help' in self.unknownargs or '-h' in self.unknownargs):
    ...
    def _resolve_param_requests(self):
        ...
        if 'parameter' in self.args and self.args.parameter is not None and len(self.args.parameter) > 0:
            ...
            for string_e in self.args.parameter:
                ...
                matches = re.findall(r'(\w+=(?:\[[^\]]*\])|(?:[^, $]+))', string_e)
    ...
    def _run_all_exps(self):
        ...
        with self.act_tags(ACTIVITIES.EXP):
            ...
            def do_runs(pool=None):
                if pool is not None:
                    results = []
                for stage, list in enumerate(self._run_q):
    ...
    def _run_all_anals(self):
        ...
        with self.act_tags(ACTIVITIES.ANAL):
            for i, analysis in enumerate(self._run_q.analyses.values()):
                if self._check_conditional_run(analysis, i):
                    self._run_one_job_locally(analysis, i)
    ...
    def _check_conditional_run(self):
        ...
        try:
            ...
            print('caught exception in _conditional_run() of type: {}'.format(e))
            pass
