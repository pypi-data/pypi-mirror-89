...
from .utils import try_find_one
...
class Rkas(BaseSimdakPaud):
    ...
    def create(
        self, rkas_data: Rab, semester_id: int = 20201, update_id: bool = False
    ) -> Rab:
        data = rkas_data.as_data()
        data.update({"yt0": "Simpan"})
        semester = semester_id or self.semester_id
        url = self._base_url + f"boppaudrkas/create/id/{self.id}/semester_id/{semester}"
        res = self._session.post(url, data=data, allow_redirects=update_id)
        if not update_id or not res.ok:
            return rkas_data
        soup = BeautifulSoup(res.text, "html.parser")
        table: List[Tag] = soup.findAll("table")
        if not table or len(table) != 2:
        ...
        return try_find_one(
            Rab.from_table(table), rkas_data, lambda n: getattr(n, "jenisbelanja")
        )
