...
class Fpmi:
	"""Fpmi commands group definition. 3 total commands, 2 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("fpmi", core, parent)

	@property
	def mcluster(self):
		"""mcluster commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_mcluster'):
			from .Fpmi_.Mcluster import Mcluster
	...
	def downlink(self):
		...
		if not hasattr(self, '_downlink'):
			from .Fpmi_.Downlink import Downlink
	...
	def clone(self) -> 'Fpmi':
		...
		new_group = Fpmi(self._core, self._base.parent)
