...
class DatasetClientBase:
    ...
    def _list_segments(self) -> List[str]:
        ...
        segments_info = self._client.open_api_get(
            "segments", params, dataset_id=self.dataset_id
        ).json()["segments"]
        return [SegmentNameWrapper.get_local_segment_name(info) for info in segments_info]
...
class DatasetClient(DatasetClientBase):
    ...
    def upload_catalog(self, catalog: Catalog) -> None:
        ...
        self._labelset_client.upload_catalog(catalog)
    ...
    def commit(self, message: str, tag: Optional[str] = None) -> "DatasetClient":
        ...
        """
        self._commit(message, tag)
        return DatasetClient(self._name, self.dataset_id, self._client)
...
class FusionDatasetClient(DatasetClientBase):
    ...
    def commit(self, message: str, tag: Optional[str] = None) -> "FusionDatasetClient":
        ...
        """
        self._commit(message, tag)
        return FusionDatasetClient(self._name, self.dataset_id, self._client)
