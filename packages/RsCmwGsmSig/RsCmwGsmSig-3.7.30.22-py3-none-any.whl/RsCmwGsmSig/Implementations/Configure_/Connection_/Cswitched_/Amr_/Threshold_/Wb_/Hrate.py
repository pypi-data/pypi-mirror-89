...
class Hrate:
	...
	def get_epsk(self) -> List[float or bool]:
		...
		Snippet: value: List[float or bool] = driver.configure.connection.cswitched.amr.threshold.wb.hrate.get_epsk() \n
		...
		"""
		response = self._core.io.query_str('CONFigure:GSM:SIGNaling<Instance>:CONNection:CSWitched:AMR:THReshold:WB:HRATe:EPSK?')
		return Conversions.str_to_float_or_bool_list(response)

	def set_epsk(self, threshold: List[float or bool]) -> None:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:CONNection:CSWitched:AMR:THReshold:WB:HRATe:EPSK \n
		Snippet: driver.configure.connection.cswitched.amr.threshold.wb.hrate.set_epsk(threshold = [1.1, True, 2.2, False, 3.3]) \n
		Selects the upper and lower limits for the codec mode swapping. The threshold sequence is following: lower 4, upper 3,
		lower 3, upper 2, lower 2, and upper 1 threshold. Value OFF disables threshold. \n
		...
		self._core.io.write(f'CONFigure:GSM:SIGNaling<Instance>:CONNection:CSWitched:AMR:THReshold:WB:HRATe:EPSK {param}')
