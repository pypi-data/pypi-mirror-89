...
class ClientMainWindow(QMainWindow):
    ...
    def set_disabled_input(self):
        ''' Метод переводящий поля ввода в состояние "неактивны"'''
        # Надпись  - получатель.
    ...
    def message(self, mes):
        ...
        # encrypted_message = base64.b64decode(message[TEXT_MESSAGE])
        ...
        self.database.save_message(self.current_chat, 'in', mes[TEXT_MESSAGE])
        sender = mes[SENDER]

        if sender == self.current_chat:
            # self.database.save_message(sender, 'in', mes[TEXT_MESSAGE])
            self.history_list_update()
        else:
