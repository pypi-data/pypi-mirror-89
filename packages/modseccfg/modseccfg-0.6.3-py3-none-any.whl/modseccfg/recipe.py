...
#    { name: user_recipes, type: bool, value: 0, description: "Load snipperts from ~/.config/modseccfg/**/*.txt" }
# license: Apache-2.0
#
...
templates = DictObj({
    ...
    "Example Exlusions": {
        ...
        "Ignore param for tag": """
            ...
            SecRule REQUEST_FILENAME "@endsWith $PATH" "id:$rand,phase:2,pass,nolog,ctl:ruleRemoveTargetByTag=attack-sqli;ARGS:pwd"
        """,

        "Ignore param for all": """
            # Removing a specific ARGS parameter from inspection for all CRS rules
            SecRule REQUEST_FILENAME "@endsWith /wp-login.php" "id:$rand,phase:2,pass,nolog,ctl:ruleRemoveTargetByTag=OWASP_CRS;ARGS:pwd"
        """,

        "Remove rule range": """
            # Removing a range of rules
            SecRule REQUEST_FILENAME "@beginsWith /admin" "id:$rand,phase:2,pass,nolog,ctl:ruleRemoveById=941000-942999"
        """
    },
        ...
        "Macro definitions": """
            ...
            <IfModule mod_macro.c>
              ...
              <Macro SecVar $VAR $VAL>
                Use NEWID "$VAR=$VAL"
                SecAction "id:%{ENV:NEWID},phase:1,nolog,pass,t:none,setvar:'$VAR=$VAL'"
              </Macro>
