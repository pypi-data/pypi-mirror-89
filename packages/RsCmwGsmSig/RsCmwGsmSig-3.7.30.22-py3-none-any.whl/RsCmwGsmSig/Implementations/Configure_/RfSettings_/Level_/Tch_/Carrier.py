...
class Carrier:
	...
	def set(self, level: float, carrier=repcap.Carrier.Default) -> None:
		...
		Defines the absolute level of the traffic channel (TCH) and the packet data channel (PDCH) . The allowed value range can
		be calculated as follows: Range (Level) = Range (Output Power) - External Attenuation - Insertion Loss + (Baseband Level
		+ 15 dB) Range (Output Power) = -130 dBm to 0 dBm (RFx COM) or -120 dBm to 13 dBm (RFx OUT) ; please also notice the
		ranges quoted in the data sheet. Insertion Loss is only relevant for internal fading, (Baseband Level + 15 dB) only for
		external fading. \n
		...
		self._core.io.write(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:LEVel:TCH:CARRier{carrier_cmd_val} {param}')
	...
	def get(self, carrier=repcap.Carrier.Default) -> float:
		...
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		response = self._core.io.query_str(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:LEVel:TCH:CARRier{carrier_cmd_val}?')
