...
class Dpcp:
	...
	@property
	def dpConnection(self):
		"""dpConnection commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_dpConnection'):
			from .Dpcp_.DpConnection import DpConnection
			self._dpConnection = DpConnection(self._core, self._base)
		return self._dpConnection

	@property
	def dpProtocol(self):
		"""dpProtocol commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_dpProtocol'):
			from .Dpcp_.DpProtocol import DpProtocol
			self._dpProtocol = DpProtocol(self._core, self._base)
		return self._dpProtocol
