...
class Bandpass:
	...
	def get_bandwidth(self) -> float:
		...
		Selects the bandwidth for the bandpass filter. \n
			:return: band_pass_bw: You can enter values between 1 kHz and 40 MHz. The setting is rounded to the closest of the following values: 1 kHz / 10 kHz / 100 kHz / 1 MHz / 10 MHz / 40 MHz Unit: Hz
		"""
	...
	def set_bandwidth(self, band_pass_bw: float) -> None:
		"""SCPI: CONFigure:GPRF:MEASurement<Instance>:IQRecorder:FILTer:BANDpass:BWIDth \n
		Snippet: driver.configure.gprfMeasurement.iqRecorder.filterPy.bandpass.set_bandwidth(band_pass_bw = 1.0) \n
		Selects the bandwidth for the bandpass filter. \n
			:param band_pass_bw: You can enter values between 1 kHz and 40 MHz. The setting is rounded to the closest of the following values: 1 kHz / 10 kHz / 100 kHz / 1 MHz / 10 MHz / 40 MHz Unit: Hz
		"""
