...
class Immediate:
	...
	def get_amplitude(self) -> float:
		...
		Sets the RF level applied to the DUT. To activate the RF output use command :​OUTPut<hw>[:​STATe] ('RF On'/'RF Off') .
			...
			- method RsSmab.Source.Power.Level.Immediate.amplitude is the values set with [:​SOURce<hw>]:​POWer[:​LEVel][:​IMMediate][:​AMPLitude]
			- RF output level is set with method RsSmab.Source.Power.power
			- OFFSet is set with OFFSet \n
			:return: amplitude: float The following settings influence the value range: OFFSet set with the command OFFSet Numerical value Sets the level UP|DOWN Varies the level step by step. The level is increased or decreased by the value set with the command method RsSmab.Source.Power.Level.Immediate.amplitude. Range: (Level_min + OFFSet) to (Level_max + OFFStet) , Unit: dBm
		"""
