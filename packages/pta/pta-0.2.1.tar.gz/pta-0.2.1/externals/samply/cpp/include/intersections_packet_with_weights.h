...
struct IntersectionsPacketWithWeights : public IntersectionsPacket<Scalar> {
	...
	IntersectionsPacketWithWeights(
		...
		const Vector<Scalar>& weights)
		: IntersectionsPacket<Scalar>(segment_starts, segment_ends, ray_indices,
			num_rays)
		, weights(weights)
	{
	...
	IntersectionsPacketWithWeights<Scalar> operator()(const IndexList& ray_indices) const {
		if (!this->one_ray_one_intersection)
			throw std::runtime_error("Not implemented yet.");
		return IntersectionsPacketWithWeights<Scalar>(
			this->segment_starts(ray_indices),
		...
		const IntersectionsPacketWithWeights<Scalar>& other) const
	{
		if (!other.one_ray_one_intersection || !this->one_ray_one_intersection)
			throw std::runtime_error("IntersectionsPacket::intersect can "
				"be used only if all rays have a single intersection");

		if (!((this->segment_ends.array() > other.segment_starts.array()) &&
			(other.segment_ends.array() > this->segment_starts.array())).all()) {
		...
		return IntersectionsPacketWithWeights<Scalar>(
			...
			this->segment_ends.cwiseMin(other.segment_ends),
			weights);
	}
	...
	Vector<Scalar> for_each_ray(Function&& ray_function) const
	{
		...
		for (Eigen::Index segment_idx = 0;
            segment_idx < this->segment_starts.size(); segment_idx++) {
			...
			if (segment_idx + 1 == this->segment_starts.size() ||
				...
				Eigen::Index num_ray_segments = 
					segment_idx - current_ray_start_idx + 1;

				// Execute the function on the ray we just completed.
				result(current_ray) = ray_function(
					this->segment_starts.segment(
						current_ray_start_idx, num_ray_segments),
					this->segment_ends.segment(
						current_ray_start_idx, num_ray_segments),
					weights.segment(
						current_ray_start_idx, num_ray_segments),
					current_ray
