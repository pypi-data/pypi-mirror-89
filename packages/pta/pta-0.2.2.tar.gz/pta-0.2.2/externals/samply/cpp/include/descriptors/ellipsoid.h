...
template<typename ScalarType>
Ellipsoid<ScalarType>::Ellipsoid(
	const Matrix<double>& E, const Vector<double>& f)
	: E_full_precision(E)
	, f_full_precision(f)
	, E_inv_((
		E.completeOrthogonalDecomposition().pseudoInverse().transpose() *
		E.completeOrthogonalDecomposition().pseudoInverse()).cast<ScalarType>())
	...
	const RaysPacket<ScalarType>& rays) const
{
	...
	auto discriminant = 
		(b.array().square().matrix() - 4 * a.cwiseProduct(c)).eval();
	...
	const std::string& optimality_criterion) const
{
	if (optimality_criterion.compare("isotropy") == 0 ||
		...
		AffineTransform<double> from_parametrization = AffineTransform<double>(
			E_full_precision, f_full_precision);
		...
		throw std::runtime_error(
			std::string("The '") + optimality_criterion + std::string("' "
