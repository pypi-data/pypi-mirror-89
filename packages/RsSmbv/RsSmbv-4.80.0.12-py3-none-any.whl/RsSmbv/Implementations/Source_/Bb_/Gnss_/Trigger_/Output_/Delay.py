...
class Delay:
	...
	def set(self, delay: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.trigger.output.delay.set(delay = 1.0, channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TRIGger:OUTPut<CH>:DELay \n
		Snippet: value: float = driver.source.bb.gnss.trigger.output.delay.get(channel = repcap.Channel.Default) \n
		Defines the delay between the signal on the marker outputs and the start of the signals. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
