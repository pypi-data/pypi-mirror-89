...
class Rframe:
	...
	def set(self, reference_frame: enums.RefFrame, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.receiver.v.location.coordinates.rframe.set(reference_frame = enums.RefFrame.PZ90, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.RefFrame:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RECeiver:[V<ST>]:LOCation:COORdinates:RFRame \n
		Snippet: value: enums.RefFrame = driver.source.bb.gnss.receiver.v.location.coordinates.rframe.get(stream = repcap.Stream.Default) \n
		Select the reference frame used to define the receiver coordinates. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'V')
