...
class PR_remover():
    ...
    def __init__(self,dataset,epochs=1,eta=5,use_pr_regularizer=True):
        """
        Arguments:
...
def PrejudiceRemover(dataset,epochs,eta=5,use_pr_regularizer=True):

    if not isinstance(dataset,ds.Dataset):
        raise TypeError("Dataset must be of type aibias.dataset.Dataset")

    print('Preparing data')
    pr_remover = PR_remover(dataset,epochs,eta,use_pr_regularizer)
