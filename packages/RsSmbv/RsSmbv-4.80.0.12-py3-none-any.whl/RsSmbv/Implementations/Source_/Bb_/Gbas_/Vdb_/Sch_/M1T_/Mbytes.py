...
class Mbytes:
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GBAS:VDB<CH>:SCH:M1T<ST>:MBYTes \n
		Snippet: value: int = driver.source.bb.gbas.vdb.sch.m1T.mbytes.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GBAS:VDB{channel_cmd_val}:SCH:M1T{stream_cmd_val}:MBYTes?')
