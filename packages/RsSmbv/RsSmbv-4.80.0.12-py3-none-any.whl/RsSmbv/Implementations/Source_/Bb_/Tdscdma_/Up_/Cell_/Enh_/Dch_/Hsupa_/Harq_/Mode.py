...
class Mode:
	...
	def set(self, mode: enums.EnhHsHarqMode, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.up.cell.enh.dch.hsupa.harq.mode.set(mode = enums.EnhHsHarqMode.CACK, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.EnhHsHarqMode:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:ENH:DCH:HSUPA:HARQ:MODE \n
		Snippet: value: enums.EnhHsHarqMode = driver.source.bb.tdscdma.up.cell.enh.dch.hsupa.harq.mode.get(stream = repcap.Stream.Default) \n
		Sets the HARQ simulation mode. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
