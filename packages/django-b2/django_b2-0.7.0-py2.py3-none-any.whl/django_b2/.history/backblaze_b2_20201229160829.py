...
class BackBlazeB2(object):
    ...
    def __init__(self, prevent_unique=True):  # by default uploads receive the name made unique as <uuid>/name
        self.prevent_unique = prevent_unique  # this can make problems if the upload library itselve handles this
            # example: django_drf_filepond, with True we receive <uuid1>/<uuid2>/name, and should so set prevent_unique=False
    ...
    def get_original_name(self, name):
        return get_original_name(name, prevent_unique=self.prevent_unique)
...
def get_original_name(name, prevent_unique=True):
    if prevent_unique:
        name = name.split('/')
        if len(name) > 1:
            del name[-2]
