...
def project(
    ...
    ctx.obj.require_cloud_access()
    config = ctx.obj.config
    deploy_project(config, deployer, project, stack_file, package_script, dryrun, output_location)


def deploy_project(config, deployer_name, project, stack_file, package_script="./bin/package.sh",  deployer=None, dryrun=False, output_location=None):
    if deployer is None:
