...
class Proxy:
    ...
    async def predict_with_prepare(self, request: Request) -> Response:
        body = await request.body()
        headers = dict(request.headers)
    ...
    async def predict_no_prepare(self, request: Request) -> Response:
        try:
    ...
    def get_routes(self) -> List[Route]:
        predict_method = self.predict_with_prepare if self.has_prepare else self.predict_no_prepare
        return [
            Route("/predict", predict_method, methods=["POST"]),
