...
class CallbackClient(Client):
    ...
    def report_statistics(self, hosts: int, services: int, credentials: int,
                          exploits: List[Tuple[str, int]], vulnerabilities: List[Tuple[str, int]],
                          passwords: List[Tuple[str, int]]) -> None:
        """Report the statistics to the server"""
        result = callback_pb2.StatisticsMessage(
            hosts=hosts, services=services, credentials=credentials
        )
