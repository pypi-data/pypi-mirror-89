...
class Extreme:
	...
	def fetch(self) -> float:
		...
		Snippet: value: float = driver.uwbMeas.multiEval.modulation.freqOffset.extreme.fetch() \n
	...
	def read(self) -> float:
		...
		Snippet: value: float = driver.uwbMeas.multiEval.modulation.freqOffset.extreme.read() \n
		Returns the frequency error relative to the channel center frequency in ppm for the selected packet number <PPDU>. \n
		Suppressed linked return values: reliability \n
			:return: offset: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
	...
	def calculate(self) -> enums.ResultStatus2:
		...
		Snippet: value: enums.ResultStatus2 = driver.uwbMeas.multiEval.modulation.freqOffset.extreme.calculate() \n
		Returns the frequency error relative to the channel center frequency in ppm for the selected packet number <PPDU>. \n
		Suppressed linked return values: reliability \n
			:return: offset: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
