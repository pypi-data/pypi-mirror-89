...
#     for i in range(adata.shape[0]//chunk_size+1):
#         log.info(i)
#         counts = np.ravel(adata.X[i*chunk_size:(i+1)*chunk_size].sum(1))
#         log.info('counts')
#         sparsefuncs.inplace_row_scale(adata.X[i*chunk_size:(i+1)*chunk_size], target_sum/counts)
#         adata.X[i*chunk_size:(i+1)*chunk_size] = sc.pp.normalize_total(adata[i*chunk_size:(i+1)*chunk_size], target_sum=target_sum, exclude_highly_expressed=True, inplace=False)['X']
...
def load_dataset(
    ...
    if partial:
        testloader = DataLoader(scdata, batch_size=batch_size, drop_last=False, shuffle=True)
    else:
