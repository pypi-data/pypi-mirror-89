...
class Data:
	...
	def set(self, data: enums.DataSour, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.up.cell.enh.dch.hsupa.data.set(data = enums.DataSour.DLISt, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.DataSour:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:ENH:DCH:HSUPA:DATA \n
		Snippet: value: enums.DataSour = driver.source.bb.tdscdma.up.cell.enh.dch.hsupa.data.get(stream = repcap.Stream.Default) \n
		The command determines the data source for the HSDPA/HSUPA channels. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
