...
def get_all(table):
    ...
    return df
def remove_ENR(i,d,cursor, conn, kwargs):
    # value = i[d['ministock_name']][:i[d['ministock_name']].index(kwargs['string'])]
    # print(i[d['ministock_name']])
    # update = '''UPDATE %s SET %s = '%s' WHERE id = %i;'''%(kwargs['table'],kwargs['column'],value, i[d['id']])
