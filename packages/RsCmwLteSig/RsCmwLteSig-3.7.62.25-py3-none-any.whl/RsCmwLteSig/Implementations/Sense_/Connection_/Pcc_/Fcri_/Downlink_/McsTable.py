from .......Internal.Core import Core
from .......Internal.CommandsGroup import CommandsGroup


# noinspection PyPep8Naming,PyAttributeOutsideInit,SpellCheckingInspection
class McsTable:
	"""McsTable commands group definition. 2 total commands, 2 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("mcsTable", core, parent)

	@property
	def determined(self):
		"""determined commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_determined'):
			from .McsTable_.Determined import Determined
			self._determined = Determined(self._core, self._base)
		return self._determined

	@property
	def ssubframe(self):
		"""ssubframe commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_ssubframe'):
			from .McsTable_.Ssubframe import Ssubframe
			self._ssubframe = Ssubframe(self._core, self._base)
		return self._ssubframe

	def clone(self) -> 'McsTable':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = McsTable(self._core, self._base.parent)
		self._base.synchronize_repcaps(new_group)
		return new_group
