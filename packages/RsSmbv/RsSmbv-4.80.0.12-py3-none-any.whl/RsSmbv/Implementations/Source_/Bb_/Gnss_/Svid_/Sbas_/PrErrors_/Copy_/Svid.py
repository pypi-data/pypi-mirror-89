...
class Svid:
	...
	def set(self, svid: enums.SvId, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:SBAS<ST>:PRERrors:COPY:SVID \n
		Snippet: driver.source.bb.gnss.svid.sbas.prErrors.copy.svid.set(svid = enums.SvId._1, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> enums.SvId:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:SBAS<ST>:PRERrors:COPY:SVID \n
		Snippet: value: enums.SvId = driver.source.bb.gnss.svid.sbas.prErrors.copy.svid.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Sets the SV ID to that the configuration form the current satellite (SVID<ch>) is applied. Both SV IDs belong to the same
		GNSS system. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sbas')
