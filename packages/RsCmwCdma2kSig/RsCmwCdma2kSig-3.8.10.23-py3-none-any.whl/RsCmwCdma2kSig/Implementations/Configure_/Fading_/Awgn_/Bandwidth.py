...
class Bandwidth:
	...
	def get_ratio(self) -> float:
		...
		"""
		response = self._core.io.query_str('CONFigure:CDMA:SIGNaling<Instance>:FADing:AWGN:BWIDth:RATio?')
	...
	def set_ratio(self, ratio: float) -> None:
		...
		self._core.io.write(f'CONFigure:CDMA:SIGNaling<Instance>:FADing:AWGN:BWIDth:RATio {param}')
	...
	def get_noise(self) -> float:
		...
		Queries the bandwidth of the AWGN inserted via the internal fading module. \n
			:return: noise_bandwidth: Range: 0.25 MHz to 80 MHz , Unit: Hz
		"""
		response = self._core.io.query_str('CONFigure:CDMA:SIGNaling<Instance>:FADing:AWGN:BWIDth:NOISe?')
		return Conversions.str_to_float(response)
