...
class Fetap:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("fetap", core, parent)

	@property
	def drc(self):
		"""drc commands group. 0 Sub-classes, 5 commands."""
		if not hasattr(self, '_drc'):
			from .Fetap_.Drc import Drc
	...
	def lback(self):
		...
		if not hasattr(self, '_lback'):
			from .Fetap_.Lback import Lback
	...
	def ack(self):
		...
		if not hasattr(self, '_ack'):
			from .Fetap_.Ack import Ack
			self._ack = Ack(self._core, self._base)
		return self._ack

	def clone(self) -> 'Fetap':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Fetap(self._core, self._base.parent)
