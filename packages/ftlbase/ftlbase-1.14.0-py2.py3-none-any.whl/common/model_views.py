...
class GenericModelView(View):
    ...
    def get_extrajavascript(self):
        ...
        try:
            script = self.form_class.extrajavascript(instance=self.object)
...
class URLGenerator(object):
    ...
    def get_create_path(self, form_class=None, **kwargs):
        """
        ...
        return re_path(
            ...
            CreateView.as_view(model=self.model, form_class=form_class,
                               success_url=self.get_cancel_url(), cancel_url=self.get_cancel_url(),
                               **kwargs),
            name='%s_create' % self.view_prefix,
        )

    def get_update_path(self, form_class=None, **kwargs):
        """
        ...
        return re_path(
            ...
            UpdateView.as_view(model=self.model, form_class=form_class, lookup_field=self.lookup_field,
                               success_url=self.get_cancel_url(), cancel_url=self.get_cancel_url(),
                               **kwargs),
            name='%s_update' % self.view_prefix,
        )
    ...
    def get_list_path(self, **kwargs):
        ...
        return re_path(
            ...
            ListView.as_view(model=self.model, form_class=self.form_table, **kwargs),
            name='%s' % self.view_prefix,
            # name='%s_list' % self.view_prefix,
        )

    def get_delete_path(self, form_class=None, **kwargs):
        """
        ...
        return re_path(
            ...
            DeleteView.as_view(model=self.model, form_class=form_class, lookup_field=self.lookup_field,
                               success_url=self.get_cancel_url(), cancel_url=self.get_cancel_url(),
                               # success_url=reverse_lazy('%s_delete' % self.view_prefix),
                               **kwargs
                               ),
            name='%s_delete' % self.view_prefix,
        )

    def get_detail_path(self, form_class=None, **kwargs):
        """
        ...
        return re_path(
            ...
            DetailView.as_view(model=self.model, form_class=form_class,lookup_field=self.lookup_field,
                               cancel_url=self.get_cancel_url(),
                               **kwargs),
            name='%s_detail' % self.view_prefix,
        )

    def get_urlpatterns(self, crud_types='crudl', paginate_by=15):
        """
        ...
        urlpatterns = []
        if 'c' in crud_types:
            urlpatterns.append(self.get_create_path())
        if 'r' in crud_types:
            urlpatterns.append(self.get_detail_path())
        if 'u' in crud_types:
            urlpatterns.append(self.get_update_path())
        if 'l' in crud_types:
            urlpatterns.append(self.get_list_path(paginate_by=paginate_by))
        if 'd' in crud_types:
            urlpatterns.append(self.get_delete_path())
