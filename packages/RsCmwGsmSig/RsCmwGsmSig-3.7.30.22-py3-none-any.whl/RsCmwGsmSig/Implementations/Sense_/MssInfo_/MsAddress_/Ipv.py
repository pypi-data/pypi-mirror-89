...
class Ipv:
	...
	def get(self, iPversion=repcap.IPversion.Default) -> List[str]:
		...
		Snippet: value: List[str] = driver.sense.mssInfo.msAddress.ipv.get(iPversion = repcap.IPversion.Default) \n
		...
		response = self._core.io.query_str(f'SENSe:GSM:SIGNaling<Instance>:MSSinfo:MSADdress:IPV{iPversion_cmd_val}?')
