...
from sadedegel.tokenize import NLTKPunctTokenizer, RegexpSentenceTokenizer, DocBuilder
...
def evaluate(v):
    ...
    y_pred = [[s.text for s in DocBuilder(doc).sents] for doc in raw]
...
def build():
    ...
    y = flatten(
        [[is_eos(span, sent['sentences']) for span in DocBuilder(raw).spans] for raw, sent in zip(raw_corpus, sent_corpus)])
