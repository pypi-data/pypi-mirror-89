...
class Mito extends React.Component<MitoProps, MitoState> {
    ...
    constructor(props: MitoProps) {
        ...
        this.setTaskpaneOpenOrClosed = this.setTaskpaneOpenOrClosed.bind(this);
    }

    /* 
        Used to open and close the taskpane
    */ 
    setTaskpaneOpenOrClosed(setTaskpaneOpen: boolean) : void {
        // If opening the taskpane, turn off cell editing mode and close modals + documentation
        if (setTaskpaneOpen) {

            // make sure that there is data in the sheet before trying to open the taskpane 
            // in order to prevent sheet crashing
            if (this.state.sheetJSONArray.length == 0) {
                return;
            }
    ...
    */
    ...
    setEditingMode(on: boolean, column: string, rowIndex: number) : void {
        if (on && !this.state.editingModeState.editingModeOn) {
            // we close the taskpane if its open
            this.setTaskpaneOpenOrClosed(false)
    ...
    */
    ...
    setModal(modalInfo: ModalInfo) : void {
        ...
        // if we're opening a modal other than the import modal, and the is no data in sheetJSON, 
        // don't open the modal to prevent the sheet from crashing

        // NOTE: move this logic inside of the taskpane itself when we make the move
        if (modalInfo.type !== ModalEnum.None && modalInfo.type !== ModalEnum.Import && this.state.sheetJSONArray.length == 0) {
            return;
        }

        // unless opening an error modal or closing a modal, close the taskpane
    ...
    */ 
    ...
    render() {
        ...
        const mitoFormulaBarAndMitosheetClass = this.state.isTaskpaneOpen ? 
            'mito-formula-bar-and-mitosheet-div mito-formula-bar-and-mitosheet-div-taskpane-open' : 
            'mito-formula-bar-and-mitosheet-div' 

        const mitoDefaultTaskpaneClass = this.state.isTaskpaneOpen ? 
            ...
            <div className="mito-container">
                <div className="mitosheet">
                    <MitoToolbar 
                        ...
                        setTaskpaneOpenOrClosed={this.setTaskpaneOpenOrClosed}
                        />
