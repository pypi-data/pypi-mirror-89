...
class RsCmwLteSig:
	...
	driver_options = "SupportedInstrModels = CMW500/CMW270/CMW280/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.7.62.0025'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwLteSig.driver_options, options, direct_session)
		self._core.driver_version = '3.7.62.0025'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '3.7.62.0025'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwLteSig version failed. Current version: '3.7.62.0025', minimum required version: '{min_version}'")
	...
	def extendedBler(self):
		"""extendedBler commands group. 5 Sub-classes, 3 commands."""
		if not hasattr(self, '_extendedBler'):
			from .Implementations.ExtendedBler import ExtendedBler
