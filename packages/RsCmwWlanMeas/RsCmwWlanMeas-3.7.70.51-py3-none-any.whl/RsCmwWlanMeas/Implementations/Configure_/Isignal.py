...
class Isignal:
	...
	def get_standard(self) -> enums.IeeeStandard:
		...
		frame trigger in the signaling application. Selecting a standard that is not compatible with the current scenario
		restores the 'Standalone' scenario. \n
	...
	def get_btype(self) -> enums.BurstType:
		...
		Sets the burst type for standard IEEE 802.11n. Do not use the command for other standards. \n
	...
	def get_bandwidth(self) -> enums.Bandwidth:
		...
		IEEE 802.11ax and trigger source set to 'HE_TB Trigger', the setting depends on the trigger configuration:
	...
	def get_modfilter(self) -> enums.ModulationFilter:
		...
		See TRIGger:WLAN:SIGN<i>:RX:MACFrame:RATE For IEEE 802.11ax and trigger source set to 'HE_TB Trigger', the setting
		depends on the trigger configuration: CONFigure:WLAN:SIGN<i>:CONNection:HETF:MCS \n
