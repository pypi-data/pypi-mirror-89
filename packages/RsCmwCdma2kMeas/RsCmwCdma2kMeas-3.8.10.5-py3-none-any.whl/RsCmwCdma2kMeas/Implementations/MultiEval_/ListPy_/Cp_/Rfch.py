...
class Rfch:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("rfch", core, parent)

	@property
	def state(self):
		"""state commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_state'):
			from .Rfch_.State import State
	...
	def current(self):
		...
		if not hasattr(self, '_current'):
			from .Rfch_.Current import Current
	...
	def average(self):
		...
		if not hasattr(self, '_average'):
			from .Rfch_.Average import Average
	...
	def maximum(self):
		...
		if not hasattr(self, '_maximum'):
			from .Rfch_.Maximum import Maximum
	...
	def minimum(self):
		...
		if not hasattr(self, '_minimum'):
			from .Rfch_.Minimum import Minimum
	...
	def clone(self) -> 'Rfch':
		...
		new_group = Rfch(self._core, self._base.parent)
