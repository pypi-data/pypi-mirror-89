...
class EventView(CustomModelViewSet, FrontendView, LiveView, AllView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        start_time = datetime.strptime(
            data['start_time'], "%Y-%m-%dT%H:%M:%S").strftime("%Y-%m-%d %H:%M:%S")
        end_time = datetime.strptime(
            data['end_time'], "%Y-%m-%dT%H:%M:%S").strftime("%Y-%m-%d %H:%M:%S")
        if start_time > date_now:
            data['time_state'] = 1
        elif start_time < date_now and end_time > date_now:
            data['time_state'] = 2
        elif end_time < date_now:
            data['time_state'] = 3
