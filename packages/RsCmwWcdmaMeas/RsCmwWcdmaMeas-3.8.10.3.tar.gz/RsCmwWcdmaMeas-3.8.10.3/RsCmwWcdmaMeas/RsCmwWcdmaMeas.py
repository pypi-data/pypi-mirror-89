...
class RsCmwWcdmaMeas:
	...
	driver_options = "SupportedInstrModels = CMW500/CMP, SupportedIdnPatterns = CMW, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,3.8.10.0003'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCmwWcdmaMeas.driver_options, options, direct_session)
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCmwWcdmaMeas version failed. Current version: '3.8.10.0003', minimum required version: '{min_version}'")
