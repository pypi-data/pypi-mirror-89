...
class Enable:
	...
	def set(self, codec_rate: bool, ims=repcap.Ims.Default, codec=repcap.Codec.Default) -> None:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:EVS:CODec<CodecIdx>:ENABle \n
		Snippet: driver.configure.data.control.ims.update.evs.codec.enable.set(codec_rate = False, ims = repcap.Ims.Default, codec = repcap.Codec.Default) \n
		...
		self._core.io.write(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:CODec{codec_cmd_val}:ENABle {param}')

	def get(self, ims=repcap.Ims.Default, codec=repcap.Codec.Default) -> bool:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:EVS:CODec<CodecIdx>:ENABle \n
		Snippet: value: bool = driver.configure.data.control.ims.update.evs.codec.enable.get(ims = repcap.Ims.Default, codec = repcap.Codec.Default) \n
		Enables or disables a codec rate for the AMR-WB IO mode of the EVS codec, for a call update. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')
			:param codec: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Codec')
		...
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		codec_cmd_val = self._base.get_repcap_cmd_value(codec, repcap.Codec)
		response = self._core.io.query_str(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:CODec{codec_cmd_val}:ENABle?')
