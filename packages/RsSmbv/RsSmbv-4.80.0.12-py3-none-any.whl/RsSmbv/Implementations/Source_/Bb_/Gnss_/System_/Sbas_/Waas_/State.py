...
class State:
	...
	def set(self, state: bool, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SYSTem:SBAS:WAAS<ST>:[STATe] \n
		Snippet: driver.source.bb.gnss.system.sbas.waas.state.set(state = False, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SYSTem:SBAS:WAAS<ST>:[STATe] \n
		Snippet: value: bool = driver.source.bb.gnss.system.sbas.waas.state.get(stream = repcap.Stream.Default) \n
		Defines if satellites from the selected GNSS system are included in the simulated satellites constellation. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sbas')
