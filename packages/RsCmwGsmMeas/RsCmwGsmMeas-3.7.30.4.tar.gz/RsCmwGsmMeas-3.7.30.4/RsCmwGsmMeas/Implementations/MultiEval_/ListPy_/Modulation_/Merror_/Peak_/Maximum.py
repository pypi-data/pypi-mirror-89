...
class Maximum:
	...
	def fetch(self) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:GSM:MEASurement<Instance>:MEValuation:LIST:MODulation:MERRor:PEAK:MAXimum?', suppressed)
		return response
	...
	def calculate(self) -> List[float]:
		...
		Use RsCmwGsmMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: mag_error_peak: float Comma-separated list of values, one per measured segment Range: -100 % to 100 % (AVERage: 0 % to 100 %, SDEViation: 0 % to 50 %) , Unit: %"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'CALCulate:GSM:MEASurement<Instance>:MEValuation:LIST:MODulation:MERRor:PEAK:MAXimum?', suppressed)
		return response
