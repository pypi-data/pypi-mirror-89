...
class Average:
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.segment.obw.average.fetch(segment = repcap.Segment.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:OBW:AVERage?', self.__class__.FetchStruct())
	...
	def calculate(self, segment=repcap.Segment.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.segment.obw.average.calculate(segment = repcap.Segment.Default) \n
		Returns occupied bandwidth (OBW) results for the segment <no> in list mode (see method RsCmwCdma2kMeas.Configure.
		MultiEval.ListPy.value) . To define the statistical length for AVERage, MAXimum and SDEviation calculation and to enable
		the calculation of the results use the command method RsCmwCdma2kMeas.Configure.MultiEval.ListPy.Segment.Spectrum.set.
		The ranges indicated below apply to all results except standard deviation results. The minimum for standard deviation
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:OBW:AVERage?', self.__class__.CalculateStruct())
