...
class RsCMPX_LteMeas:
	...
	driver_options = "SupportedInstrModels = CMX500/CMP/CMW, SupportedIdnPatterns = CMW/CMP/CMX, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,4.0.7.0004'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCMPX_LteMeas.driver_options, options, direct_session)
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_LteMeas version failed. Current version: '4.0.7.0004', minimum required version: '{min_version}'")
