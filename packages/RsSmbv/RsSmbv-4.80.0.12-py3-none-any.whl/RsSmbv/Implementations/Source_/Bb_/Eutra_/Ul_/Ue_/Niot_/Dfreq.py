...
class Dfreq:
	...
	def set(self, delta_freq: float, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.niot.dfreq.set(delta_freq = 1.0, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:NIOT:DFReq \n
		Snippet: value: float = driver.source.bb.eutra.ul.ue.niot.dfreq.get(stream = repcap.Stream.Default) \n
		Sets the frequency offset between the NB-IoT carrier and the LTE center frequency. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
