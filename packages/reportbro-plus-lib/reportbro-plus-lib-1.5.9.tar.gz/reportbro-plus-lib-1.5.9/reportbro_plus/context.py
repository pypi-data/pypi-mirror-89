...
class Context:
    ...
    def build_data(self, parameters, object_id, field, pattern=None):
        ...
        for item in parameters:
            parameter_name = re.search(r'\{(.*?)\}', item).group(1)
    ...
    def fill_parameters(self, expr, object_id, field, pattern=None):
        ...
        stack_formules = list(filter(lambda item: expr.find(item) != -1, formules))
        parameters = list(filter(lambda item: item.find('}') != -1, expr.split(" ")))
        ret = self.build_data(parameters=parameters, object_id=object_id, field=field, pattern=pattern)

        for key in ret.keys():
            if (len(stack_formules) > 0):
                stack_formules.reverse()
                for action in stack_formules:
                    ret[key] = actions[action](ret[key])
                    logging.debug(ret[key])
