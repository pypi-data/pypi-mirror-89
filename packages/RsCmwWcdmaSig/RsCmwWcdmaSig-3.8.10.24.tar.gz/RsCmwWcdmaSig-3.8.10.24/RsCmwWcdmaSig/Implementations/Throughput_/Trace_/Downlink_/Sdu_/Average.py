...
class Average:
	...
	def fetch(self) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WCDMa:SIGNaling<Instance>:THRoughput:TRACe:DL:SDU:AVERage?', suppressed)
		return response
	...
	def read(self) -> List[float]:
		...
		Use RsCmwWcdmaSig.reliability.last_value to read the updated reliability indicator. \n
			:return: downlink_sdu: Comma-separated list of N throughput trace values Range: 0 bit/s to 100E+6 bit/s, Unit: bit/s"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:WCDMa:SIGNaling<Instance>:THRoughput:TRACe:DL:SDU:AVERage?', suppressed)
		return response
