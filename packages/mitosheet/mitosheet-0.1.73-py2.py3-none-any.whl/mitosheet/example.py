...
class MitoWidget(DOMWidget):
    ...
    # aa is a state variable that is explictitly the first alpabetically, and thus 
    # shows up in the string representation of Mito, which occurs if a user
    # installs Mito incorrectly. This points them to the documentation!
    aa = t.Unicode('ERROR: if you see this message, run `help(mitosheet)` for instructions on installing Mito!').tag(sync=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set aa again, just in case
        self.aa = 'ERROR: if you see this message, run `help(mitosheet)` for instructions on installing Mito!'
...
def sheet(*args: List[pd.DataFrame]) -> MitoWidget:
    ...
    NOTE: if you have any issues with installation, please email book a demo time at https://calendly.com/trymito/mito-early-access-demo?utm_source=sheetdocstring
    """
    args = list(args)
