...
class Healthy:
	...
	def set(self, healthy_state: bool, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS<ST>:HEALthy \n
		Snippet: driver.source.bb.gnss.svid.qzss.healthy.set(healthy_state = False, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS<ST>:HEALthy \n
		Snippet: value: bool = driver.source.bb.gnss.svid.qzss.healthy.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Indicates if the selected SV ID is healthy or not. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Qzss')
