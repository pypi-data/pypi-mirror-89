...
class DerkSession:
  ...
  Attributes:
    ...
    team_stats: Stats for each team for the last episode. Numpy array of shape (n_teams, len(team_stats_keys)). See :ref:`reading-stats`
  ...
  async def step(self, action_n: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray, List[bool], List[Dict]]:
    ...
    if self.done:
      ...
      points = self.team_stats[:, self.team_stats_keys.Reward.value]
      opponent_points = self.team_stats[:, self.team_stats_keys.OpponentReward.value]
