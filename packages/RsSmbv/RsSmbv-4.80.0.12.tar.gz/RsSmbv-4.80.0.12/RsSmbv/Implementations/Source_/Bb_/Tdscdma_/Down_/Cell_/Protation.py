...
class Protation:
	...
	def set(self, protation: enums.TdscdmaPhasRot, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.down.cell.protation.set(protation = enums.TdscdmaPhasRot.AUTO, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.TdscdmaPhasRot:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:DOWN:CELL<ST>:PROTation \n
		Snippet: value: enums.TdscdmaPhasRot = driver.source.bb.tdscdma.down.cell.protation.get(stream = repcap.Stream.Default) \n
		Selects the phase rotation for the downlink pilots. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
