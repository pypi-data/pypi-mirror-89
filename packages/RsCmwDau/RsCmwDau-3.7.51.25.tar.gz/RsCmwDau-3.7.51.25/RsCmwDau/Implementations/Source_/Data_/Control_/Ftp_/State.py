...
class State:
	...
	def set(self, control: bool) -> None:
		...
		Snippet: driver.source.data.control.ftp.state.set(control = False) \n
		Starts or stops the FTP service. \n
			:param control: ON | OFF Switch the service ON or OFF
		"""
		param = Conversions.bool_to_str(control)
		self._core.io.write_with_opc(f'SOURce:DATA:CONTrol:FTP:STATe {param}')

	# noinspection PyTypeChecker
	def get(self) -> enums.DauState:
		"""SCPI: SOURce:DATA:CONTrol:FTP:STATe \n
		Snippet: value: enums.DauState = driver.source.data.control.ftp.state.get() \n
		Starts or stops the FTP service. \n
		...
		response = self._core.io.query_str_with_opc(f'SOURce:DATA:CONTrol:FTP:STATe?')
