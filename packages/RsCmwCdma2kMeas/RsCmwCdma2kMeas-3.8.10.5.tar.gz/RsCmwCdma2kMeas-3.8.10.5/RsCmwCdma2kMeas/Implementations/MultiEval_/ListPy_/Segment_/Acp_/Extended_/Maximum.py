...
class Maximum:
	...
	class FetchStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct('Maximum_Acp', DataType.FloatList, None, False, False, 41),
		...
		def __init__(self):
			...
			self.Maximum_Acp: List[float] = None
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.segment.acp.extended.maximum.fetch(segment = repcap.Segment.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:ACP:EXTended:MAXimum?', self.__class__.FetchStruct())
	...
	class CalculateStruct(StructBase):
		...
		__meta_args_list = [
			ArgStruct.scalar_int('Reliability', 'Reliability'),
			ArgStruct.scalar_int('Seg_Reliability'),
			ArgStruct('Maximum_Acp', DataType.EnumList, enums.ResultStatus2, False, False, 41),
			ArgStruct.scalar_float('Ms_Power_Wide'),
			ArgStruct.scalar_float('Ms_Power_Narrow'),
			ArgStruct.scalar_float('Out_Of_Tol_Count'),
		...
		def __init__(self):
			...
			self.Reliability: int = None
			self.Seg_Reliability: int = None
			self.Maximum_Acp: List[enums.ResultStatus2] = None
			self.Ms_Power_Wide: float = None
			self.Ms_Power_Narrow: float = None
			self.Out_Of_Tol_Count: float = None
	...
	def calculate(self, segment=repcap.Segment.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.segment.acp.extended.maximum.calculate(segment = repcap.Segment.Default) \n
		Returns the adjacent channel power (ACP) results for segment <no> in list mode (see method RsCmwCdma2kMeas.Configure.
		MultiEval.ListPy.value) . If enabled the wideband power and narrowband power results are returned, too. To define the
		statistical length for AVERage, MINimum, MAXimum and SDEviation calculation and enable the calculation of the results use
		the command method RsCmwCdma2kMeas.Configure.MultiEval.ListPy.Segment.Spectrum.set. The values described below are
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:ACP:EXTended:MAXimum?', self.__class__.CalculateStruct())
