 ...
 */
template<
	typename Derived, typename State_, typename InternalState_,
...
public:
	...
	public:
		LazyChainStateRef(
			Derived& markov_chain, const InternalState& internal_state)
		...
		{
			if (!has_state_) {
				state_ = markov_chain_.convert_from_internal_state(
					internal_state_);
				has_state_ = true;
			}
			return state_;
...
protected:
	...
	virtual State convert_from_internal_state(
		const InternalState& internal_state) = 0;
...
#define _SAMPLY_MARKOVCHAIN_TEMPLATE_DECL \
	...
	template<typename, typename> typename Logger
#define _SAMPLY_MARKOVCHAIN_TEMPLATE_SPEC \
	Derived, State, InternalState, Logger
	...
	const size_t num_steps, const InputDuration max_time)
{
	...
	for (; step_idx < num_steps && !time_limit_reached;
		 ++step_idx) {

		// Simulate the next step of the chain.
		internal_state_ = 
			static_cast<Derived&>(*this).next_state(internal_state_);

		// Notify the logger that the step has been completed.
		const auto elapsed_time =
			duration_cast<DurationFormat>(chain_clock::now() - start_time);
		LazyChainStateRef state_ref(
			static_cast<Derived&>(*this), internal_state_);
...
{
	internal_state_ = 
		static_cast<Derived&>(*this).convert_to_internal_state(state);
}

template<_SAMPLY_MARKOVCHAIN_TEMPLATE_DECL>
inline State MarkovChain<_SAMPLY_MARKOVCHAIN_TEMPLATE_SPEC>::get_state()
{
	return static_cast<Derived&>(*this).convert_from_internal_state(
		internal_state_);
}
