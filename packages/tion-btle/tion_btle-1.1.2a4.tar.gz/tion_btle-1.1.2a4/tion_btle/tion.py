...
class tion(TionDummy):
    ...
    def _connect(self, need_notifications: bool = True):
        """
        Connect to breezer with MAC-address in self.mac
        :param need_notifications: bool, should we subscribe to notification or not
        :return: None
        :raises:
            btle.BTLEDisconnectError: when got unexpected disconnect
        """
        if self.connection_status == "disc":
            try:
                ...
                if need_notifications:
                    _LOGGER.debug("_connect: notification was requested")
                    self._enable_notifications()
                else:
                    _LOGGER.debug("_connect: notification was not requested")
            except btle.BTLEDisconnectError as e:
    ...
    def _disconnect(self):
        ...
        _LOGGER.debug("_disconnect: current status is %s", status)
        if self.connection_status != "disc":
    ...
    def _enable_notifications(self):
        ...
        try:
            # todo некоторые S3 на прошивке 003С зависает в этом месте https://github.com/TionAPI/HA-tion/issues/23
            # todo некоторые Lite на HassOS ловит тут disconnect во время сопряжения https://habr.com/ru/conversations/vekser/
            data = self.notify.read()
            _LOGGER.debug("First read done. Data is %s", bytes(data).hex())
        except btle.BTLEDisconnectError as e:
            _LOGGER.critical("_enable_notifications: got '%s' while first read! Could not continue!")
            raise e
    ...
    def __bt_pair(self) -> bool:
        """
        Perform bluetooth pairing

        :return: Was it succeed?
        """
        result = False
        ...
        try:
            ...
            estat = 0
        # 0 -- fine, 19 -- paired, 20 -- Permission Denied
        if estat == 20:
            _LOGGER.critical("__bt_pair: could not pair! Permission denied. Check permissions at host")
        elif estat == 0 or estat == 19 or rsp['code'][0] == 'success':
            try:
                msg = rsp['emsg'][0]
            except KeyError:
                msg = rsp['code'][0]
            _LOGGER.debug("__bt_pair: %s", msg)
        else:
            _LOGGER.critical("__bt_pair: unexpected response: %s", rsp)
            raise TionException('pair', rsp)
        str_result = "done" if result else "failed"
        _LOGGER.debug("BT pairing is %s", str_result)
        return result
    ...
    def pair(self):
        ...
        try:
            ...
            _LOGGER.debug("pair: device-specific pairing ...")
            self._pair()
            _LOGGER.debug("pair: device pair is done")
        except Exception as e:
            _LOGGER.critical(f"pair: got exception while pair {type(e).__name__}: {str(e)}")
            raise TionException('pair', f"{type(e).__name__}: {str(e)}")
        finally:
            _LOGGER.debug("pair: disconnecting")
            self._disconnect()
