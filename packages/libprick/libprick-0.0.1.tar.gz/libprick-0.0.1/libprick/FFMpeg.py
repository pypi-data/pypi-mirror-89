...
class FFMpeg:
    ...
    def __init__(self):
        ...
        if os_family == 'Windows':
            ...
            FFMpeg.avformat = CDLL("{0}format.cpython-37m-x86_64-linux-gnu.so".format(lib_path))
            FFMpeg.avcodec = CDLL("{0}codec{1}codec.cpython-37m-x86_64-linux-gnu.so".format(lib_path, os.path.sep))
            FFMpeg.avutil = CDLL("{0}utils.cpython-37m-x86_64-linux-gnu.so".format(lib_path))
        else:
    ...
    def open(self, path):
        ...
        #avformat.av_dump_format(self.pFormatCtx, 0, None, 0)
        if res < 0:
    ...
    def __get_lib_path() -> str:
        ...
        if os_family == 'Windows':
            path = "{0}{1}Lib{1}site-packages{1}av{1}".format(sys.base_exec_prefix, os.path.sep)
            if 'VIRTUAL_ENV' in os.environ:
                path = "{0}{1}lib{1}site-packages{1}av{1}".format(os.environ['VIRTUAL_ENV'], os.path.sep)

        elif os_family == 'Linux':
            path = "/usr/lib/python3/dist-packages/av/"
            if 'VIRTUAL_ENV' in os.environ:
