...
class polynomial:
    ...
    def __call__(self, x):
        if len(x) == 1:
            ...
            coeff = ((x[0]+x[1])/2)**self.degree/sp.pi           
        else:
            raise ValueError('The encoding map "polynomial" supports only up to second order feature maps')
        return coeff
...
class polynomial_test:
    def __init__(self, degree=1):
        self.degree = degree
    def __call__(self, x):
        coeff = (reduce(lambda m, n: m + n, x)/len(x))**degree/sp.pi
        return coeff
