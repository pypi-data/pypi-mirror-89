...
class RsSmbv:
	...
	driver_options = "SupportedInstrModels = SMW/SMBV, SupportedIdnPatterns = SMW/SMBV, SimulationIdnString = 'Rohde&Schwarz,SMBV100B,100001,4.80.0.0012'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.80.0.0012'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.80.0.0012'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmbv version failed. Current version: '4.80.0.0012', minimum required version: '{min_version}'")
