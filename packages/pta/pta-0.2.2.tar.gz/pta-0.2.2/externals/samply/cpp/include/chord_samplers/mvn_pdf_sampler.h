...
public:
	 ...
	 */
	MvnPdfSampler(
		const AffineTransform<double>& samples_transform)
...
protected:
	...
	std::tuple<RaysPacket<Scalar>, IntersectionsPacketType<Scalar>,
		Vector<Scalar>, Vector<ScalarType>>
		reparametrize_rays_(
			const RaysPacket<Scalar>& rays,
	...
	SamplingHelper& sampling_helper)
{
	...
	Vector<Scalar> sampled_ts = 
		(t + ray_t_offsets).cwiseQuotient(ray_t_scale_factors);
	...
	SamplingHelper& sampling_helper)
{
	...
	if (intersections.one_ray_one_intersection) {
		...
		{
			// Choose the segment from which the next point must be sampled.
			Vector<Scalar> segment_weights = weights.cwiseProduct(
				sampling_helper.get_normal_cdf_interval<Scalar>(
					segment_starts, segment_ends));
...
{
	return sample1d(
		AffineTransform<Scalar>::identity(in_rays.origins.rows()) * in_rays,
...
template<typename ScalarType>
template<template<typename> typename IntersectionsPacketType>
inline std::tuple<RaysPacket<ScalarType>, IntersectionsPacketType<ScalarType>,
	Vector<ScalarType>, Vector<ScalarType>>
	...
	const IntersectionsPacketType<ScalarType>& in_intersections)
{
	...
	if (intersections.one_ray_one_intersection) {
		...
		intersections.segment_starts =
			in_intersections.segment_starts.cwiseProduct(
				ray_t_scale_factors(in_intersections.ray_indices));
		intersections.segment_ends =
			in_intersections.segment_ends.cwiseProduct(
				ray_t_scale_factors(in_intersections.ray_indices));
	}
	...
	Matrix<Scalar> origins = 
		rays.origins + directions * ray_t_offsets.asDiagonal();

	if (intersections.one_ray_one_intersection) {
		intersections.segment_starts =
			intersections.segment_starts - ray_t_offsets;
		intersections.segment_ends =
			intersections.segment_ends - ray_t_offsets;
	} else {
		intersections.segment_starts = intersections.segment_starts - 
			ray_t_offsets(in_intersections.ray_indices);
		intersections.segment_ends = intersections.segment_ends -
			ray_t_offsets(in_intersections.ray_indices);
	}

	return std::tuple<RaysPacket<Scalar>, IntersectionsPacketType<Scalar>,
		Vector<Scalar>, Vector<Scalar>>(
			RaysPacket<Scalar>{ origins, directions }, intersections,
			ray_t_scale_factors, ray_t_offsets);
}
