...
class Wgs:
	...
	def set(self, structure: WgsStruct, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RECeiver:[V<ST>]:LOCation:COORdinates:DMS:[WGS<CH>] \n
		Snippet: driver.source.bb.gnss.receiver.v.location.coordinates.dms.wgs.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> WgsStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RECeiver:[V<ST>]:LOCation:COORdinates:DMS:[WGS<CH>] \n
		Snippet: value: WgsStruct = driver.source.bb.gnss.receiver.v.location.coordinates.dms.wgs.get(stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
		Defines the coordinates of the geographic location of the GNSS receiver in degrees, minutes and seconds. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'V')
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Wgs')
