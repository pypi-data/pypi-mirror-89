...
class FabricModLoader (ModLoader):
    ...
    def install(self, cache_location, install_location, side):
        ...
        if side == 'client':
            self.install_client(cache_location, install_location)
        elif side == 'server':
            self.install_server(cache_location, install_location)

    def install_client(self, cache_location, install_location):
        lib_location = os.path.join(install_location, 'libraries')
        if not os.path.exists(lib_location):
            os.makedirs(lib_location)
        self._install_libraries(cache_location, lib_location, self.get_libraries('client'))

    def install_server(self, cache_location, install_location):
        ...
        with zipfile.ZipFile(launcher_jar, mode='w', compression=zipfile.ZIP_DEFLATED) as jarfile:
            ...
            for library in self.get_libraries('server'):
                ...
                try:
                    downloaded_file = self.downloader.download(url, library.get_path())
