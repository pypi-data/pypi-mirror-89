...
class Maximum:
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.segment.dwcp.maximum.fetch(segment = repcap.Segment.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:DWCP:MAXimum?', self.__class__.FetchStruct())
	...
	def calculate(self, segment=repcap.Segment.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.segment.dwcp.maximum.calculate(segment = repcap.Segment.Default) \n
		Returns the scalar channel power for the data channel results for the segment <no> in list mode. The result is extended
		to the W24 and W12 I/Q values of the data channel. Only available if subtype 2 or 3 is selected. Otherwise NAV is
		returned in list mode. To define the statistical length for AVERage, MAXimum, MINimum calculation and to enable the
		calculation of the results, use the command method RsCmwEvdoMeas.Configure.MultiEval.ListPy.Segment.Modulation.set. The
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:DWCP:MAXimum?', self.__class__.CalculateStruct())
