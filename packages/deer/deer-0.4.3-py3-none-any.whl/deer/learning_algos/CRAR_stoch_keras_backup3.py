...
def exp_dec_error(y_true, y_pred):
    return K.exp( - 2* K.sqrt( K.clip(K.sum(K.square(y_pred), axis=-1, keepdims=True),0.000001,10) )  ) # *2 distrib, *5 simple maze
...
class CRAR(LearningAlgo):
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        noise_level=max( (1.-self.update_counter/50000) , 0.2 ) #0.2
        noise_level2=max( (1.-self.update_counter/100000) , 0.4 ) #0.4
...
#
#        print ("diff enc")
#        noise_e=np.random.normal(0,noise_level/2.,Es_[:self._batch_size].shape)
...
#        print (self.transition_g_full2.train_on_batch(states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) ))
        ...
        if(self.update_counter%500==0):
            ...
            print (self.transition_g_full.predict([states_val[0][:5]]+[onehot_actions[:5],noise[:5]]))
...
#        print (self.transition_d.layers[7].get_weights())
        ...
        if (self.wait_G==0):
            loss_G1=self.transition_g_full.train_on_batch(states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) )
#            print ("after G1")
#            print (self.encoder.layers[2].get_weights())
#            print (self.transition.layers[7].get_weights())
#            print (self.transition_d.layers[7].get_weights())
            self.loss_G1=0.9*self.loss_G1+0.1*loss_G1
            loss_G2=self.encoder_g_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,np.random.normal(0,noise_level/2.,Es_.shape)], np.zeros((self._batch_size,1)) )
...
#
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.transition_g_full.optimizer.lr, self._lr/50.) # /5. for simple laby or simple catcher; /50. for distrib of laby
        K.set_value(self.encoder_g_full.optimizer.lr, self._lr/50.) # /5. for simple laby or simple catcher; /50. or distrib of laby
        K.set_value(self.transition_g_full2.optimizer.lr, self._lr)
        
        K.set_value(self.encoder.optimizer.lr, self._lr/10.)
        K.set_value(self.encoder_diff.optimizer.lr, self._lr) # /2 for simple laby or simple catcher; *1 for distrib of laby

        K.set_value(self.diff_s_s_.optimizer.lr, self._lr/10.) # /5. for simple laby or simple catcher; /1. for distrib of laby
