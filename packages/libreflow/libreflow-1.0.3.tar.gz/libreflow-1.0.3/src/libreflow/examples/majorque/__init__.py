...
from .runners import AfterEffect, Photoshop
...
class Project(baseflow.Project, flow.InjectionProvider):
    ...
    def get_default_contextual_edits(self, context_name):
        if context_name == 'settings':
            defaultContextualEdits = baseflow.Project.get_default_contextual_edits(self, 'settings')
            defaultContextualEdits.update(dict(
                ROOT_DIR='M:/',
                USER_FOLDER=os.path.join(pathlib.Path.home(), '.libreflow'),
