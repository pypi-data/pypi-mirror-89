...
class State:
	...
	def fetch(self) -> List[enums.SigChStateA]:
		...
		Snippet: value: List[enums.SigChStateA] = driver.multiEval.listPy.cto.rpiCh.state.fetch() \n
		...
		Use RsCmwCdma2kMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: rpi_ch: INVisible | ACTive | IACTive | ALIased Comma-separated list of states, one per active segment."""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:CTO:RPICh:STATe?', suppressed)
