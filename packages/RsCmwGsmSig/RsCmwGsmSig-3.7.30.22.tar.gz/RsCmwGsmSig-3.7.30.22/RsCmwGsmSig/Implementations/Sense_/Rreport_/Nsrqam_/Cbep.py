...
class Cbep:
	...
	def get(self, nsrQAM=repcap.NsrQAM.Default) -> int:
		...
		Snippet: value: int = driver.sense.rreport.nsrqam.cbep.get(nsrQAM = repcap.NsrQAM.Default) \n
		Returns the 'CV BEP', reported by the MS as dimensionless index for a 16-QAM or 32-QAM modulated DL signal with normal
		symbol rate (NSR) . \n
		...
		response = self._core.io.query_str(f'SENSe:GSM:SIGNaling<Instance>:RREPort:NSRQam{nsrQAM_cmd_val}:CBEP?')
