...
class Connector:
	...
	def set(self, cmws_connector: enums.CmwsConnector, segment=repcap.Segment.Default) -> None:
		...
		Selects the RF input connector for segment <no> for GSM list mode measurements with the R&S CMWS. This setting is only
		relevant for connector mode LIST, see method RsCmwGsmMeas.Configure.MultiEval.ListPy.SingleCmw.cmode. All segments of a
		...
		self._core.io.write(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:CMWS:CONNector {param}')
	...
	def get(self, segment=repcap.Segment.Default) -> enums.CmwsConnector:
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		response = self._core.io.query_str(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:CMWS:CONNector?')
