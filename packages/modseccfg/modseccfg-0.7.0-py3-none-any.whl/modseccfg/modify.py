...
class show:
    ...
    def __init__(self, id, mainwindow, data):
        ...
        layout += [
            ...
            [sg.Multiline("# Note that this isn't quite functional yet..", k="preview", auto_size_text=1, size=(60,4), background_color="#eee")]
        ]
        ...
        for row in show.flags:
            layout_flags.append([self.w_flag(_) for _ in row])
        # transform
        ...
        layout.append(self.w_param("status", width=10) + self.w_param("redirect", width=40))
        layout.append(self.w_ctl("ruleEngine") + self.w_param("phase", width=7, disabled=1) + self.w_param("skip", width=9))
        ...
        for name in [k for k in r.ctl.keys() if not k in have]:
            layout.append(self.w_ctl(name, width=40))
        layout.append([sg.T("")])
    ...
    def w_flag(self, name):
        state = name in self.r.flags
        return sg.Checkbox(f"{name} ", key=f"flags[{name}]", tooltip=wrap(show.tooltip.get(name)), default=state, background_color=self.colorize.get(name), enable_events=True)
    ...
    #   plain list again.)
    exclude_flags = ["ver", "rev", "severity", "tag", "accuracy", "maturity"]
