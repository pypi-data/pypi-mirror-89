...
class TranscriptionDataSource(FileDataSource):
    """Transcription data source for CMU Arctic dataset.

   Args:
        data_root (str): Data root.
    """

    def __init__(self, data_root, speaker):
        txt = join(data_root, "cmu_us_{}_arctic".format(speaker), "etc", "txt.done.data")
        with open(txt) as f:
            metadata = {}
            for l in f:
                # remove ( and )
    ...
    def collect_files(self):
        ...
        """
        return list(map(lambda s:s[1], self.metadata))
