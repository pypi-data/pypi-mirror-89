...
class Info:
	...
	def set(self, ims=repcap.Ims.Default) -> None:
		...
		Snippet: driver.configure.data.control.ims.clean.general.info.set(ims = repcap.Ims.Default) \n
		Clears the 'General IMS Info' area. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')"""
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		self._core.io.write(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:CLEan:GENeral:INFO')

	def set_with_opc(self, ims=repcap.Ims.Default) -> None:
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:CLEan:GENeral:INFO \n
		Snippet: driver.configure.data.control.ims.clean.general.info.set_with_opc(ims = repcap.Ims.Default) \n
		Clears the 'General IMS Info' area. \n
		Same as set, but waits for the operation to complete before continuing further. Use the RsCmwDau.utilities.opc_timeout_set() to set the timeout value. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')"""
		self._core.io.write_with_opc(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:CLEan:GENeral:INFO')
