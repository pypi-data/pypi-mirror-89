...
class Up:
	...
	def read(self, sequence=repcap.Sequence.Default) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:CDMA:MEASurement<Instance>:RPINterval:SEQuence{sequence_cmd_val}:TRACe:UP?', suppressed)
		return response
	...
	def fetch(self, sequence=repcap.Sequence.Default) -> List[float]:
		...
		sequence_cmd_val = self._base.get_repcap_cmd_value(sequence, repcap.Sequence)
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:CDMA:MEASurement<Instance>:RPINterval:SEQuence{sequence_cmd_val}:TRACe:UP?', suppressed)
		return response
