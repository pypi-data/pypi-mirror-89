...
class Rows:
	...
	def set(self, row_count: int, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.dpcch.hs.pcqi.rows.set(row_count = 1, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:DPCCh:HS:PCQI:ROWS \n
		Snippet: value: int = driver.source.bb.w3Gpp.mstation.dpcch.hs.pcqi.rows.get(stream = repcap.Stream.Default) \n
		Determines the number of the rows in the HARQ-ACK respectively in the PCI/CQI scheduling table. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
