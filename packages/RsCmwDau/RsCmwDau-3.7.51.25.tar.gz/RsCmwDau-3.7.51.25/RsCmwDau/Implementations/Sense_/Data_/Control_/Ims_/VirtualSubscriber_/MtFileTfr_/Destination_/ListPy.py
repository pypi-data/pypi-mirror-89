...
class ListPy:
	...
	def get(self, ims=repcap.Ims.Default, virtualSubscriber=repcap.VirtualSubscriber.Default) -> List[str]:
		...
		Snippet: value: List[str] = driver.sense.data.control.ims.virtualSubscriber.mtFileTfr.destination.listPy.get(ims = repcap.Ims.Default, virtualSubscriber = repcap.VirtualSubscriber.Default) \n
		...
		response = self._core.io.query_str(f'SENSe:DATA:CONTrol:IMS{ims_cmd_val}:VIRTualsub{virtualSubscriber_cmd_val}:MTFiletfr:DESTination:LIST?')
