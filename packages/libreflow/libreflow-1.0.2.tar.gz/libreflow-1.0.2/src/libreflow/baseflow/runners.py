...
class Blender(EditFileRunner):
    ...
    TAGS = ['Modeling', 'Sculpting', 'Animation', 'Rigging', '3D Drawing', 'Rendering', 'Simulation', 'Video Editing', 'VFX']
...
class RunnerChoiceValue(flow.values.ChoiceValue):
    ...
    def choices(self):
        ...
        runner_names = [
            nat[0] for nat in factory.find_runners(
                '*.{}'.format(self._extension.name())
            )
        ]
...
class ChangeDefaultRunner(flow.Action):
    ...
    def run(self, button):
        ...
        if self.version.get() and runner_versions and not str(self.version.get()) in runner_versions:
            self.root().session().log_debug('Version {version} of {app} unknown'.format(
...
class DefaultRunner(flow.Object):
    ...
    def get_runner(self):
        ...
        if not self.runner_name.get():
            runners = factory.find_runners(edited_filename='*.{ext}'.format(ext=self.name()))
