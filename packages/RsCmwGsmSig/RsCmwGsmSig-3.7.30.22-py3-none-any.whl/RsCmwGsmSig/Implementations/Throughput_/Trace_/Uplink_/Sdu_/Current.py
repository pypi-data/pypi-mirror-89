...
class Current:
	...
	def fetch(self) -> List[float]:
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:GSM:SIGNaling<Instance>:THRoughput:TRACe:UL:SDU:CURRent?', suppressed)
		return response
	...
	def read(self) -> List[float]:
		...
		Use RsCmwGsmSig.reliability.last_value to read the updated reliability indicator. \n
			:return: uplink_sdu: float Comma-separated list of n throughput trace values Range: 0 bit/s to 100E+6 bit/s , Unit: bit/s"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:GSM:SIGNaling<Instance>:THRoughput:TRACe:UL:SDU:CURRent?', suppressed)
		return response
