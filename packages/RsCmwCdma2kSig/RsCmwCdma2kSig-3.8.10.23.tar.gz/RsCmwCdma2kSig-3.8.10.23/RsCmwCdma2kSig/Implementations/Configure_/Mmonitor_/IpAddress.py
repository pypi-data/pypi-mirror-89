...
class IpAddress:
	...
	def set(self, index: enums.IpAddressIndex) -> None:
		...
		Select/get the target IP address for message monitoring (see method RsCmwCdma2kSig.Configure.Mmonitor.enable) . The IP
		addresses are centrally managed from the 'Setup' dialog. \n
			:param index: IP1 | IP2 | IP3
		"""
		param = Conversions.enum_scalar_to_str(index, enums.IpAddressIndex)
		self._core.io.write(f'CONFigure:CDMA:SIGNaling<Instance>:MMONitor:IPADdress {param}')
	...
	def get(self) -> GetStruct:
		...
		return self._core.io.query_struct(f'CONFigure:CDMA:SIGNaling<Instance>:MMONitor:IPADdress?', self.__class__.GetStruct())
