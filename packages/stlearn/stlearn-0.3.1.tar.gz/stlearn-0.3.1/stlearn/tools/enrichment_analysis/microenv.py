...
def microenv(
    ...
    for gene in adata.var_names:
        ...
        genes.append(gene)
        result_pearson.append(
            pearsonr(ft, adata[:, gene].X.toarray().reshape(-1, len(adata))[0])[0]
        )
    ...
    top_genes = list(
        cor_results.sort_values("Pearson_correlation", ascending=False)[:n_top].Gene
    )
    ...
    adata.uns["factor_sig"][use_data]["Factor_" + str(factor)].update(
        {"top_genes": top_genes}
    )

    print(
        "Get top genes of Factor "
        + str(factor)
        + ' is done! The top genes are stored in adata.uns["factor_sig"]["'
        + use_data
        + '"]'
        + '["Factor_'
        + str(factor)
