...
class Rasgo(Connection):
    ...
    @track_usage
    def create_feature_set(self, name: str, datasource_id: int, table_name: str, org_id: int,
    ...
    @track_usage
    def update_feature_set(self, feature_set_id: int, name: Optional[str] = None, datasource_id: Optional[int] = None,
        ...
        feature_set = api.v0.FeatureSetUpdate(id=feature_set_id,
                                              ...
                                              granularity=granularity,
                                              rawFilePath=rawFilePath)
        return self._patch(f"/feature-sets/{feature_set_id}", feature_set.dict(exclude_unset=True),
                           api_version=0).json()

    @track_usage
    def publish_feature_set(self, name: str, datasource_id: int, table_name: str, org_id: int,
                            granularity: Optional[str] = None, rawFilePath: Optional[str] = None):
        """
        ...
        return self.update_feature_set(feature_set_id=fs["id"], name=name, datasource_id=datasource_id,
                                       table_name=table_name, granularity=granularity,
                                       rawFilePath=rawFilePath) if fs else \
            self.create_feature_set(name=name, datasource_id=datasource_id, table_name=table_name, org_id=org_id,
                                    granularity=granularity, rawFilePath=rawFilePath)
    ...
    def publish_dimension(self, org_id: int, featureset_id: int, name: str, data_type: str,
        ...
        return self.update_column(dim["id"], name, data_type, featureset_id, dimensionality_id) if dim else \
            self.create_column(name=name, data_type=data_type, feature_set_id=featureset_id,
                               dimension_id=dimensionality_id)
    ...
    def publish_feature(self, org_id: int, featureset_id: int, name: str, data_type: str, code: Optional[str] = None,
        ...
        if ft:
            ...
            column = self.create_column(name=code, data_type=data_type, feature_set_id=featureset_id,
    ...
    def publish_features_from_df(self, df: pd.DataFrame, dimensions: List[str], features: List[str],
        ...
        self.grant_publisher(user_id=self._profile.get('id', None))
        now = datetime.now()
        ...
        table_name = self.data_warehouse.write_dataframe_to_table(exportable_df, table_name=featureset_name)

        # Add a reference to the FeatureSet
        featureset_name = table_name
        featureset = self.create_feature_set(name=featureset_name, datasource_id=datasource["id"],
                                             table_name=table_name, org_id=org_id, granularity=granularity,
                                             rawFilePath=None)
