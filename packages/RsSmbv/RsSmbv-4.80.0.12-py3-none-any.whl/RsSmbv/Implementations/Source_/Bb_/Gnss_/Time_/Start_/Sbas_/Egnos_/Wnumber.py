...
class Wnumber:
	...
	def get(self, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:STARt:SBAS<ST>:EGNOS:WNUMber \n
		Snippet: value: int = driver.source.bb.gnss.time.start.sbas.egnos.wnumber.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:TIME:STARt:SBAS{stream_cmd_val}:EGNOS:WNUMber?')
