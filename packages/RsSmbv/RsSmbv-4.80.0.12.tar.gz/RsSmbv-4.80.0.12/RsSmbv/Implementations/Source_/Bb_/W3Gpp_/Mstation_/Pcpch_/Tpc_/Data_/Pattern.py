...
class Pattern:
	...
	def set(self, structure: PatternStruct, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.pcpch.tpc.data.pattern.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> PatternStruct:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:PCPCh:TPC:DATA:PATTern \n
		Snippet: value: PatternStruct = driver.source.bb.w3Gpp.mstation.pcpch.tpc.data.pattern.get(stream = repcap.Stream.Default) \n
		The command determines the bit pattern for the PATTern data source selection. The maximum length of the bit pattern is 64
		bits. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
