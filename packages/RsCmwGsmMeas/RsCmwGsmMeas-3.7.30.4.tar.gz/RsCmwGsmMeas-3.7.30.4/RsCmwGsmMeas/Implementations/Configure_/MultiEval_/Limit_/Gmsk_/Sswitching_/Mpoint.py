...
class Mpoint:
	...
	def set(self, structure: MpointStruct, measPoint=repcap.MeasPoint.Default) -> None:
		...
		Snippet: driver.configure.multiEval.limit.gmsk.sswitching.mpoint.set(value = [PROPERTY_STRUCT_NAME](), measPoint = repcap.MeasPoint.Default) \n
		Defines and activates a limit line for the modulation scheme GMSK for a certain frequency offset. The specified limits
		apply at the reference power values defined by method RsCmwGsmMeas.Configure.MultiEval.Limit.Gmsk.Sswitching.plevel.
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:SSWitching:MPOint{measPoint_cmd_val}', structure)

	def get(self, measPoint=repcap.MeasPoint.Default) -> MpointStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:SSWitching:MPOint<nr> \n
		Snippet: value: MpointStruct = driver.configure.multiEval.limit.gmsk.sswitching.mpoint.get(measPoint = repcap.MeasPoint.Default) \n
		Defines and activates a limit line for the modulation scheme GMSK for a certain frequency offset. The specified limits
		apply at the reference power values defined by method RsCmwGsmMeas.Configure.MultiEval.Limit.Gmsk.Sswitching.plevel.
		Between the reference power values the limits are determined by linear interpolation. \n
			:param measPoint: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mpoint')
		...
		measPoint_cmd_val = self._base.get_repcap_cmd_value(measPoint, repcap.MeasPoint)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:SSWitching:MPOint{measPoint_cmd_val}?', self.__class__.MpointStruct())
