...
class CRAR(LearningAlgo):
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        Es__noise=Es_+np.random.normal(0,0.1,Es_.shape)
        rewards_val_noise=rewards_val+np.random.normal(0,0.1,rewards_val.shape)
        terminals_val_noise=terminals_val+np.random.normal(0,0.1,terminals_val.shape)
...
#        ## END DEBUG PURPOSES
        ...
        QD_plan=0
        for i in range(d+1):
            Qd=self.qValues_planning_abstr(encoded_x, transition, Q, d=i, branching_factor=[self._n_actions,2,2,2,2,2,2,2], repeat_factor=[2,2,2,2,2,2,2,2]).reshape(len(encoded_x),-1)
            print ("Qd,i")
            print (Qd,i)
            QD_plan+=Qd
        QD_plan=QD_plan/(d+1)
    ...
    def qValues_planning_abstr(self, state_abstr_val, transition, Q, d, branching_factor=None, repeat_factor=None):
        ...
        if (d==0):   
            ...
            if(this_branching_factor==self._n_actions):
                ...
                repeat_identity=np.repeat(identity_matrix,len(state_abstr_val)*this_branching_factor*this_repeat_factor,axis=0)
            else:
            ...
            next_x_predicted, r_vals_d0, gamma_vals_d0=transition.predict([tile3_encoded_x,repeat_identity,noise])
            ...
            q_values=r_vals_d0+gamma_vals_d0*np.amax(q_values,axis=1).flatten()
    ...
    def _compile(self):
        ...
        self.transition_g_full.compile(optimizer=optimizer2,
                  loss='binary_crossentropy')
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.transition_g_full.optimizer.lr, self._lr)
        ...
        K.set_value(self.diff_s_s_.optimizer.lr, self._lr) # /5. for simple laby or simple catcher; /1. for distrib of laby
