...
class NN():
    ...
    def __init__(self, batch_size, input_dimensions, n_actions, random_state, action_as_input=False, high_int_dim=True):
        ...
        if(high_int_dim==True):            
            self.internal_dim=3#input_dimensions[0][-2]*input_dimensions[0][-1] # In the case where the observation is a frame (or an history of frames)
        else:
            self.internal_dim=3
        ...
        for i, dim in enumerate(self._input_dimensions):
            ...
            if len(dim) == 3:
                ...
                if(self._high_int_dim==True):
                    x = Conv2D(self.internal_dim, (3, 3), padding='same', activation='tanh')(x)
                    out = x
                else:
                    out = Flatten()(x)
    ...
    def transition_model(self):
        ...
        if(self._high_int_dim==True):
            ...
            dim=(self.internal_dim,dim[1]/2,dim[2]/2)
    ...
    def R_model(self):
        ...
        if(self._high_int_dim==True):
            ...
            layers_action=Reshape((self._n_actions,dim[1],dim[2]))(layers_action)
            layers_action=Permute((2,3,1), input_shape=(self._n_actions,dim[1],dim[2]))(layers_action)    #data_format='channels_last'
    ...
    def full_Q_model(self, encoder_model, Q_model, plan_depth=0, transition_model=None, R_model=None, discount=None):#, disc_model=None):
        """
        Build a network consistent with each type of inputs
        ...
        print out._keras_shape
        disc_planned = K.variable(value=np.ones((32,1)), dtype='float32')
        disc_planned = Input(tensor=disc_planned)
        if(discount==None):
            discountK = K.variable(value=np.ones((32,1)), dtype='float32')
        else:
            discountK = K.variable(value=np.ones((32,1))*discount, dtype='float32')
        discountK = Input(tensor=discountK)        
        disc_plan=disc_planned
        disc_rewards=[]
        for d in range(plan_depth):
            input = Input(shape=(self._n_actions,))
            inputs.append(input)
            ...
            print "out._keras_shape"
            print out._keras_shape
            disc_plan=Multiply()([disc_plan,discountK]) #disc_model([out]+inputs[-1:])
        #if(self._high_int_dim==True):
        ...
        Q_out=Q_model(out)
        Q_estim = Multiply()([disc_planned,Q_out])
        if(plan_depth>0):
            Q_estim = Add()([Q_estim]+disc_rewards)

        model = Model(inputs=inputs+[disc_planned,discountK], outputs=Q_estim)
