...
from py_ios_device.src.instrument.RPC import get_usb_rpc, InstrumentRPC
from py_ios_device.src.instrument.dtxlib import auxiliary_to_pyobject
from py_ios_device.utils.exeption import PyiOSDeviceException
from py_ios_device.utils.utils import caller
from py_ios_device.src.demo.instrument_services.networking import networking
from py_ios_device.src.demo.instrument_services.activity import activity as _activity
from py_ios_device.src.demo.instrument_services.channel import channels as _channels
from py_ios_device.src.demo.instrument_services.launch_app import _launch_app as _launch_app
from py_ios_device.src.demo.instrument_services.networking import networking as _networking
from py_ios_device.src.demo.instrument_services.runningProcesses import runningProcesses as _runningProcesses
from py_ios_device.src.demo.instrument_services.sysmontap import sysmontap as _sysmontap
...
class PyiOSDevice:
    ...
    def get_performance_by_process_id(self, pid: int, on_callback):
        get_performance_by_process_id(pid, on_callback, self._device_id, self._rpc)

    def get_performance_by_app_name(self, app_name: str, on_callback):
        get_performance_by_app_name(app_name, on_callback, self._device_id, self._rpc)
...
def get_performance_by_process_id(pid: int, on_callback, device_id: str = None, rpc: InstrumentRPC = None):
    """
    使用进程 id 获取性能数据
    :param pid:
    :param on_callback:
...
def get_performance_by_app_name(app_name: str, on_callback, device_id: str = None, rpc: InstrumentRPC = None):
    """
    使用包名获取性能数据
    :param bundle_id:
    :param on_callback:
    ...
    """
    if not app_name:
        raise PyiOSDeviceException("app_name can not be null")
    process_list = get_processes(device_id)
    for _index in process_list:
        if _index.get("name") == app_name:
            return get_performance_by_process_id(_index.get("pid"), on_callback, device_id, rpc)
    raise Exception("No application with bundle ID [{}] was found".format(app_name))
