...
from .constants import PrecursorEnum
...
class ToilmoreHTTPRequester(object):
    ...
    async def empty_submit(
        ...
        if not expected_optimized_image_filename:
            expected_optimized_image_filename = (
                "/python-sdk-call/at-hash/optimized/" + file_hash
            )

        # Let's use a codified relative path so that we can test-match in tests.
        use_relative_path = (
            "/python-sdk-call/at-hash/input/" + file_hash + "/attempt_" +
            str(attempt_no)
        )
        ...
        if max_overhead:
            request_json_body["max_overhead"] = max_overhead
    ...
    async def request_presigned(self, presigned_url: str) -> Callable:
        try:
            response = await self._http_client_session.get(
                presigned_url
            )
            status_code = response.status
            if int(status_code / 100) == 5:
                raise RequestIssueException(RequestIssuesEnum.FIVE_HUNDRED)
            response_content = await response.content.read()
