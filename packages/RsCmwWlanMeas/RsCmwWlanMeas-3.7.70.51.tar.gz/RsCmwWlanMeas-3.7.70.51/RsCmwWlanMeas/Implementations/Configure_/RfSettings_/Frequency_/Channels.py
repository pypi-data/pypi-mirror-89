...
class Channels:
	...
	def set(self, channel: float, channels=repcap.Channels.Default) -> None:
		...
		The command logic depends on the standard. This description applies to the standards IEEE 802.11ac and 802.11ax.
		For other standards, see method RsCmwWlanMeas.Configure.RfSettings.Frequency.Channels.set. A setting command sets channel
		number <Ch> to the channel index <Channel>. The other 20-MHz channels of the bandwidth are configured automatically,
		resulting in a sequence of channel indices with the increment 4, see examples. A query returns the channel indices of all
		20-MHz channels as comma-separated list.
