...
def clean():
    ...
    click.echo(click.style('\n-- Deleted temporary data and results\n',fg='red'))
...
def solve(economy,sector,years,scenario,ignore,solver):
    ...
    """
    click.echo('\n Solving model for:\n')
    click.echo(' - {}'.format(economy))
    click.echo(' - {}'.format(sector))
    click.echo(' - {}'.format(years))
    click.echo(' - {}'.format(scenario))
    tic = time.time()
...
def load_data_config():
    ...
    click.echo('    ...successfully read in data configuration\n')
    return keep_list
...
def solve_model(solve_state,solver):
    ...
    try:
        ...
        click.echo ("Successfully created the directory %s " % path)
...
def write_results(results_tables,economy,sector,model_start):
    with pd.ExcelWriter('./results/{}/{}_{}_results_{}.xlsx'.format(economy,economy,sector,model_start)) as writer:
        for k, v in results_tables.items():
            v.to_excel(writer, sheet_name=k, merge_cells=False)
    click.echo(click.style('\n-- Results are available in the folder /results/{}'.format(economy),fg='magenta'))
    return None
