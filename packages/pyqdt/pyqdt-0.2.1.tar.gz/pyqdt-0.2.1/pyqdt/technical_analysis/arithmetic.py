...
class ArithmeticContainer(object):
    ...
    def __getattr__(self, item):
        ...
        if item.lower() in ['ref', 'abs', 'max', 'min', 'sum',
...
class ArithmeticOperator(object):
    ...
    def __getattr__(self, item):
        ...
        elif item in ['sum', 'ma', 'ema', 'mema', 'sma', 'hhv', 'llv']:
            stat = self._fetch_stat()
...
class ArithmeticStatistics(object):
    ...
    def sum(self, x, n=0):
        x = self._val(x)
        self._update(x, n)
        self._result = float(numpy.sum(self._data[-n:]))
        return self._result

    def ma(self, x, n):
        x = self._val(x)
        self._update(x, n)
        self._result = float(numpy.mean(self._data[-n:])) if len(self._data) >= n else None
        return self._result
    ...
    def hhv(self, x, n):
        x = self._val(x)
        self._update(x, n)
        self._result = float(numpy.max(self._data[-n:]))
        return self._result

    def llv(self, x, n):
        x = self._val(x)
        self._update(x, n)
        self._result = float(numpy.min(self._data[-n:]))
        return self._result
...
def arithmetic_wrapper(func):
    ...
    def _wrapper(*args, **kwargs):
        if args and type(args[0]) is pandas.Series:
            data = args[0]
            ...
            try:
                ...
                if type(returns) is list:
                    arith.result = (0 for i in range(len(returns)))
                else:
                    arith.result = 0
            #arith.result = func(*args[1:], **kwargs)
            return arith.result
...
def MAX(*args):
    """最大值"""
    return ARITH.MAX(*args)


def MIN(*args):
    """最小值"""
    return ARITH.MIN(*args)
