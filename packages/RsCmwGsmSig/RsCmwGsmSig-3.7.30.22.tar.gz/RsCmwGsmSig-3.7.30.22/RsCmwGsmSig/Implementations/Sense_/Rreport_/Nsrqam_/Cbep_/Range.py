...
class Range:
	...
	def get(self, nsrQAM=repcap.NsrQAM.Default) -> GetStruct:
		...
		Snippet: value: GetStruct = driver.sense.rreport.nsrqam.cbep.range.get(nsrQAM = repcap.NsrQAM.Default) \n
		Returns the CV BEP range, corresponding to the 'CV BEP' index reported by the MS for a 16-QAM or 32-QAM modulated DL
		signal with normal symbol rate (NSR) . \n
		...
		return self._core.io.query_struct(f'SENSe:GSM:SIGNaling<Instance>:RREPort:NSRQam{nsrQAM_cmd_val}:CBEP:RANGe?', self.__class__.GetStruct())
