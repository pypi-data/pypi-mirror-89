...
class State:
	...
	def fetch(self) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.olpControl.state.fetch() \n
		...
		INITiate..., STOP..., ABORt... to change the measurement state. \n
			:return: state: OFF | RUN | RDY OFF: measurement switched off, no resources allocated, no results available (when entered after ABORt...) RUN: measurement running (after INITiate..., READ...) , synchronization pending or adjusted, resources active or queued RDY: measurement has been terminated, valid results are available"""
		response = self._core.io.query_str(f'FETCh:WCDMa:MEASurement<Instance>:OLPControl:STATe?')
