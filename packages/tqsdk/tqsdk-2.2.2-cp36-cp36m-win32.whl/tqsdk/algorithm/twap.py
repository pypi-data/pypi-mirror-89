...
class Twap(object):
    ...
    **注意**：
    ...
    async def _forward_trade_chan(self, trade_chan, to_chan):
    ...
    async def _insert_order(self, volume, end_time, strict_end_time):
        volume_left = volume
        try:
            update_chan = TqChan(self._api)
            self._api.register_update_notify(self._quote, update_chan)
            trade_chan = TqChan(self._api)  # 手数更新 chan, 转发到 update_chan
            ...
            forward_trade_chan_task = self._api.create_task(self._forward_trade_chan(trade_chan, update_chan))
            async for item in update_chan:
                if isinstance(item, dict) and item["_from"] == "trade":
                    v = item["_volume"]
                    volume_left = volume_left - (v if self._direction == "BUY" else -v)
                    if volume_left == 0:
                        break
                elif _get_trade_timestamp(self._quote.datetime, float('nan')) > strict_end_time:
                    break
        finally:
            forward_trade_chan_task.cancel()
            self._order_task._task.cancel()
            await asyncio.gather(forward_trade_chan_task, self._order_task._task, return_exceptions=True)
