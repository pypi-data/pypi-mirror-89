...
class State:
	...
	def fetch(self) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.cpo.state.fetch() \n
		Return the states of the channels for channel phase offset (CPO) measurements. The values listed below in curly brackets
		{} are returned for each active segment: {...}seg 1, {...}seg 2, ..., {...}seg n. The number of active segments n is
		determined by method RsCmwCdma2kMeas.Configure.MultiEval.ListPy.count. The number to the left of each result parameter is
		provided for easy identification of the parameter position within the result array. \n
			:return: structure: for return value, see the help for FetchStruct structure arguments."""
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:CPO:STATe?', self.__class__.FetchStruct())
