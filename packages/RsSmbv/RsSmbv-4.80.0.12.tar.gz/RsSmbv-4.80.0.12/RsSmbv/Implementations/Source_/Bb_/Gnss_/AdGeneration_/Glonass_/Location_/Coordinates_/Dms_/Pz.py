...
class Pz:
	...
	def set(self, structure: PzStruct, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GLONass<ST>:LOCation:COORdinates:DMS:PZ<CH> \n
		Snippet: driver.source.bb.gnss.adGeneration.glonass.location.coordinates.dms.pz.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
	...
	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> PzStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GLONass<ST>:LOCation:COORdinates:DMS:PZ<CH> \n
		Snippet: value: PzStruct = driver.source.bb.gnss.adGeneration.glonass.location.coordinates.dms.pz.get(stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
		Sets the geographic reference location in degrees, minutes and seconds. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Glonass')
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pz')
