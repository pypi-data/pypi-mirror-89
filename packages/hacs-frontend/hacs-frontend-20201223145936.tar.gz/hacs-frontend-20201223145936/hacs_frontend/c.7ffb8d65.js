      ...
      >
        ...
        <div class=${d({content:!0,narrow:this.narrow})} @scroll=${this._loadMore}>
          <div class=${d({list:!0,narrow:this.narrow})}>
            ${t.sort((e,t)=>"name"===this._sortBy?e.name.toLocaleLowerCase()<t.name.toLocaleLowerCase()?-1:1:e[this._sortBy]>t[this._sortBy]?-1:1).slice(0,this._load).map(e=>a`<paper-icon-item
                  class=${d({narrow:this.narrow})}
                  @click=${()=>this._openInformation(e)}
                >
                  ${"integration"===e.category?a`
                        <img
            ...
            ${0===t.length?a`<p>${s("dialog_add_repo.no_match")}</p>`:""}
          </div>
        </div>
      </hacs-dialog>
