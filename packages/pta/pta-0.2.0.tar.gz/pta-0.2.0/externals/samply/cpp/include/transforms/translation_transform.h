...
class TranslationTransform : public AffineTransformBase<
	TranslationTransform<Scalar, Storage>, Storage> {
public:	
	...
	const Eigen::MatrixBase<MatrixType>& matrix) const
{
	if (Base::get_linear().rows() == Base::get_linear().cols()) {
		...
		Matrix<Scalar> result = Matrix<Scalar>::Zero(
			Base::get_linear().rows(), matrix.cols());
		result.block(0, 0, std::min(result.rows(), matrix.rows()),
			...
			matrix.block(0, 0, std::min(result.rows(), matrix.rows()),
				std::min(result.cols(), matrix.cols())).colwise() + 
					Base::get_shift();
		return result;
	}
}
...
{
	return TranslationTransform<Scalar>(
		Base::get_linear().transpose(), -Base::get_shift());
}
