...
class Wtfi:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("wtfi", core, parent)

	@property
	def current(self):
		"""current commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_current'):
			from .Wtfi_.Current import Current
	...
	def average(self):
		...
		if not hasattr(self, '_average'):
			from .Wtfi_.Average import Average
	...
	def maximum(self):
		...
		if not hasattr(self, '_maximum'):
			from .Wtfi_.Maximum import Maximum
	...
	def minimum(self):
		...
		if not hasattr(self, '_minimum'):
			from .Wtfi_.Minimum import Minimum
	...
	def clone(self) -> 'Wtfi':
		...
		new_group = Wtfi(self._core, self._base.parent)
