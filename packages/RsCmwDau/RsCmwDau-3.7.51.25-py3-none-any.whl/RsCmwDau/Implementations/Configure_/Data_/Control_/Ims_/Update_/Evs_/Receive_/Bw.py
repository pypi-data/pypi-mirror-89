...
class Bw:
	...
	def set(self, rx_bw: enums.Bandwidth, ims=repcap.Ims.Default) -> None:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:EVS:RECeive:BW \n
		Snippet: driver.configure.data.control.ims.update.evs.receive.bw.set(rx_bw = enums.Bandwidth.FB, ims = repcap.Ims.Default) \n
		Selects the codec bandwidths supported in the EVS primary mode in the uplink (receive) direction, for a call update and
		...
		self._core.io.write(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:RECeive:BW {param}')

	# noinspection PyTypeChecker
	def get(self, ims=repcap.Ims.Default) -> enums.Bandwidth:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:EVS:RECeive:BW \n
		Snippet: value: enums.Bandwidth = driver.configure.data.control.ims.update.evs.receive.bw.get(ims = repcap.Ims.Default) \n
		Selects the codec bandwidths supported in the EVS primary mode in the uplink (receive) direction, for a call update and
		separate configuration. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')
		...
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		response = self._core.io.query_str(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:RECeive:BW?')
