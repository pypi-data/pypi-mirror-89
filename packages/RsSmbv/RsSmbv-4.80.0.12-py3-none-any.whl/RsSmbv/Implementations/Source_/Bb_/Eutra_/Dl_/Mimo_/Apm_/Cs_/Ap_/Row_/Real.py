...
class Real:
	...
	def set(self, ant_port_map_dat: float, antennaPort=repcap.AntennaPort.Default, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:MIMO:APM:CS:AP<DIR>:ROW<ST>:REAL<CH> \n
		Snippet: driver.source.bb.eutra.dl.mimo.apm.cs.ap.row.real.set(ant_port_map_dat = 1.0, antennaPort = repcap.AntennaPort.Default, stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
	...
	def get(self, antennaPort=repcap.AntennaPort.Default, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:MIMO:APM:CS:AP<DIR>:ROW<ST>:REAL<CH> \n
		Snippet: value: float = driver.source.bb.eutra.dl.mimo.apm.cs.ap.row.real.get(antennaPort = repcap.AntennaPort.Default, stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
		Define the mapping of the antenna ports to the physical antennas. \n
			:param antennaPort: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Ap')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Row')
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Real')
