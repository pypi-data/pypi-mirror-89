...
class Dselect:
	...
	def set(self, dselect: str, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS<ST>:SIGNal:L5Band:L5S:DATA:NMESsage:DSELect \n
		Snippet: driver.source.bb.gnss.svid.qzss.signal.l5Band.l5S.data.nmessage.dselect.set(dselect = '1', channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS<ST>:SIGNal:L5Band:L5S:DATA:NMESsage:DSELect \n
		Snippet: value: str = driver.source.bb.gnss.svid.qzss.signal.l5Band.l5S.data.nmessage.dselect.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Selects an existing data list file from the default directory or from the specific directory. Refer to 'Accessing Files
		in the Default or Specified Directory' for general information on file handling in the default and in a specific
		directory. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
