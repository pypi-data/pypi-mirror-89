...
class Extended:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.data.measurement.throughput.trace.overall.ulink.extended.read() \n
		...
		return self._core.io.query_struct(f'READ:DATA:MEASurement<MeasInstance>:THRoughput:TRACe:OVERall:ULINk:EXTended?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:DATA:MEASurement<Instance>:THRoughput:TRACe:OVERall:ULINk:EXTended \n
		Snippet: value: ResultData = driver.data.measurement.throughput.trace.overall.ulink.extended.fetch() \n
		Query the values of the overall throughput trace in uplink (ULINk) or downlink (DLINk) direction. The trace values are
		returned from right to left (last to first measurement) . There are two values per interval: <Reliability>, {<Counter>,
		<Result>}interval n, {...}interval n-1, ... The counter is useful if you want to perform repeated queries and combine the
		returned result traces. To configure the number of intervals, see method RsCmwDau.Configure.Data.Measurement.Throughput.
		mcount. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:DATA:MEASurement<MeasInstance>:THRoughput:TRACe:OVERall:ULINk:EXTended?', self.__class__.ResultData())
