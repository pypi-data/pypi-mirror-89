...
class ApSelect:
	...
	def set(self, pusch_ap_sel: enums.Nr5GpuschAp, channel=repcap.Channel.Default, stream=repcap.Stream.Default, antennaSelect=repcap.AntennaSelect.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:SCHed:CELL<CH>:SUBF<ST>:USER:BWPart:ALLoc:PUSCh:[DMRS]:APSelect<S2US> \n
		Snippet: driver.source.bb.nr5G.scheduling.cell.subf.user.bwPart.alloc.pusch.dmrs.apSelect.set(pusch_ap_sel = enums.Nr5GpuschAp.AP0, channel = repcap.Channel.Default, stream = repcap.Stream.Default, antennaSelect = repcap.AntennaSelect.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default, antennaSelect=repcap.AntennaSelect.Default) -> enums.Nr5GpuschAp:
		"""SCPI: [SOURce<HW>]:BB:NR5G:SCHed:CELL<CH>:SUBF<ST>:USER:BWPart:ALLoc:PUSCh:[DMRS]:APSelect<S2US> \n
		Snippet: value: enums.Nr5GpuschAp = driver.source.bb.nr5G.scheduling.cell.subf.user.bwPart.alloc.pusch.dmrs.apSelect.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default, antennaSelect = repcap.AntennaSelect.Default) \n
		Each layer of a PUSCH allocation is mapped to a certain antenna port. By the command the antenna ports are selected which
		are used for the transmission of the PUSCH allocation. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Subf')
