...
class Current:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.listPy.cp.data.current.fetch() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:CP:DATA:CURRent?', suppressed)
		return response
	...
	def calculate(self) -> List[enums.ResultStatus2]:
		...
		Snippet: value: List[enums.ResultStatus2] = driver.multiEval.listPy.cp.data.current.calculate() \n
		Returns the RMS power (statistical values) of the data channel for all active list mode segments. The values described
		below are returned by FETCh commands. CALCulate commands return limit check results instead, one value for each result
		listed below. \n
		Use RsCmwEvdoMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: data: float Comma-separated list of values, one per active segment. Range: -60 dB to +10 dB , Unit: dB"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:CP:DATA:CURRent?', suppressed)
