...
class AyncExtractor:
    ...
    async def _get_links(self, rules, response):
        ...
        for rule in rules:
            ...
            extractor_redirect_switch=response.request.meta.get("extractor.redirect.switch", False)
            if ex_req[key_map["same_domain_only"]] and redirect_urls is not None and len(redirect_urls) > 0 and not extractor_redirect_switch:
