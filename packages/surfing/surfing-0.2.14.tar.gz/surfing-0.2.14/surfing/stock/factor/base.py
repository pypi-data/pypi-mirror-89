...
class Factor(Calculator, metaclass=Singleton):
    ...
    _factor_names_set: Set[str] = set()

    def __init__(self, f_name: str, f_type: Union[StockFactorType, FundFactorType], f_level: str = 'basic'):
        assert f_name not in self._factor_names_set, f'duplicated factor with name {f_name}!!!'
        self._f_name: str = f_name
        # 加到这个set里，可以检查是否有重名的(名字写错的)factor
        self._factor_names_set.add(f_name)
