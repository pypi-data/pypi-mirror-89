...
synapse_types = {
    ...
    'depressing, stochastic':      sim.StochasticTsodyksMarkramSynapseEM(**depressing),
    'facilitating, deterministic': sim.TsodyksMarkramSynapse(**facilitating),
    'facilitating, stochastic':    sim.StochasticTsodyksMarkramSynapseEM(**facilitating),
}
...
for label, p in populations.items():
    filename = normalized_filename("Results", "stochastic_tsodyksmarkram__em_%s" % label,
...
if options.plot_figure:
    ...
    figure_filename = "Results/stochastic_tsodyksmarkram_em_{}.png".format(options.simulator)
    panels = []
