...
class ScpiChars(ScpiTypeBase):
    ...
    @classmethod
    def parse(cls, transport: Type[ScpiTransportBase]) -> 'ScpiChars':
...
class ScpiNumber(ScpiTypeBase):
    ...
    @classmethod
    def parse(cls, transport: Type[ScpiTransportBase]):
...
class ScpiArbStream(ScpiTypeBase, io.RawIOBase):
    ...
    def __init__(self, transport: Type[ScpiTransportBase], length: int) -> None:
...
class ScpiEvent(ScpiTypeBase):
    ...
    def as_string(self) -> str:
        ...
        return f'{self._code:d}: {self._description}' + (' ' + self._info if self._info is not None else '')
