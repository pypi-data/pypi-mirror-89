...
class DownloaderFinder(BaseFinder):
    ...
    def _load(self, staticfiles_urls, variable_name):
        if not isinstance(staticfiles_urls, dict):
            raise ImproperlyConfigured('Value of "{}" is not a dict.'.format(variable_name))
            ...
            if isinstance(url, (list, tuple)):
                try:
                    ...
                    raise ImproperlyConfigured(
                        ...
                        'urls or tuples (url, algorithm, checksum).'.format(variable_name)
                    )
                if algorithm not in hashlib.algorithms_available:
                    raise ImproperlyConfigured('The algorythm "{}" is not available.'.format(algorithm))
            else:
