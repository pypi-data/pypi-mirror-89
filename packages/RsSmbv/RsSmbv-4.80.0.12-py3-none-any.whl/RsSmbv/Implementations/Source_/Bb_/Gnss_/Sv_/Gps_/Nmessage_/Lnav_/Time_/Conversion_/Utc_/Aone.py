...
class Aone:
	...
	def set(self, a_1: int, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:GPS<ST>:NMESsage:LNAV:TIME:CONVersion:UTC<CH>:AONE \n
		Snippet: driver.source.bb.gnss.sv.gps.nmessage.lnav.time.conversion.utc.aone.set(a_1 = 1, stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:GPS{stream_cmd_val}:NMESsage:LNAV:TIME:CONVersion:UTC{channel_cmd_val}:AONE {param}')

	def get(self, stream=repcap.Stream.Default, channel=repcap.Channel.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:GPS<ST>:NMESsage:LNAV:TIME:CONVersion:UTC<CH>:AONE \n
		Snippet: value: int = driver.source.bb.gnss.sv.gps.nmessage.lnav.time.conversion.utc.aone.get(stream = repcap.Stream.Default, channel = repcap.Channel.Default) \n
		Sets the parameter A1. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Gps')
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Utc')
		...
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SV:GPS{stream_cmd_val}:NMESsage:LNAV:TIME:CONVersion:UTC{channel_cmd_val}:AONE?')
