...
from high_vision.Segmentation.deeplab.deeplab import Deeplab_xcep_pascal
from high_vision.Segmentation.deeplab.deeplab import Deeplab_xcep_ade20k
import cv2
import time
...
class semantic_segmentation():
  ...
  def process_camera_ade20k(self, cam, overlay = True,  check_fps = True, frames_per_second = 25, 
                            output_video_name = None, show_frames = True, frame_name = "Output", verbose = 0):
    capture = cam
    width = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
    if frames_per_second is not None:
