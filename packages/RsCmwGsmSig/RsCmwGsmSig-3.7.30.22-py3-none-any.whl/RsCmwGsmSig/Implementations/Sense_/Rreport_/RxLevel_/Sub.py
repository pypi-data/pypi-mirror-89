...
class Sub:
	...
	def get_range(self) -> RangeStruct:
		...
		Snippet: value: RangeStruct = driver.sense.rreport.rxLevel.sub.get_range() \n
		...
		"""
		return self._core.io.query_struct('SENSe:GSM:SIGNaling<Instance>:RREPort:RXLevel:SUB:RANGe?', self.__class__.RangeStruct())

	def get_value(self) -> int:
		"""SCPI: SENSe:GSM:SIGNaling<Instance>:RREPort:RXLevel:SUB \n
		Snippet: value: int = driver.sense.rreport.rxLevel.sub.get_value() \n
		Returns the 'RX Level Sub' reported by the MS as dimensionless power level. \n
			:return: rx_level: Range: 0 to 63
		"""
		response = self._core.io.query_str('SENSe:GSM:SIGNaling<Instance>:RREPort:RXLevel:SUB?')
