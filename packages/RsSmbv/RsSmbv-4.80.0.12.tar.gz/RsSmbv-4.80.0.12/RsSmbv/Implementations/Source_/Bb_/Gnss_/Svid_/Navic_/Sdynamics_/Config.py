...
class Config:
	...
	def set(self, predefined_confi: enums.DopplerConfig, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:NAVic<ST>:SDYNamics:CONFig \n
		Snippet: driver.source.bb.gnss.svid.navic.sdynamics.config.set(predefined_confi = enums.DopplerConfig.USER, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> enums.DopplerConfig:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:NAVic<ST>:SDYNamics:CONFig \n
		Snippet: value: enums.DopplerConfig = driver.source.bb.gnss.svid.navic.sdynamics.config.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Selects between the predefined velocity profiles or a user-defined one. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Navic')
