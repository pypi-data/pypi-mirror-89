...
class QuoteData(object):
    ...
    def _parse_period_start(self, period, date, count):
        ...
        if period in ['day', 'daily', '1d']:
            ...
            week_list.sort()
            w = week_list[-count]
            return trade_list[week_maps.index(w)].timetuple()
    ...
    def execute_rehabilitation(self, data, fq='pre', fq_vol=False, columns=None) -> pandas.DataFrame:
        ...
        for index, row in xrxd.iterrows():
            ...
            if fq_cols_px:
                n = self._get_price_precision(row['code'])
                if ts_date <= ts_ref:
    ...
    def get_trade_days(self, start_date=None, end_date=None, count=None) -> list:
        ...
        if dt_start or dt_end or count:
            ...
            if dt_start:
                result = result[numpy.where(result >= dt_start)]
            if dt_end:
                result = result[numpy.where(result <= dt_end)]
    ...
    def get_quote_static(self, date=None, fields=None, exch=None) -> pandas.DataFrame:
        ...
        for x in exch:
            ...
            df = df.copy()
            df['code'] = df['code'] + '.' + x
            df['paused'] = df['paused'].astype('bool_')
            if fields:
    ...
    def get_quote_kdata(self, security, period='day', fq='pre', fq_vol=False, fields=None,
        ...
        if mode_static:
            ...
            for c in security:
                ...
                if append_paused:
                    if period_type == 'day':
                        df['paused'] = df.apply(lambda x: x['volume'] == 0 and x['money'] == 0, axis=1)
                    elif period_type == 'mn1':
                        c_period = 'day'
                        ...
                        df['paused'] = df['datetime'].astype('datetime64[D]').isin(c_skip_days)
                if skip_paused:
                    df = df[~df['paused']]
                if not prep_count:
    ...
    def get_technical_analysis(self, security, indications, period='day', fq='pre', fq_vol=False, fields=None,
        ...
        for v in indications:
            ...
            # 字段重名处理
            if [x for x in cols_fn if x in df.columns]:
            ...
            arithmetic_reset()
            se = df.apply(func, **kw)
            df[cols_fn] = pandas.DataFrame(se.tolist(), index=df.index)
        ...
        if not count:
            ...
            c_groupby = df.groupby(by='code')
            for x, y in c_groupby:
