...
class Catalog:
	...
	def get(self, stream=repcap.Stream.Default) -> List[str]:
		"""SCPI: [SOURce<HW>]:IQ:DPD<ST>:SHAPing:NORMalized:DATA:CATalog \n
		Snippet: value: List[str] = driver.source.iq.dpd.shaping.normalized.data.catalog.get(stream = repcap.Stream.Default) \n
