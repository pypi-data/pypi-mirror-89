...
class Adsc:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("adsc", core, parent)

	@property
	def state(self):
		"""state commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_state'):
			from .Adsc_.State import State
	...
	def current(self):
		...
		if not hasattr(self, '_current'):
			from .Adsc_.Current import Current
	...
	def average(self):
		...
		if not hasattr(self, '_average'):
			from .Adsc_.Average import Average
	...
	def maximum(self):
		...
		if not hasattr(self, '_maximum'):
			from .Adsc_.Maximum import Maximum
	...
	def minimum(self):
		...
		if not hasattr(self, '_minimum'):
			from .Adsc_.Minimum import Minimum
	...
	def clone(self) -> 'Adsc':
		...
		new_group = Adsc(self._core, self._base.parent)
