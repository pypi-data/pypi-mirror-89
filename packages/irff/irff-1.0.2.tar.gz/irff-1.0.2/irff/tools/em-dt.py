...
def cross_val_fit_print_error(EMs_prepared, EMs_labels, model=None,
    ...
    # 交叉验证 并 输出误差
    scores = cross_validate(model, EMs_prepared, EMs_labels, scoring=scoring, cv=cv)
    print(scores)
...
def get_data_():
    ...
    for s in (strat_train_set, strat_test_set):
        s.drop(["density_cat"], axis=1, inplace=True)
    ...
    # for d,l in zip(dataset_use['DT'],dataset_use['density']):
    #     print(d,l)
    # for ldata in train_inputs:
    #     for l in train_inputs[ldata]:
    #         if l=='-':
    #            print(l,ldata)
...
def mlp():
    train_inputs,train_labels,test_inputs,test_labels,column_list = get_data_()
    mlr = MLPRegressor((32, 32), max_iter=10000)
...
def xg():
    train_inputs,train_labels,test_inputs,test_labels,column_list = get_data_()
    xgr = xgb.XGBRegressor(n_estimators=500,reg_lambda=3,booster='gbtree',max_depth=10)
...
def rf():
    ...
    kf = KFold(n_splits=5)
    for train_index, test_index in kf.split(train_inputs, train_labels):
        x_ = train_inputs.iloc[train_index] 
        y_ = train_labels.iloc[train_index]
        rfr.fit(x_, y_)
        score_test = rfr.score(train_inputs.iloc[test_index],train_labels.iloc[test_index])
...
def cada():
    train_inputs,train_labels,test_inputs,test_labels,column_list = get_data_()
    ada = AdaBoostRegressor(DecisionTreeRegressor(max_depth=10,splitter='best'),
                            n_estimators=300,random_state=37,
                            loss='exponential',)
    ada.fit(train_inputs, train_labels)
    cross_val_fit_print_error(train_inputs,train_labels, model=ada)
    show_results(ada,test_inputs,test_labels,title="AdaBoost Regression (test set)")


def ada():
    train_inputs,train_labels,test_inputs,test_labels,column_list = get_data_()
    adar = AdaBoostRegressor(DecisionTreeRegressor(max_depth=10,splitter='best'),
                            n_estimators=300,random_state=37,
                            loss='exponential',learning_rate=0.5)
    ...
    for train_index, test_index in kf.split(train_inputs, train_labels):
        ...
        y_ = train_labels.iloc[train_index]
        adar.fit(x_, y_)
        score_test = adar.score(train_inputs.iloc[test_index],train_labels.iloc[test_index])
        print(' * score on test set:',score_test)
    adar.fit(train_inputs,train_labels)

    # scores_cvs_new = cross_validate(adar,train_inputs,train_labels,cv=5,return_train_score=False)
    # print(sorted(scores_cvs_new.keys()))
...
def adags():
    ...
    # --------------------调参--------------------------------------------------------
    # 随机森林
    param_grid  = [{'n_estimators': [500,600,700,800],'learning_rate':[0.1,0.3,0.5,0.7]}]
    adar = AdaBoostRegressor(DecisionTreeRegressor(max_depth=10),
                            n_estimators=500, random_state=37,
                            learning_rate=0.5)
    # train
    grid_search = GridSearchCV(adar, param_grid, cv=9,
                               scoring='neg_mean_squared_error', return_train_score=True)
    grid_search.fit(train_inputs, train_labels)
    print('best params: ', grid_search.best_params_)
    cvres = grid_search.cv_results_
    ...
    # 模型
    final_model = grid_search.best_estimator_
    show_results(final_model,train_inputs,train_labels,title="AdaBoosted Regression (train set)")
    show_results(final_model,test_inputs,test_labels,title="AdaBoosted Regression (test set)")
...
if __name__ == '__main__':
   ''' use commond like ./em-sklearn.py ada to run it'''
   parser = argparse.ArgumentParser()
   argh.add_commands(parser, [rf,ada,adags,rfgs,sv,svgs,krgs,kr,xg,mlp,knn,gb,cada])
