...
class ChannelCrawler(CrawlerObject):
    def __init__(self):
        super().__init__()
        # maybe these should be in build
        self.PARENT_PATH = config.get("channelfilepath", "parent_path")
        self.channel_video_list_workfile = config.get("channelfilepath", "channel_video_list_workfile")
        self.channel_list_workfile = config.get("channelfilepath", "channel_list_workfile")
        self.channel_list_dir = config.get("channelfilepath", "channel_list_dir")
        self.channel_data_dir = config.get("channelfilepath", "channel_data_dir")
    ...
    def __build__(self):
        ...
        try:
            os.mkdir(self.channel_list_dir)
        except OSError:
            print("Directory already exists %s" % self.channel_list_dir)
        else:
            print("Successfully created the directory %s " % self.channel_list_dir)

        try:
            os.mkdir(self.channel_data_dir)
        except OSError:
            print("Directory already exists %s" % self.channel_data_dir)
        else:
            print("Successfully created the directory %s " % self.channel_data_dir)
        super().__build__()
