    ...
    async def main_loop(self, bot, token, bot_id,log):
        ...
        while not self.bot.is_closed():
            ...
            try:
                ...
                if guilds == before:
                    if log is True:
                        print("KOR|UniqueBots|서버수가 동일하네요. 잠시후에 다시요청할께요!\nENG|UniqueBots|Fail Server count same")
                    ...
                    if log is True:
                        print(f"KOR|UniqueBots|서버수를 성공적으로 갱신했어요! 현재 서버수는 {code}이네요.\nENG|UniqueBots|SUCCESS! server: {code}")
                        await asyncio.sleep(1800)
                    else:
                        await asyncio.sleep(1800)
            except KeyError as e:
                if log is True:
                    print(f"KOR|UniqueBots|에러가 발생했어요! 에러메시지: {e}\nENG|UniqueBots|ERROR! error msg: {e}")
                    await asyncio.sleep(1800)
                    pass
                else:
                    await asyncio.sleep(1800)
