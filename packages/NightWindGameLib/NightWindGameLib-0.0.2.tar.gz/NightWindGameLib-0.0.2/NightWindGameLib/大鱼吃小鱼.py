...
class Player(Sprite):
    def __init__(self, image):
        ...
        self.append_texture(load_texture('player.png', mirrored=True))
        self.size = 2
        self.level = 0
...
class Game(Window):
    ...
    def setup(self):
        self.bg = Sprite('newsea.png')
        self.bg.center_x = WIDTH // 2
        self.bg.center_y = HEIGHT // 2
        self.color = ["黄", "绿", "红", "紫", "蓝"]
        self.fishes = ['{}鱼.png'.format(self.color[i]) for i in range(5)]
        ...
        self.game_over = Status('game_over.png')
        self.game_pause_status = False
        self.game_pause = Status('game_pause.png')
        self.achievement = 300
        self.achieve_goal = False
        self.eat_sound = load_sound('eat_sound.wav')
        self.game_over_sound = load_sound('game_over_sound.wav')
    ...
    def on_draw(self):
        ...
        draw_text(f'分数：{self.score}', 0, HEIGHT - 25, ac.color.WHITE,
                  font_name=('simhei', 'PingFang'), font_size=20)
        draw_text(f'目标：达到{self.achievement}积分', 125, HEIGHT - 25, ac.color.WHITE,
                  font_name=('simhei', 'PingFang'), font_size=20)
        ...
        if self.game_over_status:
            ...
            if int(self.total_time) % 2 == 0:
                draw_text('按下 Tab 键重新开始游戏', WIDTH // 2 - 150, HEIGHT // 2 - 200, 
                          ac.color.WHITE, font_name=('simhei', 'PingFang'), font_size=20)
    ...
    def upgrade_till_next_level(self):
        if self.player.level >= self.score:
            ...
            draw_text('目标达成！', WIDTH // 2 - 210, HEIGHT // 2 - 50, ac.color.RED,
