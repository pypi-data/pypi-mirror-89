...
class All:
	...
	def get(self, stream=repcap.Stream.Default) -> List[int]:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID:GALileo<ST>:LIST:ALL \n
		Snippet: value: List[int] = driver.source.bb.gnss.svid.galileo.listPy.all.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_int_list(f'SOURce<HwInstance>:BB:GNSS:SVID:GALileo{stream_cmd_val}:LIST:ALL?')
		return response
