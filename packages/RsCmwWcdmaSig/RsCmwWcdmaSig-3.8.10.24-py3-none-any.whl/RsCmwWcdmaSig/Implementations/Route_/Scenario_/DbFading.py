...
class DbFading:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("dbFading", core, parent)

	@property
	def flexible(self):
		"""flexible commands group. 0 Sub-classes, 2 commands."""
		if not hasattr(self, '_flexible'):
			from .DbFading_.Flexible import Flexible
	...
	def get_external(self) -> ExternalStruct:
		...
		Snippet: value: ExternalStruct = driver.route.scenario.dbFading.get_external() \n
		...
		"""
		return self._core.io.query_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DBFading:EXTernal?', self.__class__.ExternalStruct())

	def set_external(self, value: ExternalStruct) -> None:
		"""SCPI: ROUTe:WCDMa:SIGNaling<instance>:SCENario:DBFading[:EXTernal] \n
		Snippet: driver.route.scenario.dbFading.set_external(value = ExternalStruct()) \n
		Activates the 'Dual Carrier / Dual Band Fading: External' scenario and selects the signal paths. For possible connector
		and converter values, see 'Values for Signal Path Selection'. \n
			:param value: see the help for ExternalStruct structure arguments.
		"""
		self._core.io.write_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DBFading:EXTernal', value)
	...
	def get_internal(self) -> InternalStruct:
		...
		Snippet: value: InternalStruct = driver.route.scenario.dbFading.get_internal() \n
		Activates the 'Dual Carrier / Dual Band Fading: Internal' scenario and selects the signal paths. To set the signaling
		unit manually, use the command method RsCmwWcdmaSig.Route.Scenario.DbFading.Flexible.internal instead. For possible
		...
		"""
		return self._core.io.query_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DBFading:INTernal?', self.__class__.InternalStruct())

	def set_internal(self, value: InternalStruct) -> None:
		"""SCPI: ROUTe:WCDMa:SIGNaling<instance>:SCENario:DBFading:INTernal \n
		Snippet: driver.route.scenario.dbFading.set_internal(value = InternalStruct()) \n
		Activates the 'Dual Carrier / Dual Band Fading: Internal' scenario and selects the signal paths. To set the signaling
		unit manually, use the command method RsCmwWcdmaSig.Route.Scenario.DbFading.Flexible.internal instead. For possible
		connector and converter values, see 'Values for Signal Path Selection'. \n
			:param value: see the help for InternalStruct structure arguments.
		"""
		self._core.io.write_struct('ROUTe:WCDMa:SIGNaling<Instance>:SCENario:DBFading:INTernal', value)

	def clone(self) -> 'DbFading':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = DbFading(self._core, self._base.parent)
