...
async def send(uri, data):
    async with websockets.connect('wss://cpa-global-wechat.bluewebonline.com/ws/wechat/login/'+uri) as websocket:
...
class WeChatLoginWebSocketView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        token = jwt_create_token_wechat(openid=user_queryset.openid, unionid=user_queryset.unionid,
                                        user_id=user_queryset.pk, wechat_id=user_queryset.wechat_id)
