...
def copy_template(source: str = '', target: str = '.'):
    CMD = f'xcopy /s/e {source} {target}' if os.name == 'nt' else f'cp -r {source} {target}'
...
def cmd():
    ...
    arg.add_argument('-i', '--install', default=-1, help='-1不安装(默认),支持jdk|jmeter|chromdriver|jenkins|git|node')
    arg.add_argument('-v', '--version', type=str, default='latest', help='指定软件的版本')
    params = arg.parse_args()
    package_path_list = [p for p in sys.path if '\\lib\\site-packages' in p]
    package_path = package_path_list[0] if len(package_path_list)>0 else os.popen('pip show xiaobaiauto2').readlines()[7].split(' ')[-1][:-1]
    if params.install in [0, 'jdk']:
        ...
        copy_template(f'{package_path}{step}xiaobaiauto2{step}test{step}ApiTest', params.dir)
    elif params.template in [1, 'web']:
        copy_template(f'{package_path}{step}xiaobaiauto2{step}test{step}WebTest', params.dir)
    elif params.template in [2, 'app']:
        copy_template(f'{package_path}{step}xiaobaiauto2{step}test{step}AppTest', params.dir)
    else:
