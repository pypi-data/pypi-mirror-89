...
class Mcs:
	...
	def set(self, mcs_index: enums.McsIndex, dummy=repcap.Dummy.Default) -> None:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:DFRame:HEMU:DUMMy<index>:MCS \n
		Snippet: driver.configure.connection.dframe.hemu.dummy.mcs.set(mcs_index = enums.McsIndex.MCS, dummy = repcap.Dummy.Default) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:DFRame:HEMU:DUMMy{dummy_cmd_val}:MCS {param}')

	# noinspection PyTypeChecker
	def get(self, dummy=repcap.Dummy.Default) -> enums.McsIndex:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:DFRame:HEMU:DUMMy<index>:MCS \n
		Snippet: value: enums.McsIndex = driver.configure.connection.dframe.hemu.dummy.mcs.get(dummy = repcap.Dummy.Default) \n
		Sets the modulation and coding scheme for the corresponding dummy user. \n
			:param dummy: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Dummy')
			:return: mcs_index: MCS | MCS1 | MCS2 | MCS3 | MCS4 | MCS5 | MCS6 | MCS7 | MCS8 | MCS9 | MCS10 | MCS11 MCS, MCS1,...,MCS11: MCS 0 to MCS 11"""
		dummy_cmd_val = self._base.get_repcap_cmd_value(dummy, repcap.Dummy)
		response = self._core.io.query_str(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:DFRame:HEMU:DUMMy{dummy_cmd_val}:MCS?')
