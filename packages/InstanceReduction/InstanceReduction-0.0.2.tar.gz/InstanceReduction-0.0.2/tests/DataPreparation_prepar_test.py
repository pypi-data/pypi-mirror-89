...
import pytest
from tests.const import names, len_all, tuples, n_classes, n_features
...
@pytest.fixture(params = ["tests\\test_csv\\only_cols.csv", "tests\\test_csv\\only_cols.csv"])
...
def test_not_enough_cols():
    with pytest.raises(Exception, match="minimum 2 columns"):
        DataPreparation(filepath="tests\\test_csv\\one_col.csv", sep = ";")

def test_missing_values():
    with pytest.raises(Exception, match="Nan"):
        DataPreparation(filepath="tests\\test_csv\\missed.csv", sep = ";")

def test_non_numeric():
    with pytest.raises(Exception, match="non numeric"):
        DataPreparation(filepath="tests\\test_csv\\non_numeric.csv", sep = ";")