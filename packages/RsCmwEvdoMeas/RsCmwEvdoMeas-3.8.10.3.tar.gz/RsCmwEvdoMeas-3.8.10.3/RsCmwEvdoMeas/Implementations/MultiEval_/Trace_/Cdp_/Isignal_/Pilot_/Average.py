...
class Average:
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.cdp.isignal.pilot.average.read() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:EVDO:MEASurement<Instance>:MEValuation:TRACe:CDP:ISIGnal:PILot:AVERage?', suppressed)
		return response

	def fetch(self) -> List[float]:
		"""SCPI: FETCh:EVDO:MEASurement<instance>:MEValuation:TRACe:CDP:ISIGnal:PILot:AVERage \n
		Snippet: value: List[float] = driver.multiEval.trace.cdp.isignal.pilot.average.fetch() \n
		Return values of the code domain power (CDP) I-signal and Q-signal bar graphs. The results of the current, average,
		maximum and minimum bar graphs can be retrieved. Only the pilot part of a physical layer subtype 0/1 measurement is
		retrieved. For RRI part and subtype 2 or 3 measurements, see method RsCmwEvdoMeas.MultiEval.Trace.Cdp.Isignal.Rri.Current.
		fetch etc. The values described below are returned by FETCh and READ commands. CALCulate commands return limit check
		...
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:TRACe:CDP:ISIGnal:PILot:AVERage?', suppressed)
		return response
	...
	def calculate(self) -> List[enums.ResultStatus2]:
		...
		Snippet: value: List[enums.ResultStatus2] = driver.multiEval.trace.cdp.isignal.pilot.average.calculate() \n
		Return values of the code domain power (CDP) I-signal and Q-signal bar graphs. The results of the current, average,
		maximum and minimum bar graphs can be retrieved. Only the pilot part of a physical layer subtype 0/1 measurement is
		retrieved. For RRI part and subtype 2 or 3 measurements, see method RsCmwEvdoMeas.MultiEval.Trace.Cdp.Isignal.Rri.Current.
		fetch etc. The values described below are returned by FETCh and READ commands. CALCulate commands return limit check
		...
		Use RsCmwEvdoMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: isig_pilo_aver_cdp: float The number of results depends on the physical layer subtype (see method RsCmwEvdoMeas.Configure.MultiEval.plSubtype) : SF=15 for subtype 0/1 and SF=31 for subtypes 2 and 3 (only NAV values) . Range: -70 dB to 0 dB, Unit: dB"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:TRACe:CDP:ISIGnal:PILot:AVERage?', suppressed)
