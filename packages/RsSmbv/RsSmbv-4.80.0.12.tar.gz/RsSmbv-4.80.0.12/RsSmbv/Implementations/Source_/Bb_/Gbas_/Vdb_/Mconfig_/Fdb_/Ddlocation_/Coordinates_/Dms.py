...
class Dms:
	...
	def set(self, structure: DmsStruct, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.gbas.vdb.mconfig.fdb.ddlocation.coordinates.dms.set(value = [PROPERTY_STRUCT_NAME](), channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> DmsStruct:
		"""SCPI: [SOURce<HW>]:BB:GBAS:VDB<CH>:MCONfig:FDB<ST>:DDLocation:COORdinates:DMS \n
		Snippet: value: DmsStruct = driver.source.bb.gbas.vdb.mconfig.fdb.ddlocation.coordinates.dms.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Defines the coordinates of the Delta DERP location in degrees, minutes and seconds. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Vdb')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fdb')
