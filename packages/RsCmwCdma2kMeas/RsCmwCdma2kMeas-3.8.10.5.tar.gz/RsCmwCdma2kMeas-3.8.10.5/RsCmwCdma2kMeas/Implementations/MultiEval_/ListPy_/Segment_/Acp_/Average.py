...
class Average:
	...
	class FetchStruct(StructBase):
		...
		__meta_args_list = [
			...
			ArgStruct('Average_Acp', DataType.FloatList, None, False, False, 21),
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.segment.acp.average.fetch(segment = repcap.Segment.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:ACP:AVERage?', self.__class__.FetchStruct())
	...
	def calculate(self, segment=repcap.Segment.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.segment.acp.average.calculate(segment = repcap.Segment.Default) \n
		Returns the adjacent channel power (ACP) results for segment <no> in list mode (see method RsCmwCdma2kMeas.Configure.
		MultiEval.ListPy.value) . If enabled the wideband power and narrowband power results are returned, too. To define the
		statistical length for AVERage, MINimum, MAXimum and SDEviation calculation and enable the calculation of the results use
		the command method RsCmwCdma2kMeas.Configure.MultiEval.ListPy.Segment.Spectrum.set. The values described below are
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:ACP:AVERage?', self.__class__.CalculateStruct())
