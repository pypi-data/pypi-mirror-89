...
class Dms:
	...
	def set(self, structure: DmsStruct, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.gbas.vdb.mconfig.dfLocation.coordinates.dms.set(value = [PROPERTY_STRUCT_NAME](), channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> DmsStruct:
		"""SCPI: [SOURce<HW>]:BB:GBAS:VDB<CH>:MCONfig:DFLocation:COORdinates:DMS \n
		Snippet: value: DmsStruct = driver.source.bb.gbas.vdb.mconfig.dfLocation.coordinates.dms.get(channel = repcap.Channel.Default) \n
		Defines the coordinates of the Delta FPAD location in degrees, minutes and seconds. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Vdb')
