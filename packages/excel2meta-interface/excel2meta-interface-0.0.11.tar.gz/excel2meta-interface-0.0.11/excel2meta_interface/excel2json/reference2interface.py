...
class Reference2Interface:
    ...
    def process_ref_data_source_file(self, data_source_file, is_source=True):
        ...
        for data_source in sources:
            ...
            # create a copy in the sub folder source or target
            ...
            if is_source and self.one_on_ones is not None:
                todo = True
                for pattern in self.one_on_patterns:
                    # print("Checking for one-on-one", item["label"])
                    # zone_pattern = re.compile(pattern["pattern"])
    ...
    def process_one_on_one(self, zone, source, label, original_content):
        ...
        directory = path.join(self.lineage_output, self.one_on_one_name)
        makedirs(directory, exist_ok=True)
        file_name = path.join(directory, label + "-" + source + "-1on1_entity")
        with open(file_name + ".json", "w", encoding="utf8") as out:
            out.write(content)
    ...
    def process_one_on_one_attributes(self, zone, source, label, attributes):
        ...
        directory = path.join(self.lineage_output, self.one_on_one_name)
        makedirs(directory, exist_ok=True)
        file_name = path.join(directory, label + "-" + source["dataset"] + "-1on1_attributes.json")
        with open(file_name, "w", encoding="utf8") as out:
            json.dump(json_content, out, indent=4)
    ...
    def process_map_data_source_file(self, data_source_file):
        ...
        makedirs(directory, exist_ok=True)
        csv_file_name = path.join(directory, "lineage_for_data_sources" + ".csv")
        with open(csv_file_name, "w", newline="", encoding="utf8") as f_csv_file:
            ...
            for src_tgt_data_source in src_tgt_data_sources:
                ...
                src_path = path.join("edl://", "FileSystem"
                                     , src_tgt_data_source["source"], src_tgt_data_source["source_dataset"])
                tgt_path = path.join("Rock_Anthems_Playlist://", "FileSystem"
                                     , src_tgt_data_source["target"], src_tgt_data_source["target_dataset"])
                col_writer.writerow(["core.DataSetDataFlow", "", "", src_path, tgt_path])
    ...
    def process_map_zone_file(self, zone_file):
        ...
        directory = path.join(self.lineage_output, self.one_on_one_name, "lineage")
        makedirs(directory, exist_ok=True)
        csv_file_name = path.join(directory, "lineage_for_zones" + ".csv")
        with open(csv_file_name, "w", newline="", encoding="utf8") as f_csv_file:
            col_writer = csv.writer(f_csv_file)
            col_writer.writerow(self.edc_lineage_column_header)
            for src_tgt_zone in src_tgt_zones:
                # TODO: Get EDC Resource name from input or config
                # print("src_tgt_zone is:", src_tgt_zone)
                src_path = path.join("edl://", "FileSystem", src_tgt_zone["source"])
                tgt_path = path.join("Rock_Anthems_Playlist://", "FileSystem", src_tgt_zone["target"])
                col_writer.writerow(["core.DataSourceDataFlow", "", "", src_path, tgt_path])
