...
class FormatPy:
	...
	def set(self, position_format: enums.PositionFormat, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:QZSS<ST>:COORdinates:FORMat \n
		Snippet: driver.source.bb.gnss.adGeneration.qzss.coordinates.formatPy.set(position_format = enums.PositionFormat.DECimal, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.PositionFormat:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:QZSS<ST>:COORdinates:FORMat \n
		Snippet: value: enums.PositionFormat = driver.source.bb.gnss.adGeneration.qzss.coordinates.formatPy.get(stream = repcap.Stream.Default) \n
		Sets the format in which the coordinates of the reference location are set. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Qzss')
