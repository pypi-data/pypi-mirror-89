...
from .maputils import GenericItemMap, CreateItemAction, CreateGenericItemAction, ClearMapAction
...
class CreateAssetAction(CreateGenericItemAction):
    ...
    def run(self, button):
        ...
        if item is None:
            ...
            msg += "\n<h3>Could not create asset <i>{}</i>...</h3>".format(self.asset_name.get())
...
class DropAssetAction(flow.ConnectAction):
    ...
    def accept_label(self, objects, urls):
        asset_objects = list(filter(
            lambda obj: isinstance(obj, Asset),
            objects
        ))
