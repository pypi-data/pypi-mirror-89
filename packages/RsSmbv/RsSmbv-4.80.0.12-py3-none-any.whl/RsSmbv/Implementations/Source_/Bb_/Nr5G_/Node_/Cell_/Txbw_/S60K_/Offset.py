...
class Offset:
	...
	def set(self, scs_offset: float, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CH>:TXBW:S60K<ST>:OFFSet \n
		Snippet: driver.source.bb.nr5G.node.cell.txbw.s60K.offset.set(scs_offset = 1.0, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:NR5G:NODE:CELL<CH>:TXBW:S60K<ST>:OFFSet \n
		Snippet: value: float = driver.source.bb.nr5G.node.cell.txbw.s60K.offset.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Sets the offset between the usable RB and the common RBs. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'S60K')
