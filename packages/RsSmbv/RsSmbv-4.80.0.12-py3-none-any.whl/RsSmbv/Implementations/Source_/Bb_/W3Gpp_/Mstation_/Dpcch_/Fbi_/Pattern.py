...
class Pattern:
	...
	def set(self, structure: PatternStruct, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.dpcch.fbi.pattern.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> PatternStruct:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:DPCCh:FBI:PATTern \n
		Snippet: value: PatternStruct = driver.source.bb.w3Gpp.mstation.dpcch.fbi.pattern.get(stream = repcap.Stream.Default) \n
		The command determines the bit pattern when the PATTern data source is selected for the FBI field. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
