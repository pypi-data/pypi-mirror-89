...
class GPGTool(Command):
	...
	def _gpg_(self):
		...
		opts = ['--always-trust', '--pinentry-mode=loopback']
		if osname == 'nt':
			opts = opts + ['--batch']
	...
	def _gpgcheck(self):
		...
		plain = r' ~^ test ^~ '
		enc = self.stdo(f'gpg -e -r {gpgkey}', inputs=plain)
		_plain = str(self.stdo('gpg --batch -r {gpgkey} --always-trust -d', inputs=str(enc)))
		if _plain == plain:
			setattr(self, '_gpgselfcheck', True)
			return True
		else:
			print(plain, _plain)
			if getin(quest):
