...
class ManagedConnection(Connection):
    ...
    def __init__(
        ...
        """
        debug = os.environ.get("COMPILER_GYM_SERVICE_DEBUG") == "1"
        ...
        working_directory = Path(mkdtemp(prefix="CompilerGymService-"))
        ...
        # local service binary output to stderr. By default, the output of
        # the service is discarded.
        ostream = sys.stderr if debug else subprocess.DEVNULL
        ...
        wait_secs = 0.1
        port_path = working_directory / "port.txt"
        end_time = time() + port_init_max_seconds
        while time() < end_time:
            returncode = self.process.poll()
            if returncode is not None:
                raise ServiceError(
                    f"Serviced terminated with returncode: {returncode}"
                )
