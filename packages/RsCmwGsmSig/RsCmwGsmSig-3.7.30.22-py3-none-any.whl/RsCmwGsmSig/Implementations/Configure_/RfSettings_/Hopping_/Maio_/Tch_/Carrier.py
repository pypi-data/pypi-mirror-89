...
class Carrier:
	...
	def set(self, maio: int, carrier=repcap.Carrier.Default) -> None:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:MAIO:TCH[:CARRier<Carrier>] \n
		Snippet: driver.configure.rfSettings.hopping.maio.tch.carrier.set(maio = 1, carrier = repcap.Carrier.Default) \n
		Specifies the mobile allocation index offset (MAIO) . \n
			:param maio: Range: 0 to 63
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')"""
		param = Conversions.decimal_value_to_str(maio)
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		self._core.io.write(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:MAIO:TCH:CARRier{carrier_cmd_val} {param}')

	def get(self, carrier=repcap.Carrier.Default) -> int:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:MAIO:TCH[:CARRier<Carrier>] \n
		Snippet: value: int = driver.configure.rfSettings.hopping.maio.tch.carrier.get(carrier = repcap.Carrier.Default) \n
		Specifies the mobile allocation index offset (MAIO) . \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
			:return: maio: Range: 0 to 63"""
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		response = self._core.io.query_str(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:MAIO:TCH:CARRier{carrier_cmd_val}?')
