...
class Impedance:
	...
	def set(self, impedance: enums.ImpG50G10K, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.connector.user.trigger.impedance.set(impedance = enums.ImpG50G10K.G10K, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'CONNector:USER{channel_cmd_val}:TRIGger:IMPedance {param}')

	# noinspection PyTypeChecker
	def get(self, channel=repcap.Channel.Default) -> enums.ImpG50G10K:
		"""SCPI: CONNector:USER<CH>:TRIGger:IMPedance \n
		Snippet: value: enums.ImpG50G10K = driver.connector.user.trigger.impedance.get(channel = repcap.Channel.Default) \n
		Selects the input impedance for the external trigger/clock inputs, when method RsSgt.Connector.User.Omode.set is set to
		TRIGger or CIN/COUT. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'CONNector:USER{channel_cmd_val}:TRIGger:IMPedance?')
