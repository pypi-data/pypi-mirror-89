...
class Supl:
	...
	def reliability(self):
		...
		if not hasattr(self, '_reliability'):
			from .Supl_.Reliability import Reliability
	...
	def state(self):
		...
		if not hasattr(self, '_state'):
			from .Supl_.State import State
			self._state = State(self._core, self._base)
		return self._state

	# noinspection PyTypeChecker
	class RexStruct(StructBase):
	...
	def get_rex(self) -> RexStruct:
		"""SCPI: SOURce:DATA:CONTrol:SUPL:REX \n
		Snippet: value: RexStruct = driver.source.data.control.supl.get_rex() \n
		No command help available \n
			:return: structure: for return value, see the help for RexStruct structure arguments.
		"""
		return self._core.io.query_struct('SOURce:DATA:CONTrol:SUPL:REX?', self.__class__.RexStruct())
