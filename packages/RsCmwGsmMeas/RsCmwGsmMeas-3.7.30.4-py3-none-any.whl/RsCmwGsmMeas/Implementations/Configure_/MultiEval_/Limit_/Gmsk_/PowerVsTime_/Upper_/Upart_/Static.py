...
class Static:
	...
	def set(self, structure: StaticStruct, usefulPart=repcap.UsefulPart.Default) -> None:
		...
		Snippet: driver.configure.multiEval.limit.gmsk.powerVsTime.upper.upart.static.set(value = [PROPERTY_STRUCT_NAME](), usefulPart = repcap.UsefulPart.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:PVTime:UPPer:UPARt{usefulPart_cmd_val}:STATic', structure)

	def get(self, usefulPart=repcap.UsefulPart.Default) -> StaticStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:PVTime:UPPer:UPARt<nr>:STATic \n
		Snippet: value: StaticStruct = driver.configure.multiEval.limit.gmsk.powerVsTime.upper.upart.static.get(usefulPart = repcap.UsefulPart.Default) \n
		These commands define and activate upper limit lines for the measured power vs. time. The lines apply to the modulation
		schemes GMSK, 8PSK (EPSK) or 16-QAM (QAM16) . Each line consists of three sections: rising edge (REDGe) , useful part
		(UPARt) and falling edge (FEDGe) . Each section consists of several areas for which relative and absolute limits can be
		defined (if both are defined the higher limit overrules the lower one) . \n
		...
		usefulPart_cmd_val = self._base.get_repcap_cmd_value(usefulPart, repcap.UsefulPart)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:PVTime:UPPer:UPARt{usefulPart_cmd_val}:STATic?', self.__class__.StaticStruct())
