...
r"""Functional dependencies (FDs) are a major tool for database design.

Introduction
============
The module
[functional_dependencies](https://gitlab.com/oer/cs/programming/-/blob/master/functional_dependencies.py)
...
(I do not know what a "candidate" key is.  We do not need that term here.)
...
FDs and X of attributes, the closure of X with respect to F is the largest set
of attributes Y such that X -> Y is implied by F.
...
"small" set G of FDs such that the FDs implied by F and G are the same.
...
>>> logging.debug("\\n".join([str(threenf) for threenf in normalisiert]))
>>> len(normalisiert) == 3
True
...
>>> logging.debug("\\n".join([str(threenf) for threenf in normalized]))
>>> len(normalized) == 5
True
...
>>> logging.debug("\\n".join([str(threenf) for threenf in starschema]))
>>> len(starschema) == 4
True
"""
    ...
    def lminimize(self, lhs, rhs):
        ...
        for attr in lhs:
            ...
            if not self.isimplied(cand, rhs):
                ...
                logging.debug(
                    "lminimize: removed %s from lhs %s for rhs %s",
                    attr, _set2string(lhs), _set2string(rhs))
        return cand
    ...
    def basis(self):
        ...
        for fdep in rcopy:
            ...
            if not result.isimplied(fdep.lhs, fdep.rhs):
                ...
                logging.debug("basis: removed %s", fdep)
