...
class TbsIndex:
	...
	def get(self, cell_name: str, subframe: int, cword=repcap.Cword.Default) -> GetStruct:
		...
		Snippet: value: GetStruct = driver.configure.signaling.lte.cell.ueScheduling.userDefined.sassignment.uplink.cword.tbsIndex.get(cell_name = '1', subframe = 1, cword = repcap.Cword.Default) \n
		Queries the transport block size index for the UL subframe with the index <Subframe>, code word <no>, for user-defined
		scheduling. \n
		...
		return self._core.io.query_struct(f'CONFigure:SIGNaling:LTE:CELL:UESCheduling:UDEFined:SASSignment:UL:CWORd{cword_cmd_val}:TBSindex? {param}'.rstrip(), self.__class__.GetStruct())
