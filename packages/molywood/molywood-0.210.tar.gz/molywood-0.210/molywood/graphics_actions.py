    ...
    if 'figure' in action.overlays[ovl].keys():
        ...
        try:
            ...
            box_params = {'boxstyle': "round", 'ec': (0.0, 0.0, 0.0), 'fc': (1.0, 1.0, 1.0), 'alpha': 0.8,
                          'lw': tsize*0.1}
            if 'arrow' in box_params_raw:
                text_origin = 0.0018 * tsize
            else:
                text_origin = 0.0007 * tsize
            if box_params_raw not in ['t', 'true', 'y', 'yes']:
                ...
                for param_pair in box_params_raw:
                    ...
                    bkey = boxkey if boxkey not in param_maps.keys() else param_maps[boxkey]
        ...
        try:
            ...
            fig = plt.figure(figsize=[r / 100 for r in res])
        text_dict = {'ha': 'left', 'va': 'bottom', 'clip_on': False, 'size': tsize, 'color': tcolor}
        if box_params:
