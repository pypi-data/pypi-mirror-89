...
class Users:
	...
	def set(self, users: enums.TdscdmaTotalUsers, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.up.cell.users.set(users = enums.TdscdmaTotalUsers._10, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:USERs {param}')

	# noinspection PyTypeChecker
	def get(self, stream=repcap.Stream.Default) -> enums.TdscdmaTotalUsers:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:USERs \n
		Snippet: value: enums.TdscdmaTotalUsers = driver.source.bb.tdscdma.up.cell.users.get(stream = repcap.Stream.Default) \n
		Sets the total number of users of the cell. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: users: 2| 4| 6| 8| 10| 12| 14| 16"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{stream_cmd_val}:USERs?')
