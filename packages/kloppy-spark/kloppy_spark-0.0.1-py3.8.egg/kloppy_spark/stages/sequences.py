...
class Sequences(Stage):
    def process(self, pipeline: "Pipeline", inputs: DataFrame) -> DataFrame:
        ...
        return inputs \
            .withColumn("seq", F.lag(F.col("team_id")).over(windowSpec) != F.col("team_id")) \
            .withColumn("seq", (F.col("event_type").isin(OPEN_SEQUENCE) & F.col("seq")) | F.col("event_type").isin(CLOSE_SEQUENCE)) \
            .withColumn("seq_no", F.sum(F.col("seq").cast("long")).over(windowSpec)) \
            .rdd.map(lambda e: (e.seq_no, e)).groupByKey().mapValues(list).filter(
