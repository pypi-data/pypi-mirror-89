...
def data_available_check(symbol='rb2105'):
    ...
    sql_str = """SELECT `datetime` FROM dbbardata 
    where symbol=%s and `interval`='1m' order by `datetime` desc limit 1"""
    sql_latest_trade_date_str = """SELECT trade_date FROM md_integration.wind_trade_date 
    where exch_code='SZSE' and trade_date<curdate() order by trade_date desc limit 1
    """
    try:
        trade_date_latest = database.execute_sql(sql_latest_trade_date_str).fetchone()[0]
        datetime_max = database.execute_sql(sql_str, [symbol]).fetchone()[0]
        if trade_date_latest <= datetime_max.date() and datetime_max.hour > 22:
            logger.info("当前数据截止时间 %s 是最新日期数据", datetime_max)
            return True
        else:
            logger.info("当前数据截止时间 %s 不是最新日期数据", datetime_max)
            return False
    finally:
        database.close()
