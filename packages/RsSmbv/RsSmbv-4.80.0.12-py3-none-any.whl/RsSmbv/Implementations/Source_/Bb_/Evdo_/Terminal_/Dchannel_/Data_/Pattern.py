...
class Pattern:
	...
	def set(self, structure: PatternStruct, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.evdo.terminal.dchannel.data.pattern.set(value = [PROPERTY_STRUCT_NAME](), stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> PatternStruct:
		"""SCPI: [SOURce<HW>]:BB:EVDO:TERMinal<ST>:DCHannel:DATA:PATTern \n
		Snippet: value: PatternStruct = driver.source.bb.evdo.terminal.dchannel.data.pattern.get(stream = repcap.Stream.Default) \n
		Selects the bit pattern for the data source. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Terminal')
