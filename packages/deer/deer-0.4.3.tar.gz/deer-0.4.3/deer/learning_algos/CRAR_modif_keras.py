...
from .NN_CRAR_modif_keras import NN # Default Neural network used
...
class CRAR(LearningAlgo):
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        if(self.update_counter%500==0):
            ...
            if(Es.ndim==4):
                print (np.transpose(Es, (0, 3, 1, 2))[0],np.transpose(ETs[0], (0, 3, 1, 2))[0],np.transpose(Es_, (0, 3, 1, 2))[0])    # data_format='channels_last' --> 'channels_first'
            else:
                print (Es[0],ETs[0],Es_[0])
            print ("R[0]")
            print (R[0])
        ...
        self.loss_T+=self.diff_Tx_x_.train_on_batch( states_val+next_states_val+[onehot_actions]+[(1-terminals_val)] , np.repeat(np.zeros_like(Es),2,axis=-1) ) #repeat 2 because two levels of complexity
...
#        ## END DEBUG PURPOSES
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.encoder_productl.optimizer.lr, self._lr)
