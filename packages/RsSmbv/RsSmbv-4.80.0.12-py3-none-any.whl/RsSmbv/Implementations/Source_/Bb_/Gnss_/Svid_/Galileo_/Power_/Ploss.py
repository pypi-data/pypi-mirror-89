...
class Ploss:
	...
	def set(self, path_loss_state: bool, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GALileo<ST>:POWer:PLOSs \n
		Snippet: driver.source.bb.gnss.svid.galileo.power.ploss.set(path_loss_state = False, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SVID{channel_cmd_val}:GALileo{stream_cmd_val}:POWer:PLOSs {param}')

	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GALileo<ST>:POWer:PLOSs \n
		Snippet: value: bool = driver.source.bb.gnss.svid.galileo.power.ploss.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		If enabled, the power of the SV ID signals is reduced to account for the free space attenuation. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Galileo')
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SVID{channel_cmd_val}:GALileo{stream_cmd_val}:POWer:PLOSs?')
