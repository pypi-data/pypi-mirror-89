...
class Maximum:
	...
	def fetch(self, segment=repcap.Segment.Default) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.multiEval.listPy.segment.cto.maximum.fetch(segment = repcap.Segment.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:CTO:MAXimum?', self.__class__.FetchStruct())
	...
	def calculate(self, segment=repcap.Segment.Default) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.multiEval.listPy.segment.cto.maximum.calculate(segment = repcap.Segment.Default) \n
		Returns channel time offset (CTO) results for the segment <no> in list mode (see method RsCmwCdma2kMeas.Configure.
		MultiEval.ListPy.value) . To define the statistical length for AVERage and MAXimum calculation and to enable the
		calculation of the results, use the command method RsCmwCdma2kMeas.Configure.MultiEval.ListPy.Segment.Modulation.set. The
		values described below are returned by FETCh commands. CALCulate commands return limit check results instead, one value
		...
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CALCulate:CDMA:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:CTO:MAXimum?', self.__class__.CalculateStruct())
