...
"""
        self.prep_command_ouput("test", "", 0)
        self.prep_command_ouput("test2", "", 0)
        self.prep_command_ouput("", "error", 0)
        mm = MechanicalMarkdown(test_data)
        success, report = mm.exectute_steps(False)
        self.assertTrue(success)
        calls = [call(['bash', '-c', 'echo "test"'],
                 ...
                 call(['bash', '-c', 'echo "test2"'],
                      stdout=subprocess.PIPE,
                      stderr=subprocess.PIPE,
                      universal_newlines=True,
                      env=os.environ),
                 call().communicate(timeout=60),
                 call(['bash', '-c', 'echo "error" >2'],
                      stdout=subprocess.PIPE,
                      stderr=subprocess.PIPE,
                      universal_newlines=True,
                      env=os.environ),
...
would run 'bash -c' with command: `echo "test2"`
---- Expected stdout ----
test
test2
---- end stdout ----
...
Step: step 2
----
would run 'bash -c' with command: `echo "foo"`
would run 'bash -c' with command: `echo "bar" >2`
---- Expected stdout ----
foo
---- end stdout ----
---- Expected stderr ----
