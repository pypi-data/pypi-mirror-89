...
class Pipeline:
    ...
    def _setup_config(self, lang):
        ...
        if self._cache_dir is None:
            master_config._cache_dir = 'cache/merino'
        else:
    ...
    def set_active(self, lang):
        assert lang in self.added_langs
    ...
    def add(self, lang):
        if lang in self.added_langs:
            ...
            treebank_name = lang2treebank[lang]
            if tbname2training_id[treebank_name] % 2 == 1:
                self._mwt_model[lang] = MWTWrapper(self._config, treebank_name=treebank_name, use_gpu=self._gpu)
            # lemma
            self._lemma_model[lang] = LemmaWrapper(self._config, treebank_name=treebank_name, use_gpu=self._gpu)
            # ner if possible
            if lang in lang2nercorpus:
                self._ner_model[lang] = NERClassifier(self._config, lang)
                self._ner_model[lang].to(self._config.device)
    ...
    def ssplit(self, in_doc):  # assuming input is a document
        in_doc, lstrip_offset = normalize_input(in_doc)

        eval_batch_size = tbname2tokbatchsize.get(lang2treebank[self.active_lang], 2)
        # load input text
        config = self._config
                    ...
                    if (wp_p == 2 or wp_p == 4):
                        ...
                        sentences.append(
                            {ID: len(sentences) + 1, TEXT: in_doc[sent_span[0]: sent_span[1]],
                             DSPAN: (lstrip_offset + sent_span[0], lstrip_offset + sent_span[1])})
                        current_sent = []
    ...
    def tokenize(self, in_sent):  # assuming input is a sentence
        in_sent, lstrip_offset = normalize_input(in_sent)
        eval_batch_size = tbname2tokbatchsize.get(lang2treebank[self.active_lang], 2)
        # load input text
        config = self._config
            ...
            for t, wp_p in zip(para_text, wp_pred):
                ...
                if wp_p >= 1:
                    ...
                    additional_info = {'current_len': len(tokens),
                                       SSPAN: (lstrip_offset + para_start + local_position - len(tok),
                                               lstrip_offset + para_start + local_position)}
                    current_sent += [(tok, wp_p, additional_info)]
                    current_tok = ''
                    if (wp_p == 2 or wp_p == 4):
    ...
    def dtokenize(self, in_doc):  # assuming input is a document
        in_doc, lstrip_offset = normalize_input(in_doc)
        ...
        for j in range(len(paragraphs)):
            ...
            for t, wp_p in zip(para_text, wp_pred):
                ...
                if wp_p >= 1:
                    ...
                    if len(tok) <= 0:
                        current_tok = ''
                        continue
                    additional_info = {DSPAN: (lstrip_offset + para_start + local_position - len(tok),
                                               lstrip_offset + para_start + local_position)}
                    current_sent += [(tok, wp_p, additional_info)]
                    current_tok = ''
                    if (wp_p == 2 or wp_p == 4):
                        ...
                        doc.append({
                            ID: len(doc) + 1,
                            TEXT: in_doc[processed_sent[0][DSPAN][0] - lstrip_offset: processed_sent[-1][DSPAN][
                                                                                          1] - lstrip_offset],
            ...
            if len(current_sent):
                ...
                doc.append({
                    ID: len(doc) + 1,
                    TEXT: in_doc[
                          processed_sent[0][DSPAN][0] - lstrip_offset: processed_sent[-1][DSPAN][1] - lstrip_offset],
                    TOKENS: processed_sent,
                    DSPAN: (processed_sent[0][DSPAN][0], processed_sent[-1][DSPAN][1])
                })
    ...
    def posdep(self, in_sent):  # assuming input is a sentence
        if type(in_sent) == str:  # input sentence is an untokenized string in this case
            in_sent = self.tokenize(in_sent)
        ...
        for batch in DataLoader(test_set, batch_size=tbname2tagbatchsize.get(self._config.treebank_name, 12),
                                shuffle=False, collate_fn=test_set.collate_fn):
    ...
    def dposdep(self, in_doc):  # assuming input is a document
        if type(in_doc) == str:  # in_doc is an untokenized string in this case
            in_doc = self.dtokenize(in_doc)
    ...
    def lemmatize(self, in_sent):
        if type(in_sent) == str:
            in_sent = self.tokenize(in_sent)
            in_sent = self.posdep(in_sent)
        dlemmatize_sent = deepcopy(in_sent)
        lemmatized_sent = self._lemma_model[self._config.active_lang].predict([{ID: 1, TOKENS: dlemmatize_sent}])[0][
            TOKENS]
        return lemmatized_sent

    def dlemmatize(self, in_doc):  # assuming input is a document
        if type(in_doc) == str:  # in_doc is a raw string in this case
            in_doc = self.dtokenize(in_doc)
            in_doc = self.dposdep(in_doc)

        dlemmatize_doc = deepcopy(in_doc)
        lemmatized_doc = self._lemma_model[self._config.active_lang].predict(dlemmatize_doc)
    ...
    def ner(self, in_sent):  # assuming input is a document
        if type(in_sent) == str:
            in_sent = self.tokenize(in_sent)
    ...
    def dner(self, in_doc):  # assuming input is a document
        if type(in_doc) == str:
            in_doc = self.dtokenize(in_doc)
    ...
    def __call__(self, input, is_sent=False):
        if is_sent:
            tokenized_sent = self.tokenize(input)
            tagged_sent = self.posdep(tokenized_sent)
            out = self.lemmatize(tagged_sent)
            if self._config.active_lang in lang2nercorpus:  # ner if possible
                out = self.ner(out)
        else:
            tokenized_doc = self.dtokenize(input)
            tagged_doc = self.dposdep(tokenized_doc)
            out = self.dlemmatize(tagged_doc)
            if self._config.active_lang in lang2nercorpus:  # ner if possible
                out = self.dner(out)
        return out
