...
class Lower:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.listPy.obw.frequency.lower.fetch() \n
		...
		Use RsCmwEvdoMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: obw_lower: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:OBW:FREQuency:LOWer?', suppressed)
		return response
