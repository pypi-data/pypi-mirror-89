...
# v - value
# p - permissions
def create_mapped_file(filemap, path):
...
def chunkify(files):
  ...
  for f in files:
    if os.path.islink(f):
      mapped_file = create_mapped_file(filemap['v'], f)
      mapped_file['t'] = 'l'
      mapped_file['v'] = os.readlink(f)
    ...
    mapped_file['t'] = 'f'
    mapped_file['v'] = []
    mapped_file['p'] = os.stat(f)[ST_MODE]
    for chunk in fastcdc(f, 2**11, 2**14, 2**15, hf=blake3.blake3, fat=True):
  ...
  with tempfile.NamedTemporaryFile('w', delete=False) as f:
    ...
    with tempfile.NamedTemporaryFile('w', delete=False) as g:
      ...
      safe_non_stream_api_call(
        ...
        'transmit',
        json={
          'chunks': { x: base64.b64encode(chunks[x]) for x in missing_chunks }
        }
      )
  return filemap
