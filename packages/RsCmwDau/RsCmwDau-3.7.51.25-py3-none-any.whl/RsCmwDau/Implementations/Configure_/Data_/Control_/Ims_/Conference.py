...
class Conference:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("conference", core, parent)

	@property
	def factory(self):
		"""factory commands group. 1 Sub-classes, 1 commands."""
		if not hasattr(self, '_factory'):
			from .Conference_.Factory import Factory
	...
	def max(self):
		...
		if not hasattr(self, '_max'):
			from .Conference_.Max import Max
	...
	def clone(self) -> 'Conference':
		...
		new_group = Conference(self._core, self._base.parent)
