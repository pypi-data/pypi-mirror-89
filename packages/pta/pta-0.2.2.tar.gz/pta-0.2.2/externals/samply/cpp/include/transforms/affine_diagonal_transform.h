...
class AffineDiagonalTransform : public AffineTransformBase<
	AffineDiagonalTransform<Scalar, Storage>, Storage> {
public:
	using Base = AffineTransformBase<
		AffineDiagonalTransform<Scalar, Storage>, Storage>;
	using Base::Base;
...
AffineDiagonalTransform<Scalar, Storage>::operator*(
	const Eigen::MatrixBase<MatrixType>& matrix) const
{
	return (Base::get_diagonal().asDiagonal() * matrix).colwise() + 
		Base::get_shift();
}
...
{
	Matrix<Scalar> inv_T = Matrix<Scalar>::Zero(
		Base::get_linear().cols(), Base::get_linear().rows());
