...
class Path:
	...
	def set(self, structure: PathStruct, path=repcap.Path.Default) -> None:
		...
		self._core.io.write_struct(f'CONFigure:EVDO:SIGNaling<Instance>:IQIN:PATH{path_cmd_val}', structure)
	...
	def get(self, path=repcap.Path.Default) -> PathStruct:
		...
		path_cmd_val = self._base.get_repcap_cmd_value(path, repcap.Path)
		return self._core.io.query_struct(f'CONFigure:EVDO:SIGNaling<Instance>:IQIN:PATH{path_cmd_val}?', self.__class__.PathStruct())
