...
class Carrier:
	...
	def set(self, enable: bool, carrier=repcap.Carrier.Default) -> None:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:ENABle:TCH[:CARRier<Carrier>] \n
		Snippet: driver.configure.rfSettings.hopping.enable.tch.carrier.set(enable = False, carrier = repcap.Carrier.Default) \n
		...
		self._core.io.write(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:ENABle:TCH:CARRier{carrier_cmd_val} {param}')

	def get(self, carrier=repcap.Carrier.Default) -> bool:
		"""SCPI: CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:ENABle:TCH[:CARRier<Carrier>] \n
		Snippet: value: bool = driver.configure.rfSettings.hopping.enable.tch.carrier.get(carrier = repcap.Carrier.Default) \n
		Enable or disable frequency hopping in the downlink traffic channel. \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')
			:return: enable: OFF | ON"""
		carrier_cmd_val = self._base.get_repcap_cmd_value(carrier, repcap.Carrier)
		response = self._core.io.query_str(f'CONFigure:GSM:SIGNaling<Instance>:RFSettings:HOPPing:ENABle:TCH:CARRier{carrier_cmd_val}?')
