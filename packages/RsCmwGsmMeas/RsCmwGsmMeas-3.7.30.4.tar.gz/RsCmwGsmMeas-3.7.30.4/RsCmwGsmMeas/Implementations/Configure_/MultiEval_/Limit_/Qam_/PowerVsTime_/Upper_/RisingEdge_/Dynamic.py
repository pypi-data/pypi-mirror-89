...
class Dynamic:
	...
	def set(self, structure: DynamicStruct, qamOrder=repcap.QamOrder.Default, risingEdge=repcap.RisingEdge.Default, rangePcl=repcap.RangePcl.Default) -> None:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM<ModOrder>:PVTime:UPPer:REDGe<nr>:DYNamic<Range> \n
		Snippet: driver.configure.multiEval.limit.qam.powerVsTime.upper.risingEdge.dynamic.set(value = [PROPERTY_STRUCT_NAME](), qamOrder = repcap.QamOrder.Default, risingEdge = repcap.RisingEdge.Default, rangePcl = repcap.RangePcl.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:PVTime:UPPer:REDGe{risingEdge_cmd_val}:DYNamic{rangePcl_cmd_val}', structure)

	def get(self, qamOrder=repcap.QamOrder.Default, risingEdge=repcap.RisingEdge.Default, rangePcl=repcap.RangePcl.Default) -> DynamicStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM<ModOrder>:PVTime:UPPer:REDGe<nr>:DYNamic<Range> \n
		Snippet: value: DynamicStruct = driver.configure.multiEval.limit.qam.powerVsTime.upper.risingEdge.dynamic.get(qamOrder = repcap.QamOrder.Default, risingEdge = repcap.RisingEdge.Default, rangePcl = repcap.RangePcl.Default) \n
		These commands define and activate dynamic (PCL-dependent) corrections to the upper limit lines for the measured power vs.
		time. The corrections apply to the modulation schemes GMSK, 8PSK (EPSK) or 16-QAM (QAM16) and to the three limit line
		sections: rising edge (REDGe) , useful part (UPARt) and falling edge (FEDGe) . Each limit line section consists of
		several areas (<no>) . Each dynamic correction is defined for up to five different PCL ranges (<Range>) ). \n
		...
		risingEdge_cmd_val = self._base.get_repcap_cmd_value(risingEdge, repcap.RisingEdge)
		rangePcl_cmd_val = self._base.get_repcap_cmd_value(rangePcl, repcap.RangePcl)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:PVTime:UPPer:REDGe{risingEdge_cmd_val}:DYNamic{rangePcl_cmd_val}?', self.__class__.DynamicStruct())
