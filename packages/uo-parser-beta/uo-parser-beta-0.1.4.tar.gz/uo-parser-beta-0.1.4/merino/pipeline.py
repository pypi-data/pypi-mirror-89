...
class Pipeline:
    def __init__(self, lang, cache_dir=None, gpu=True):
        ...
        for lang in self.added_langs:
            if lang in lang2nercorpus:
                ...
                self._ner_model[lang].to(self._config.device).half()
    ...
    def ssplit(self, in_doc):  # assuming input is a document
        if type(in_doc) == str and in_doc.isspace():
            return []
    ...
    def posdep(self, input, is_sent=False):
        if type(input) == str and input.isspace():
            ...
            return []
        if is_sent:
            if type(input) == list and len(input) > 0 and type(input[0]) == str:
                input = [{ID: k + 1, TEXT: w} for k, w in enumerate(input)]
            return self._posdep_sent(in_sent=input)
        else:
            if type(input) == list and len(input) > 0 and type(input[0]) == list and len(input[0]) > 0 and type(
                    input[0][0]) == str:
