...
class Imsi:
	"""Imsi commands group definition. 2 total commands, 1 Sub-groups, 1 group commands
	...
	@property
	def apn(self):
		"""apn commands group. 0 Sub-classes, 1 commands."""
	...
	def get_value(self) -> List[str]:
		...
		Snippet: value: List[str] = driver.sense.data.control.epdg.connections.imsi.get_value() \n
		Queries all IMSIs for which ePDG connections are established. \n
			:return: imsi: Comma-separated list of 10 values Each IMSI is returned as a string value. The remaining values are filled with INV.
		"""
		response = self._core.io.query_str('SENSe:DATA:CONTrol:EPDG:CONNections:IMSI?')
