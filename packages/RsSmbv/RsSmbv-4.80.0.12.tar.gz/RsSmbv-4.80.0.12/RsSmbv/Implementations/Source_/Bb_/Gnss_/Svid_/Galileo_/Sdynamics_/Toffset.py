...
class Toffset:
	...
	def set(self, time_offset: float, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GALileo<ST>:SDYNamics:TOFFset \n
		Snippet: driver.source.bb.gnss.svid.galileo.sdynamics.toffset.set(time_offset = 1.0, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GALileo<ST>:SDYNamics:TOFFset \n
		Snippet: value: float = driver.source.bb.gnss.svid.galileo.sdynamics.toffset.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Sets a time delay before the profile is applied. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Galileo')
