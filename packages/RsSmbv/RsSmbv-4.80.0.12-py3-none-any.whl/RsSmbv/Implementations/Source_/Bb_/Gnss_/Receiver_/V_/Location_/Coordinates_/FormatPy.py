...
class FormatPy:
	...
	def set(self, position_format: enums.PositionFormat, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.receiver.v.location.coordinates.formatPy.set(position_format = enums.PositionFormat.DECimal, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.PositionFormat:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RECeiver:[V<ST>]:LOCation:COORdinates:FORMat \n
		Snippet: value: enums.PositionFormat = driver.source.bb.gnss.receiver.v.location.coordinates.formatPy.get(stream = repcap.Stream.Default) \n
		Sets the format in which the latitude and longitude are set. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'V')
