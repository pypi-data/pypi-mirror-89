...
class BatchEndpointPair:
    __slots__ = ["prepare_endpoint", "process_endpoint", "has_prepare"]

    def __init__(self, prepare_endpoint: Optional[Endpoint], process_endpoint: Endpoint) -> None:
        self.prepare_endpoint = prepare_endpoint
        self.process_endpoint = process_endpoint
        self.has_prepare = prepare_endpoint is not None

    def bind(self, instance: Any) -> None:
        if self.prepare_endpoint:
    ...
    async def prepare(self, request: Request) -> Response:
        if not self.has_prepare:
            raise RuntimeError("prepare endpoint has not been provided")
