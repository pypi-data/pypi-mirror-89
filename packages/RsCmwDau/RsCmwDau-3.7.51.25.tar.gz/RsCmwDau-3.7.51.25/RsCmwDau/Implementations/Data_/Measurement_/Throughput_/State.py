...
class State:
	...
	def fetch(self) -> enums.ResourceState:
		...
		Snippet: value: enums.ResourceState = driver.data.measurement.throughput.state.fetch() \n
		...
		INITiate..., STOP..., ABORt... to change the measurement state. \n
			:return: meas_state: OFF | RUN | RDY OFF: measurement off, no resources allocated, no results RUN: measurement running, synchronization pending or adjusted, resources active or queued RDY: measurement terminated, valid results can be available"""
		response = self._core.io.query_str(f'FETCh:DATA:MEASurement<MeasInstance>:THRoughput:STATe?')
