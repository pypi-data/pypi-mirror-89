...
class Average:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.hack.trace.throughput.carrier.average.fetch() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:WCDMa:SIGNaling<Instance>:HACK:TRACe:THRoughput:CARRier<Carrier>:AVERage?', suppressed)
		return response

	def read(self) -> List[float]:
		"""SCPI: READ:WCDMa:SIGNaling<instance>:HACK:TRACe:THRoughput:CARRier<carrier>:AVERage \n
		Snippet: value: List[float] = driver.hack.trace.throughput.carrier.average.read() \n
		Returns the current throughput trace results per carrier. The number of results depends on the configured number of
		subframes to be measured per measurement cycle, see method RsCmwWcdmaSig.Configure.Hack.msFrames. For each 100 subframes,
		one result is returned. The results of the average and current traces can be retrieved. \n
		Use RsCmwWcdmaSig.reliability.last_value to read the updated reliability indicator. \n
		Global Repeated Capabilities: repcap.Carrier
			:return: average: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:WCDMa:SIGNaling<Instance>:HACK:TRACe:THRoughput:CARRier<Carrier>:AVERage?', suppressed)
		return response
