...
class Cw:
	...
	def get_value(self) -> float:
		...
		Sets the frequency of the RF output signal in the selected path.
			...
			:return: fixed: float The following settings influence the value range: An offset set with the command method RsSmab.Source.Frequency.offset Numerical value Sets the frequency in CW and sweep mode UP|DOWN Varies the frequency step by step in user mode. The frequency is increased or decreased by the value set with the command method RsSmab.Source.Frequency.Cw.value. Range: (RFmin + OFFSet) to (RFmax + OFFSet)
		"""
