...
def generate_mai(m, k, N, M, sparse_variables_1=None, sparse_variables_2=None, structure='identity',sigma=1):
    mean = np.zeros(N + M)
    ...
    for _ in range(k):
        sparse_vec = p[_] * np.outer(up[:, _], vp[:, _])
    # Cross Bit
