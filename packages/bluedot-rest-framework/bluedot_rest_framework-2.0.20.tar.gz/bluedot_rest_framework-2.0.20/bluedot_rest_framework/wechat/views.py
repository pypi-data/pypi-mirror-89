...
class OAuth(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if code:
            ...
            wechat_queryset = UesrModel.objects.filter(
                openid=user_info['openid']).first()
            if wechat_queryset:
                wechat_queryset.update(**user_data)
            else:
                wechat_queryset = UesrModel.objects.create(**user_data)

            token = jwt_create_token_wechat(
                openid=wechat_queryset.openid, unionid=wechat_queryset.unionid, wechat_id=wechat_queryset.pk)
            link_type = '?'
            if "?" in referer_uri:
                link_type = '&'
