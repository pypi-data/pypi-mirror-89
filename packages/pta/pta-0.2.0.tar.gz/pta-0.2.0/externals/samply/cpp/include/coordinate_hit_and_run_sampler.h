...
class CoordinateHitAndRunSampler : public MarkovChain<
...
public:
	 ...
	 */
	typedef MarkovChain<
		CoordinateHitAndRunSampler, Matrix<Scalar>, Matrix<Scalar>, Logger> Base;
...
protected:
	...
	InternalState next_state(const InternalState& state) override
	{
		...
		auto axis_ids = sampling_helper.get_random_integers(
			0, state.rows() - 1, state.cols());
		...
		if (transforms_.is_Fs_equal_Fi()) {
			t = chord_sampler_Fs.sample1d(
				rays_Fi, intersections, sampling_helper);
		}
		else {
