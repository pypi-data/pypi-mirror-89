...
class Static:
	...
	def set(self, structure: StaticStruct, fallingEdge=repcap.FallingEdge.Default) -> None:
		...
		Snippet: driver.configure.multiEval.limit.epsk.powerVsTime.upper.fallingEdge.static.set(value = [PROPERTY_STRUCT_NAME](), fallingEdge = repcap.FallingEdge.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:PVTime:UPPer:FEDGe{fallingEdge_cmd_val}:STATic', structure)

	def get(self, fallingEdge=repcap.FallingEdge.Default) -> StaticStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:PVTime:UPPer:FEDGe<nr>:STATic \n
		Snippet: value: StaticStruct = driver.configure.multiEval.limit.epsk.powerVsTime.upper.fallingEdge.static.get(fallingEdge = repcap.FallingEdge.Default) \n
		These commands define and activate upper limit lines for the measured power vs. time. The lines apply to the modulation
		schemes GMSK, 8PSK (EPSK) or 16-QAM (QAM16) . Each line consists of three sections: rising edge (REDGe) , useful part
		(UPARt) and falling edge (FEDGe) . Each section consists of several areas for which relative and absolute limits can be
		defined (if both are defined the higher limit overrules the lower one) . \n
		...
		fallingEdge_cmd_val = self._base.get_repcap_cmd_value(fallingEdge, repcap.FallingEdge)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:EPSK:PVTime:UPPer:FEDGe{fallingEdge_cmd_val}:STATic?', self.__class__.StaticStruct())
