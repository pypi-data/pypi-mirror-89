...
class PreprocessedTrainingData:
    ...
    def __init__(self, agent_data: dict, *, predict_single: bool = False, shuffle: bool = True, seed: int = 0):
        ...
        self.conversations = []
        for entry in agent_data["trainingConversations"]:
            conv = Conversation.parse_obj(entry['conversation'])
            if conv.num_agent_turns > 0:
                # These are training conversations, so we need to ensure each one
                # has some labels for supervised learning.
    ...
    def encode_raw_conversations(self, conversations: t.List[dict]) -> t.Dict[str, np.ndarray]:
        ...
        encoded_convs, _ = self._encode_conversations(convs, training=False)
