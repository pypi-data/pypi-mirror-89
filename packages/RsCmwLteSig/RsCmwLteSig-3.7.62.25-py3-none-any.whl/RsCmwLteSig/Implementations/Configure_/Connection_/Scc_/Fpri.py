...
class Fpri:
	"""Fpri commands group definition. 3 total commands, 2 Sub-groups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("fpri", core, parent)

	@property
	def mcluster(self):
		"""mcluster commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_mcluster'):
			from .Fpri_.Mcluster import Mcluster
	...
	def downlink(self):
		...
		if not hasattr(self, '_downlink'):
			from .Fpri_.Downlink import Downlink
	...
	def clone(self) -> 'Fpri':
		...
		new_group = Fpri(self._core, self._base.parent)
