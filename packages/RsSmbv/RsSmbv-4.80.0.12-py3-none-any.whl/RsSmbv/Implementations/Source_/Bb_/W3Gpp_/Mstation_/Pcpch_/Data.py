...
class Data:
	...
	def set(self, data: enums.DataSour, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.pcpch.data.set(data = enums.DataSour.DLISt, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.DataSour:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:PCPCh:DATA \n
		Snippet: value: enums.DataSour = driver.source.bb.w3Gpp.mstation.pcpch.data.get(stream = repcap.Stream.Default) \n
		The command determines the data source for the PCPCH. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
