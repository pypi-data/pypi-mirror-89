...
class Awgn:
	...
	def get_enable(self) -> bool:
		...
		Enables or disables AWGN insertion via the fading module. For multi-carrier, the same settings are applied to all
		carriers. Thus it is sufficient to configure one carrier. \n
			:return: enable: OFF | ON
		"""
		response = self._core.io.query_str('CONFigure:EVDO:SIGNaling<Instance>:FADing:AWGN:ENABle?')
	...
	def set_enable(self, enable: bool) -> None:
		...
		self._core.io.write(f'CONFigure:EVDO:SIGNaling<Instance>:FADing:AWGN:ENABle {param}')
	...
	def get_sn_ratio(self) -> float:
		...
		"""
		response = self._core.io.query_str('CONFigure:EVDO:SIGNaling<Instance>:FADing:AWGN:SNRatio?')
	...
	def set_sn_ratio(self, ratio: float) -> None:
		...
		self._core.io.write(f'CONFigure:EVDO:SIGNaling<Instance>:FADing:AWGN:SNRatio {param}')
