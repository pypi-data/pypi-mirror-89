...
class BwCommon:
	...
	def set(self, bw_common: enums.Bandwidth, ims=repcap.Ims.Default, virtualSubscriber=repcap.VirtualSubscriber.Default) -> None:
		...
		Snippet: driver.configure.data.control.ims.virtualSubscriber.evs.bwCommon.set(bw_common = enums.Bandwidth.FB, ims = repcap.Ims.Default, virtualSubscriber = repcap.VirtualSubscriber.Default) \n
		Selects the codec bandwidths supported in the EVS primary mode. The setting applies only if the uplink (receive) and the
		downlink (send) are configured together, see method RsCmwDau.Configure.Data.Control.Ims.VirtualSubscriber.Evs.Synch.
		Select.set. \n
		...
		self._core.io.write(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:VIRTualsub{virtualSubscriber_cmd_val}:EVS:BWCommon {param}')

	# noinspection PyTypeChecker
	def get(self, ims=repcap.Ims.Default, virtualSubscriber=repcap.VirtualSubscriber.Default) -> enums.Bandwidth:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:VIRTualsub<VirtualSubscriber>:EVS:BWCommon \n
		Snippet: value: enums.Bandwidth = driver.configure.data.control.ims.virtualSubscriber.evs.bwCommon.get(ims = repcap.Ims.Default, virtualSubscriber = repcap.VirtualSubscriber.Default) \n
		Selects the codec bandwidths supported in the EVS primary mode. The setting applies only if the uplink (receive) and the
		downlink (send) are configured together, see method RsCmwDau.Configure.Data.Control.Ims.VirtualSubscriber.Evs.Synch.
		Select.set. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')
		...
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		virtualSubscriber_cmd_val = self._base.get_repcap_cmd_value(virtualSubscriber, repcap.VirtualSubscriber)
		response = self._core.io.query_str(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:VIRTualsub{virtualSubscriber_cmd_val}:EVS:BWCommon?')
