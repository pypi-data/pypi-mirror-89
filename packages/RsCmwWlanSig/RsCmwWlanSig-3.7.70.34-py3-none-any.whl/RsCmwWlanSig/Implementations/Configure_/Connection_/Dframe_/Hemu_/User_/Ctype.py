...
class Ctype:
	...
	def set(self, coding_type: enums.CodingType, user=repcap.User.Default) -> None:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:DFRame:HEMU:USER<index>:CTYPe \n
		Snippet: driver.configure.connection.dframe.hemu.user.ctype.set(coding_type = enums.CodingType.BCC, user = repcap.User.Default) \n
		...
		self._core.io.write(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:DFRame:HEMU:USER{user_cmd_val}:CTYPe {param}')

	# noinspection PyTypeChecker
	def get(self, user=repcap.User.Default) -> enums.CodingType:
		"""SCPI: CONFigure:WLAN:SIGNaling<instance>:CONNection:DFRame:HEMU:USER<index>:CTYPe \n
		Snippet: value: enums.CodingType = driver.configure.connection.dframe.hemu.user.ctype.get(user = repcap.User.Default) \n
		Selects the coding type related to a user for HE MU PPDU. \n
			:param user: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:return: coding_type: LDPC | BCC"""
		user_cmd_val = self._base.get_repcap_cmd_value(user, repcap.User)
		response = self._core.io.query_str(f'CONFigure:WLAN:SIGNaling<Instance>:CONNection:DFRame:HEMU:USER{user_cmd_val}:CTYPe?')
