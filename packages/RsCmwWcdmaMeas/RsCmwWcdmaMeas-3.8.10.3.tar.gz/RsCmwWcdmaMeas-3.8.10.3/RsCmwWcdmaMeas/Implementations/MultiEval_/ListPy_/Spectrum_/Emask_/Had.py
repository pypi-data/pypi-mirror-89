...
class Had:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("had", core, parent)

	@property
	def current(self):
		"""current commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_current'):
			from .Had_.Current import Current
	...
	def average(self):
		...
		if not hasattr(self, '_average'):
			from .Had_.Average import Average
	...
	def maximum(self):
		...
		if not hasattr(self, '_maximum'):
			from .Had_.Maximum import Maximum
	...
	def clone(self) -> 'Had':
		...
		new_group = Had(self._core, self._base.parent)
