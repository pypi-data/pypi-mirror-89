...
class Range:
	...
	def set(self, structure: RangeStruct, ims=repcap.Ims.Default, virtualSubscriber=repcap.VirtualSubscriber.Default) -> None:
		...
		Snippet: driver.configure.data.control.ims.virtualSubscriber.evs.send.bitrate.range.set(value = [PROPERTY_STRUCT_NAME](), ims = repcap.Ims.Default, virtualSubscriber = repcap.VirtualSubscriber.Default) \n
		Selects the bit-rate range supported in the EVS primary mode in the downlink (send) direction. The value must be
		...
		self._core.io.write_struct(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:VIRTualsub{virtualSubscriber_cmd_val}:EVS:SEND:BITRate:RANGe', structure)

	def get(self, ims=repcap.Ims.Default, virtualSubscriber=repcap.VirtualSubscriber.Default) -> RangeStruct:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:VIRTualsub<VirtualSubscriber>:EVS:SEND:BITRate:RANGe \n
		Snippet: value: RangeStruct = driver.configure.data.control.ims.virtualSubscriber.evs.send.bitrate.range.get(ims = repcap.Ims.Default, virtualSubscriber = repcap.VirtualSubscriber.Default) \n
		Selects the bit-rate range supported in the EVS primary mode in the downlink (send) direction. The value must be
		compatible to the selected bandwidth. The setting applies only if the uplink and the downlink are configured separately,
		see method RsCmwDau.Configure.Data.Control.Ims.VirtualSubscriber.Evs.Synch.Select.set. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')
		...
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		virtualSubscriber_cmd_val = self._base.get_repcap_cmd_value(virtualSubscriber, repcap.VirtualSubscriber)
		return self._core.io.query_struct(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:VIRTualsub{virtualSubscriber_cmd_val}:EVS:SEND:BITRate:RANGe?', self.__class__.RangeStruct())
