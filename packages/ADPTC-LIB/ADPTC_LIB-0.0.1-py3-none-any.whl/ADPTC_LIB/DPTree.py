...
#** 按照 Theta = 密度/距离 从小到大，取前 20% 排除 离群点
def split_outlier(tree,local_density,closest_dis_denser):
    outlier_forest = {}
    #** 按照 Theta = 密度/距离 从小到大，取前 20% 排除 离群点
    theta = local_density/closest_dis_denser
    sorted_theta_index = np.argsort(theta)
    # wanted_outliers = np.where(theta<(np.mean(local_density)/1.2*0.2))[0] #* 方法 一：密度平均值/距离阈值    前百分之20
    # wanted_outliers = np.argsort(theta[wanted_outliers])
    wanted_outliers = sorted_theta_index[:int(len(theta)*0.2)]  #*  方法 二
    print(wanted_outliers)
    for node_id in wanted_outliers:
        if(node_id in tree.node_dir.keys()):
            node = tree.node_dir[node_id]
            parent_id = node.parent_id
            offspring = tree.get_subtree_offspring_id(node_id,[node_id])
            offspring_with_parent = tree.get_subtree_offspring_id(node_id,[node_id,parent_id])
            node_entropy = tree.calcu_subtree_entropy(offspring,local_density,closest_dis_denser)
            node_with_parent_entropy = tree.calcu_subtree_entropy(offspring_with_parent,local_density,closest_dis_denser)
            if(node_entropy == 0 and node_with_parent_entropy<0.8):
                print("异常点：",node_id)
                outlier_forest[node_id] = tree.remove_subtree(node_id)
                continue
            elif(node_entropy != 0 and np.abs(node_entropy-node_with_parent_entropy)>0.2):
                print("异常点：",node_id)
                outlier_forest[node_id] = tree.remove_subtree(node_id)
                pass
            pass
        pass
    return outlier_forest
