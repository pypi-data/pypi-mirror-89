...
def configure(
    ...
    *,
        ...
        for trans in transactions.pkgs:
            if not isinstance(trans, Delete):
                ...
                if set(to_fetch) - set(can_fetch) and not is_downloaded(trans.pkg):
                    ...
                    state.UNFETCHED = to_fetch
                    state.A = trans.pkg.get_default_sources()
                    state.USE = trans.pkg.get_use()[0]
                    _sandbox_execute_pybuild(
                        trans.pkg.FILE,
                        "nofetch",
                        Permissions(),
        ...
        if isinstance(trans, Delete):
            ...
            if deselect is None or deselect:
                if trans.pkg.CPN in get_set("selected-packages"):
                    ...
                    remove_set("world", trans.pkg.CPN)
    ...
    for trans in merged.pkgs:
        pkg = trans.pkg
        warning_path = os.path.join(env.TMP_DIR, "warnings", pkg.ATOM.CPF)
        info_path = os.path.join(env.TMP_DIR, "messages", pkg.ATOM.CPF)
        messages = []
    ...
    for set_name in selected_sets:
        if set_name not in BUILTIN_SETS:
            if deselect or delete or depclean:
                remove_set("world_sets", Atom(set_name))
            else:
                add_set("world_sets", Atom(set_name))
