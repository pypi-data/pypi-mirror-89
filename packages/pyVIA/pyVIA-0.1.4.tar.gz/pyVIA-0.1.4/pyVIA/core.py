...
def draw_trajectory_gams(X_dimred, sc_supercluster_nn, cluster_labels, super_cluster_labels, super_edgelist, x_lazy,
    ...
    for fst_i in final_super_terminal:
        ...
        #print('ll_',set(ll_))
    for e_i, (start, end) in enumerate(super_edgelist):#enumerate(list(set(ll_))):# : use the ll_ if you want to simplify the curves
        ...
        ax2.plot(XX, preds, linewidth=1.5, c='#323538')  # #2 1 #dimgray
        ...
        if direction_arrow == 1:
            ...
            ax2.arrow(xp[closest_loc], preds[closest_loc], xp[closest_loc + step] - xp[closest_loc],
                      preds[closest_loc + step] - preds[closest_loc], shape='full', lw=0, length_includes_head=False, #True
...
class VIA:
    def __init__(self, data, true_label=None, anndata=None, dist_std_local=2, jac_std_global='median',
                 ...
                 too_big_factor=0.4,resolution_parameter = 1.0,partition_type = "ModularityVP", small_pop=10, jac_weighted_edges=True, knn=30, n_iter_leiden=5, random_seed=42,
                 ...
                 full_neighbor_array='', full_distance_array ='', embedding=None, df_annot=None, preserve_disconnected_after_pruning=False,
    ...
    def get_gene_expression(self, gene_exp, title_gene=""):
        ...
        for i in range(n_terminal_states):
            ...
            if len(np.where(sc_bp[:, i] > 0.8)[
                ...
                print('len xval, yg', len(xval), len(yg))
                ax.plot(xval, yg, color=cmap_[i], linewidth=3.5, zorder=3,  label='TS:' + str(self.terminal_clusters[i]))#linestyle=(0, (5, 2, 1, 2)),    dash_capstyle='round'
            plt.legend()
    ...
    def run_subPARC(self):
        ...
        for comp_i in large_components:  # range(n_components):
            ...
            if self.super_terminal_cells == False:
                ...
                for i in terminal_clus_ai:
                    if markov_hitting_times_ai[i]> np.percentile(np.asarray(markov_hitting_times_ai), self.pseudotime_threshold_TS):
        ...
        if remove_outliers_low.size ==0: threshold_low = 0
        else: threshold_low = np.percentile(remove_outliers_low, 5)
    ...
    def draw_piechart_graph(self, ax, ax1, type_pt='pt', gene_exp='', title ='' ):
        ...
        for i, ax_i in enumerate([ax1]):
            ...
            if type_pt=='pt':  pt = self.markov_hitting_times
            else: pt = gene_exp
...
def main_Human(ncomps=80, knn=30, v0_random_seed=7, run_palantir_func=False):
    ...
    v1 = VIA(adata_counts.obsm['X_pca'][:, 0:ncomps], true_label, jac_std_global=0.15, dist_std_local=1, knn=knn,
             ...
             csr_array_locally_pruned=v0.csr_array_locally_pruned, random_seed=v0_random_seed)  # *.4super_terminal_cells = tsi_list #3root=1,
    v1.run_VIA()
    labels = v1.labels
...
def main_Toy_comparisons(ncomps=10, knn=30, random_seed=42, dataset='Toy3', root_user='M1',
    ...
    v0 = VIA(adata_counts.obsm['X_pca'][:, 0:ncomps], true_label, jac_std_global=0.15, dist_std_local=1, knn=knn,
             too_big_factor=0.3, root_user=root_user, preserve_disconnected=True, dataset='toy', random_seed=random_seed, is_coarse=True)  # *.4 root=2,
    v0.run_VIA()
    super_labels = v0.labels
...
def main_EB_clean(ncomps=30, knn=20, v0_random_seed=21, foldername='/home/shobi/Trajectory/Datasets/EB_Phate/'):
    ...
    v1 = VIA(input_data, time_labels, jac_std_global=0.15, dist_std_local=1, knn=knn,
             ...
             super_terminal_cells=tsi_list, root_user=1,is_coarse=False, full_neighbor_array=v0.full_neighbor_array, full_distance_array=v0.full_distance_array,ig_full_graph=v0.ig_full_graph,
    ...
    draw_sc_evolution_trajectory_dijkstra(v1, Y_phate, knn_hnsw, v0.full_graph_shortpath, idx=np.arange(0, input_data.shape[0]))
...
def main_EB(ncomps=30, knn=20, v0_random_seed=21):
    ...
    v1 = VIA(input_data, time_labels, jac_std_global=0.15, dist_std_local=1, knn=knn,
             ...
             csr_array_locally_pruned=v0.csr_array_locally_pruned, full_distance_array=v0.full_distance_array,full_neighbor_array=v0.full_neighbor_array,
...
def main_mESC(knn=30, v0_random_seed=42, run_palantir_func=False):
    ...
    v0 = VIA(adata.X, true_label_int, jac_std_global=0.3, dist_std_local=1, knn=knn,
             ...
             do_magic_bool=True, is_coarse=True, preserve_disconnected=False, pseudotime_threshold_TS=40, x_lazy=0.95, alpha_teleport=0.99)  # *.4 root=1,
    v0.run_VIA()
    ...
    v1 = VIA(adata.X, true_label_int, jac_std_global=0.15, dist_std_local=1, knn=knn,
             ...
             super_terminal_clusters=v0.terminal_clusters, random_seed=v0_random_seed,full_neighbor_array=v0.full_neighbor_array,full_distance_array = v0.full_distance_array,
             ig_full_graph=v0.ig_full_graph, csr_array_locally_pruned=v0.csr_array_locally_pruned, pseudotime_threshold_TS=40)
...
def main_scATAC_zscores(knn=20,ncomps = 30):
    ...
    df = pd.read_csv("/home/shobi/Trajectory/Datasets/scATAC_Hemato/zscore_scaled_kmer.tsv", sep='\t') #TF Zcores from STREAM NOT the original buenrostro corrected PCs
    df=df.transpose()
    ...
    gene_dict = {'ENSG00000165702_LINE1058_GFI1B_D':'GFI1B','ENSG00000162676_LINE1033_GFI1_D_N2':'GFI1','ENSG00000114853_LINE833_ZBTB47_I':'cDC','ENSG00000105610_LINE802_KLF1_D':'KLF1 (MkP)','ENSG00000119919_LINE2287_NKX23_I': "NKX32", 'ENSG00000164330_LINE251_EBF1_D_N2': 'EBF1 (Bcell)',
...
def main_scATAC_Hemato(knn=20):
    ...
    cell_types = ['GMP', 'HSC', 'MEP', 'CLP', 'CMP', 'LMuPP','MPP', 'pDC', 'mono', 'UNK']
    cell_dict = {'UNK': 'aqua', 'pDC': 'purple', 'mono': 'gold', 'GMP': 'orange', 'MEP': 'red', 'CLP': 'aqua','HSC': 'black', 'CMP': 'moccasin', 'MPP': 'darkgreen', 'LMuPP': 'limegreen'}
...
def wrapper(adata,true_label, embedding, knn=10,jac_std_global=0.15,root=0,dataset='', random_seed=42, v0_toobig=0.3, v1_toobig=0.1, marker_genes=[],ncomps=20):
...
def main_faced():
    ...
    def make_df(dataset ='mcf7',prefix = 'T1_M', start_g1=700 ):
        ...
        cols_keep_OR_q2=  ['Dry Mass' ,'Area','Volume','Phase Entropy Mean','Phase Entropy Radial Distribution','Peak Phase','Phase Var','Phase Entropy Var','Phase Range', 'Phase Kurtosis','Phase Arrangement Skewness','Phase Arrangement Var','Phase STD Variance','Mean Phase Arrangement','Phase Entropy Kurtosis','Phase Entropy Skewness','Dry Mass var','Phase Fiber Pixel >Median','Phase Skewness','Phase STD Skewness','Phase Fiber Pixel >Upper Percentile','Phase Entropy Centroid Displacement','Phase STD Mean','Eccentricity','AspectRatio','Phase STD Centroid Displacement','Phase STD Radial Distribution','Phase Fiber Centroid Displacement']
        ...
        cols_keep_AND = ['Dry Mass', 'Area', 'Volume', 'Phase Entropy Mean', 'Phase Entropy Radial Distribution','Phase Range', 'Peak Phase']#,'Orientation']# NEED ORIENTATION FOR CYCLIC LOOP
        cols_keep = ['Area', 'Volume', 'AspectRatio', 'Orientation', 'Dry Mass', 'Dry Mass Density', 'Dry Mass var', 'Dry Mass Skewness', 'Peak Phase', 'Phase Var', 'Phase Skewness', 'Phase Kurtosis', 'Phase Range', 'Phase Min', 'Phase Centroid Displacement', 'Phase STD Mean', 'Phase STD Variance', 'Phase STD Skewness', 'Phase STD Kurtosis', 'Phase STD Centroid Displacement', 'Phase STD Radial Distribution', 'Phase Entropy Mean', 'Phase Entropy Var', 'Phase Entropy Skewness', 'Phase Entropy Kurtosis', 'Phase Entropy Centroid Displacement', 'Phase Entropy Radial Distribution', 'Phase Fiber Centroid Displacement', 'Phase Fiber Radial Distribution', 'Phase Fiber Pixel >Upper Percentile', 'Phase Fiber Pixel >Median', 'Mean Phase Arrangement', 'Phase Arrangement Var', 'Phase Arrangement Skewness', 'Phase Orientation Var', 'Phase Orientation Kurtosis']
        cols_keep_test=['Dry Mass' ,'Area','Volume','Phase Entropy Mean','Phase Entropy Radial Distribution','Peak Phase','Phase Var','Phase Entropy Var','Phase Range', 'Phase Kurtosis','Phase Arrangement Skewness','Phase Arrangement Var','Phase STD Variance','Mean Phase Arrangement','Phase Entropy Kurtosis','Phase Entropy Skewness','Dry Mass var','Phase Fiber Pixel >Median','Phase Skewness','Phase STD Skewness','Phase Fiber Pixel >Upper Percentile','Phase Entropy Centroid Displacement','Phase STD Mean','Eccentricity','AspectRatio','Phase STD Centroid Displacement','Phase STD Radial Distribution','Phase Fiber Centroid Displacement', 'Circularity']
    ...
    cell_dict = {'T1_M1':'red', 'T2_M1':'yellowgreen', 'T1_M2':'orange', 'T2_M2':'darkgreen', 'T1_M3':'yellow', 'T2_M3':'blue'}
    ...
    v0 = VIA(X_in, true_label, jac_std_global=jac_std_global, dist_std_local=1, knn=knn,
             ...
             do_magic_bool=True, is_coarse=True, preserve_disconnected=True, preserve_disconnected_after_pruning=True, pseudotime_threshold_TS=40)  # *.4 root=1,
    v0.run_VIA()
...
def main():
    ...
    if dataset == 'Human':
        ...
        df_palantir = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/palantir_pt_50000cells_knn10.csv')
        truth = df_palantir['days']
        pt = df_palantir['palantir']
        correlation = pt.corr(truth)
        print('corr Palantir knn10', correlation)
        print('')

        df_palantir = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/palantir_pt_50000cells_knn20.csv')
        truth = df_palantir['days']
        pt = df_palantir['palantir']
        correlation = pt.corr(truth)
        print('corr Palantir knn20', correlation)
        print('')

        df_palantir = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/palantir_pt_50000cells_knn30.csv')
        truth = df_palantir['days']
        pt = df_palantir['palantir']
        correlation = pt.corr(truth)
        print('corr Palantir knn30', correlation)
        print('')

        df_palantir = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/palantir_pt_50000cells_knn40.csv')
        truth = df_palantir['days']
        pt = df_palantir['palantir']
        correlation = pt.corr(truth)
        print('corr Palantir knn40', correlation)
        print('')

        df_palantir = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/palantir_pt_50000cells_knn50.csv')
        truth = df_palantir['days']
        pt = df_palantir['palantir']
        correlation = pt.corr(truth)
        print('corr Palantir knn50', correlation)
        print('')

        df_sling = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/Slingshot_Kmeans15_PC5_pt.csv')
        df_sling['mean'] = df_sling[['curve1','curve2','curve3','curve4','curve5']].mean(axis=1)
        print('df head sling', df_sling.head())
        truth = df_sling['days']
        pt= df_sling['mean']
        correlation = pt.corr(truth)
        print('corr Slingshot', correlation)
        print('')

        df_sling = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/Slingshot_Kmeans15_28Feats_2000cells_pt.csv')
        df_sling['mean'] = df_sling[['curve1', 'curve2', 'curve3', 'curve4', 'curve5']].mean(axis=1)
        print('df head sling', df_sling.head())
        truth = df_sling['days']
        pt = df_sling['mean']
        correlation = pt.corr(truth)
        print('corr Slingshot 28 Feats 2000cells', correlation)
        print('')

        df_sling = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/Slingshot_Kmeans15_28Feats_5000cells_pt.csv')
        df_sling['mean'] = df_sling[['curve1','curve2', 'curve3']].mean(axis=1)#, 'curve4']].mean(axis=1)
        print('df head sling', df_sling.head())
        truth = df_sling['days']
        pt = df_sling['mean']
        correlation = pt.corr(truth)
        print('corr Slingshot 28 Feats 5000cells', correlation)
        print('')

        df= pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn10_v0.csv')
        truth = df['days']
        pt = df['via_knn']
        correlation = pt.corr(truth)
        print('corr via knn10', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn20_v4.csv')
        truth = df['days']
        pt = df['via_knn']
        correlation = pt.corr(truth)
        print('corr via knn20', correlation)
        pt_via1 = df['via_v1']
        correlation = pt_via1.corr(truth)
        print('corr via1 knn20', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn30.csv')
        truth = df['days']
        pt = df['via_knn']
        correlation = pt.corr(truth)
        print('corr viaknn 30', correlation)
        pt_via1 = df['via_v1']
        correlation = pt_via1.corr(truth)
        print('corr via1 knn30', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn20_v1.csv')
        truth = df['days']
        pt = df['via_knn']
        pt_via1 = df['via_v1']
        correlation = pt.corr(truth)
        print('corr viaknn20', correlation)
        correlation = pt_via1.corr(truth)
        print('corr via1 knn20', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn20_v2.csv')
        truth = df['days']
        pt = df['via_knn']
        pt_via1 = df['via_v1']
        correlation = pt.corr(truth)
        ...
        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/paga_dpt_knn10_v1.csv')
        truth = df['days']
        pt = df['paga_dpt']
        correlation = pt.corr(truth)
        print('paga knn10 corr', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/paga_dpt_knn20_v1.csv')
        truth = df['days']
        pt = df['paga_dpt']
        correlation = pt.corr(truth)
        print('paga knn 20 corr', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/paga_dpt_knn30_v1.csv')
        truth = df['days']
        pt = df['paga_dpt']
        correlation = pt.corr(truth)
        print('paga knn30 corr', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/paga_dpt_knn40.csv')
        truth = df['days']
        pt = df['paga_dpt']
        correlation = pt.corr(truth)
        print('paga knn40 corr', correlation)


        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/paga_dpt_knn50.csv')
        truth = df['days']
        pt = df['paga_dpt']
        correlation = pt.corr(truth)
        print('paga knn50 corr', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn10_v2.csv')
        truth = df['days']
        pt = df['via_knn']
        pt_via1 = df['via_v1']
        correlation = pt.corr(truth)
        print('corr via0', correlation)
        correlation = truth.corr(pt_via1)
        print('corr via1', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn10_v6.csv')
        truth = df['days']
        pt = df['via_knn']
        pt_via1 = df['via_v1']
        correlation = pt.corr(truth)
        print('corr via0 knn10', correlation)
        correlation = truth.corr(pt_via1)
        print('corr via1 knn10', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn30_v0.csv')
        truth = df['days']
        pt = df['via_knn']
        pt_via1 = df['via_v1']
        correlation = pt.corr(truth)
        print('corr via0 knn30', correlation)
        correlation = truth.corr(pt_via1)
        print('corr via1 knn30', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn40_v1.csv')
        truth = df['days']
        pt = df['via_knn']
        correlation = pt.corr(truth)
        print('via0 knn40 corr', correlation)
        pt_via1 = df['via_v1']
        correlation = pt_via1.corr(truth)
        print('corr via1 knn40', correlation)

        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn40_v2.csv')
        truth = df['days']
        pt = df['via_knn']
        correlation = pt.corr(truth)
        print('via0 knn40 corr', correlation)
        ...
        df = pd.read_csv('/home/shobi/Trajectory/Datasets/mESC/via_pt_knn50_v1.csv')
        truth = df['days']
        pt = df['via_knn']
        correlation = pt.corr(truth)
        print('via0 knn50 corr', correlation)
        pt_via1 = df['via_v1']
        correlation = pt_via1.corr(truth)
        print('corr via1 knn50', correlation)
