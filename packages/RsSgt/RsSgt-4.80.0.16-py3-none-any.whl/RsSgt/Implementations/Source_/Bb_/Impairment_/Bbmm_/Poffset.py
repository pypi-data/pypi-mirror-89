...
class Poffset:
	...
	def set(self, phase_offset: float, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.impairment.bbmm.poffset.set(phase_offset = 1.0, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce:BB:IMPairment:BBMM{channel_cmd_val}:POFFset {param}')

	def get(self, channel=repcap.Channel.Default) -> float:
		"""SCPI: [SOURce]:BB:IMPairment:BBMM<CH>:POFFset \n
		Snippet: value: float = driver.source.bb.impairment.bbmm.poffset.get(channel = repcap.Channel.Default) \n
		No command help available \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Bbmm')
			:return: phase_offset: No help available"""
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce:BB:IMPairment:BBMM{channel_cmd_val}:POFFset?')
