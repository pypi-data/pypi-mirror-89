...
class Mpoint:
	...
	def set(self, structure: MpointStruct, measPoint=repcap.MeasPoint.Default) -> None:
		...
		apply above the high power reference value and below the low power reference value defined by method RsCmwGsmMeas.
		Configure.MultiEval.Limit.Gmsk.Smodulation.rpower. Between the two reference power values, the limits are determined by
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:SMODulation:MPOint{measPoint_cmd_val}', structure)
	...
	def get(self, measPoint=repcap.MeasPoint.Default) -> MpointStruct:
		...
		measPoint_cmd_val = self._base.get_repcap_cmd_value(measPoint, repcap.MeasPoint)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:GMSK:SMODulation:MPOint{measPoint_cmd_val}?', self.__class__.MpointStruct())
