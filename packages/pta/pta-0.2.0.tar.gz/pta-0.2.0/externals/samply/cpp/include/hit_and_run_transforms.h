...
public:
	...
	HitAndRunTransforms(
		...
		, Fd_to_Fi_linear(
			(descriptor_Fi.to_frame().linear() * 
			descriptor_Fd.from_frame().linear()).template cast<TransformationScalar>())
		, Fd_to_Fs_linear(make_Fd_to_Fs_transform(
			descriptor_Fd, descriptor_Fi, sampler_Fs))
...
protected:
	size_t find_cheapest_linear_transform(
		const std::vector<
			AffineTransform<TransformationScalar>, Eigen::aligned_allocator<
				AffineTransform<TransformationScalar>>>& transforms) const
	{
		assert(!transforms.empty());

		// Look for an identity transform
		auto optimal_transform_it = 
		...
		optimal_transform_it = std::find_if(transforms.begin(), transforms.end(),
			[](const AffineTransform<TransformationScalar> & transform) {
