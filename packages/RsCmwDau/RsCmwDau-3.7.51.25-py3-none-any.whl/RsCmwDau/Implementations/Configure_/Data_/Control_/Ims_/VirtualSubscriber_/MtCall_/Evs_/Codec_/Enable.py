...
class Enable:
	...
	def set(self, codec_rate: bool, ims=repcap.Ims.Default, virtualSubscriber=repcap.VirtualSubscriber.Default, codec=repcap.Codec.Default) -> None:
		...
		Snippet: driver.configure.data.control.ims.virtualSubscriber.mtCall.evs.codec.enable.set(codec_rate = False, ims = repcap.Ims.Default, virtualSubscriber = repcap.VirtualSubscriber.Default, codec = repcap.Codec.Default) \n
		...
		self._core.io.write(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:VIRTualsub{virtualSubscriber_cmd_val}:MTCall:EVS:CODec{codec_cmd_val}:ENABle {param}')

	def get(self, ims=repcap.Ims.Default, virtualSubscriber=repcap.VirtualSubscriber.Default, codec=repcap.Codec.Default) -> bool:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:VIRTualsub<VirtualSubscriber>:MTCall:EVS:CODec<CodecIdx>:ENABle \n
		Snippet: value: bool = driver.configure.data.control.ims.virtualSubscriber.mtCall.evs.codec.enable.get(ims = repcap.Ims.Default, virtualSubscriber = repcap.VirtualSubscriber.Default, codec = repcap.Codec.Default) \n
		No command help available \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')
			:param virtualSubscriber: optional repeated capability selector. Default value: Nr1 (settable in the interface 'VirtualSubscriber')
			:param codec: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Codec')
		...
		virtualSubscriber_cmd_val = self._base.get_repcap_cmd_value(virtualSubscriber, repcap.VirtualSubscriber)
		codec_cmd_val = self._base.get_repcap_cmd_value(codec, repcap.Codec)
		response = self._core.io.query_str(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:VIRTualsub{virtualSubscriber_cmd_val}:MTCall:EVS:CODec{codec_cmd_val}:ENABle?')
