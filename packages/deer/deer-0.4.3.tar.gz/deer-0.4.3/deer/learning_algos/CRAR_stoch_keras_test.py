...
#def binary_crossentropy_from_logits_clipped(target, output, from_logits=False):
#    """Binary crossentropy between an output tensor and a target tensor from logits.
#    Arguments:
...
#        A tensor.
#    """
#    return K.clip( binary_crossentropy(y_true=target, y_pred=output) , 0.01, None )
...
class CRAR(LearningAlgo):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_norm=0, freeze_interval=1000, batch_size=32, update_rule="rmsprop", random_state=np.random.RandomState(), double_Q=False, neural_network=NN, **kwargs):
        ...
        self.loss_G1, self.loss_G2, self.loss_G=0.68, 0.68, 0.68
        self.loss_D1,self.loss_D2,self.loss_D=0.68, 0.68, 0.68
        self.loss_Q=0
        self.loss_disentangle_t=0
        self.loss_disambiguate1=0
        self.loss_disambiguate2=0
        self.wait_D,self.wait_G=0,0
        self.transition_g_full_lrdiv = kwargs.get('transition_g_full_lrdiv',100) #was 100 but at least low dim good 10
        self.encoder_g_full_lrdiv = kwargs.get('transition_g_full_lrdiv',100) #was 100 but at least low dim good 10
        self.exp_dec_error_C = kwargs.get('transition_g_full_lrdiv',2)
        self.encoder_diff_lrdiv = kwargs.get('encoder_diff_lrdiv',1)
        self.diff_s_s_lrdiv = kwargs.get('diff_s_s_lrdiv',10) #5io10?
        self.encoder_lrdiv = kwargs.get('encoder_lrdiv',10)
        self.full_Q_lrdiv = kwargs.get('full_Q_lrdiv',100) #must be high but not too high because it helps! (e.g. 100 is good)
        self.transition_d_lrdiv = kwargs.get('transition_d_lrdiv',1)
        ...
        self.encoder_frozen = self.learn_and_plan.encoder_model() # not frozen actually... :)
        ...
        self.transition_d_withencD = self.learn_and_plan.transition_discr_model_withencD(self.encoder_frozen,self.transition_d)
        self.transition_d_withencD2 = self.learn_and_plan.transition_discr_model_withencD(self.encoder_frozen,self.transition_d)
        
        self.full_Q=self.learn_and_plan.full_Q_model(self.encoder,self.Q,0,self._df)
        self.full_Q2=self.learn_and_plan.full_Q_model(self.encoder,self.Q,0,self._df)
        ...
        self.params_encoder = [ param
                    ...
                    for param in layer.trainable_weights ]
        self.params_encoder_frozen = [ param
                    for layer in self.encoder_frozen.layers
                    for param in layer.trainable_weights ]
        ...
        self.transition_d_full = self.learn_and_plan.transition_g_full_model(self.encoder_frozen, self.encoder, self.transition, self.transition_d)
        self.encoder_d_full = self.learn_and_plan.encoder_g_full_model(self.encoder_frozen, self.encoder, self.transition_d)
        self.transition_g_full = self.learn_and_plan.transition_g_full_model(self.encoder_frozen, self.encoder, self.transition, self.transition_d)
        self.encoder_g_full = self.learn_and_plan.encoder_g_full_model(self.encoder_frozen, self.encoder, self.transition_d)
        self.transition_g_full2 = self.learn_and_plan.transition_g_full_model(self.encoder_frozen, self.encoder, self.transition, self.transition_d)
    ...
    def exp_dec_error(self, y_true, y_pred):
        return K.exp( - self.exp_dec_error_C * K.sqrt( K.clip(K.sum(K.square(y_pred), axis=-1, keepdims=True),0.000001,10) )  ) # *2 distrib, *5 simple maze
...
#        for i,(param,next_param) in enumerate(zip(self.params_encoder, self.params_encoder_frozen)):
...
#            K.set_value(next_param,K.get_value(param))
        ...
        noise_level=self.noise_level/2. #0.2 #max( (1.-self.update_counter/50000) , 0.1 ) #0.2
        noise_level2=self.noise_level #used for r and gamma #max( (1.-self.update_counter/50000) , 0.1 ) #0.4
        
        noise_Es = np.random.normal(0,noise_level,Es.shape)*0 #
        noise_Es2 = np.random.normal(0,noise_level,Es.shape)*0 #noise_Es #good
        noise_r = np.random.normal(0,noise_level2,rewards_val.shape)
        noise_gamma = np.random.normal(0,noise_level2,terminals_val.shape)
        noise_Q_abstr = np.random.normal(0, noise_level, Es.shape)
        ...
        if(self.update_counter%2000==0):
            ...
            if (self._high_int_dim==True):
                ...
                plt.subplot(2,3,6)
                plt.imshow(Es_[0,:,:,0:3])
                plt.show()
                plt.savefig('plot123.pdf')
                
                
        # Fit the transition discriminator
        # First half of the transitions are "True" and the other half is generated.
        true_transitions=np.concatenate(  ( np.ones((self._batch_size,1)) , np.zeros((self._batch_size,1)) )  ,  axis=0  )
        Es+=noise_Es # if *1 --> bisimilar states tend to collapse!
        Es_+=noise_Es2 #noise_Es2 = noise_Es
        Es__double=np.append(Es_,ETs,axis=0)
        rewards_val+=np.random.normal(0,noise_level2,(self._batch_size))
        rewards_val_double=np.append(np.expand_dims(rewards_val,axis=1),R,axis=0)
        terminals_val+=np.random.normal(0,noise_level2,(self._batch_size))
        terminals_val_double=np.append(np.expand_dims(terminals_val,axis=1),terminals,axis=0)
        ...
        if(len(Es.shape)==4):
            Es_double=np.tile(Es,(2,1,1,1))
            noise_Es_double=np.tile(noise_Es,(2,1,1,1))
        elif(len(Es.shape)==2):
            Es_double=np.tile(Es,(2,1))
            noise_Es_double=np.tile(noise_Es,(2,1))
        ...
        for s in states_val:
            ...
            for i in range(len(s.shape)-1):
                mytuple=mytuple+(1,)
            states_val_double.append( np.tile(s,mytuple) ) # tile along the first axis
            
        Es_double+=np.random.normal(0,noise_level,Es_double.shape)
        onehot_actions_double=np.tile(onehot_actions,(2,1))
        
#        print ("diff")
#        print (self.transition_d_full.predict(states_val+[onehot_actions,noise] ))
#        print (self.transition_g_full.predict(states_val+[onehot_actions,noise] ))
#
#        print (self.transition_d_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) ))
#        print (self.transition_d_full.train_on_batch(states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) ))
#        print (self.transition_g_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) ))
#        print (self.transition_g_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) ))
#        print (self.transition_d_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) ))
#        print (self.transition_g_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) ))
#        print (self.transition_g_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) ))
#
#        print ("diff enc")
#        noise_e=np.random.normal(0,noise_level,Es_[:self._batch_size].shape)
#        print (self.encoder_d_full.predict(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e] ))
#        print (self.encoder_g_full.predict(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e] ))
#
#        print (self.encoder_d_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e], np.ones((self._batch_size,1))) )
#        print (self.encoder_g_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e], np.zeros((self._batch_size,1))) )
#        print (self.encoder_g_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e], np.zeros((self._batch_size,1))) )
#        print (self.encoder_d_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e], np.ones((self._batch_size,1))) )
#        print (self.encoder_g_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e], np.zeros((self._batch_size,1))) )
#        print (self.encoder_g_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,noise_e], np.zeros((self._batch_size,1))) ) 
#        
#        print (self.transition_g_full2.train_on_batch(states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) ))
#        print (self.transition_g_full2.train_on_batch(states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) ))

        if(self.update_counter%500==0):
            print (true_transitions[-5:])
            print (self.transition_d.predict([Es_double[-5:],onehot_actions_double[-5:],Es__double[-5:],rewards_val_double[-5:],terminals_val_double[-5:]]))
            print ("generator_discrim predicted transitions vs true transitions")
            print (self.transition_g_full.predict([noise_Es[-5:]]+[states_val[0][-5:]]+[onehot_actions[-5:],noise[-5:]]))

            #a,b,c,d=self.encoder.layers[2].get_weights()[0],self.encoder_frozen.layers[2].get_weights()[0],self.transition.layers[7].get_weights()[0], self.transition_d.layers[7].get_weights()[0]
            #print (a,b,c,d)
            print ("discrim predicted transitions vs true transitions")
            print (true_transitions[:5])
            print (self.transition_d.predict([Es_double[:5],onehot_actions_double[:5],Es__double[:5],rewards_val_double[:5],terminals_val_double[:5]]))
            print ("generator_discrim predicted transitions vs true transitions")
            print (self.transition_g_full.predict([noise_Es[:5]]+[states_val[0][:5]]+[onehot_actions[:5],noise[:5]]))
            #print (self.transition_d_full.predict([states_val[0][:5]]+[onehot_actions[:5],noise[:5]]) )
            print (rewards_val_double[:5],terminals_val_double[:5])
            for i in range(1):
                loss_D=self.transition_d.train_on_batch([Es_double,onehot_actions_double,Es__double,rewards_val_double,terminals_val_double], true_transitions )
                #loss_D1=self.transition_d_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) )
                #loss_D2=self.encoder_d_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,np.random.normal(0,noise_level,Es_[:self._batch_size].shape)], np.ones((self._batch_size,1)) )
                #true_transitions=np.concatenate(  ( np.ones((self._batch_size,1)) , np.zeros((self._batch_size,1)) )  ,  axis=0  )
                #loss_D=self.transition_d.train_on_batch([Es,onehot_actions,Es_,rewards_val,terminals_val], true_transitions )
            ...
            print ("generator_discrim predicted transitions vs true transitions")
            print (self.transition_g_full.predict([noise_Es[-5:]]+[states_val[0][-5:]]+[onehot_actions[-5:],noise[-5:]]))
            #print (self.encoder.layers[2].get_weights()[0].all()==a.all())
            #print (self.encoder_frozen.layers[2].get_weights()[0].all()==b.all())
            #print (self.transition.layers[7].get_weights()[0].all()==c.all())
            #print (self.transition_d.layers[7].get_weights()[0].all()==d.all())
            #a,b,c,d=self.encoder.layers[2].get_weights()[0],self.encoder_frozen.layers[2].get_weights()[0],self.transition.layers[7].get_weights()[0], self.transition_d.layers[7].get_weights()[0]
            print (self.transition_d.predict([Es_double[:5],onehot_actions_double[:5],Es__double[:5],rewards_val_double[:5],terminals_val_double[:5]]))
            print ("generator_discrim predicted transitions vs true transitions")
            print (self.transition_g_full.predict([noise_Es[:5]]+[states_val[0][:5]]+[onehot_actions[:5],noise[:5]]))
            ...
            self.transition_g_full2.train_on_batch([noise_Es]+states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) )
            print ("after train tr_G")
            print (true_transitions[-5:])
            print (self.transition_d.predict([Es_double[-5:],onehot_actions_double[-5:],Es__double[-5:],rewards_val_double[-5:],terminals_val_double[-5:]]))
            print ("generator_discrim predicted transitions vs true transitions")
            print (self.transition_g_full.predict([noise_Es[-5:]]+[states_val[0][-5:]]+[onehot_actions[-5:],noise[-5:]]))
            ...
            self.encoder_g_full.train_on_batch([noise_Es]+states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,noise_Es2,noise_r,noise_gamma], np.zeros((self._batch_size,1)) )
            print ("after train enc_G")
            print (true_transitions[-5:])
            print (self.transition_d.predict([Es_double[-5:],onehot_actions_double[-5:],Es__double[-5:],rewards_val_double[-5:],terminals_val_double[-5:]]))
            print ("generator_discrim predicted transitions vs true transitions")
            print (self.transition_g_full.predict([noise_Es[-5:]]+[states_val[0][-5:]]+[onehot_actions[-5:],noise[-5:]]))
        ...
        #print (self.transition_d.layers)
#        print ("before D")
#        print (self.encoder.layers[2].get_weights())
#        print (self.transition.layers[7].get_weights())
#        print (self.transition_d.layers[7].get_weights())
...
#            K.set_value(next_param,K.get_value(param))
        ...
        if (self.wait_D==0):
#            loss_D=self.transition_d.train_on_batch([Es_double,onehot_actions_double,Es__double,rewards_val_double,terminals_val_double], true_transitions )
#            self.loss_D=0.9*self.loss_D+0.1*loss_D
            loss_D=self.transition_d_withencD.train_on_batch(states_val_double+[noise_Es_double,onehot_actions_double, Es__double, rewards_val_double, terminals_val_double], true_transitions )
            self.transition_d_withencD2.train_on_batch(states_val_double+[noise_Es_double,onehot_actions_double, Es__double, rewards_val_double, terminals_val_double], true_transitions )
            self.loss_D=0.9*self.loss_D+0.1*loss_D
            #while(self.loss_D>0.65):
            #    loss_D=self.transition_d.train_on_batch([Es,onehot_actions,Es__noise,rewards_val_noise,terminals_val_noise], true_transitions )
            #    #loss_D2=self.transition_d.train_on_batch([Es,onehot_actions,Es__noise,rewards_val_noise,terminals_val_noise], true_transitions )
            #    #print (loss_D)
            #    self.loss_D=0.95*self.loss_D+0.05*loss_D
#            loss_D1=self.transition_d_full.train_on_batch([noise_Es]+states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) )
##            print ("after D1")
##            print (self.encoder.layers[2].get_weights())
##            print (self.transition.layers[7].get_weights())
##            print (self.transition_d.layers[7].get_weights())
#            loss_D2=self.encoder_d_full.train_on_batch([noise_Es]+states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,noise_Es2,noise_r,noise_gamma], np.ones((self._batch_size,1)) )
##            print ("after D2")
##            print (self.encoder.layers[2].get_weights())
##            print (self.transition.layers[7].get_weights())
##            print (self.transition_d.layers[7].get_weights())
#            self.loss_D1=0.95*self.loss_D1+0.05*loss_D1
#            self.loss_D2=0.95*self.loss_D2+0.05*loss_D2
#            self.loss_D=(self.loss_D1+self.loss_D2)/2.
            #if(self.loss_D<0.4):
            #    self.wait_D=4
            #if(self.loss_D<0.2):
            #    self.wait_D=8
            #elif(self.loss_D<0.1):
            #    self.wait_D=32
        else:
            self.wait_D-=1
...
#        print (self.transition_d.layers[7].get_weights())
        ...
        #    #new_vals=K.get_value(param)/2.+K.get_value(next_param)/2.
        #    #K.set_value(next_param,new_vals)
        #    K.set_value(param,K.get_value(next_param))
        
        # Fit transition and encoder by trying to fool the discriminator
        if (self.wait_G==0):
            loss_G1=0
            #loss_G1=self.transition_g_full.train_on_batch([noise_Es]+states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) )
#            print ("after G1")
#            print (self.encoder.layers[2].get_weights())
#            print (self.transition.layers[7].get_weights())
#            print (self.transition_d.layers[7].get_weights())
            self.loss_G1=0.9*self.loss_G1+0.1*loss_G1
            loss_G2=0
            #loss_G2=self.encoder_g_full.train_on_batch([noise_Es]+states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,noise_Es2,noise_r,noise_gamma], np.zeros((self._batch_size,1)) )
            #print ("loss_G,loss_G2"+str(loss_G)+str(loss_G2))
#            print ("after G2")
#            print (self.encoder.layers[2].get_weights())
#            print (self.transition.layers[7].get_weights())
#            print (self.transition_d.layers[7].get_weights())
            self.loss_G2=0.9*self.loss_G2+0.1*loss_G2
            self.transition_g_full2.train_on_batch([noise_Es]+states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) )
            self.loss_G=(self.loss_G1+self.loss_G2)/2.
...
#        print (self.transition_d.layers[7].get_weights())
        ...
        # As compared to tanh activation on the abstract repr, this allows a soft constraint
        # this less likley poses less problems, e.g. with an encoder that tries to get very large values pre activation to
        # allow the transition function to fit correctly the states
...
#        self.loss_disentangle_t+=self.diff_s_s_.train_on_batch(states_val+next_states_val, np.reshape(np.zeros_like(Es),(self._batch_size,-1)))
        ...
        if(self.update_counter%2000==0):
            print ("self.loss_G1, self.loss_G2, self.loss_D")
            print (self.loss_G1, self.loss_G2, self.loss_D)
            print ("self.loss_Q/2000., self.loss_disentangle_t/2000., self.loss_disambiguate1/2000., self.loss_disambiguate2/2000.")
            print (self.loss_Q/2000., self.loss_disentangle_t/2000., self.loss_disambiguate1/2000., self.loss_disambiguate2/2000.)
        ...
        next_q_vals = self.full_Q_target.predict(next_states_val+[np.zeros(Es.shape)])
        
        if(self._double_Q==True):
            next_q_vals_current_qnet=self.full_Q.predict(next_states_val+[np.zeros(Es.shape)])
        ...
        q_vals=self.full_Q.predict(states_val+[np.zeros(Es.shape)])
        ...
        loss_Q=0
        #loss_Q=self.full_Q.train_on_batch(states_val+[noise_Q_abstr] , q_vals )
        #self.full_Q2.train_on_batch(states_val+[noise_Q_abstr] , q_vals ) 
        self.loss_Q+=loss_Q
        ...
        return np.sqrt(loss_Q),loss_ind
...
#        ## END DEBUG PURPOSES
        ...
        QD_plan=0
        for i in range(d+1):
            Qd=self.qValues_planning_abstr(encoded_x, transition, Q, d=i, branching_factor=[self._n_actions,2,2,2,2,2,2,2], repeat_factor=[5,4,3,3,3,3,3,3]).reshape(len(encoded_x),-1) #1,1,1,1,1,1]).reshape(len(encoded_x),-1)#5,4,3,3,3,3,3,3]).reshape(len(encoded_x),-1)
            print ("Qd,i")
            print (Qd,i)
            QD_plan+=Qd
        QD_plan=QD_plan/(d+1)
    ...
    def _compile(self):
        ...
        if (self._update_rule=="sgd"):
            ...
            optimizer3=SGD(lr=self._lr, momentum=self._momentum, nesterov=False, clipvalue=self._clip_norm) # to possibly modify them separately
            optimizer3b=SGD(lr=self._lr, momentum=self._momentum, nesterov=False, clipvalue=self._clip_norm) # to possibly modify them separately
            ...
            optimizer8=SGD(lr=self._lr, momentum=self._momentum, nesterov=False, clipvalue=self._clip_norm)
        elif (self._update_rule=="rmsprop"):
            print (self._rho,self._rms_epsilon,self._clip_norm)
            optimizerQ=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
            optimizerQ2=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
            ...
            optimizer1b=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
            ...
            optimizer3b=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm) # to possibly modify them separately
            ...
            optimizer8=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
#        elif (self._update_rule=="rmsprop"):
#            print (self._rho,self._rms_epsilon,self._clip_norm)
#            optimizer=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
#            optimizer1=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm) # Different optimizers for each network; 
#            optimizer2=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm) # to possibly modify them separately
#            optimizer2b=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm) # to possibly modify them separately
#            optimizer3=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm) # to possibly modify them separately
#            optimizer3b=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm) # to possibly modify them separately
#            optimizer4=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
#            optimizer5=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
#            optimizer6=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
#            optimizer7=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)

        else:
            raise Exception('The update_rule '+self._update_rule+' is not implemented.')
        
        self.full_Q.compile(optimizer=optimizerQ, loss='mse')

        #freezeLayer(self.encoder)
#        self.encoder_frozen.trainable=False
        self.encoder.trainable=False
        #freezeLayer(self.transition)
        self.transition.trainable=False
        self.full_Q2.compile(optimizer=optimizerQ2, loss='mse')

        self.transition_d.compile(optimizer=optimizer1,
                  loss='binary_crossentropy')#_from_logits_clipped)
        self.transition_d_withencD.compile(optimizer=optimizer1,
                  loss='binary_crossentropy')#_from_logits_clipped)
        
        self.encoder_d_full.compile(optimizer=optimizer7,
                  loss='binary_crossentropy')
        self.transition_d_full.compile(optimizer=optimizer8,
                  loss='binary_crossentropy')

        self.encoder_frozen.trainable=False
        self.transition_d_withencD2.compile(optimizer=optimizer1b,
                  loss='binary_crossentropy')#_from_logits_clipped)
        
        #unfreezeLayer(self.transition)
        self.transition.trainable=True
        #freezeLayer(self.transition_d)
        self.transition_d.trainable=False
        self.transition_g_full2.compile(optimizer=optimizer2,
                  loss='binary_crossentropy') # transition are updated, encoder is not

        #freezeLayer(self.transition)
        self.encoder.trainable=True
        self.transition.trainable=False
        self.encoder_g_full.compile(optimizer=optimizer3,
                  loss='binary_crossentropy') # encoder (for s') is updated, transition is not
        #unfreezeLayer(self.transition)
        self.transition.trainable=True

        self.transition_g_full.compile(optimizer=optimizer3b,
                  loss='binary_crossentropy') # encoder  (for s) is updated (as well as transition)

        #unfreezeLayer(self.transition_d)
        self.transition_d.trainable=True


        #unfreezeLayer(self.encoder)
        self.encoder.compile(optimizer=optimizer4,
                  loss=mean_squared_error_p)
        self.encoder_diff.compile(optimizer=optimizer5,
                  loss=self.exp_dec_error)
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.full_Q.optimizer.lr, self._lr/self.full_Q_lrdiv)
        K.set_value(self.full_Q2.optimizer.lr, self._lr)

        K.set_value(self.transition_d_withencD.optimizer.lr, self._lr/self.transition_d_lrdiv)
        K.set_value(self.transition_d_withencD2.optimizer.lr, self._lr)
        K.set_value(self.transition_d_full.optimizer.lr, self._lr/self.transition_d_lrdiv)
        K.set_value(self.encoder_d_full.optimizer.lr, self._lr/self.transition_d_lrdiv)
        K.set_value(self.transition_g_full.optimizer.lr, self._lr/self.transition_g_full_lrdiv) # /5. for simple laby or simple catcher; /50. for distrib of laby
        K.set_value(self.encoder_g_full.optimizer.lr, self._lr/self.encoder_g_full_lrdiv) # /5. for simple laby or simple catcher; /50. or distrib of laby
        K.set_value(self.transition_g_full2.optimizer.lr, self._lr/100.)
        
        K.set_value(self.encoder.optimizer.lr, self._lr/self.encoder_lrdiv)
        K.set_value(self.encoder_diff.optimizer.lr, self._lr/self.encoder_diff_lrdiv) # /2 for simple laby or simple catcher; *1 for distrib of laby

        K.set_value(self.diff_s_s_.optimizer.lr, self._lr/self.diff_s_s_lrdiv) # /5. for simple laby or simple catcher; /1. for distrib of laby
