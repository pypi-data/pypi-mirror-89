...
class BaseCollector(object):
    ...
    def initialize(self, PROJECT_DIR):
        ...
        self.execution_state = "POLLING" # even in non c2c case this variable is used
        ...
        while True:
            try:
                ...
                # initialising state for c2c
                self.task_params = self.build_task_params()
                break
    ...
    def run(self):
        ...
        if self.is_running():
                ...
                for future in futures.as_completed(all_futures):
                    ...
                    try:
                        ...
                        self.log.error(f'''Thread: {api_type} {e}''', extra={"status": SourceHealthStatus.StatusError,
                                                                             "errorType": ErrorType.ERROR_CONFIG},
                                       exc_info=True)
                        self.execution_state = "STOP_POLLING"
            ...
            if self.execution_state != "STOP_POLLING":
                if has_any_error:
                    self.execution_state = "BACKOFF"
                else:
                    self.execution_state = "POLLING"
    ...
    def execute(self):
        if self.collection_config["ENVIRONMENT"] == "ctoc":
            ...
            self.execution_state = "POLLING"
            while self.execution_state != "STOP_POLLING":
                calling_time = time.time()
                self.run()
                execution_time = time.time() - calling_time

                if self.execution_state == "BACKOFF":

                    sleep_time = base * polling_interval_secs
                    if sleep_time > max_back_off_secs:
                        self.execution_state = "STOP_POLLING"
                        self.log.info(
                            f'''Max backoff limit reached backoff: {base}''')
                        break
                    else:
                        base = base << 1

                elif self.execution_state == "POLLING":
                    # resetting to normal state
                    base = 2
                    sleep_time = polling_interval_secs

                self.log.debug(f'''execution_state: {self.execution_state} backoff: {base} last_execution_time: {execution_time} sleep_time: {sleep_time - execution_time}''')
