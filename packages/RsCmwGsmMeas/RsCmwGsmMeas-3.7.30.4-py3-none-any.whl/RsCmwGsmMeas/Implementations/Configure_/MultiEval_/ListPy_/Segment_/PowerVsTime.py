...
class PowerVsTime:
	...
	def set(self, structure: PowerVsTimeStruct, segment=repcap.Segment.Default) -> None:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIST:SEGMent<nr>:PVTime \n
		Snippet: driver.configure.multiEval.listPy.segment.powerVsTime.set(value = [PROPERTY_STRUCT_NAME](), segment = repcap.Segment.Default) \n
		Defines the statistical length for averaging and enables the power vs. time measurement in segment no. <no>; see 'List
		Mode'. \n
			:param structure: for set value, see the help for PowerVsTimeStruct structure arguments.
			:param segment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Segment')"""
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:PVTime', structure)

	def get(self, segment=repcap.Segment.Default) -> PowerVsTimeStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIST:SEGMent<nr>:PVTime \n
		Snippet: value: PowerVsTimeStruct = driver.configure.multiEval.listPy.segment.powerVsTime.get(segment = repcap.Segment.Default) \n
		Defines the statistical length for averaging and enables the power vs. time measurement in segment no. <no>; see 'List
		Mode'. \n
			:param segment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Segment')
			:return: structure: for return value, see the help for PowerVsTimeStruct structure arguments."""
		segment_cmd_val = self._base.get_repcap_cmd_value(segment, repcap.Segment)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIST:SEGMent{segment_cmd_val}:PVTime?', self.__class__.PowerVsTimeStruct())
