...
from high_vision.Segmentation.deeplab.semantic import semantic_segmentation
from high_vision.Segmentation.maskrcnn.instance import instance_segmentation
from high_vision.Segmentation.cdcl.inference_15parts import run_image, run_video, run_camera
import cv2
import os
import sys
...
class Segmentation:
    ...
    """
    This is Segmentation Class in High-Vision Library, it provides the support of State-Of-The-Art Models 
    ...
    Note: Custom Segmenation only Supports Mask-RCNN
        ...
        Use_MaskRCNN()                              # To Specify which Model to Use
        Use_DeepLabv3()                             # To Specify which Model to Use
        Use_PersonPart()                            # To Specify which Model to Use
    ...
    def Use_MaskRCNN(self, weights_path: str = None):
        ...
        if weights_path is None:
            path = 'high_vision/weights/maskrcnn_weights.h5'
            if os.path.isfile(path):
                ...
                self.weights_path = tf.keras.utils.get_file('maskrcnn_weights.h5',
                                                            'https://github.com/fahima10/high-vision/releases/download/v0.0/mask_rcnn_coco.h5',
                                                            cache_subdir='weights/', cache_dir='high_vision')
        else:
    ...
    def Use_DeepLabv3(self, weights_path: str = None):
        ...
        if weights_path is None:
            path = 'high_vision/weights/deeplab_weights.h5'
            if os.path.isfile(path):
                print('Found Existing Weights File...\nLoading Existing File...')
                self.weights_path = path
            else:
                ...
                self.weights_path = tf.keras.utils.get_file('deeplab_weights.h5',
                                                            'https://github.com/fahima10/high-vision/releases/download/v0.0/deeplabv3_xception65_ade20k.h5',
                                                            cache_subdir='weights/', cache_dir='high_vision')
        else:
            if os.path.isfile(weights_path):
                self.weights_path = weights_path
    ...
    def Detect_From_Image(self, input_path:str, output_path:str, show_boxes:bool = False):
        ...
        if self.modelLoaded != True:
            ...
            if self.modelType == 'maskrcnn':
                ...
                _ = run_image(input_path, output_path, self.weights_p)

            else:
                raise RuntimeError(
                    'Invalid ModelType: Valid Types are "MaskRCNN"\t"DeepLabv3"\t"CDCL".')
    ...
    def Detect_From_Video(self, input_path:str, output_path:str, show_boxes:bool = False, fps:int = 25):
        ...
        if self.modelType == 'cdcl':
            vid = run_video(input_path, output_path, fps, self.weights_p)
            sys.exit()
    ...
    def Detect_From_Camera(self, cam, output_path, show_boxes = False, fps:int = 25):
        ...
        """[This function is used to segment objects from Live Camera Stream]

        Args:
            cam (str): [instance of camera or webcam from which objects are segmented]
            output_path (str): [path to save the output video with mp4/avi extension]
            show_boxes (bool, optional): [wether to show the boxes of detected objects, Only Mask-RCNN supports it]. Defaults to False.
            fps (int, optional): [frames per second for video processing]
        ...
        if self.modelType == 'maskrcnn':
            _, output_video = self.model.process_camera(cam, show_boxes, output_path, show_frames=True, frames_per_second=fps,
                                                        frame_name="Output", verbose=0, check_fps=True, custom=None)
            
        elif self.modelType == 'deeplab':
            _, output_video = self.model.process_camera_ade20k(cam, output_video_name=output_path, frames_per_second=fps)
            ...
            raise RuntimeError("Invalid ModelType: Valid Types are MaskRCNN\tDeepLabv3\tCDCL")
    ...
    def Detect_Custom_Objects_From_Camera(self, cam, custom_objects:dict, output_path:str, show_boxes:bool=True, fps:int = 25):
        
        
        """[This function is used to detect custom objects from Live Camera Streams, it will only detect those objects which
        are set to True in dictionary returned from Custom_Objects() function.]
        ...
        if self.modelType == 'maskrcnn':
            _, output_video = self.model.process_camera(cam, show_boxes, output_path, show_frames=True, frames_per_second=fps,
                                                        frame_name="Output", verbose=0, check_fps=True, custom=custom_objects)
