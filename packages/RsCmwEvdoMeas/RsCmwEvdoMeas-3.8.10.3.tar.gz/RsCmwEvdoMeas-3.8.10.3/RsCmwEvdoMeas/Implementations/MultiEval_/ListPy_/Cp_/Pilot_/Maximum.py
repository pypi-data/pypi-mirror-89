...
class Maximum:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.listPy.cp.pilot.maximum.fetch() \n
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:EVDO:MEASurement<Instance>:MEValuation:LIST:CP:PILot:MAXimum?', suppressed)
		return response
	...
	def calculate(self) -> List[enums.ResultStatus2]:
		...
		Snippet: value: List[enums.ResultStatus2] = driver.multiEval.listPy.cp.pilot.maximum.calculate() \n
		Returns the RMS power (statistical values) of the pilot channel for all active list mode segments. The values described
		below are returned by FETCh commands. CALCulate commands return limit check results instead, one value for each result
		listed below. \n
		Use RsCmwEvdoMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: pilot: float Comma-separated list of values, one per active segment. Range: -60 dB to +10 dB , Unit: dB"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'CALCulate:EVDO:MEASurement<Instance>:MEValuation:LIST:CP:PILot:MAXimum?', suppressed)
