...
class DIETClassifier20201220(DIETClassifier):
    ...
    def batch_process(self, training_data):
        count = 5
        print('self.component_config["prediction_condition"] is not False', self.component_config["prediction_condition"] is not False)
        print('self.component_config["entity_recognition"]', self.component_config["entity_recognition"])
        print('self.component_config["prediction_entity_write_attribute"] is not False', self.component_config["prediction_entity_write_attribute"] is not False)
        print('self.component_config["prediction_entity_write_attribute"]', self.component_config["prediction_entity_write_attribute"])
            ...
            for j, message in enumerate(batch_data):                
                ...
                if self.component_config["intent_classification"]:
                    ...
                    if self.component_config["prediction_intent_write_attribute"] is not False:
                        setattr(message, self.component_config["prediction_intent_write_attribute"], intent)
                ...
                if self.component_config["entity_recognition"]:
                    ...
                    if self.component_config["prediction_entity_write_attribute"] is not False:
                        setattr(message, self.component_config["prediction_entity_write_attribute"], entities)
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        if self.component_config["intent_classification"]:
            intent = message.get(INTENT)
            if self.component_config["prediction_intent_write_attribute"] is not False:
                setattr(message, self.component_config["prediction_intent_write_attribute"], intent, add_to_output=True)
    
        if self.component_config["entity_recognition"]:
            entities = message.get(ENTITIES, [])        
            if self.component_config["prediction_entity_write_attribute"] is not False:
                setattr(message, self.component_config["prediction_entity_write_attribute"], entities, add_to_output=True)
