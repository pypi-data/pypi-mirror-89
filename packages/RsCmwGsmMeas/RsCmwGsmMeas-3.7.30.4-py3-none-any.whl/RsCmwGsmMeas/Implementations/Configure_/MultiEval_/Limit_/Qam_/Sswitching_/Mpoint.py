...
class Mpoint:
	...
	def set(self, structure: MpointStruct, qamOrder=repcap.QamOrder.Default, measPoint=repcap.MeasPoint.Default) -> None:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM<ModOrder>:SSWitching:MPOint<nr> \n
		Snippet: driver.configure.multiEval.limit.qam.sswitching.mpoint.set(value = [PROPERTY_STRUCT_NAME](), qamOrder = repcap.QamOrder.Default, measPoint = repcap.MeasPoint.Default) \n
		...
		self._core.io.write_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:SSWitching:MPOint{measPoint_cmd_val}', structure)

	def get(self, qamOrder=repcap.QamOrder.Default, measPoint=repcap.MeasPoint.Default) -> MpointStruct:
		"""SCPI: CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM<ModOrder>:SSWitching:MPOint<nr> \n
		Snippet: value: MpointStruct = driver.configure.multiEval.limit.qam.sswitching.mpoint.get(qamOrder = repcap.QamOrder.Default, measPoint = repcap.MeasPoint.Default) \n
		Define and activate a limit line for the modulation schemes 8PSK and 16-QAM for a certain frequency offset. The specified
		limits apply at the reference power values defined by method RsCmwGsmMeas.Configure.MultiEval.Limit.Epsk.Sswitching.
		plevel and method RsCmwGsmMeas.Configure.MultiEval.Limit.Qam.Sswitching.Plevel.set. Between the reference power values
		the limits are determined by linear interpolation. \n
		...
		qamOrder_cmd_val = self._base.get_repcap_cmd_value(qamOrder, repcap.QamOrder)
		measPoint_cmd_val = self._base.get_repcap_cmd_value(measPoint, repcap.MeasPoint)
		return self._core.io.query_struct(f'CONFigure:GSM:MEASurement<Instance>:MEValuation:LIMit:QAM{qamOrder_cmd_val}:SSWitching:MPOint{measPoint_cmd_val}?', self.__class__.MpointStruct())
