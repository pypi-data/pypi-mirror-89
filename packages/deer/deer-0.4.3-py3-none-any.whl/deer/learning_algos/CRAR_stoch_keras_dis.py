...
from .NN_CRAR_stoch_dis_keras import NN # Default Neural network used
...
class CRAR(LearningAlgo):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_norm=0, freeze_interval=1000, batch_size=32, update_rule="rmsprop", random_state=np.random.RandomState(), double_Q=False, neural_network=NN, **kwargs):
        ...
        self.loss_discrim_mask_d,self.loss_discrim_mask_g =0, 0
        self.wait_D,self.wait_G=0,0
        self.dis = kwargs.get('dis',False)
        self.transition_g_full_lrdiv = kwargs.get('transition_g_full_lrdiv',100)
        self.encoder_g_full_lrdiv = kwargs.get('encoder_g_full_lrdiv',100)
        self.exp_dec_error_C = kwargs.get('exp_dec_error_C',2)
        self.encoder_diff_lrdiv = kwargs.get('encoder_diff_lrdiv',1)
        self.diff_s_s_lrdiv = kwargs.get('diff_s_s_lrdiv',10)
        self.encoder_lrdiv = kwargs.get('encoder_lrdiv',10)
        self.discrim_masked_full_lrdiv = kwargs.get('discrim_masked_full_lrdiv',10.)
        self.noise_level = kwargs.get('noise_level',0.2)
        ...
        self.encoder_frozen = self.learn_and_plan.encoder_model()
        ...
        # used to disentangle info in the layers
        self.discrim_masked = self.learn_and_plan.transition_discr_model(discrim_mask=True)
        self.discrim_masked_full = self.learn_and_plan.discrim_masked_full_model(self.encoder, self.encoder_frozen, self.transition, self.discrim_masked)
        #self.discrim_masked_full2 = self.learn_and_plan.discrim_masked_full_model(self.encoder, self.transition, self.discrim_masked)
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        if(self.dis==True):
            mask=np.random.randint(2, size=(self._batch_size,self._internal_dim)) #to improve?
        else:
            mask=np.ones((self._batch_size, self._internal_dim))
            
        Es_=self.encoder.predict(next_states_val)#self.encoder_target.predict(next_states_val)
        Es=self.encoder.predict(states_val)
        
        noise_level=self.noise_level #0.2 #max( (1.-self.update_counter/50000) , 0.1 ) #0.2
        noise_level2=self.noise_level #max( (1.-self.update_counter/50000) , 0.1 ) #0.4
        
        noise_Es = np.random.normal(0,noise_level/2.,Es.shape)
        noise_Es2 = np.random.normal(0,noise_level/2.,Es_.shape)
        noise_Q_abstr = np.random.normal(0, noise_level/2., Es.shape)

        noise_tr = np.random.normal(0, 1, Es.shape)

        ETs, R, terminals=self.transition.predict([Es,onehot_actions,noise_tr])
...
#                plt.savefig('plot96.pdf')
        ...
        Es_+=np.random.normal(0,noise_level/2.,Es_.shape)
        ...
        Es_double+=np.random.normal(0,noise_level/2.,Es_double.shape)
        onehot_actions_double=np.tile(onehot_actions,(2,1))
        mask_double=np.concatenate(  ( mask , np.ones(mask.shape) )  ,  axis=0  ) # first half has some inputs masked (if dis=True), second half not
        mask2_double=np.concatenate(  ( mask , mask )  ,  axis=0  ) # both halves have some inputs masked (if dis=True)
        whether_masked=np.concatenate(  ( np.ones((self._batch_size,1)) , np.zeros((self._batch_size,1)) )  ,  axis=0  )

        if(self.update_counter%500==0):
            print (true_transitions[-5:])
            print (self.transition_d.predict([Es_double[-5:],onehot_actions_double[-5:],Es__double[-5:],mask_double[-5:],rewards_val_double[-5:],terminals_val_double[-5:]]))
            print ("generator_discrim predicted transitions vs true transitions")
            print (self.transition_g_full.predict([noise_Es[-5:]]+[states_val[0][-5:]]+[onehot_actions[-5:],noise_tr[-5:],mask[-5:]]))
...
#            for i in range(1):
#                loss_D=self.transition_d.train_on_batch([Es_double,onehot_actions_double,Es__double,mask_double,rewards_val_double,terminals_val_double], true_transitions )
#                #loss_D1=self.transition_d_full.train_on_batch(states_val+[onehot_actions,noise], np.zeros((self._batch_size,1)) )
#                #loss_D2=self.encoder_d_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,np.random.normal(0,noise_level/2.,Es_[:self._batch_size].shape)], np.ones((self._batch_size,1)) )
#                #true_transitions=np.concatenate(  ( np.ones((self._batch_size,1)) , np.zeros((self._batch_size,1)) )  ,  axis=0  )
...
#            #print (self.transition_d_full.predict([states_val[0][:5]]+[onehot_actions[:5],noise[:5]]) )
#            
#            self.transition_g_full2.train_on_batch([noise_Es]+states_val+[onehot_actions,noise_tr,mask], np.ones((self._batch_size,1)) )
#            print ("after train tr_G")
#            print (true_transitions[-5:])
#            print (self.transition_d.predict([Es_double[-5:],onehot_actions_double[-5:],Es__double[-5:],mask_double[-5:],rewards_val_double[-5:],terminals_val_double[-5:]]))
#            print ("generator_discrim predicted transitions vs true transitions")
#            print (self.transition_g_full.predict([noise_Es[-5:]]+[states_val[0][-5:]]+[onehot_actions[-5:],noise_tr[-5:],mask_double[-5:]]))
#            #print (self.encoder.layers[2].get_weights()[0].all()==a.all())
#            #print (self.encoder_frozen.layers[2].get_weights()[0].all()==b.all())
#            #print (self.transition.layers[7].get_weights()[0].all()==c.all())
#            #print (self.transition_d.layers[7].get_weights()[0].all()==d.all())
...
#
#
#            self.encoder_g_full.train_on_batch([noise_Es]+states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,noise_Es2,mask], np.zeros((self._batch_size,1)) )
...
#        print (self.transition_d.layers[7].get_weights())
        ...
        if (self.wait_D==0):
            loss_D=self.transition_d.train_on_batch([Es_double,onehot_actions_double,Es__double,mask2_double,rewards_val_double,terminals_val_double], true_transitions )
...
##            print (self.transition.layers[7].get_weights())
##            print (self.transition_d.layers[7].get_weights())
#            loss_D2=self.encoder_d_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,np.random.normal(0,noise_level/2.,Es_[:self._batch_size].shape)], np.ones((self._batch_size,1)) )
##            print ("after D2")
##            print (self.encoder.layers[2].get_weights())
##            print (self.transition.layers[7].get_weights())
##            print (self.transition_d.layers[7].get_weights())
...
#        print (self.transition_d.layers[7].get_weights())
        ...
        if (self.wait_G==0):
            loss_G1=self.transition_g_full.train_on_batch([noise_Es]+states_val+[onehot_actions,noise_tr, mask], np.ones((self._batch_size,1)) )
#            print ("after G1")
#            print (self.encoder.layers[2].get_weights())
#            print (self.transition.layers[7].get_weights())
#            print (self.transition_d.layers[7].get_weights())
            self.loss_G1=0.9*self.loss_G1+0.1*loss_G1
            loss_G2=self.encoder_g_full.train_on_batch([noise_Es]+states_val+[onehot_actions]+next_states_val+[rewards_val,terminals_val,noise_Es2, mask], np.zeros((self._batch_size,1)) )
...
#            print (self.transition_d.layers[7].get_weights())
        ...
        # Training of the discrim_mask
        if(self.dis):
            states_val_double=[]
            for s in states_val:
                states_val_double.append( np.append(s,s,axis=0) )            
            noise_Es_double=np.append(noise_Es,noise_Es,axis=0)
            noise_tr_double=np.append(noise_tr,noise_tr,axis=0)
            noise_Es2_double=np.append(noise_Es2,noise_Es2,axis=0)
            
            self.loss_discrim_mask_g += self.discrim_masked_full.train_on_batch(states_val_double+ [onehot_actions_double, mask_double, mask2_double, noise_Es_double, noise_tr_double, noise_Es2_double], whether_masked )
...
#        print (self.transition_d.layers[7].get_weights())
        ...
        if(self.update_counter%2000==0):
            ...
            print (self.loss_Q/2000., self.loss_disentangle_t/2000., self.loss_disambiguate1/2000., self.loss_disambiguate2/2000.)
            if(self.dis):
                print ("self.loss_discrim_mask_d/2000.,self.loss_discrim_mask_g/2000.")
                print (self.loss_discrim_mask_d/2000.,self.loss_discrim_mask_g/2000.)
...
#        ## END DEBUG PURPOSES
    ...
    def _compile(self):
        ...
        if (self._update_rule=="sgd"):
            ...
            optimizer9=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
            optimizer10=RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
#        elif (self._update_rule=="rmsprop"):
#            print (self._rho,self._rms_epsilon,self._clip_norm)
...
#            optimizer7=Adam(lr=self._lr)#RMSprop(lr=self._lr, rho=self._rho, epsilon=self._rms_epsilon, clipvalue=self._clip_norm)
        ...
        if(self.dis):
            #self.discrim_masked.compile(optimizer=optimizer9,
            #      loss='binary_crossentropy')
            self.discrim_masked_full.compile(optimizer=optimizer10,
                  loss='binary_crossentropy')
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.transition_d.optimizer.lr, self._lr/2.)
        #K.set_value(self.transition_d_full.optimizer.lr, self._lr/2.)
        #K.set_value(self.encoder_d_full.optimizer.lr, self._lr/2.)
        ...
        if(self.dis):
            K.set_value(self.discrim_masked_full.optimizer.lr, self._lr/self.discrim_masked_full_lrdiv) #
