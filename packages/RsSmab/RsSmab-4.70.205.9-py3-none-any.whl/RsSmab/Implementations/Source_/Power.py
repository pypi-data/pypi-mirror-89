...
class Power:
	...
	def get_manual(self) -> float:
		...
		Sets the level for the subsequent sweep step if method RsSmab.Source.Sweep.Power.Mode.value. Use a separate command for
		each sweep step. \n
			:return: manual: float You can select any level within the setting range, where: STARt is set with method RsSmab.Source.Power.start STOP is set with method RsSmab.Source.Power.stop OFFSet is set with OFFSet Range: (STARt + OFFSet) to (STOP + OFFSet) , Unit: dBm
		"""
	...
	def set_manual(self, manual: float) -> None:
		...
		Sets the level for the subsequent sweep step if method RsSmab.Source.Sweep.Power.Mode.value. Use a separate command for
		each sweep step. \n
			:param manual: float You can select any level within the setting range, where: STARt is set with method RsSmab.Source.Power.start STOP is set with method RsSmab.Source.Power.stop OFFSet is set with OFFSet Range: (STARt + OFFSet) to (STOP + OFFSet) , Unit: dBm
		"""
	...
	def get_mode(self) -> enums.LfFreqMode:
		...
		Selects the operating mode of the instrument to set the output level. \n
			:return: mode: CW| FIXed| SWEep CW|FIXed Operates at a constant level. CW and FIXed are synonyms. To set the output level value, use the command [:​SOURcehw]:​POWer[:​LEVel][:​IMMediate][:​AMPLitude]. SWEep Sets sweep mode. Set the range and current level with the commands: method RsSmab.Source.Power.start and method RsSmab.Source.Power.stop, method RsSmab.Source.Power.manual.
		"""
	...
	def set_start(self, start: float) -> None:
		...
		Sets the RF start/stop level in sweep mode. \n
			:param start: float Sets the setting range calculated as follows: (Level_min + OFFSet) to (Level_max + OFFSet) Where the values are set with the commands: OFFSet method RsSmab.Source.Power.start method RsSmab.Source.Power.stop Range: Minimum level to maximum level , Unit: dBm
		"""
	...
	def get_stop(self) -> float:
		...
		Sets the RF start/stop level in sweep mode. \n
			:return: stop: float Sets the setting range calculated as follows: (Level_min + OFFSet) to (Level_max + OFFSet) Where the values are set with the commands: OFFSet method RsSmab.Source.Power.start method RsSmab.Source.Power.stop Range: Minimum level to maximum level , Unit: dBm
		"""
	...
	def set_stop(self, stop: float) -> None:
		...
		Sets the RF start/stop level in sweep mode. \n
			:param stop: float Sets the setting range calculated as follows: (Level_min + OFFSet) to (Level_max + OFFSet) Where the values are set with the commands: OFFSet method RsSmab.Source.Power.start method RsSmab.Source.Power.stop Range: Minimum level to maximum level , Unit: dBm
		"""
