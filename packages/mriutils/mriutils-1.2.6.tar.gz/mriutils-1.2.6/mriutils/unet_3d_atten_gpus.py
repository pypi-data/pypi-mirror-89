...
from mriutils.norm import Normalization
from keras.callbacks import EarlyStopping
from mriutils.metrics2 import ErrorMetrics
...
class UNet3D_Atten():
    ...
    def train(self, train_set, data_mode, label_mode, test_set, epochs = 15, batch_size = 8, save_interval = 5):
        ...
        pred = Normalization(pred, 'label').norm()
        errors = []
        mse_error = 0.0
        ssim_error = 0.0
        psnr_error = 0.0
        num = len(pred)
        for i in range(num):
            error = ErrorMetrics(pred[i], y_test[i])
            mse_error += float(error['MSE'])
            ssim_error += float(error['SSIM'])
            psnr_error += float(error['PSNR'])
            errors.append(error)
        print(mse_error / num)
        print(ssim_error / num)
        print(psnr_error / num)
