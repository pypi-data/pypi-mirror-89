    ...
    for i in range(df.shape[0]):
        try:
            for wb in json.loads(df.iloc[i]['table_fields']):
                assert 'workbook' in wb.keys(), "table_fields缺少对象workbook"
                assert 'attach_name' in wb.keys(), "table_fields缺少对象attach_name"
                for st in wb['workbook']:
                    assert 'sheet' in wb.keys(), "workbook缺少对象sheet"
                    assert 'sheet_name' in st[k].keys(), "workbook缺少对象sheet_name"
                    for k, v in st.items():
                        assert 'from' in st[k].keys(), "sheet缺少对象from"
                        assert 'select' in st[k].keys(), "sheet缺少对象select"
                        assert 'as' in st[k].keys(), "sheet缺少对象as"
