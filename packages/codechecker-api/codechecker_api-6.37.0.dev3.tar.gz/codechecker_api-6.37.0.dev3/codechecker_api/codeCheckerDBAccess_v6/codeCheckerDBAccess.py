...
class Client(Iface):
    ...
    def exportData(self, runId):
        ...
        Parameters:
         - runId
        """
        self.send_exportData(runId)
        return self.recv_exportData()

    def send_exportData(self, runId):
    ...
    def importData(self, export):
        ...
        Parameters:
         - export
        """
        self.send_importData(export)
        return self.recv_importData()

    def send_importData(self, export):
...
class Processor(Iface, TProcessor):
    ...
    def process_exportData(self, seqid, iprot, oprot):
        ...
        result = exportData_result()
        try:
            result.success = self._handler.exportData(args.runId)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except codechecker_api_shared.ttypes.RequestFailed as requestError:
    ...
    def process_importData(self, seqid, iprot, oprot):
        ...
        result = importData_result()
        try:
            result.success = self._handler.importData(args.export)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except codechecker_api_shared.ttypes.RequestFailed as requestError:
...
class importData_args(object):
    ...
    def write(self, oprot):
        ...
        if self.export is not None:
            oprot.writeFieldBegin('export', TType.STRUCT, 1)
            self.export.write(oprot)
            oprot.writeFieldEnd()
        oprot.writeFieldStop()
        oprot.writeStructEnd()
...
importData_args.thrift_spec = (
    None,  # 0
    (1, TType.STRUCT, 'export', [ExportData, None], None, ),  # 1
)
