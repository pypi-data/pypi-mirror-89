...
class OffTime:
	...
	def set(self, off_time: int, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.lora.trigger.output.offTime.set(off_time = 1, channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:LORA:TRIGger:OUTPut<CH>:OFFTime \n
		Snippet: value: int = driver.source.bb.lora.trigger.output.offTime.get(channel = repcap.Channel.Default) \n
		Sets the number of samples during which the marker output is on or off. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
