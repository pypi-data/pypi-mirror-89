...
class ScSpacing:
	...
	def get(self, stream=repcap.Stream.Default) -> enums.EutraSubCarrierSpacing:
		...
		Snippet: value: enums.EutraSubCarrierSpacing = driver.source.bb.eutra.ul.ue.niot.frc.scSpacing.get(stream = repcap.Stream.Default) \n
		Queries the subcarrier spacing. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
			:return: sub_carr_spacing: S15| S375"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{stream_cmd_val}:NIOT:FRC:SCSPacing?')
