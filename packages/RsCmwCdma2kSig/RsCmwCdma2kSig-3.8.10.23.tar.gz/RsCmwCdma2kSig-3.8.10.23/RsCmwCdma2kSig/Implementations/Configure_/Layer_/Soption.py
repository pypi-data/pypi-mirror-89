...
class Soption:
	...
	def get_first(self) -> enums.ServiceOption:
		...
		Snippet: value: enums.ServiceOption = driver.configure.layer.soption.get_first() \n
		Queries the current primary service option used during the connection to the mobile station, see also '1st Service
		Option'. Setting this value has no effect as the service option parameter is a result of the session negotiation. \n
		...
		"""
		response = self._core.io.query_str('CONFigure:CDMA:SIGNaling<Instance>:LAYer:SOPTion:FIRSt?')
	...
	def set_first(self, service_option: enums.ServiceOption) -> None:
		"""SCPI: CONFigure:CDMA:SIGNaling<Instance>:LAYer:SOPTion:FIRSt \n
		Snippet: driver.configure.layer.soption.set_first(service_option = enums.ServiceOption.SO1) \n
		Queries the current primary service option used during the connection to the mobile station, see also '1st Service
		Option'. Setting this value has no effect as the service option parameter is a result of the session negotiation. \n
		...
		self._core.io.write(f'CONFigure:CDMA:SIGNaling<Instance>:LAYer:SOPTion:FIRSt {param}')
