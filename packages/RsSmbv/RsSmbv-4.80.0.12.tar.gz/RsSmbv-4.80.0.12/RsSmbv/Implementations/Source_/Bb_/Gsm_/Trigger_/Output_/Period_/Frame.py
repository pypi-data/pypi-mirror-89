...
class Frame:
	...
	def set(self, frame: int, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.gsm.trigger.output.period.frame.set(frame = 1, channel = repcap.Channel.Default) \n
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:OUTPut<CH>:PERiod:[FRAMe] \n
		Snippet: value: int = driver.source.bb.gsm.trigger.output.period.frame.get(channel = repcap.Channel.Default) \n
		Sets the repetition rate for the frame clock at the marker outputs. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
