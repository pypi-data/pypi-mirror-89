...
class Pybuild1(FullPybuild):
    ...
    * ``module``: Indicates that the installed source tree contains a
    ...
    def warn(self, string: str):
        ...
        """
        warnings_dir = os.path.join(env.TMP_DIR, "warnings")
        os.makedirs(warnings_dir, exist_ok=True)
        with open(os.path.join(warnings_dir, self.ATOM.CPF), "a+") as file:
            print(string, file)
        warning(string)
    ...
    def info(self, string: str):
        ...
        args:
            string: String to display
        """
        info_dir = os.path.join(env.TMP_DIR, "messages")
        os.makedirs(info_dir, exist_ok=True)
        with open(os.path.join(info_dir, self.ATOM.CPF), "a+") as file:
            print(string, file)
        info(string)
