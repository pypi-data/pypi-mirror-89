...
class TPipeline:
    ...
    def _set_up_config(self, training_config):
        ...
        self._test_txt_fpath = training_config['test_txt_fpath'] if 'test_txt_fpath' in training_config else None
        self._test_conllu_fpath = training_config[
            'test_conllu_fpath'] if 'test_conllu_fpath' in training_config else None
        ...
        self._test_bio2_fpath = training_config['test_bio2_fpath'] if 'test_bio2_fpath' in training_config else None

        master_config.train_conllu_fpath = self._train_conllu_fpath
        master_config.dev_conllu_fpath = self._dev_conllu_fpath
        master_config.test_conllu_fpath = self._test_conllu_fpath
    ...
    def _prepare_tokenize(self):
        ...
        if self._test_txt_fpath is not None and self._test_conllu_fpath is not None:
            self.test_set = TokenizeDataset(
                self._config,
                txt_fpath=self._test_txt_fpath,
                conllu_fpath=self._test_conllu_fpath,
                evaluate=True
            )
    ...
    def _prepare_posdep(self):
        ...
        if not os.path.exists(in_conllu['dev']) and not os.path.exists(in_conllu['test']):
            in_conllu = {
        ...
        if self._test_conllu_fpath is not None:
    ...
    def _prepare_ner(self):
        ...
        if self._test_bio2_fpath:
            self.test_set = NERDataset(
                config=self._config,
                bio2_fpath=self._test_bio2_fpath,
                evaluate=False
            )
            self.test_set.numberize()
            self.test_batch_num = len(self.test_set) // self._config.batch_size + \
    ...
    def _train_tokenize(self):
        ...
        for epoch in range(self._config.max_epoch):
            ...
            if self._test_txt_fpath and self._test_conllu_fpath:
                test_score = self._eval_tokenize(data_set=self.test_set, batch_num=self.test_batch_num,
            ...
            if epoch <= 30 or dev_score['average'] > best_dev['average']:
                ...
                if self._test_txt_fpath and self._test_conllu_fpath:
                    os.system('mv {} {}'.format(
                        os.path.join(self._config._save_dir, 'preds',
                                     'tokenizer.test.conllu' + '.epoch-{}'.format(epoch)),
                        os.path.join(self._config._save_dir, 'preds', 'tokenizer.test.conllu'),
                    ))

            self._printlog('-' * 30 + ' Best dev CoNLLu score: epoch {}'.format(best_epoch) + '-' * 30)
            self._printlog(get_ud_performance_table(dev_score))
            if self._test_txt_fpath and self._test_conllu_fpath:
                self._printlog('-' * 30 + ' Best test CoNLLu score: epoch {}'.format(best_epoch) + '-' * 30)
    ...
    def _train_posdep(self):
        ...
        for epoch in range(self._config.max_epoch):
            ...
            dev_score = self._eval_posdep(data_set=self.dev_set, batch_num=self.dev_batch_num,
                                          name='dev', epoch=epoch)
            if self._test_conllu_fpath:
                test_score = self._eval_posdep(data_set=self.test_set, batch_num=self.test_batch_num,
                                               name='test', epoch=epoch)
            ...
            if epoch <= 30 or dev_score['average'] > best_dev['average']:
                ...
                if self._test_conllu_fpath:
                    os.system('mv {} {}'.format(
                        os.path.join(self._config._save_dir, 'preds',
                                     'tagger.test.conllu' + '.epoch-{}'.format(epoch)),
                        os.path.join(self._config._save_dir, 'preds', 'tagger.test.conllu'),
                    ))

                # printout current best dev, best test
            self._printlog('-' * 30 + ' Best dev CoNLLu score: epoch {}'.format(best_epoch) + '-' * 30)
            self._printlog(get_ud_performance_table(dev_score))
            if self._test_conllu_fpath:
                self._printlog('-' * 30 + ' Best test CoNLLu score: epoch {}'.format(best_epoch) + '-' * 30)
    ...
    def _train_ner(self):
        ...
        for epoch in range(self._config.max_epoch):
            ...
            dev_score = self._eval_ner(data_set=self.dev_set, batch_num=self.dev_batch_num,
                                       name='dev', epoch=epoch)
            if self._test_bio2_fpath:
                test_score = self._eval_ner(data_set=self.test_set, batch_num=self.test_batch_num,
                                            name='test', epoch=epoch)
            ...
            if self._test_bio2_fpath:
                self._printlog('-' * 30)
                self._printlog('Best test F1 score: epoch {}, F1: {:.2f}'.format(best_epoch, best_test['f1']))
        print('Training done!')
