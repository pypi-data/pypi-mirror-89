...
class Slope:
	...
	def set(self, slope: enums.SlopeType, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.connector.user.trigger.slope.set(slope = enums.SlopeType.NEGative, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'CONNector:USER{channel_cmd_val}:TRIGger:SLOPe {param}')

	# noinspection PyTypeChecker
	def get(self, channel=repcap.Channel.Default) -> enums.SlopeType:
		"""SCPI: CONNector:USER<CH>:TRIGger:SLOPe \n
		Snippet: value: enums.SlopeType = driver.connector.user.trigger.slope.get(channel = repcap.Channel.Default) \n
		Sets the polarity of the active slope of an applied instrument trigger/clock. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:return: slope: NEGative| POSitive"""
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'CONNector:USER{channel_cmd_val}:TRIGger:SLOPe?')
