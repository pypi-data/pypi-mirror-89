...
class CaPeriod:
	...
	def set(self, const_acc_period: float, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GLONass<ST>:SDYNamics:CAPeriod \n
		Snippet: driver.source.bb.gnss.svid.glonass.sdynamics.caPeriod.set(const_acc_period = 1.0, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SVID{channel_cmd_val}:GLONass{stream_cmd_val}:SDYNamics:CAPeriod {param}')

	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GLONass<ST>:SDYNamics:CAPeriod \n
		Snippet: value: float = driver.source.bb.gnss.svid.glonass.sdynamics.caPeriod.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Sets the time duration during that acceleration is applied and thus the velocity varies. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Glonass')
		...
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SVID{channel_cmd_val}:GLONass{stream_cmd_val}:SDYNamics:CAPeriod?')
