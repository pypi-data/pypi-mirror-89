...
class Increment:
	...
	def set(self, increment: int, channel=repcap.Channel.Default, availableUser=repcap.AvailableUser.Default) -> None:
		...
		Snippet: driver.source.bb.wlnn.fblock.user.mac.scontrol.fragment.increment.set(increment = 1, channel = repcap.Channel.Default, availableUser = repcap.AvailableUser.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, availableUser=repcap.AvailableUser.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:WLNN:FBLock<CH>:[USER<DI>]:MAC:SCONtrol:FRAGment:INCRement \n
		Snippet: value: int = driver.source.bb.wlnn.fblock.user.mac.scontrol.fragment.increment.get(channel = repcap.Channel.Default, availableUser = repcap.AvailableUser.Default) \n
		Defines the number of packets required to increment the counter of the fragment bits of the sequence control. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fblock')
			:param availableUser: optional repeated capability selector. Default value: Nr0 (settable in the interface 'User')
