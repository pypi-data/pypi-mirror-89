...
import requests
import json
import udpipe_parser.text_preprocessor
import nltk
nltk.download('punkt')
...
def extract_dets(word):
  result = []
  for det in dets:
    if re.findall('^' + det + ' ', word) != [] or re.findall(' ' + det + ' ', word) != []:
      result.append(det)
  return result
...
class UDPipeParser:
      ...
      for p in parsed:
        if p.word.lower() not in self.propn_nouns:
          if ( (('INFN' in p.tag or 'VERB' in p.tag) and ('incl' not in p.tag) ) or ('долж' in p.word or 'нужн' in p.word)):
            pred_here = p
        if (('NOUN' in p.tag or 'NPRO' in p.tag or 'ADJF' in p.tag or 'ADJS' in p.tag) and ('nomn' in p.tag) or p.word in self.propn_nouns):
          subj_here = p
    if pred_here == False:
      j = -1
  ...
  def _parse_noun_phrase(self,current, root,tag='lemma'):
      ...
      for k in range(len(current.children)):
        ...
        if is_a_tag(current.children[k],'NOUN') or is_a_tag(current.children[k],'PRON') or\
         is_a_tag(current.children[k],'PROPN') or is_a_tag(current.children[k],'VERB') or\
         is_a_tag(current.children[k],'ADV'):
          ...
          elif  (('nmod' in current.children[k].token['deprel'] or 'appos' in current.children[k].token['deprel']) or 'advmod' in current.children[k].token['deprel']):
