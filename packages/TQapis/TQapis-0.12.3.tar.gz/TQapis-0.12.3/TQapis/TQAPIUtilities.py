...
def formatted_grid_fx(connection, base_date, base_currency):
    #
    # Changes/resets the password
    #
    request_formatted_grid_fx = TQRequests.request_function_formatted_grid_fx(base_date,base_currency)
    message = connection.send(request_formatted_grid_fx)
    return make_results(message, connection)
