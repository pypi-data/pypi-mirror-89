...
class Send:
	...
	def set(self) -> None:
		...
		Snippet: driver.configure.cell.horder.send.set() \n
		Triggers the HS-SCCH order type 1, according to the preconfiguration of UL/DL and queries the frame number, subframe
		number and acknowledgment related to the HS-SCCH order execution. For preconfiguration, refer to method RsCmwWcdmaSig.
		Configure.Cell.Carrier.Horder.downlink etc. \n
		"""
		self._core.io.write(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HORDer:SEND')

	def set_with_opc(self) -> None:
		"""SCPI: CONFigure:WCDMa:SIGNaling<instance>:CELL:HORDer:SEND \n
		Snippet: driver.configure.cell.horder.send.set_with_opc() \n
		Triggers the HS-SCCH order type 1, according to the preconfiguration of UL/DL and queries the frame number, subframe
		number and acknowledgment related to the HS-SCCH order execution. For preconfiguration, refer to method RsCmwWcdmaSig.
		Configure.Cell.Carrier.Horder.downlink etc. \n
		...
		"""
		self._core.io.write_with_opc(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HORDer:SEND')
	...
	def get(self) -> GetStruct:
		"""SCPI: CONFigure:WCDMa:SIGNaling<instance>:CELL:HORDer:SEND \n
		Snippet: value: GetStruct = driver.configure.cell.horder.send.get() \n
		Triggers the HS-SCCH order type 1, according to the preconfiguration of UL/DL and queries the frame number, subframe
		number and acknowledgment related to the HS-SCCH order execution. For preconfiguration, refer to method RsCmwWcdmaSig.
		Configure.Cell.Carrier.Horder.downlink etc. \n
			:return: structure: for return value, see the help for GetStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:WCDMa:SIGNaling<Instance>:CELL:HORDer:SEND?', self.__class__.GetStruct())
