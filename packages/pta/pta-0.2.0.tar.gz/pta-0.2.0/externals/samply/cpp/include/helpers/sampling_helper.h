...
public:
	...
	Scalar get_random_uniform_scalar(
	...
	Matrix<Scalar> get_random_uniform(
		const Scalar lower_bound, const Scalar upper_bound,
		const Eigen::Index num_rows, const Eigen::Index num_cols);
	...
	Eigen::Matrix<
		typename Derived1::Scalar, Derived1::RowsAtCompileTime,
	 ...
	 */
	template<typename Scalar>
	Matrix<Scalar> get_random_normals(
		const Scalar mean, const Scalar std_dev,
	 ...
	 */
	template<typename Scalar>
	Matrix<Scalar> get_random_directions(
		const int num_dimensions, const int num_directions);
	 ...
	 */
	Vector<Eigen::Index> get_random_integers(
		const Eigen::Index min_value, const Eigen::Index max_value,
	 ...
	 */
	template<typename Scalar>
	Eigen::Index sample_index_with_weights(
	...
	Scalar get_random_truncated_normal(
		const Scalar mean, const Scalar sigma,
		const Scalar lower_bound, const Scalar upper_bound);
	 ...
	 */
	template<typename Scalar>
	Vector<Scalar> get_normal_cdf_interval(
...
Eigen::Matrix<
	typename Derived1::Scalar, Derived1::RowsAtCompileTime,
	Derived1::ColsAtCompileTime>
SamplingHelper::get_random_uniform(
	const Eigen::MatrixBase<Derived1>& lower_bounds,
	const Eigen::MatrixBase<Derived2>& upper_bounds)
{
	// Validate inputs.
...
SamplingHelper::get_random_normals(
	const Scalar mean, const Scalar std_dev,
	const int num_rows, const int num_cols)
{
	assert(num_rows > 0 && num_cols > 0);
...
template<typename Scalar>
inline Matrix<Scalar>
SamplingHelper::get_random_directions(
	const int num_dimensions, const int num_directions)
{
	auto random_normals =
	...
	const size_t num_values)
{
	std::uniform_int_distribution<> distribution(
		min_value, max_value);
	return Vector<Eigen::Index>::NullaryExpr(
		num_values, [&]() { return distribution(generator); });
}
...
inline Eigen::Index SamplingHelper::sample_index_with_weights(
	const Vector<Scalar>& index_weights)
{
	std::discrete_distribution<Eigen::Index> distribution(
		index_weights.begin(), index_weights.end());
	return distribution(generator);
}
...
inline Scalar SamplingHelper::get_random_truncated_normal(
	...
	const Scalar lower_bound, const Scalar upper_bound)
{
	return static_cast<Scalar>(rtnorm(
		generator, lower_bound, upper_bound, mean, sigma));
}
...
template<typename Scalar>
inline Vector<Scalar> SamplingHelper::get_normal_cdf_interval(
	const Vector<Scalar>& a, const Vector<Scalar>& b)
{
