...
def SMEclust(
    ...
    stlearn.spatial.morphology.adjust(
        adata, use_data=use_data, radius=radius, method=adjust_method
    )
    ...
    if use_label == "louvain":
        stlearn.tl.clustering.louvain(adata, random_state=random_state)
    elif use_label == "kmeans":
        stlearn.tl.clustering.kmeans(adata, use_data=use_data, n_clusters=n_clusters)
    else:
