...
class MyQNetwork(QNetwork):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_delta=0, freeze_interval=1000, batch_size=32, network_type=None, update_rule="rmsprop", batch_accumulator="sum", random_state=np.random.RandomState(), double_Q=False, neural_network=NN):
        ...
        # We use the memory replay for the NTM (--> no write function)
        M=10 # Number of memory locations (=time steps)
        
        memory = []     # list of symbolic variables for each of the type of observations, reward, terminal
                        # --> [ T.tensor4 if observation of element=matrix, T.tensor3 if vector, T.tensor 2 if scalar ]
        self.memory_shared=[]
        
        T.matrix("memory")
        self.memory_shared=theano.shared(
            np.zeros((N, M), dtype=theano.config.floatX),broadcastable=(False, True))
        
        for i, dim in enumerate(self._input_dimensions):
            if len(dim) == 3:
                memory.append(T.tensor4("%s_%s" % ("memory_state", i)))
            elif len(dim) == 2:
                memory.append(T.tensor3("%s_%s" % ("memory_state", i)))
            elif len(dim) == 1:
                memory.append( T.matrix("%s_%s" % ("memory_state", i)) )
                
            self.memory_shared.append(theano.shared(np.zeros((batch_size,) + dim, dtype=theano.config.floatX) , borrow=False)) #FIXME dim (include M)

        memory.append( T.matrix("%s_%s" % ("memory_action")) )
        memory.append( T.matrix("%s_%s" % ("memory_reward")) )
        memory.append( T.matrix("%s_%s" % ("memory_terminal")) )
        self.memory_shared.append(theano.shared(np.zeros((batch_size,) + (1), dtype=theano.config.floatX) , borrow=False)) #FIXME dim (include M)
        self.memory_shared.append(theano.shared(np.zeros((batch_size,) + (1), dtype=theano.config.floatX) , borrow=False)) #FIXME dim (include M)
        self.memory_shared.append(theano.shared(np.zeros((batch_size,) + (1), dtype=theano.config.floatX) , borrow=False)) #FIXME dim (include M)
        
        read_head = T.vector("read")
        self.read_head=theano.shared(
            np.ones((M), dtype=theano.config.floatX)*0.01,
            broadcastable=(False, True))
            
        my_ntm=NTM(self._batch_size, self._input_dimensions, self._n_actions, self._random_state)
        self.q_vals, self.params, shape_after_conv = my_ntm._buildNTM(memory)

        
    def get_read_in_memory_replay():
