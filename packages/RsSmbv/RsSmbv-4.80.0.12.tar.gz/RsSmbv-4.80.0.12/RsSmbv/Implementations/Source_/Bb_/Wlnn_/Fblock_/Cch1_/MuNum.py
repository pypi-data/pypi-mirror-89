...
class MuNum:
	...
	def set(self, mu_num_ch_1: int, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.wlnn.fblock.cch1.muNum.set(mu_num_ch_1 = 1, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:WLNN:FBLock<CH>:CCH1:MUNum<ST> \n
		Snippet: value: int = driver.source.bb.wlnn.fblock.cch1.muNum.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Sets the number of MU-MIMO users for each RU and station of the first content channel. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fblock')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'MuNum')
