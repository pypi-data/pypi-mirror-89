...
class Preamble:
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.w3Gpp.mstation.udtx.preamble.get(channel = repcap.Channel.Default) \n
		Queries the preamble length in slots, when the corresponding UE-DTX cycle is applied. The preamble length is fixed to 2
		slots. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Preamble')
			:return: preamble: integer Range: 2 to 2"""
		channel_cmd_val = self._base.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation:UDTX:PREamble{channel_cmd_val}?')
