...
class EcpFormat:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("ecpFormat", core, parent)
	...
	def fetch(self, flow_id: int) -> FetchStruct:
		...
		Snippet: value: FetchStruct = driver.data.measurement.ipAnalysis.ipcSecurity.capplication.handshake.offered.ecpFormat.fetch(flow_id = 1) \n
		Queries information about the elliptic curve point formats offered during the handshake for a specific connection. After
		the reliability indicator, two results are returned for each format: <Reliability>, {<ECPFormatID>,
		<ECPFormatName>}Format 1, {...}Format 2, ... \n
		...
		return self._core.io.query_struct(f'FETCh:DATA:MEASurement<MeasInstance>:IPANalysis:IPCSecurity:CAPPlication:HANDshake:OFFered:ECPFormat? {param}', self.__class__.FetchStruct())
