...
class Data:
	...
	def set(self, data: enums.TpcDataSour, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.dpcch.tpc.data.set(data = enums.TpcDataSour.DLISt, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> enums.TpcDataSour:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:DPCCh:TPC:DATA \n
		Snippet: value: enums.TpcDataSour = driver.source.bb.w3Gpp.mstation.dpcch.tpc.data.get(stream = repcap.Stream.Default) \n
		The command determines the data source for the TPC field of the DPCCH. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
