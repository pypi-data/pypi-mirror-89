...
class SnnRatio:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("snnRatio", core, parent)
	...
	def set(self, structure: SnnRatioStruct, audioInput=repcap.AudioInput.Default) -> None:
		"""SCPI: CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:AIN<Nr>:SNNRatio \n
		Snippet: driver.configure.afRf.measurement.multiEval.limit.audioInput.snnRatio.set(value = [PROPERTY_STRUCT_NAME](), audioInput = repcap.AudioInput.Default) \n
		No command help available \n
			:param structure: for set value, see the help for SnnRatioStruct structure arguments.
			:param audioInput: optional repeated capability selector. Default value: Nr1 (settable in the interface 'AudioInput')"""
		audioInput_cmd_val = self._base.get_repcap_cmd_value(audioInput, repcap.AudioInput)
		self._core.io.write_struct(f'CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:AIN{audioInput_cmd_val}:SNNRatio', structure)
