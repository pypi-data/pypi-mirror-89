    ...
    getSelectedLayers: function (polygon) {
        ...
        this.map.eachLayer(function (layer) {
           ...
           if (layer === _this.polygon) {
			...
			if(layer.options.type=="circleMarker" && layer instanceof L.CircleMarker){
                layers.push(layer);
            }
			if(layer.options.type=="polygon" && layer instanceof L.Polygon){
				layers.push(layer);
			}
        });
		...
		for(var i=0; i<layers.length; i++){
			//console.log(layers[i].options.id);
			if(layers[i] instanceof L.Circle){
				alert("Is a marker");
			}
        ...
        var selectedLayers = layers.filter(function (layer) {
            ...
            if (layer.options.type=="polygon" && layer instanceof L.Polygon) {
				var layerGeometry = L.circleMarker(layer.getCenter()).toGeoJSON().geometry;
                return boolean_point_in_polygon_1.default(layerGeometry, lassoPolygonGeometry);
            }
            return false;
        });
		//alert("selected " + selectedLayers.length);
...
}, nameIndexes: {"@turf/boolean-point-in-polygon":1}, result: initialModResult_674497323404793172}, {fun: function(exports, module, require) {
...
}, nameIndexes: {"@turf/invariant":2}, result: initialModResult_674497323404793172}, {fun: function(exports, module, require) {
...
}, nameIndexes: {"@turf/helpers":3}, result: initialModResult_674497323404793172}, {fun: function(exports, module, require) {
