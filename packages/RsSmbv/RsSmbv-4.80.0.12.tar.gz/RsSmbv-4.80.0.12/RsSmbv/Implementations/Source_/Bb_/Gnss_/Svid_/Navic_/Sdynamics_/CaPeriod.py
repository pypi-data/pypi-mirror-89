...
class CaPeriod:
	...
	def set(self, const_acc_period: float, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:NAVic<ST>:SDYNamics:CAPeriod \n
		Snippet: driver.source.bb.gnss.svid.navic.sdynamics.caPeriod.set(const_acc_period = 1.0, channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:NAVic<ST>:SDYNamics:CAPeriod \n
		Snippet: value: float = driver.source.bb.gnss.svid.navic.sdynamics.caPeriod.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default) \n
		Sets the time duration during that acceleration is applied and thus the velocity varies. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Navic')
