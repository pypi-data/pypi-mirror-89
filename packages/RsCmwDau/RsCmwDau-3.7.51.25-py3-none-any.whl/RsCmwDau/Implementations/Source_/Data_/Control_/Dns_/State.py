...
class State:
	...
	def set(self, control: bool) -> None:
		...
		Snippet: driver.source.data.control.dns.state.set(control = False) \n
		...
		self._core.io.write_with_opc(f'SOURce:DATA:CONTrol:DNS:STATe {param}')

	# noinspection PyTypeChecker
	def get(self) -> enums.DauState:
		"""SCPI: SOURce:DATA:CONTrol:DNS:STATe \n
		Snippet: value: enums.DauState = driver.source.data.control.dns.state.get() \n
		Starts or stops the local DNS server. \n
		...
		response = self._core.io.query_str_with_opc(f'SOURce:DATA:CONTrol:DNS:STATe?')
