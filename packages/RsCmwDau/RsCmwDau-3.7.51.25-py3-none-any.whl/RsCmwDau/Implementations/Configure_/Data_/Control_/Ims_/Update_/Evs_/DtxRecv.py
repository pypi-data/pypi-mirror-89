...
class DtxRecv:
	...
	def set(self, dtx_recv: enums.DtxRecv, ims=repcap.Ims.Default) -> None:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:EVS:DTXRecv \n
		Snippet: driver.configure.data.control.ims.update.evs.dtxRecv.set(dtx_recv = enums.DtxRecv.DISable, ims = repcap.Ims.Default) \n
		...
		self._core.io.write(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:DTXRecv {param}')

	# noinspection PyTypeChecker
	def get(self, ims=repcap.Ims.Default) -> enums.DtxRecv:
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:EVS:DTXRecv \n
		Snippet: value: enums.DtxRecv = driver.configure.data.control.ims.update.evs.dtxRecv.get(ims = repcap.Ims.Default) \n
		Specifies the SDP parameter 'dtx-recv' for a call update. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')
			:return: dtx_recv: DISable | ENABle | NP Disable, enable, not present"""
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		response = self._core.io.query_str(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:EVS:DTXRecv?')
