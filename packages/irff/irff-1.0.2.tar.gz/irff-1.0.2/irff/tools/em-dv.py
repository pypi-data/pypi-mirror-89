...
def get_data_():
    excel_name = 'Parameters-v5-20200807-ZL.xlsx'
...
def xg():
    train_inputs,train_labels,test_inputs,test_labels,column_list = get_data_()
    xgr = xgb.XGBRegressor(reg_lambda=3,max_depth=20,learning_rate=0.3)
...
def ada():
    ...
    ada = AdaBoostRegressor(DecisionTreeRegressor(max_depth=20),
                            n_estimators=200, random_state=42,learning_rate=0.5)
    # train
    ada.fit(train_inputs, train_labels)
    score = ada.score(train_inputs, train_labels)
    print(' * train set score: ',score)
    score = ada.score(test_inputs, test_labels)
    print(' * test set score: ',score)
    ...
    for i in range(len(column_list)-1):
        print(column_list[i],feature_importances[i])
    # pprint.pprint(sorted(zip(feature_importances, column_list), reverse=True))
    show_results(ada,train_inputs,train_labels,title="Boosted Decision Tree Regression")
