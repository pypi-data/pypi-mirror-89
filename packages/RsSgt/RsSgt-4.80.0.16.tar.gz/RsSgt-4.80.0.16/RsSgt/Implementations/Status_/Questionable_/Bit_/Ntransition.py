...
class Ntransition:
	...
	def set(self, ntransition: str, bitNumber=repcap.BitNumber.Default) -> None:
		...
		Snippet: driver.status.questionable.bit.ntransition.set(ntransition = '1', bitNumber = repcap.BitNumber.Default) \n
	...
	def get(self, bitNumber=repcap.BitNumber.Default) -> str:
		"""SCPI: STATus:QUEStionable:BIT<BITNR>:NTRansition \n
		Snippet: value: str = driver.status.questionable.bit.ntransition.get(bitNumber = repcap.BitNumber.Default) \n
		No command help available \n
			:param bitNumber: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Bit')
