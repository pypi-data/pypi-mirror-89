...
class Limit:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.multiEval.trace.cde.isignal.limit.fetch() \n
		...
		Use RsCmwCdma2kMeas.reliability.last_value to read the updated reliability indicator. \n
			:return: isig_limit: float Return the exceeded limits as float values. The number of results depends on the selected spreading factor: SF=16, 32, 64."""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:CDMA:MEASurement<Instance>:MEValuation:TRACe:CDE:ISIGnal:LIMit?', suppressed)
		return response
