...
def aeh(X, Y, C, Ks=None, max_iter=10):
    ...
    for j in range(max_iter):
        ...
        log_likelihood = (
            -(N + G) / 2.0 * np.log(2.0 * np.pi)
            - (N + G) / 2.0 * np.log(cost.min(1).sum() / (N + G))
            - (N + G) / 2.0
        )

        logging.info(
            "Iteration {}, log likelihood: {:.2f}".format(j + 1, log_likelihood)
        )
...
def spatial_patterns(X, exp_mat, DE_mll_results, C, max_iter=10, kernel_space=None):
    ...
    cres = pd.DataFrame(
        {"g": DE_mll_results["g"], "pattern": patterns, "membership": cost.min(1) / N}
    )
