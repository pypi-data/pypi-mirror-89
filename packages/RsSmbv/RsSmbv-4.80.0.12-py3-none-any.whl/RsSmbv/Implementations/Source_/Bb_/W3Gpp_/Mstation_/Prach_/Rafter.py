...
class Rafter:
	...
	def set(self, repeatafter: int, stream=repcap.Stream.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.prach.rafter.set(repeatafter = 1, stream = repcap.Stream.Default) \n
	...
	def get(self, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:PRACh:RAFTer \n
		Snippet: value: int = driver.source.bb.w3Gpp.mstation.prach.rafter.get(stream = repcap.Stream.Default) \n
		Sets the number of access slots after that the PRACH structure is repeated. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
