...
class CRAR(LearningAlgo):
    ...
    def __init__(self, environment, rho=0.9, rms_epsilon=0.0001, momentum=0, clip_norm=0, freeze_interval=1000, batch_size=32, update_rule="rmsprop", random_state=np.random.RandomState(), double_Q=False, neural_network=NN, **kwargs):
        ...
        self.loss_G, self.loss_G2=0.68, 0.68
        self.loss_D=0.68
        self.loss_Q=0
    ...
    def train(self, states_val, actions_val, rewards_val, next_states_val, terminals_val):
        ...
        noise_level2=max( (1.-self.update_counter/100000) , 0.2 ) #0.2
        ...
        rewards_val[:self._batch_size//2]+=np.random.normal(0,noise_level2,(self._batch_size//2))
        terminals_val[self._batch_size//2:]=terminals[self._batch_size//2:,0]
        terminals_val[:self._batch_size//2]+=np.random.normal(0,noise_level2,(self._batch_size//2))
        ...
        if(self.update_counter%2000==0):
            ...
            print (self.transition_d.predict([Es[:5],onehot_actions[:5],Es__noise[:5],rewards_val_noise[:5],terminals_val_noise[:5]]))
            ...
            print (rewards_val_noise[:5],terminals_val_noise[:5])
        ...
        if (self.wait_G==0):
            loss_G=self.transition_g_full.train_on_batch(states_val+[onehot_actions,noise], np.ones((self._batch_size,1)) )
            self.loss_G=0.95*self.loss_G+0.05*loss_G
            loss_G2=self.encoder_g_full.train_on_batch(states_val+[onehot_actions]+next_states_val+[rewards_val_noise,terminals_val_noise,np.random.normal(0,noise_level/2.,Es_[:self._batch_size].shape)], np.zeros((self._batch_size,1)) )
            #print ("loss_G,loss_G2"+str(loss_G)+str(loss_G2))
            self.loss_G2=0.95*self.loss_G2+0.05*loss_G2
        ...
        if(self.update_counter%2000==0):
            print ("self.loss_G, self.loss_G2, self.loss_D.")
            print (self.loss_G, self.loss_G2, self.loss_D)
...
#        ## END DEBUG PURPOSES
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(self.transition_d.optimizer.lr, self._lr/4.)
        ...
        #K.set_value(self.diff_s_s_.optimizer.lr, self._lr/1./10.) # /5. for simple laby or simple catcher; /1. for distrib of laby
