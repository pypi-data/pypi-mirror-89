...
/**
	...
	*/
template<
	typename ChainState, typename DurationFormat = std::chrono::milliseconds>
...
public:
	...
	/**
		...
		*/
	void log(
		const ChainState& chain_state, const DurationFormat total_time,
		const size_t step_idx)
	{
		...
		if (interval_time >= log_period_) {
			...
			const double double_interval_time = std::chrono::duration_cast<
				DoubleSeconds>(interval_time).count();
			const double elapsed_time = std::chrono::duration_cast<
				DoubleSeconds>(total_time).count();
			...
			output_stream_ << std::fixed
				<< std::setw(10) << std::setprecision(0) << worker_id_ << " |"
				<< std::setw(10) << std::setprecision(2) << elapsed_time << " |"
				<< std::setw(10) << std::setprecision(0) << step_idx + 1 << " |"
				<< std::setw(10) << std::setprecision(2) << steps_rate << " |"
				<< std::endl;
	...
	void start()
	{
		...
		output_stream_ <<
			"------------------------------------------------" << std::endl;
		output_stream_ <<
			"    worker |  time (s) |     steps |   steps/s  " << std::endl;
		output_stream_ <<
			"------------------------------------------------" << std::endl;
		print_progress(output_stream_.str());
		output_stream_.str("");
	...
	void stop(const bool time_limit_reached = false)
	{
		if (time_limit_reached)
			output_stream_
			<< "Time limit reached. The chain might not have converged."
			<< std::endl;
		else
