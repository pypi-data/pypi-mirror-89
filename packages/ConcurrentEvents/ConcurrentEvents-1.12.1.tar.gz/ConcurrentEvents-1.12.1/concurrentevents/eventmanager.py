...
from concurrentevents._exceptions import Cancel, StartError, EventError
...
class EventManager:
    ...
    def add_handlers(self, *handlers):
        ...
        for h in handlers:
            try:
                ...
                for var in dir(h):
                    fn = getattr(h, var)
                    if callable(h) and hasattr(fn, 'event'):
                        self.handlers[fn.event].append(fn)
            except Exception as e:
                logger.error(f"{fn} is not a valid handler {e}")
    ...
    def start(self):
        ...
        while not (self.exit or len(self.__futures.keys()) == 0):
            debug_info = [len(list(self.__futures)), len([ft for ft in list(self.__futures.keys()) if ft.running()]),
