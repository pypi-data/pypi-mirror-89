...
def get_kaniko_manifest(bento, path, registry, uid=None):
    if ':' in bento:
        name, version = bento.split(':')
    else:
        name = bento
    ...
    pod_name = '{}-{}'.format(camel_to_kebab(name), uid)
    manifest = gen_kaniko_manifest(name, pod_name, version, path, registry)
    click.echo(manifest)
    return manifest
...
def containerize(ctx, bento, registry):
    ...
    manifest = ctx.invoke(get_kaniko_manifest, bento=bento, path=path, registry=registry)
...
def delete_containerize_job(ctx, bento, pod, registry):
    path = ctx.invoke(get_saved_path, bento=bento)
    uid = pod.split('-')[-1]
    manifest = ctx.invoke(get_kaniko_manifest, bento=bento, path=path, registry=registry, uid=uid)
