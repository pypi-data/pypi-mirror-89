...
class mysql2postgresql:
    ...
    def selecttoinsert(self, table:str) -> None:
        ...
        while count > 0:
            ...
            self.insertinto(iter(self.dbx.fetchall()), table)
    ...
    def main(self) -> None:
        ...
        if len(self.tables) == 0:
            ...
            tables:list = [table_name[0] for table_name in iter(self.dbx.fetchall())]
            self.close_mysql()
        else:
    ...
    def run(self):
        if cython:
            tqdm.write('Running in Cython version')
            runner.mysql2postgresql(self.connect_mysql_kwargs, self.connectpostgresql_kwargs, self.tables)
