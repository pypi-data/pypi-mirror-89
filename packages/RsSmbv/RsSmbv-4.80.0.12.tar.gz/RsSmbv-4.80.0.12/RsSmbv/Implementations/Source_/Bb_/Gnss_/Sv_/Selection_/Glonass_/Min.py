...
class Min:
	...
	def set(self, minimum_sv_s: int, stream=repcap.Stream.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:SELection:GLONass<ST>:MIN \n
		Snippet: driver.source.bb.gnss.sv.selection.glonass.min.set(minimum_sv_s = 1, stream = repcap.Stream.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:GLONass{stream_cmd_val}:MIN {param}')

	def get(self, stream=repcap.Stream.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:SELection:GLONass<ST>:MIN \n
		Snippet: value: int = driver.source.bb.gnss.sv.selection.glonass.min.get(stream = repcap.Stream.Default) \n
		Sets the minimum and maximum number of satellites per GNSS system that can be included in the satellite constellation. \n
			:param stream: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Glonass')
			:return: minimum_sv_s: No help available"""
		stream_cmd_val = self._base.get_repcap_cmd_value(stream, repcap.Stream)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:GLONass{stream_cmd_val}:MIN?')
