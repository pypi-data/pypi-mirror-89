...
class Perform:
	...
	def set(self, ims=repcap.Ims.Default) -> None:
		...
		Snippet: driver.configure.data.control.ims.update.rcs.chat.perform.set(ims = repcap.Ims.Default) \n
		Initiates a chat message transfer to the DUT. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')"""
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		self._core.io.write(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:RCS:CHAT:PERForm')

	def set_with_opc(self, ims=repcap.Ims.Default) -> None:
		ims_cmd_val = self._base.get_repcap_cmd_value(ims, repcap.Ims)
		"""SCPI: CONFigure:DATA:CONTrol:IMS<Suffix>:UPDate:RCS:CHAT:PERForm \n
		Snippet: driver.configure.data.control.ims.update.rcs.chat.perform.set_with_opc(ims = repcap.Ims.Default) \n
		Initiates a chat message transfer to the DUT. \n
		Same as set, but waits for the operation to complete before continuing further. Use the RsCmwDau.utilities.opc_timeout_set() to set the timeout value. \n
			:param ims: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Ims')"""
		self._core.io.write_with_opc(f'CONFigure:DATA:CONTrol:IMS{ims_cmd_val}:UPDate:RCS:CHAT:PERForm')
