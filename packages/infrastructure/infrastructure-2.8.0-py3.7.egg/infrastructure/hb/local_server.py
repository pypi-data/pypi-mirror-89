...
# @Last Modified time: 2020-12-24 21:25:46
...
class LocalServer(object):
	def __init__(self,alreadyRecord,JavaCoverageReportUrl="",
		...
		self.mergeFile = JAVA_COV["mergeFile"].format(week=getWeek(onlyWeek=True),
			service_name=self.alreadyRecord.service_name)
		self.destfile = JAVA_COV["destfile"].format(week=getWeek(onlyWeek=True),service_name=self.alreadyRecord.service_name,recordID=self.alreadyRecord.id)
	...
	def operateExec(self):
		if len(os.listdir(self.destfile)) > 1:
			...
			os.rename(self.jacocoIt.format(week=getWeek(onlyWeek=True),service_name=self.alreadyRecord.service_name,
					recordID=self.alreadyRecord.id,index=0),self.jacocoIt.format(week=getWeek(onlyWeek=True),service_name=self.alreadyRecord.service_name,
