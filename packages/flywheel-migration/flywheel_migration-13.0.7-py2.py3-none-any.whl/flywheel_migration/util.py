...
def get_dicom_record_attributes(dcm):
    """Returns a list of attributes in the record in a dotty notation format.

    Args:
        dcm (pydicom.FileDataset): A pydicom DataElement.

    Returns:
        list: List of attributes in the record in a dotty notation format (e.g.
          ``['PatientID', ..., 'AnatomicRegionSequence.0.CodeValue', ...].
    """
    # TODO: to be moved to a DicomRecord class
    dotty_attrs = []
    keyword_list = sorted(dcm.dir())
    for keyword in keyword_list:
        try:
            data_element = dcm[keyword]
            if keyword in dcm and data_element.VR == "SQ":
                sequence = data_element.value
                seq_attrs = []
                for i, dataset in enumerate(sequence):
                    attrs = get_dicom_record_attributes(dataset)
                    # concatenate parent keyword and index to attrs items and
                    # append to seq_attrs list
                    seq_attrs += list(
                        map(
                            lambda x, parent=keyword, idx=i: f"{parent}.{idx}.{x}",
                            attrs,
                        )
                    )
            ...
            log.debug("Exception raised while walking record", exc_info=True)
